...
BASE="disptext_set_layout_params"
ENTRY="DISPTEXT_SetLayoutParams"
ENTRY_SASC_REGEX="^DISPTEXT_SetLayoutParams[A-Za-z0-9_]*:$"
ENTRY_SEM="DISPTEXT_SETLAYOUTPARAMS:"
ENTRY_ALT_SEM="DISPTEXT_SETLAYOUTPARAM"
...
awk -v ENTRY_PREFIX="$ENTRY_SEM" -v ENTRY_ALT_PREFIX="$ENTRY_ALT_SEM" -f src/decomp/scripts/semantic_filter_sasc_disptext_set_layout_params.awk "${OUT_DIR}/${BASE}.original.norm.s" >"${OUT_DIR}/${BASE}.original.semantic.txt"
awk -v ENTRY_PREFIX="$ENTRY_SEM" -v ENTRY_ALT_PREFIX="$ENTRY_ALT_SEM" -f src/decomp/scripts/semantic_filter_sasc_disptext_set_layout_params.awk "${OUT_DIR}/${BASE}.sasc.norm.s" >"${OUT_DIR}/${BASE}.sasc.semantic.txt"
