...
SRC_C="src/decomp/c/replacements/diskio1_append_attr_flag_ditto_gcc.c"
ORIG_ASM="src/modules/groups/a/g/diskio1.s"
OUT_DIR="build/decomp/c_trial_gcc"
GEN_ASM="${OUT_DIR}/diskio1_append_attr_flag_ditto.generated.gcc.s"
ORIG_SLICE="${OUT_DIR}/diskio1_append_attr_flag_ditto.original_slice.s"
GEN_SLICE="${OUT_DIR}/diskio1_append_attr_flag_ditto.generated_slice.s"
ORIG_NORM="${OUT_DIR}/diskio1_append_attr_flag_ditto.original_slice.norm.s"
GEN_NORM="${OUT_DIR}/diskio1_append_attr_flag_ditto.generated_slice.norm.s"
...
awk '
$0 ~ /^DISKIO1_AppendAttrFlagDitto:?$/ {in_func=1}
in_func {
    if ($0 ~ /^;------------------------------------------------------------------------------/) exit
    print
}
...
awk '
$0 ~ /^_?DISKIO1_AppendAttrFlagDitto:?$/ {in_func=1}
in_func { print }
' "$GEN_ASM" > "$GEN_SLICE"
...
awk -f src/decomp/scripts/semantic_filter_diskio1_append_attr_flag_ditto.awk "$ORIG_NORM" > "$ORIG_SEM"
awk -f src/decomp/scripts/semantic_filter_diskio1_append_attr_flag_ditto.awk "$GEN_NORM" > "$GEN_SEM"
