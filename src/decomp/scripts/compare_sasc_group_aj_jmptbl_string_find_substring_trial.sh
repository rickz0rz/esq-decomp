...
BASE="group_aj_jmptbl_string_find_substring"
ENTRY_ORIG="GROUP_AJ_JMPTBL_STRING_FindSubstring"
ENTRY_SASC="GROUP_AJ_JMPTBL_STRING_FindSubst"
TARGET="STRING_FindSubstring"
mkdir -p "$OUT_DIR"
./sc-build-with-dis.sh "$SASC_SRC" >"${OUT_DIR}/sc_build_${BASE}.log" 2>&1
awk -v e="^${ENTRY_ORIG}:$" '$0 ~ e {inf=1} inf { if ($0 ~ /^GROUP_AJ_JMPTBL_FORMAT_RawDoFmtWithScratchBuffer:$/) exit; print }' "$ORIG_ASM" >"${OUT_DIR}/${BASE}.original.s"
awk -v e="^${ENTRY_SASC}:$" '$0 ~ e {inf=1} inf { if ($0 ~ /^GROUP_AJ_JMPTBL_FORMAT_RawDoFmtW:$/) exit; print }' "$SASC_DIS" >"${OUT_DIR}/${BASE}.sasc.dis.s"
