...

{
    ...
    if (u ~ /WDISP_PALETTETRIPLESRBASE/) has_palette_base = 1
    if (u ~ /ESQ_COPPERSTATUSDIGITSA/) has_list_a = 1
    if (u ~ /ESQ_COPPERSTATUSDIGITSB/) has_list_b = 1
    if (u ~ /ESQ_BUMPCOLORTOWARDTARGETS/) has_bump_call = 1
    if ((u ~ /MOVE\.W D0,0\(A2,D5\.W\)/ || u ~ /MOVE\.W D0,0\(A3,D5\.W\)/ || u ~ /MOVE\.W D0,\(A[23]\)/ || u ~ /MOVE\.W D[0-7],\(A3\)/ || u ~ /MOVE\.W D[0-7],\$0\(A3,D[0-7]\.W\)/) && u !~ /MOVE\.W D[0-7],D[0-7]/) has_dual_store = 1
    if ((u ~ /MOVE\.W D0,0\(A2,D5\.W\)/ || u ~ /MOVE\.W D0,\(A2\)/ || u ~ /MOVE\.W D0,\(A5\)/ || u ~ /MOVE\.W D[0-7],\(A5\)/ || u ~ /MOVE\.W D[0-7],\$0\(A5,D[0-7]\.W\)/ || u ~ /MOVE\.W D[0-7],\$0\(A3,D[0-7]\.W\)/) && u !~ /MOVE\.W D[0-7],D[0-7]/) has_primary_store = 1
...
END {
    ...
    print "HAS_PALETTE_BASE=" has_palette_base
    print "HAS_LIST_A=" has_list_a
    print "HAS_LIST_B=" has_list_b
    print "HAS_BUMP_CALL=" has_bump_call
