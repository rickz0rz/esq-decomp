...
SASC_SRC="diskio_save_config_to_file_handle.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/a/g/diskio.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="diskio_save_config_to_file_handle"
ENTRY="DISKIO_SaveConfigToFileHandle"
ENTRY_SASC_REGEX="^DISKIO_SaveConfigToFileHandle[A-Za-z0-9_]*:$"
...
awk -v e="^${ENTRY}:$" '
  $0 ~ e {in_func=1}
  in_func {
    if ($0 ~ /^DISKIO_SaveConfigToFileHandle_Return:/) exit
    if ($0 ~ /^;!======/) exit
    print
  }
...
awk -f src/decomp/scripts/semantic_filter_sasc_diskio_save_config_to_file_handle.awk "${OUT_DIR}/${BASE}.original.norm.s" >"${OUT_DIR}/${BASE}.original.semantic.txt"
awk -f src/decomp/scripts/semantic_filter_sasc_diskio_save_config_to_file_handle.awk "${OUT_DIR}/${BASE}.sasc.norm.s" >"${OUT_DIR}/${BASE}.sasc.semantic.txt"
