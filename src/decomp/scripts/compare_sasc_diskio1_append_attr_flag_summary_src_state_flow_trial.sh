...
SASC_SRC="diskio1_append_attr_helpers.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/a/g/diskio1.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="diskio1_append_attr_flag_summary_src_state_flow"
ENTRY="DISKIO1_AppendAttrFlagSummarySrc"
ENTRY_SASC_REGEX="^DISKIO1_AppendAttrFlagSummarySrc[A-Za-z0-9_]*:$"
...
awk -v e="^${ENTRY}:$" '
  ...
  in_func {
    if ($0 ~ /^; FUNC: /) exit
    if (($0 ~ /^[A-Z0-9_]+:$/ || $0 ~ /^_?[A-Z0-9_]+:$/) && $0 !~ e) exit
    if ($0 ~ /^;!======/) exit
    print
  }
...
awk -v e="^${ENTRY}:$" -v e2="$ENTRY_SASC_REGEX" '
  $0 ~ e || $0 ~ e2 {in_func=1}
  in_func {
    if ($0 ~ /^DISKIO1_AppendAttrFlagVideoTagDi[A-Za-z0-9_]*:$/) exit
...
awk -f src/decomp/scripts/semantic_filter_sasc_diskio1_append_attr_flag_summary_src_state_flow.awk "${OUT_DIR}/${BASE}.original.norm.s" >"${OUT_DIR}/${BASE}.original.semantic.txt"
awk -f src/decomp/scripts/semantic_filter_sasc_diskio1_append_attr_flag_summary_src_state_flow.awk "${OUT_DIR}/${BASE}.sasc.norm.s" >"${OUT_DIR}/${BASE}.sasc.semantic.txt"
