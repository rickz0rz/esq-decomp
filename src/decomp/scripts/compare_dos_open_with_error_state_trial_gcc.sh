...
SRC_C="src/decomp/c/replacements/dos_open_with_error_state_gcc.c"
ORIG_ASM="src/modules/submodules/unknown20.s"
OUT_DIR="build/decomp/c_trial_gcc"
GEN_ASM="${OUT_DIR}/dos_open_with_error_state.generated.gcc.s"
ORIG_SLICE="${OUT_DIR}/dos_open_with_error_state.original_slice.s"
GEN_SLICE="${OUT_DIR}/dos_open_with_error_state.generated_slice.s"
ORIG_NORM="${OUT_DIR}/dos_open_with_error_state.original_slice.norm.s"
GEN_NORM="${OUT_DIR}/dos_open_with_error_state.generated_slice.norm.s"
...
awk '
$0 ~ /^DOS_OpenWithErrorState:?$/ {in_func=1}
in_func {
    if ($0 ~ /^;!======/) {
        exit
    }
...
awk '
$0 ~ /^_?DOS_OpenWithErrorState:?$/ {in_func=1}
in_func { print }
' "$GEN_ASM" > "$GEN_SLICE"
...
awk -f src/decomp/scripts/semantic_filter_dos_open_with_error_state.awk "$ORIG_NORM" > "$ORIG_SEM"
awk -f src/decomp/scripts/semantic_filter_dos_open_with_error_state.awk "$GEN_NORM" > "$GEN_SEM"
