...
BASE="group_aw_jmptbl_wdisp_sprintf"
ENTRY_ORIG="GROUP_AW_JMPTBL_WDISP_SPrintf"
ENTRY_SASC="GROUP_AW_JMPTBL_WDISP_SPrintf"
TARGET="WDISP_SPrintf"

mkdir -p "$OUT_DIR"
./sc-build-with-dis.sh "$SASC_SRC" >"${OUT_DIR}/sc_build_${BASE}.log" 2>&1
awk -v e="^${ENTRY_ORIG}:$" '$0 ~ e {inf=1} inf { if ($0 ~ /^GROUP_AW_JMPTBL_ESQ_SetCopperEffect_OffDisableHighlight:$/) exit; print }' "$ORIG_ASM" >"${OUT_DIR}/${BASE}.original.s"
awk -v e="^${ENTRY_SASC}:$" '$0 ~ e {inf=1} inf { if ($0 ~ /^GROUP_AW_JMPTBL_ESQ_SetCopperEff:$/) exit; print }' "$SASC_DIS" >"${OUT_DIR}/${BASE}.sasc.dis.s"
