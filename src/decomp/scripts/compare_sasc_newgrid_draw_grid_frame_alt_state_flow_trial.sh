...
BASE="newgrid_draw_grid_frame_alt_state_flow"
ENTRY_ORIG="NEWGRID_DrawGridFrameAlt"
...
awk -v e="^${ENTRY_ORIG}:$" '
  $0 ~ e {in_func=1}
  in_func {
    if ($0 ~ /^NEWGRID_HandleAltGridState:$/) exit
    print
  }
...
awk -f src/decomp/scripts/semantic_filter_sasc_newgrid_draw_grid_frame_alt_state_flow.awk "${OUT_DIR}/${BASE}.original.norm.s" >"${OUT_DIR}/${BASE}.original.semantic.txt"
awk -f src/decomp/scripts/semantic_filter_sasc_newgrid_draw_grid_frame_alt_state_flow.awk "${OUT_DIR}/${BASE}.sasc.norm.s" >"${OUT_DIR}/${BASE}.sasc.semantic.txt"
