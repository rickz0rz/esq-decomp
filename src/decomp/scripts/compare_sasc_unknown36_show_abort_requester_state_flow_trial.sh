...
BASE="unknown36_show_abort_requester_state_flow"
...
awk -v entry="^${ENTRY_ORIG}:$" '
  $0 ~ entry { in_func = 1 }
  in_func {
    if ($0 ~ /^DEBUG_STR_UserAbortRequested:$/ || $0 ~ /^__const:$/) exit
    print
  }
...
awk -f src/decomp/scripts/semantic_filter_sasc_unknown36_show_abort_requester_state_flow.awk \
