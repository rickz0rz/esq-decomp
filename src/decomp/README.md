...
- Current hybrid-integration note (March 13, 2026): `modules/groups/a/u/gcommand4.s` is now wired into `src/decomp/replacements.map` as a passthrough hybrid boundary. Its restored SAS/C coverage currently includes `GCOMMAND_SaveBrushResult`, so future object-level replacement work can land in that brush-result helper module without another root include-graph edit.
...
- Current hybrid-integration note (March 13, 2026): `modules/groups/a/h/diskio2.s` is now wired into `src/decomp/replacements.map` as a passthrough hybrid boundary. Its restored SAS/C coverage already spans the current non-`JMPTBL` data-file transfer, sync, and ini/oinfo helper set in that module, so future object-level replacement work can land there without another root include-graph edit.
