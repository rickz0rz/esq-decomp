...
cd "$ROOT_DIR"
SASC_SRC="textdisp2_jmptbl_stubs.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/b/a/textdisp2.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="textdisp2_jmptbl_esqiff_playnextexternalassetframe"
ENTRY_ORIG="TEXTDISP2_JMPTBL_ESQIFF_PlayNextExternalAssetFrame"
ENTRY_SASC="TEXTDISP2_JMPTBL_ESQIFF_PlayNext"
TARGET="ESQIFF_PlayNextExternalAssetFrame"
mkdir -p "$OUT_DIR"
./sc-build-with-dis.sh "$SASC_SRC" >"${OUT_DIR}/sc_build_${BASE}.log" 2>&1
awk -v e="^${ENTRY_ORIG}:$" '$0 ~ e {inf=1} inf { if ($0 ~ /^TEXTDISP2_JMPTBL_/ && $0 !~ e) exit; if ($0 ~ /^;!======/) exit; print }' "$ORIG_ASM" >"${OUT_DIR}/${BASE}.original.s"
