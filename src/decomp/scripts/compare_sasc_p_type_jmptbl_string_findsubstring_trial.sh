...
cd "$ROOT_DIR"
SASC_SRC="p_type_jmptbl_stubs.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/b/a/p_type.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="p_type_jmptbl_string_findsubstring"
ENTRY_ORIG="P_TYPE_JMPTBL_STRING_FindSubstring"
ENTRY_SASC="P_TYPE_JMPTBL_STRING_FindSubstri"
TARGET="STRING_FindSubstring"
mkdir -p "$OUT_DIR"
./sc-build-with-dis.sh "$SASC_SRC" >"${OUT_DIR}/sc_build_${BASE}.log" 2>&1
awk -v e="^${ENTRY_ORIG}:$" '$0 ~ e {inf=1} inf { if ($0 ~ /^P_TYPE_JMPTBL_/ && $0 !~ e) exit; if ($0 ~ /^;!======/) exit; print }' "$ORIG_ASM" >"${OUT_DIR}/${BASE}.original.s"
