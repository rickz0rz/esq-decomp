...
cd "$ROOT_DIR"
SASC_SRC="script2_jmptbl_stubs.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/b/a/script2.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="script2_jmptbl_esq_capturectrlbit4streambufferbyte"
ENTRY_ORIG="SCRIPT2_JMPTBL_ESQ_CaptureCtrlBit4StreamBufferByte"
ENTRY_SASC="SCRIPT2_JMPTBL_ESQ_CaptureCtrlBi"
TARGET="ESQ_CaptureCtrlBit4StreamBufferByte"
mkdir -p "$OUT_DIR"
./sc-build-with-dis.sh "$SASC_SRC" >"${OUT_DIR}/sc_build_${BASE}.log" 2>&1
awk -v e="^${ENTRY_ORIG}:$" '$0 ~ e {inf=1} inf { if ($0 ~ /^SCRIPT2_JMPTBL_/ && $0 !~ e) exit; if ($0 ~ /^;!======/) exit; print }' "$ORIG_ASM" >"${OUT_DIR}/${BASE}.original.s"
