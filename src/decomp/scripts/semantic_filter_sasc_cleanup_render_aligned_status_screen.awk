BEGIN {
    ...
    has_today_label = 0
    has_tonight_label = 0
    has_dst_compute_banner_index = 0
    has_adjust_month_index = 0
...

{
    ...
    if (u ~ /GROUP_AD_JMPTBL_DST_COMPUTEBANNERINDEX/ || u ~ /DST_COMPUTEBANNERINDEX/) has_dst_compute_banner_index = 1
    if (u ~ /GROUP_AD_JMPTBL_DATETIME_ADJUSTMONTHINDEX/ || u ~ /DATETIME_ADJUSTMONTHINDEX/) has_adjust_month_index = 1
...
END {
    ...
    print "HAS_DAY_SUFFIX_PATH=" (has_format_entry_time && has_tomorrow_label && has_today_label && has_tonight_label &&
        has_dst_compute_banner_index && has_adjust_month_index)
    ...
    print "HAS_FINAL_TEXT_DRAW_SETUP=" (has_pen_override && has_trim_text && has_draw_inset && has_set_drmd && has_set_apen)
    print "HAS_MODE2_FILL=" (has_get_rast_port && get_height_count >= 2 && has_rect_fill && has_noop)
