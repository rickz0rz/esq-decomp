...
SRC_C="src/decomp/c/replacements/esqfunc_draw_esc_menu_version_gcc.c"
ORIG_ASM="src/modules/groups/a/n/esqfunc.s"
OUT_DIR="build/decomp/c_trial_gcc"
GEN_ASM="${OUT_DIR}/esqfunc_draw_esc_menu_version.generated.gcc.s"
ORIG_SLICE="${OUT_DIR}/esqfunc_draw_esc_menu_version.original_slice.s"
GEN_SLICE="${OUT_DIR}/esqfunc_draw_esc_menu_version.generated_slice.s"
ORIG_NORM="${OUT_DIR}/esqfunc_draw_esc_menu_version.original_slice.norm.s"
GEN_NORM="${OUT_DIR}/esqfunc_draw_esc_menu_version.generated_slice.norm.s"
...
awk '$0 ~ /^ESQFUNC_DrawEscMenuVersion:?$/ {in_func=1} in_func { if ($0 ~ /^;!======/) exit; print }' "$ORIG_ASM" > "$ORIG_SLICE"
awk '$0 ~ /^_?ESQFUNC_DrawEscMenuVersion:?$/ {in_func=1} in_func { print }' "$GEN_ASM" > "$GEN_SLICE"
...
awk -f src/decomp/scripts/semantic_filter_esqfunc_draw_esc_menu_version.awk "$ORIG_NORM" > "$ORIG_SEM"
awk -f src/decomp/scripts/semantic_filter_esqfunc_draw_esc_menu_version.awk "$GEN_NORM" > "$GEN_SEM"
