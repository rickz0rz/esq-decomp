...
- Current hybrid-integration note (March 13, 2026, later re-validated): `modules/groups/a/z/locavail2.s` is now wired into `src/decomp/replacements.map` as a passthrough hybrid boundary. Its restored SAS/C coverage already spans the current non-`JMPTBL` intuition override helpers (`LOCAVAIL2_AutoRequestNoOp` and `LOCAVAIL2_DisplayAlertDelayAndReboot`) in that module, so future object-level replacement work can land there without another root include-graph edit.
...
- Current hybrid-integration note (March 13, 2026, later re-validated): `modules/submodules/unknown40.s` is now wired into `src/decomp/replacements.map` as a passthrough hybrid boundary. Its restored SAS/C coverage already spans the current non-`JMPTBL` battery-clock and DOS wrapper helpers (`BATTCLOCK_GetSecondsFromBatteryBackedClock`, `BATTCLOCK_WriteSecondsToBatteryBackedClock`, `DOS_Delay`, and `DOS_SystemTagList`), so future object-level replacement work can land there without another root include-graph edit.
