...
- Current triage state (March 13, 2026): the maintained `run_sasc_core_sweep.sh` set, the remaining out-of-sweep `compare_sasc_memory_allocate_trial.sh` / `compare_sasc_esqfunc_draw_esc_menu_version_trial.sh` checks, and a full local rerun of the current `compare_sasc_*` script set all currently produce zero-byte semantic diffs. Future triage should start from genuinely GCC-only behavior targets rather than re-checking the already-green SAS/C lane. One concrete example is `ESQIFF_ShowExternalAssetWithCopperFx`, whose `_return` tail is covered in SAS/C while the main body still remains in the GCC staging lane.
