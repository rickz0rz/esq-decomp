...
BASE="esqdisp_draw_status_banner_impl_state_flow"
ENTRY="ESQDISP_DrawStatusBanner_Impl"
ENTRY_SASC_REGEX="^ESQDISP_DrawStatusBanner_Im[A-Za-z0-9_]*:$"
NEXT_LABEL="^ESQDISP_MirrorPrimaryEntriesToSecondaryIfEmpty:$"
...
awk -v e="^${ENTRY}:$" -v e2="$ENTRY_SASC_REGEX" '
    $0 ~ e || $0 ~ e2 {in_func=1}
    in_func {
        if (($0 ~ /^ESQDISP_[A-Za-z0-9_]+:$/ || $0 ~ /^_?ESQDISP_[A-Za-z0-9_]+:$/) &&
            $0 !~ e && $0 !~ e2) exit
...
awk -f src/decomp/scripts/semantic_filter_sasc_esqdisp_draw_status_banner_impl_state_flow.awk \
