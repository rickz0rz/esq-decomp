...
SRC_C="src/decomp/c/replacements/group_am_jmptbl_kybd_initialize_input_devices_gcc.c"
ORIG_ASM="src/modules/groups/a/m/xjump.s"
OUT_DIR="build/decomp/c_trial_gcc"
GEN_ASM="${OUT_DIR}/group_am_jmptbl_kybd_initialize_input_devices.generated.gcc.s"
ORIG_SLICE="${OUT_DIR}/group_am_jmptbl_kybd_initialize_input_devices.original_slice.s"
GEN_SLICE="${OUT_DIR}/group_am_jmptbl_kybd_initialize_input_devices.generated_slice.s"
ORIG_NORM="${OUT_DIR}/group_am_jmptbl_kybd_initialize_input_devices.original_slice.norm.s"
GEN_NORM="${OUT_DIR}/group_am_jmptbl_kybd_initialize_input_devices.generated_slice.norm.s"
...
awk -v start_label="GROUP_AM_JMPTBL_KYBD_InitializeInputDevices" -v next_label="GROUP_AM_JMPTBL_ESQ_CheckCompatibleVideoChip" '
...
awk '$0 ~ /^_?GROUP_AM_JMPTBL_KYBD_InitializeInputDevices:?$/ {in_func=1} in_func { print }' "$GEN_ASM" > "$GEN_SLICE"
...
awk -f "src/decomp/scripts/semantic_filter_group_am_jmptbl_kybd_initialize_input_devices.awk" "$ORIG_NORM" > "$ORIG_SEM"
awk -f "src/decomp/scripts/semantic_filter_group_am_jmptbl_kybd_initialize_input_devices.awk" "$GEN_NORM" > "$GEN_SEM"
