...
LONG TEXTDISP_JMPTBL_NEWGRID_ShouldOpenEditor(const char *entry){return NEWGRID_ShouldOpenEditor(entry);}
LONG TEXTDISP_JMPTBL_ESQDISP_TestEntryGridEligibility(const void *entry, LONG index){return ESQDISP_TestEntryGridEligibility(entry, (WORD)index);}
