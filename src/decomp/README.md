...
- Current hybrid-integration note (March 13, 2026): `modules/groups/a/p/esqshared.s` is now wired into `src/decomp/replacements.map` as a passthrough hybrid boundary. Its restored SAS/C coverage already spans a useful non-`JMPTBL` foothold for entry/default setup, title-token filtering, and selection-code helpers in that module, so future object-level replacement work can land there without another root include-graph edit.
...
- Current hybrid-integration note (March 13, 2026): `modules/groups/a/y/locavail.s` is now wired into `src/decomp/replacements.map` as a passthrough hybrid boundary. Its restored SAS/C coverage already spans the current non-`JMPTBL` local-availability filter helpers in that module, so future object-level replacement work can land there without another root include-graph edit.
...
- Current hybrid-integration note (March 13, 2026, later re-validated): `modules/groups/b/a/tliba1.s` is now wired into `src/decomp/replacements.map` as a passthrough hybrid boundary. Its restored SAS/C coverage already spans the current non-`JMPTBL` formatted-text, clock-format, and style parsing helpers in that module, so future object-level replacement work can land there without another root include-graph edit.
- Current hybrid-integration note (March 13, 2026, later re-validated): `modules/groups/b/a/tliba3.s` is now wired into `src/decomp/replacements.map` as a passthrough hybrid boundary. Its restored SAS/C coverage already spans the current non-`JMPTBL` view-mode, pattern-table, and scale/overlay helper cluster in that module, so future object-level replacement work can land there without another root include-graph edit.
- Current hybrid-integration note (March 13, 2026, later re-validated): `modules/groups/b/a/newgrid1.s` is now wired into `src/decomp/replacements.map` as a passthrough hybrid boundary. Its restored SAS/C coverage already spans the current non-`JMPTBL` grid drawing, entry scan, and schedule-state helper cluster in that module, so future object-level replacement work can land there without another root include-graph edit.
...
- Current hybrid-integration note (March 13, 2026): `modules/groups/b/a/textdisp3.s` is now wired into `src/decomp/replacements.map` as a passthrough hybrid boundary. Its restored SAS/C coverage already spans an initial non-`JMPTBL` text/layout helper slice in that module, so future object-level replacement work can land there without another root include-graph edit.
- Current hybrid-integration note (March 14, 2026): `modules/groups/a/j/disptext2.s` is now wired into `src/decomp/replacements.map` as a passthrough hybrid boundary. Its restored SAS/C coverage already spans the DATETIME conversion/format helper cluster in that module, so future object-level replacement work can land there without another root include-graph edit.
...
- Current hybrid-integration note (March 13, 2026): `modules/submodules/unknown15.s` is now wired into `src/decomp/replacements.map` as a passthrough hybrid boundary. Its restored SAS/C coverage already spans `STREAM_ReadLineWithLimit`, so future object-level replacement work can land in that stream line-reader helper module without another root include-graph edit.
- Current hybrid-integration note (March 13, 2026, later re-validated): `modules/submodules/unknown5.s` is now wired into `src/decomp/replacements.map` as a passthrough hybrid boundary. Its restored SAS/C coverage already spans the current non-`JMPTBL` string helper cluster (`STRING_AppendN`, `STRING_CompareN`, `STRING_CompareNoCase`, `STRING_CompareNoCaseN`, and `STRING_CopyPadNul`), so future object-level replacement work can land there without another root include-graph edit.
- Current hybrid-integration note (March 13, 2026, later re-validated): `modules/submodules/unknown6.s`, `unknown8.s`, `unknown9.s`, `unknown12.s`, `unknown16.s`-`unknown20.s`, `unknown23.s`, `unknown28.s`, `unknown31.s`, `unknown35.s`, `unknown37.s`, and `unknown39.s` are now wired into `src/decomp/replacements.map` as passthrough hybrid boundaries. Their restored SAS/C coverage already spans the current single-helper footholds for `STRING_AppendAtNull`, the decimal/octal formatters, `ALLOC_AllocFromFreeList`, the buffered close and `DOS_*WithErrorState` wrappers, `HANDLE_GetEntryByIndex`, `WDISP_FormatWithCallback`, `BUFFER_EnsureAllocated`, `HANDLE_OpenWithMode`, `HANDLE_CloseByIndex`, and `GRAPHICS_BltBitMapRastPort`, so future object-level replacement work can land in those helper modules without another root include-graph edit.
...
- Current hybrid-integration note (March 13, 2026): `modules/submodules/unknown22.s` is now wired into `src/decomp/replacements.map` as a passthrough hybrid boundary. Its restored SAS/C coverage already spans the current non-`JMPTBL` arithmetic/msgport helper cluster (`ALLOCATE_AllocAndInitializeIOStdReq`, `DOS_CloseWithSignalCheck`, `MATH_DivS32`, `MATH_DivU32`, `MATH_Mulu32`, and `SIGNAL_CreateMsgPortWithSignal`), so future object-level replacement work can land there without another root include-graph edit.
- Current hybrid-integration note (March 13, 2026): `modules/submodules/unknown24.s` is now wired into `src/decomp/replacements.map` as a passthrough hybrid boundary. Its restored SAS/C coverage already spans the current non-`JMPTBL` memlist/signed-long helper cluster (`MEMLIST_AllocTracked`, `MEMLIST_FreeAll`, `PARSE_ReadSignedLongSkipClass3`, and `PARSE_ReadSignedLongSkipClass3_Alt`), so future object-level replacement work can land there without another root include-graph edit.
