...
cd "$ROOT_DIR"
SASC_SRC="group_ai_jmptbl_stubs.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/a/i/xjump.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="group_ai_jmptbl_tliba1_draw_text_with_inset_segments"
ENTRY_ORIG="GROUP_AI_JMPTBL_TLIBA1_DrawTextWithInsetSegments"
ENTRY_SASC="GROUP_AI_JMPTBL_TLIBA1_DrawTextW"
TARGET="TLIBA1_DrawTextWithInsetSegments"
mkdir -p "$OUT_DIR"
./sc-build-with-dis.sh "$SASC_SRC" >"${OUT_DIR}/sc_build_${BASE}.log" 2>&1
awk -v e="^${ENTRY_ORIG}:$" '$0 ~ e {inf=1} inf { if ($0 ~ /^GROUP_AI_JMPTBL_FORMAT_FormatToBuffer2:$/) exit; print }' "$ORIG_ASM" >"${OUT_DIR}/${BASE}.original.s"
awk -v e="^${ENTRY_SASC}:$" '$0 ~ e {inf=1} inf { if ($0 ~ /^GROUP_AI_JMPTBL_FORMAT_FormatToB:$/) exit; print }' "$SASC_DIS" >"${OUT_DIR}/${BASE}.sasc.dis.s"
