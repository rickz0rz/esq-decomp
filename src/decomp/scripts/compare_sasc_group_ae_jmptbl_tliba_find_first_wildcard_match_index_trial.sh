...
BASE="group_ae_jmptbl_tliba_find_first_wildcard_match_index"
ENTRY_ORIG="GROUP_AE_JMPTBL_TLIBA_FindFirstWildcardMatchIndex"
ENTRY_SASC="GROUP_AE_JMPTBL_TLIBA_FindFirstW"
TARGET="TLIBA_FindFirstWildcardMatchInde"
mkdir -p "$OUT_DIR"
./sc-build-with-dis.sh "$SASC_SRC" >"${OUT_DIR}/sc_build_${BASE}.log" 2>&1
awk -v e="^${ENTRY_ORIG}:$" '$0 ~ e {inf=1} inf { if ($0 ~ /^GROUP_AE_JMPTBL_SCRIPT_BuildTokenIndexMap:$/) exit; print }' "$ORIG_ASM" >"${OUT_DIR}/${BASE}.original.s"
awk -v e="^${ENTRY_SASC}:$" '$0 ~ e {inf=1} inf { if ($0 ~ /^GROUP_AE_JMPTBL_SCRIPT_BuildToke:$/) exit; print }' "$SASC_DIS" >"${OUT_DIR}/${BASE}.sasc.dis.s"
