...
BASE="script3_handle_serial_ctrl_cmd_state_flow"
ENTRY="SCRIPT_HandleSerialCtrlCmd"
ENTRY_SASC_REGEX="^SCRIPT_HandleSerialCtrlC[A-Za-z0-9_]*:$"

mkdir -p "$OUT_DIR"

./sc-build-with-dis.sh "$SASC_SRC" >"${OUT_DIR}/sc_build_${BASE}.log" 2>&1

awk -v start="^${ENTRY}:$" -v next_label="^SCRIPT_HandleBrushCommand:$" '
    $0 ~ start { in_func=1 }
    in_func {
...
awk -f src/decomp/scripts/semantic_filter_sasc_script3_handle_serial_ctrl_cmd_state_flow.awk \
