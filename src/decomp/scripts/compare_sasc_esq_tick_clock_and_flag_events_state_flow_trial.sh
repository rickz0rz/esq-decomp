...
BASE="esq_tick_clock_and_flag_events_state_flow"
...
awk -v e="^${ENTRY}:$" -v e2="$ENTRY_SASC_REGEX" '
    $0 ~ e || $0 ~ e2 { inf=1 }
    inf {
        if (($0 ~ /^ESQ_[A-Za-z0-9_]+:$/ || $0 ~ /^_?ESQ_[A-Za-z0-9_]+:$/) && $0 !~ e && $0 !~ e2) exit
...
awk -f src/decomp/scripts/semantic_filter_sasc_esq_tick_clock_and_flag_events_state_flow.awk "${OUT_DIR}/${BASE}.original.norm.s" >"${OUT_DIR}/${BASE}.original.semantic.txt"
awk -f src/decomp/scripts/semantic_filter_sasc_esq_tick_clock_and_flag_events_state_flow.awk "${OUT_DIR}/${BASE}.sasc.norm.s" >"${OUT_DIR}/${BASE}.sasc.semantic.txt"
