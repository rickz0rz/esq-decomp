...
SRC_C="src/decomp/c/replacements/string_append_at_null_gcc.c"
ORIG_ASM="src/modules/submodules/unknown6.s"
OUT_DIR="build/decomp/c_trial_gcc"
GEN_ASM="${OUT_DIR}/string_append_at_null.generated.gcc.s"
ORIG_SLICE="${OUT_DIR}/string_append_at_null.original_slice.s"
GEN_SLICE="${OUT_DIR}/string_append_at_null.generated_slice.s"
ORIG_NORM="${OUT_DIR}/string_append_at_null.original_slice.norm.s"
GEN_NORM="${OUT_DIR}/string_append_at_null.generated_slice.norm.s"
...
awk '
$0 ~ /^STRING_AppendAtNull:?$/ {in_func=1}
in_func {
    if ($0 ~ /^;!======/) {
        exit
    }
...
awk '
$0 ~ /^_?STRING_AppendAtNull:?$/ {in_func=1}
in_func { print }
' "$GEN_ASM" > "$GEN_SLICE"
...
awk -f src/decomp/scripts/semantic_filter_str_append.awk "$ORIG_NORM" > "$ORIG_SEM"
awk -f src/decomp/scripts/semantic_filter_str_append.awk "$GEN_NORM" > "$GEN_SEM"
