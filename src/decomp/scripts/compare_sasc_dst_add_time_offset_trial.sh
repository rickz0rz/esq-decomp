...
awk '$0 ~ /^DST_AddTimeOffset:$/ {in_func=1} in_func { if ($0 ~ /^;!======/) exit; print }' "$ORIG_ASM" >"${OUT_DIR}/dst_add_time_offset.original.s"
awk '$0 ~ /^[[:space:]]*DST_AddTimeOffset:/ {in_func=1} in_func { if ($0 ~ /^[[:space:]]*__const:$/) exit; print }' "$SASC_DIS" >"${OUT_DIR}/dst_add_time_offset.sasc.dis.s"
