...
awk '$0 ~ /^DATETIME_NormalizeMonthRange:$/ {in_func=1} in_func { if ($0 ~ /^;!======/) exit; print }' "$ORIG_ASM" >"${OUT_DIR}/datetime_normalize_month_range.original.s"
awk '$0 ~ /^[[:space:]]*DATETIME_NormalizeMonthRange:/ {in_func=1} in_func { if ($0 ~ /^[[:space:]]*__const:$/) exit; print }' "$SASC_DIS" >"${OUT_DIR}/datetime_normalize_month_range.sasc.dis.s"
