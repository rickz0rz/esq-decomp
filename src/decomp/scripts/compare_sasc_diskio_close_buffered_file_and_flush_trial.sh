...
SASC_SRC="diskio_close_buffered_file_and_flush.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/a/g/diskio.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="diskio_close_buffered_file_and_flush"
ENTRY="DISKIO_CloseBufferedFileAndFlush"
...
awk -v e="^${ENTRY}:$" '
  $0 ~ e {in_func=1}
  in_func {
    if ($0 ~ /^DISKIO_CloseBufferedFileAndFlush_Return:/) exit
    if ($0 ~ /^;!======/) exit
    print
  }
...
awk -f src/decomp/scripts/semantic_filter_sasc_diskio_close_buffered_file_and_flush.awk "${OUT_DIR}/${BASE}.original.norm.s" >"${OUT_DIR}/${BASE}.original.semantic.txt"
awk -f src/decomp/scripts/semantic_filter_sasc_diskio_close_buffered_file_and_flush.awk "${OUT_DIR}/${BASE}.sasc.norm.s" >"${OUT_DIR}/${BASE}.sasc.semantic.txt"
