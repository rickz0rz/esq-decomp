...
BASE="group_at_jmptbl_dos_system_taglist"
ENTRY_ORIG="GROUP_AT_JMPTBL_DOS_SystemTagList"
ENTRY_SASC="GROUP_AT_JMPTBL_DOS_SystemTagLis"
TARGET="DOS_SystemTagList"

mkdir -p "$OUT_DIR"
./sc-build-with-dis.sh "$SASC_SRC" >"${OUT_DIR}/sc_build_${BASE}.log" 2>&1

awk -v e="^${ENTRY_ORIG}:$" '$0 ~ e {inf=1} inf { if ($0 ~ /^GROUP_AT_JMPTBL_ED1_WaitForFlagAndClearBit1:$/) exit; print }' "$ORIG_ASM" >"${OUT_DIR}/${BASE}.original.s"
