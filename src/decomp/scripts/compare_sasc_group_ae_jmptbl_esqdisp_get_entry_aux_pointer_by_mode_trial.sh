...
cd "$ROOT_DIR"
SASC_SRC="group_ae_jmptbl_stubs_core.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/a/e/xjump.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="group_ae_jmptbl_esqdisp_get_entry_aux_pointer_by_mode"
ENTRY_ORIG="GROUP_AE_JMPTBL_ESQDISP_GetEntryAuxPointerByMode"
ENTRY_SASC="GROUP_AE_JMPTBL_ESQDISP_GetEntry"
TARGET="ESQDISP_GetEntryAuxPointerByMode"
mkdir -p "$OUT_DIR"
./sc-build-with-dis.sh "$SASC_SRC" >"${OUT_DIR}/sc_build_${BASE}.log" 2>&1
awk -v e="^${ENTRY_ORIG}:$" '$0 ~ e {inf=1} inf { if ($0 ~ /^GROUP_AE_JMPTBL_ESQDISP_GetEntryPointerByMode:$/) exit; print }' "$ORIG_ASM" >"${OUT_DIR}/${BASE}.original.s"
awk -v e="^${ENTRY_SASC}:$" '$0 ~ e {inf=1} inf { if ($0 ~ /^GROUP_AE_JMPTBL_LADFUNC_ParseHex:$/) exit; print }' "$SASC_DIS" >"${OUT_DIR}/${BASE}.sasc.dis.s"
