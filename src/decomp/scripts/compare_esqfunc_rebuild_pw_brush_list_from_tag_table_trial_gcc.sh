...
SRC_C="src/decomp/c/replacements/esqfunc_rebuild_pw_brush_list_from_tag_table_gcc.c"
ORIG_ASM="src/modules/groups/a/n/esqfunc.s"
OUT_DIR="build/decomp/c_trial_gcc"
GEN_ASM="${OUT_DIR}/esqfunc_rebuild_pw_brush_list_from_tag_table.generated.gcc.s"
ORIG_SLICE="${OUT_DIR}/esqfunc_rebuild_pw_brush_list_from_tag_table.original_slice.s"
GEN_SLICE="${OUT_DIR}/esqfunc_rebuild_pw_brush_list_from_tag_table.generated_slice.s"
ORIG_NORM="${OUT_DIR}/esqfunc_rebuild_pw_brush_list_from_tag_table.original_slice.norm.s"
GEN_NORM="${OUT_DIR}/esqfunc_rebuild_pw_brush_list_from_tag_table.generated_slice.norm.s"
...
awk '$0 ~ /^ESQFUNC_RebuildPwBrushListFromTagTable:?$/ {in_func=1} in_func { if ($0 ~ /^;!======/) exit; print }' "$ORIG_ASM" > "$ORIG_SLICE"
awk '$0 ~ /^_?ESQFUNC_RebuildPwBrushListFromTagTable:?$/ {in_func=1} in_func { print }' "$GEN_ASM" > "$GEN_SLICE"
...
awk -f src/decomp/scripts/semantic_filter_esqfunc_rebuild_pw_brush_list_from_tag_table.awk "$ORIG_NORM" > "$ORIG_SEM"
awk -f src/decomp/scripts/semantic_filter_esqfunc_rebuild_pw_brush_list_from_tag_table.awk "$GEN_NORM" > "$GEN_SEM"
