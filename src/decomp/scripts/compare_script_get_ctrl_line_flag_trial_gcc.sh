...
SRC_C="src/decomp/c/replacements/script_get_ctrl_line_flag_gcc.c"
ORIG_ASM="src/modules/groups/b/a/script2.s"
OUT_DIR="build/decomp/c_trial_gcc"
GEN_ASM="${OUT_DIR}/script_get_ctrl_line_flag.generated.gcc.s"
ORIG_SLICE="${OUT_DIR}/script_get_ctrl_line_flag.original_slice.s"
GEN_SLICE="${OUT_DIR}/script_get_ctrl_line_flag.generated_slice.s"
ORIG_NORM="${OUT_DIR}/script_get_ctrl_line_flag.original_slice.norm.s"
GEN_NORM="${OUT_DIR}/script_get_ctrl_line_flag.generated_slice.norm.s"
...
awk '
$0 ~ /^SCRIPT_GetCtrlLineFlag:?$/ {in_func=1}
in_func {
    if ($0 ~ /^;!======/) {
        exit
    }
...
awk '
$0 ~ /^_?SCRIPT_GetCtrlLineFlag:?$/ {in_func=1}
in_func { print }
' "$GEN_ASM" > "$GEN_SLICE"
...
awk -f src/decomp/scripts/semantic_filter_script_get_ctrl_line_flag.awk "$ORIG_NORM" > "$ORIG_SEM"
awk -f src/decomp/scripts/semantic_filter_script_get_ctrl_line_flag.awk "$GEN_NORM" > "$GEN_SEM"
