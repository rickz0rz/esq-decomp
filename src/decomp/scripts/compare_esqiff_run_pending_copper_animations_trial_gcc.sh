...
SRC_C="src/decomp/c/replacements/esqiff_run_pending_copper_animations_gcc.c"
ORIG_ASM="src/modules/groups/a/n/esqiff.s"
OUT_DIR="build/decomp/c_trial_gcc"
GEN_ASM="${OUT_DIR}/esqiff_run_pending_copper_animations.generated.gcc.s"
ORIG_SLICE="${OUT_DIR}/esqiff_run_pending_copper_animations.original_slice.s"
GEN_SLICE="${OUT_DIR}/esqiff_run_pending_copper_animations.generated_slice.s"
ORIG_NORM="${OUT_DIR}/esqiff_run_pending_copper_animations.original_slice.norm.s"
GEN_NORM="${OUT_DIR}/esqiff_run_pending_copper_animations.generated_slice.norm.s"
...
awk '$0 ~ /^ESQIFF_RunPendingCopperAnimations:?$/ {in_func=1} in_func { if ($0 ~ /^;!======/) exit; print }' "$ORIG_ASM" > "$ORIG_SLICE"
awk '$0 ~ /^_?ESQIFF_RunPendingCopperAnimations:?$/ {in_func=1} in_func { print }' "$GEN_ASM" > "$GEN_SLICE"
...
awk -f src/decomp/scripts/semantic_filter_esqiff_run_pending_copper_animations.awk "$ORIG_NORM" > "$ORIG_SEM"
awk -f src/decomp/scripts/semantic_filter_esqiff_run_pending_copper_animations.awk "$GEN_NORM" > "$GEN_SEM"
