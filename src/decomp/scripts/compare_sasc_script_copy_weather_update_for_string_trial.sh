...
SASC_SRC="script_copy_weather_update_text.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/b/a/script3.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="script_copy_weather_update_for_string"
ENTRY="SCRIPT_CopyWeatherUpdateForString"
ENTRY_SASC_REGEX="^SCRIPT_CopyWeatherUpdateForStrin[A-Za-z0-9_]*:$"

mkdir -p "$OUT_DIR"

./sc-build-with-dis.sh "$SASC_SRC" >"${OUT_DIR}/sc_build_${BASE}.log" 2>&1

awk -v e="^; FUNC: ${ENTRY} " '$0 ~ e {in_func=1} in_func { if ($0 ~ /^;!======/) exit; print }' \
    "$ORIG_ASM" >"${OUT_DIR}/${BASE}.original.s"

awk -v e="$ENTRY_SASC_REGEX" '
    $0 ~ e { in_func=1 }
    in_func {
        if (($0 ~ /^SCRIPT_[A-Za-z0-9_]+:$/ || $0 ~ /^_?SCRIPT_[A-Za-z0-9_]+:$/) && $0 !~ e) exit
...
awk -f src/decomp/scripts/semantic_filter_sasc_script_copy_weather_update_for_string.awk \
