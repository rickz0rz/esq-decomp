...
cd "$ROOT_DIR"
SASC_SRC="parseini2_jmptbl_stubs.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/b/a/parseini2.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="parseini2_jmptbl_battclock_getsecondsfrombatterybackedclock"
ENTRY_ORIG="PARSEINI2_JMPTBL_BATTCLOCK_GetSecondsFromBatteryBackedClock"
ENTRY_SASC="PARSEINI2_JMPTBL_BATTCLOCK_GetSe"
TARGET="BATTCLOCK_GetSecondsFromBatteryBackedClock"
mkdir -p "$OUT_DIR"
./sc-build-with-dis.sh "$SASC_SRC" >"${OUT_DIR}/sc_build_${BASE}.log" 2>&1
awk -v e="^${ENTRY_ORIG}:$" '$0 ~ e {inf=1} inf { if ($0 ~ /^PARSEINI2_JMPTBL_/ && $0 !~ e) exit; if ($0 ~ /^;!======/) exit; print }' "$ORIG_ASM" >"${OUT_DIR}/${BASE}.original.s"
