...
SRC_C="src/decomp/c/replacements/group_ab_jmptbl_unknown2_a_stub0_gcc.c"
ORIG_ASM="src/modules/groups/a/b/xjump.s"
OUT_DIR="build/decomp/c_trial_gcc"
GEN_ASM="${OUT_DIR}/group_ab_jmptbl_unknown2_a_stub0.generated.gcc.s"
ORIG_SLICE="${OUT_DIR}/group_ab_jmptbl_unknown2_a_stub0.original_slice.s"
GEN_SLICE="${OUT_DIR}/group_ab_jmptbl_unknown2_a_stub0.generated_slice.s"
ORIG_NORM="${OUT_DIR}/group_ab_jmptbl_unknown2_a_stub0.original_slice.norm.s"
GEN_NORM="${OUT_DIR}/group_ab_jmptbl_unknown2_a_stub0.generated_slice.norm.s"
...
awk '
$0 ~ /^GROUP_AB_JMPTBL_UNKNOWN2A_Stub0:?$/ {in_func=1}
in_func {
    if ($0 ~ /^GROUP_AB_JMPTBL_NEWGRID_ShutdownGridResources:?$/) {
        exit
    }
    print
}
...
awk '
$0 ~ /^_?GROUP_AB_JMPTBL_UNKNOWN2A_Stub0:?$/ {in_func=1}
in_func { print }
' "$GEN_ASM" > "$GEN_SLICE"
...
awk -f "src/decomp/scripts/semantic_filter_group_ab_jmptbl_unknown2_a_stub0.awk" "$ORIG_NORM" > "$ORIG_SEM"
awk -f "src/decomp/scripts/semantic_filter_group_ab_jmptbl_unknown2_a_stub0.awk" "$GEN_NORM" > "$GEN_SEM"
