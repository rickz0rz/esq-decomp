...
SASC_SRC="diskio1_dump_program_info_attr_table.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/a/g/diskio1.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="diskio1_dump_program_info_attr_table"
ENTRY="DISKIO1_DumpProgramInfoAttrTable"
ENTRY_SASC_REGEX="^DISKIO1_DumpProgramInfoAttrTab[A-Za-z0-9_]*:$"
ENTRY_SEM=""
ENTRY_ALT_SEM=""
...
awk '
  $0 ~ /^    LINK\.W  A5,#-48$/ {in_func=1}
  in_func {
    if ($0 ~ /^DISKIO1_DumpProgramInfoAttrTable_Return:/) exit
    print
  }
...
awk -v ENTRY_PREFIX="$ENTRY_SEM" -v ENTRY_ALT_PREFIX="$ENTRY_ALT_SEM" -f src/decomp/scripts/semantic_filter_sasc_diskio1_dump_program_info_attr_table.awk "${OUT_DIR}/${BASE}.original.norm.s" >"${OUT_DIR}/${BASE}.original.semantic.txt"
awk -v ENTRY_PREFIX="$ENTRY_SEM" -v ENTRY_ALT_PREFIX="$ENTRY_ALT_SEM" -f src/decomp/scripts/semantic_filter_sasc_diskio1_dump_program_info_attr_table.awk "${OUT_DIR}/${BASE}.sasc.norm.s" >"${OUT_DIR}/${BASE}.sasc.semantic.txt"
