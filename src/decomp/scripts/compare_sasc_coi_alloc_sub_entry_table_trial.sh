...
cd "$ROOT_DIR"
SASC_SRC="coi_alloc_sub_entry_table.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/a/e/coi.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="coi_alloc_sub_entry_table"
ENTRY="COI_AllocSubEntryTable"
ENTRY_SASC_REGEX="^COI_AllocSubEntryTable[A-Za-z0-9_]*:$"
mkdir -p "$OUT_DIR"
...
awk -f src/decomp/scripts/semantic_filter_sasc_coi_alloc_sub_entry_table.awk "${OUT_DIR}/${BASE}.original.norm.s" >"${OUT_DIR}/${BASE}.original.semantic.txt"
awk -f src/decomp/scripts/semantic_filter_sasc_coi_alloc_sub_entry_table.awk "${OUT_DIR}/${BASE}.sasc.norm.s" >"${OUT_DIR}/${BASE}.sasc.semantic.txt"
