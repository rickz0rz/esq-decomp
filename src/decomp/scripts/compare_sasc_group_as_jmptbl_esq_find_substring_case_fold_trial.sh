...
BASE="group_as_jmptbl_esq_find_substring_case_fold"
ENTRY_ORIG="GROUP_AS_JMPTBL_ESQ_FindSubstringCaseFold"
ENTRY_SASC="GROUP_AS_JMPTBL_ESQ_FindSubstrin"
TARGET="ESQ_FindSubstringCaseFold"

mkdir -p "$OUT_DIR"
./sc-build-with-dis.sh "$SASC_SRC" >"${OUT_DIR}/sc_build_${BASE}.log" 2>&1

awk -v e="^${ENTRY_ORIG}:$" '$0 ~ e {inf=1} inf { print }' "$ORIG_ASM" >"${OUT_DIR}/${BASE}.original.s"
