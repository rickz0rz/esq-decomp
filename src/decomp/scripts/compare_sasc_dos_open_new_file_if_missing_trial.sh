...
awk '$0 ~ /^DOS_OpenNewFileIfMissing:$/ {in_func=1} in_func { if ($0 ~ /^;!======/) exit; print }' "$ORIG_ASM" >"${OUT_DIR}/dos_open_new_file_if_missing.original.s"
awk '$0 ~ /^DOS_OpenNewFileIfMissing:$/ {in_func=1} in_func { if ($0 ~ /^DOS_DeleteAndRecreateFile:$/) exit; print }' "$SASC_DIS" >"${OUT_DIR}/dos_open_new_file_if_missing.sasc.dis.s"
