...
LONG NEWGRID_DrawGridFrameAndRows(char *ctx, LONG rowColorIndex)
{
    ...
    rp = &ctxView->rastPort;
    _LVOSetAPen(Global_REF_GRAPHICS_LIBRARY, (char *)rp, NEWGRID_SetRowColor(ctx, 0, rowColorIndex));
    rowHeight = (LONG)NEWGRID_RowHeightPx;
    _LVORectFill(Global_REF_GRAPHICS_LIBRARY, (char *)rp, 0, 0, 695, rowHeight + 3);

    x = 42;
    yBase = 0;

    if (DISPTEXT_GetTotalLineCount() - 1 == 0) {
        LONG textWidth;
        LONG pad;

        textWidth = DISPTEXT_MeasureCurrentLineLength((char *)rp);
        pad = 612 - textWidth;
