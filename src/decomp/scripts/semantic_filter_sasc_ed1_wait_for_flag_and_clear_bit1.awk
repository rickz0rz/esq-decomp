...

{
    ...
    if (l ~ /^ANDI\.[WL] #\$?FFFD,D0$/ || l ~ /^AND\.[WL] #\$?FFFD,D0$/ || l ~ /^AND\.[WL] #\$?FFFD,ESQIFF_EXTERNALASSETFLAGS(\(A[0-7]\))?$/) {
        has_mask = 1
    }

    if (l ~ /(JSR|BSR).*ESQIFF_RELOADEXTERNALASSETCATALO/) has_call = 1

    if (l ~ /^CLR\.L -\(A7\)$/ || l ~ /^MOVEQ(\.L)? #\$?0,D[0-7]$/ || l ~ /^PEA 0\.W$/) has_arg0 = 1
