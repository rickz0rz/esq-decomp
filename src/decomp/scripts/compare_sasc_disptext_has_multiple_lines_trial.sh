...
BASE="disptext_has_multiple_lines"
ENTRY="DISPTEXT_HasMultipleLines"
ENTRY_SASC_REGEX="^DISPTEXT_HasMultipleLines[A-Za-z0-9_]*:$"
ENTRY_SEM="DISPTEXT_HASMULTIPLELINES:"
ENTRY_ALT_SEM="DISPTEXT_HASMULTIPLELINE"
...
awk -v ENTRY_PREFIX="$ENTRY_SEM" -v ENTRY_ALT_PREFIX="$ENTRY_ALT_SEM" -f src/decomp/scripts/semantic_filter_sasc_disptext_has_multiple_lines.awk "${OUT_DIR}/${BASE}.original.norm.s" >"${OUT_DIR}/${BASE}.original.semantic.txt"
awk -v ENTRY_PREFIX="$ENTRY_SEM" -v ENTRY_ALT_PREFIX="$ENTRY_ALT_SEM" -f src/decomp/scripts/semantic_filter_sasc_disptext_has_multiple_lines.awk "${OUT_DIR}/${BASE}.sasc.norm.s" >"${OUT_DIR}/${BASE}.sasc.semantic.txt"
