...
ENTRY="STRING_CopyPadNul"

mkdir -p "$OUT_DIR"

./sc-build-with-dis.sh "$SASC_SRC" >"${OUT_DIR}/sc_build_string_copy_pad_nul.log" 2>&1

awk -v e="^${ENTRY}:$" '$0 ~ e {in_func=1} in_func { if ($0 ~ /^;!======/) exit; print }' "$ORIG_ASM" >"${OUT_DIR}/string_copy_pad_nul.original.s"
awk -v e="^${ENTRY}:$" '$0 ~ e {in_func=1} in_func { if ($0 ~ /^STRING_CompareN:$/) exit; print }' "$SASC_DIS" >"${OUT_DIR}/string_copy_pad_nul.sasc.dis.s"
