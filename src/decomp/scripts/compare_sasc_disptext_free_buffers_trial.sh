...
BASE="disptext_free_buffers"
ENTRY="DISPTEXT_FreeBuffers"
ENTRY_SASC_REGEX="^DISPTEXT_FreeBuffers[A-Za-z0-9_]*:$"
ENTRY_SEM="DISPTEXT_FREEBUFFERS:"
ENTRY_ALT_SEM="DISPTEXT_FREEBUFFER"
...
awk -v ENTRY_PREFIX="$ENTRY_SEM" -v ENTRY_ALT_PREFIX="$ENTRY_ALT_SEM" -f src/decomp/scripts/semantic_filter_sasc_disptext_free_buffers.awk "${OUT_DIR}/${BASE}.original.norm.s" >"${OUT_DIR}/${BASE}.original.semantic.txt"
awk -v ENTRY_PREFIX="$ENTRY_SEM" -v ENTRY_ALT_PREFIX="$ENTRY_ALT_SEM" -f src/decomp/scripts/semantic_filter_sasc_disptext_free_buffers.awk "${OUT_DIR}/${BASE}.sasc.norm.s" >"${OUT_DIR}/${BASE}.sasc.semantic.txt"
