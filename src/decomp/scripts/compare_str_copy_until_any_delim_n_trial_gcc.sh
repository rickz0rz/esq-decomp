...
SRC_C="src/decomp/c/replacements/str_copy_until_any_delim_n_gcc.c"
ORIG_ASM="src/modules/submodules/unknown7.s"
OUT_DIR="build/decomp/c_trial_gcc"
GEN_ASM="${OUT_DIR}/str_copy_until_any_delim_n.generated.gcc.s"
ORIG_SLICE="${OUT_DIR}/str_copy_until_any_delim_n.original_slice.s"
GEN_SLICE="${OUT_DIR}/str_copy_until_any_delim_n.generated_slice.s"
ORIG_NORM="${OUT_DIR}/str_copy_until_any_delim_n.original_slice.norm.s"
GEN_NORM="${OUT_DIR}/str_copy_until_any_delim_n.generated_slice.norm.s"
...
awk '
$0 ~ /^STR_CopyUntilAnyDelimN:?$/ {in_func=1}
in_func {
    if ($0 ~ /^;!======/) {
        exit
    }
...
awk '
$0 ~ /^_?STR_CopyUntilAnyDelimN:?$/ {in_func=1}
in_func { print }
' "$GEN_ASM" > "$GEN_SLICE"
...
awk -f src/decomp/scripts/semantic_filter_str_copy_until_any_delim_n.awk "$ORIG_NORM" > "$ORIG_SEM"
awk -f src/decomp/scripts/semantic_filter_str_copy_until_any_delim_n.awk "$GEN_NORM" > "$GEN_SEM"
