...
SASC_SRC="esqiff_play_next_external_asset_frame.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/a/n/esqiff.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="esqiff_play_next_external_asset_frame"
ENTRY="ESQIFF_PlayNextExternalAssetFrame"
ENTRY_SASC_REGEX="^ESQIFF_PlayNextExternalAssetFram[A-Za-z0-9_]*:$"
...
awk -v e="^${ENTRY}:$" '
  $0 ~ e {in_func=1}
  in_func {
    if ($0 ~ /^; FUNC: ESQIFF_PlayNextExternalAssetFrame_Return/) exit
    if ($0 ~ /^;!======/) exit
    print
  }
...
awk -f src/decomp/scripts/semantic_filter_sasc_esqiff_play_next_external_asset_frame.awk "${OUT_DIR}/${BASE}.original.norm.s" >"${OUT_DIR}/${BASE}.original.semantic.txt"
awk -f src/decomp/scripts/semantic_filter_sasc_esqiff_play_next_external_asset_frame.awk "${OUT_DIR}/${BASE}.sasc.norm.s" >"${OUT_DIR}/${BASE}.sasc.semantic.txt"
