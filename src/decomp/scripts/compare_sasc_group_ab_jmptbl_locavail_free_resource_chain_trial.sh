...
BASE="group_ab_jmptbl_locavail_free_resource_chain"
ENTRY_ORIG="GROUP_AB_JMPTBL_LOCAVAIL_FreeResourceChain"
ENTRY_SASC="GROUP_AB_JMPTBL_LOCAVAIL_FreeRes"
TARGET="LOCAVAIL_FreeResourceChain"
mkdir -p "$OUT_DIR"
./sc-build-with-dis.sh "$SASC_SRC" >"${OUT_DIR}/sc_build_${BASE}.log" 2>&1
awk -v e="^${ENTRY_ORIG}:$" '$0 ~ e {inf=1} inf { if ($0 ~ /^GROUP_AB_JMPTBL_GRAPHICS_FreeRaster:$/) exit; print }' "$ORIG_ASM" >"${OUT_DIR}/${BASE}.original.s"
awk -v e="^${ENTRY_SASC}:$" '$0 ~ e {inf=1} inf { if ($0 ~ /^GROUP_AB_JMPTBL_GRAPHICS_FreeRas:$/) exit; print }' "$SASC_DIS" >"${OUT_DIR}/${BASE}.sasc.dis.s"
