...
SRC_C="src/decomp/c/replacements/esqiff_jmptbl_brush_populate_brush_list_gcc.c"
ORIG_ASM="src/modules/groups/a/n/esqiff.s"
OUT_DIR="build/decomp/c_trial_gcc"
GEN_ASM="${OUT_DIR}/esqiff_jmptbl_brush_populate_brush_list.generated.gcc.s"
ORIG_SLICE="${OUT_DIR}/esqiff_jmptbl_brush_populate_brush_list.original_slice.s"
GEN_SLICE="${OUT_DIR}/esqiff_jmptbl_brush_populate_brush_list.generated_slice.s"
ORIG_NORM="${OUT_DIR}/esqiff_jmptbl_brush_populate_brush_list.original_slice.norm.s"
GEN_NORM="${OUT_DIR}/esqiff_jmptbl_brush_populate_brush_list.generated_slice.norm.s"
...
awk '
$0 ~ /^ESQIFF_JMPTBL_BRUSH_PopulateBrushList:?$/ {in_func=1}
in_func {
    if ($0 ~ /^ESQIFF_JMPTBL_ESQ_NoOp_0074:?$/) {
        exit
    }
    print
}
...
awk '
$0 ~ /^_?ESQIFF_JMPTBL_BRUSH_PopulateBrushList:?$/ {in_func=1}
in_func { print }
' "$GEN_ASM" > "$GEN_SLICE"
...
awk -f src/decomp/scripts/semantic_filter_esqiff_jmptbl_brush_populate_brush_list.awk "$ORIG_NORM" > "$ORIG_SEM"
awk -f src/decomp/scripts/semantic_filter_esqiff_jmptbl_brush_populate_brush_list.awk "$GEN_NORM" > "$GEN_SEM"
