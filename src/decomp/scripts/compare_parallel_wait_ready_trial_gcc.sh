...
SRC_C="src/decomp/c/replacements/parallel_wait_ready_gcc.c"
ORIG_ASM="src/modules/submodules/unknown42.s"
OUT_DIR="build/decomp/c_trial_gcc"
GEN_ASM="${OUT_DIR}/parallel_wait_ready.generated.gcc.s"
ORIG_SLICE="${OUT_DIR}/parallel_wait_ready.original_slice.s"
GEN_SLICE="${OUT_DIR}/parallel_wait_ready.generated_slice.s"
ORIG_NORM="${OUT_DIR}/parallel_wait_ready.original_slice.norm.s"
GEN_NORM="${OUT_DIR}/parallel_wait_ready.generated_slice.norm.s"
...
awk '$0 ~ /^PARALLEL_WaitReady:?$/ {in_func=1} in_func { if ($0 ~ /^;!======/) exit; print }' "$ORIG_ASM" > "$ORIG_SLICE"
awk '$0 ~ /^_?PARALLEL_WaitReady:?$/ {in_func=1} in_func { print }' "$GEN_ASM" > "$GEN_SLICE"
...
awk -f src/decomp/scripts/semantic_filter_parallel_wait_ready.awk "$ORIG_NORM" > "$ORIG_SEM"
awk -f src/decomp/scripts/semantic_filter_parallel_wait_ready.awk "$GEN_NORM" > "$GEN_SEM"
