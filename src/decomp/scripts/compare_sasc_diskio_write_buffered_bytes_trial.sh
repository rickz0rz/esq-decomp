...
SASC_SRC="diskio_write_buffered_bytes.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/a/g/diskio.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="diskio_write_buffered_bytes"
ENTRY="DISKIO_WriteBufferedBytes"
ENTRY_SASC_REGEX="^DISKIO_WriteBufferedBytes[A-Za-z0-9_]*:$"
...
awk -v e="^${ENTRY}:$" '
  $0 ~ e {in_func=1}
  in_func {
    if ($0 ~ /^DISKIO_WriteBufferedBytes_Return:/) exit
    if ($0 ~ /^;!======/) exit
    print
  }
...
awk -f src/decomp/scripts/semantic_filter_sasc_diskio_write_buffered_bytes.awk "${OUT_DIR}/${BASE}.original.norm.s" >"${OUT_DIR}/${BASE}.original.semantic.txt"
awk -f src/decomp/scripts/semantic_filter_sasc_diskio_write_buffered_bytes.awk "${OUT_DIR}/${BASE}.sasc.norm.s" >"${OUT_DIR}/${BASE}.sasc.semantic.txt"
