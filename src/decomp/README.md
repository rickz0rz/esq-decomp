...
- Current hybrid-integration note (March 13, 2026): `modules/groups/a/l/ed3.s` is now wired into `src/decomp/replacements.map` as a passthrough hybrid boundary. Its restored SAS/C coverage currently includes `SET_A_PEN_1_B_PEN_6_DRMD_1_DRAW_TEXT_OR_CURSOR` and `SET_A_PEN_1_B_PEN_6_DRMD_1_DRAW_LINE_OR_PAGE`, so future object-level replacement work can land in that ED3 draw-helper module without another root include-graph edit.
...
- Current hybrid-integration note (March 13, 2026, later re-validated): `modules/groups/a/n/esqfunc.s` is now wired into `src/decomp/replacements.map` as a passthrough hybrid boundary. Its restored SAS/C coverage already spans the current non-`JMPTBL` status, clock, and UI-service helpers in that module, so future object-level replacement work can land there without another root include-graph edit.
- Current hybrid-integration note (March 13, 2026, later re-validated): `modules/groups/a/n/esqiff.s` is now wired into `src/decomp/replacements.map` as a passthrough hybrid boundary. Its restored SAS/C coverage already spans the current non-`JMPTBL` external-asset, weather-overlay, and copper-animation helpers in that module, so future object-level replacement work can land there without another root include-graph edit.
