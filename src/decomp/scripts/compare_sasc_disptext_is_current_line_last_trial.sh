...
BASE="disptext_is_current_line_last"
ENTRY="DISPTEXT_IsCurrentLineLast"
ENTRY_SASC_REGEX="^DISPTEXT_IsCurrentLineLast[A-Za-z0-9_]*:$"
ENTRY_SEM="DISPTEXT_ISCURRENTLINELAST:"
ENTRY_ALT_SEM="DISPTEXT_ISCURRENTLINELAS"
...
awk -v ENTRY_PREFIX="$ENTRY_SEM" -v ENTRY_ALT_PREFIX="$ENTRY_ALT_SEM" -f src/decomp/scripts/semantic_filter_sasc_disptext_is_current_line_last.awk "${OUT_DIR}/${BASE}.original.norm.s" >"${OUT_DIR}/${BASE}.original.semantic.txt"
awk -v ENTRY_PREFIX="$ENTRY_SEM" -v ENTRY_ALT_PREFIX="$ENTRY_ALT_SEM" -f src/decomp/scripts/semantic_filter_sasc_disptext_is_current_line_last.awk "${OUT_DIR}/${BASE}.sasc.norm.s" >"${OUT_DIR}/${BASE}.sasc.semantic.txt"
