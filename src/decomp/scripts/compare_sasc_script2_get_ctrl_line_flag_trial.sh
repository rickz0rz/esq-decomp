...
SASC_SRC="script2_get_ctrl_line_flag.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/b/a/script2.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="script2_get_ctrl_line_flag"
ENTRY="SCRIPT_GetCtrlLineFlag"
ENTRY_SASC_REGEX="^SCRIPT_GetCtrlLineFla[A-Za-z0-9_]*:$"

mkdir -p "$OUT_DIR"

./sc-build-with-dis.sh "$SASC_SRC" >"${OUT_DIR}/sc_build_${BASE}.log" 2>&1

awk -v start="^${ENTRY}:$" -v next_label="^SCRIPT_WriteCtrlShadowToSerdat:$" '
    $0 ~ start { in_func=1 }
    in_func {
...
awk -f src/decomp/scripts/semantic_filter_sasc_script2_get_ctrl_line_flag.awk \
