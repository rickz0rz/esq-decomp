...
- Current ED note (March 19, 2026): `src/decomp/sas_c/ed_handle_editor_input.c` was audited next because its semantic lane was green while the raw diff stayed large. One real mismatch is now fixed: the ESC case returns immediately like the asm instead of falling through into the final sync/update logic. `semantic_filter_sasc_ed_handle_editor_input.awk` was tightened accordingly and the lane is currently back to an empty semantic diff.
