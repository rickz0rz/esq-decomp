...
SRC_C="src/decomp/c/replacements/format_u32_to_octal_string_gcc.c"
ORIG_ASM="src/modules/submodules/unknown9.s"
OUT_DIR="build/decomp/c_trial_gcc"
GEN_ASM="${OUT_DIR}/format_u32_octal.generated.gcc.s"
ORIG_SLICE="${OUT_DIR}/format_u32_octal.original_slice.s"
GEN_SLICE="${OUT_DIR}/format_u32_octal.generated_slice.s"
ORIG_NORM="${OUT_DIR}/format_u32_octal.original_slice.norm.s"
GEN_NORM="${OUT_DIR}/format_u32_octal.generated_slice.norm.s"
...
awk '
$0 ~ /^FORMAT_U32ToOctalString:?$/ {in_func=1}
in_func {
    if ($0 ~ /^;!======/) {
        exit
    }
...
awk '
$0 ~ /^_?FORMAT_U32ToOctalString:?$/ {in_func=1}
in_func { print }
' "$GEN_ASM" > "$GEN_SLICE"
...
awk -f src/decomp/scripts/semantic_filter_u32oct.awk "$ORIG_NORM" > "$ORIG_SEM"
awk -f src/decomp/scripts/semantic_filter_u32oct.awk "$GEN_NORM" > "$GEN_SEM"
