...
SASC_SRC="group_au_jmptbl_stubs.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/a/u/xjump.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="group_au_jmptbl_brush_append_brush_node"
ENTRY_ORIG="GROUP_AU_JMPTBL_BRUSH_AppendBrushNode"
ENTRY_SASC="GROUP_AU_JMPTBL_BRUSH_AppendBrus"
TARGET="BRUSH_AppendBrushNode"

mkdir -p "$OUT_DIR"
./sc-build-with-dis.sh "$SASC_SRC" >"${OUT_DIR}/sc_build_${BASE}.log" 2>&1

awk -v e="^${ENTRY_ORIG}:$" '$0 ~ e {inf=1} inf { if ($0 ~ /^GROUP_AU_JMPTBL_BRUSH_PopulateBrushList:$/) exit; print }' "$ORIG_ASM" >"${OUT_DIR}/${BASE}.original.s"
awk -v e="^${ENTRY_SASC}:$" '$0 ~ e {inf=1} inf { if ($0 ~ /^GROUP_AU_JMPTBL_BRUSH_PopulateBr:$/) exit; print }' "$SASC_DIS" >"${OUT_DIR}/${BASE}.sasc.dis.s"
