...
- Current hybrid-integration note (March 13, 2026, later re-validated): `modules/groups/b/a/parseini.s` is now wired into `src/decomp/replacements.map` as a passthrough hybrid boundary. Its restored SAS/C coverage already spans the current non-`JMPTBL` config, weather, and font-loading helpers in that module, so future object-level replacement work can land there without another root include-graph edit.
