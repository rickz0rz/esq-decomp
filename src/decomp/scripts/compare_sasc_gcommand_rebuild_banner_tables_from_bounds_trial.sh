...
SASC_SRC="gcommand_rebuild_banner_tables_from_bounds.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/a/u/gcommand3.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="gcommand_rebuild_banner_tables_from_bounds"
ENTRY="GCOMMAND_RebuildBannerTablesFromBounds"
ENTRY_SASC_REGEX="^GCOMMAND_RebuildBannerTablesFrom[A-Za-z0-9_]*:$"
...
awk -v e="^${ENTRY}:$" -v e2="$ENTRY_SASC_REGEX" '$0 ~ e || $0 ~ e2 {in_func=1} in_func { if (($0 ~ /^GCOMMAND_[A-Za-z0-9_]+:$/ || $0 ~ /^_?GCOMMAND_[A-Za-z0-9_]+:$/) && $0 !~ e && $0 !~ e2) exit; if ($0 ~ /^XREF / || $0 ~ /^XDEF / || $0 ~ /^ END$/ || $0 ~ /^END$/ || $0 ~ /^__const:$/) exit; print }' \
...
awk -f src/decomp/scripts/semantic_filter_sasc_gcommand_rebuild_banner_tables_from_bounds.awk \
