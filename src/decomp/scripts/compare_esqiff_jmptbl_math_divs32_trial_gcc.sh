...
SRC_C="src/decomp/c/replacements/esqiff_jmptbl_math_divs32_gcc.c"
ORIG_ASM="src/modules/groups/a/n/esqiff.s"
OUT_DIR="build/decomp/c_trial_gcc"
GEN_ASM="${OUT_DIR}/esqiff_jmptbl_math_divs32.generated.gcc.s"
ORIG_SLICE="${OUT_DIR}/esqiff_jmptbl_math_divs32.original_slice.s"
GEN_SLICE="${OUT_DIR}/esqiff_jmptbl_math_divs32.generated_slice.s"
ORIG_NORM="${OUT_DIR}/esqiff_jmptbl_math_divs32.original_slice.norm.s"
GEN_NORM="${OUT_DIR}/esqiff_jmptbl_math_divs32.generated_slice.norm.s"
...
awk '
$0 ~ /^ESQIFF_JMPTBL_MATH_DivS32:?$/ {in_func=1}
in_func {
    if ($0 ~ /^ESQIFF_JMPTBL_TEXTDISP_FindEntryIndexByWildcard:?$/) {
        exit
    }
    print
}
...
awk '
$0 ~ /^_?ESQIFF_JMPTBL_MATH_DivS32:?$/ {in_func=1}
in_func { print }
' "$GEN_ASM" > "$GEN_SLICE"
...
awk -f src/decomp/scripts/semantic_filter_esqiff_jmptbl_math_divs32.awk "$ORIG_NORM" > "$ORIG_SEM"
awk -f src/decomp/scripts/semantic_filter_esqiff_jmptbl_math_divs32.awk "$GEN_NORM" > "$GEN_SEM"
