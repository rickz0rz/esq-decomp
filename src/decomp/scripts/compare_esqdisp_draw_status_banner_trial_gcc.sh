...
GEN_ASM="${OUT_DIR}/esqdisp_draw_status_banner.generated.gcc.s"
ORIG_SLICE="${OUT_DIR}/esqdisp_draw_status_banner.original_slice.s"
GEN_SLICE="${OUT_DIR}/esqdisp_draw_status_banner.generated_slice.s"
ORIG_NORM="${OUT_DIR}/esqdisp_draw_status_banner.original_slice.norm.s"
GEN_NORM="${OUT_DIR}/esqdisp_draw_status_banner.generated_slice.norm.s"
...
awk '$0 ~ /^ESQDISP_DrawStatusBanner:?$/ {in_func=1} in_func { if ($0 ~ /^;!======/) exit; print }' "$ORIG_ASM" > "$ORIG_SLICE"
awk '$0 ~ /^_?ESQDISP_DrawStatusBanner:?$/ {in_func=1} in_func { print }' "$GEN_ASM" > "$GEN_SLICE"
...
awk -f src/decomp/scripts/semantic_filter_esqdisp_draw_status_banner.awk "$ORIG_NORM" > "$ORIG_SEM"
awk -f src/decomp/scripts/semantic_filter_esqdisp_draw_status_banner.awk "$GEN_NORM" > "$GEN_SEM"
