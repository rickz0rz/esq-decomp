...
SASC_SRC="esq_read_serial_rbf_byte.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/a/a/app.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="esq_read_serial_rbf_byte"
ENTRY="ESQ_ReadSerialRbfByte"
ENTRY_SASC_REGEX="^ESQ_ReadSerialRbfByte[A-Za-z0-9_]*:$"
ENTRY_SEM="ESQ_READSERIALRBFBYTE:"
ENTRY_ALT_SEM="ESQ_READSERIALRBFBYTE"
...
awk -v ENTRY_PREFIX="$ENTRY_SEM" -v ENTRY_ALT_PREFIX="$ENTRY_ALT_SEM" -f src/decomp/scripts/semantic_filter_sasc_esq_read_serial_rbf_byte.awk "${OUT_DIR}/${BASE}.original.norm.s" >"${OUT_DIR}/${BASE}.original.semantic.txt"
awk -v ENTRY_PREFIX="$ENTRY_SEM" -v ENTRY_ALT_PREFIX="$ENTRY_ALT_SEM" -f src/decomp/scripts/semantic_filter_sasc_esq_read_serial_rbf_byte.awk "${OUT_DIR}/${BASE}.sasc.norm.s" >"${OUT_DIR}/${BASE}.sasc.semantic.txt"
