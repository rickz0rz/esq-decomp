...

{
    ...
    if (u ~ /^SUBA?\.L A[0-7],A[0-7]$/ || u ~ /^SUBA?\.L A[0-7],D[0-7]$/ || u ~ /^SUB\.L D[0-7],D[0-7]$/) has_len_subtract = 1

    if (u ~ /^MOVE\.B \(A[0-7]\)\+,D[0-7]$/ || u ~ /^MOVE\.B \(A[0-7]\)\+,\(A[0-7]\)$/) has_char_load = 1

    if (u ~ /GLOBAL_PREALLOCHANDLENODE1_WRITE/) has_write_remaining_ref = 1
    if (u ~ /GLOBAL_PREALLOCHANDLENODE1_BUFFE/) has_buffer_cursor_ref = 1
    if (u ~ /GLOBAL_PREALLOCHANDLENODE1([^A-Z0-9_]|$)/) has_prealloc_node_ref = 1

    if (u ~ /STREAM_BUFFEREDPUTCORFLUSH/) has_stream_putc_call = 1
    if (u ~ /PEA -1\.W/ || u ~ /PEA \(\$?FFFFFFFF\)\.W/ || u ~ /MOVEQ #-1,D[0-7]/ || u ~ /#-1/) has_flush_minus_one = 1
    if (u == "RTS") has_rts = 1
}
