...
SRC_C="src/decomp/c/replacements/esqiff2_read_rbf_bytes_with_xor_gcc.c"
ORIG_ASM="src/modules/groups/a/o/esqiff2.s"
OUT_DIR="build/decomp/c_trial_gcc"
GEN_ASM="${OUT_DIR}/esqiff2_read_rbf_bytes_with_xor.generated.gcc.s"
ORIG_SLICE="${OUT_DIR}/esqiff2_read_rbf_bytes_with_xor.original_slice.s"
GEN_SLICE="${OUT_DIR}/esqiff2_read_rbf_bytes_with_xor.generated_slice.s"
ORIG_NORM="${OUT_DIR}/esqiff2_read_rbf_bytes_with_xor.original_slice.norm.s"
GEN_NORM="${OUT_DIR}/esqiff2_read_rbf_bytes_with_xor.generated_slice.norm.s"
...
awk '$0 ~ /^ESQIFF2_ReadRbfBytesWithXor:?$/ {in_func=1} in_func { if ($0 ~ /^;------------------------------------------------------------------------------/) exit; print }' "$ORIG_ASM" > "$ORIG_SLICE"
awk '$0 ~ /^_?ESQIFF2_ReadRbfBytesWithXor:?$/ {in_func=1} in_func { print }' "$GEN_ASM" > "$GEN_SLICE"
...
awk -f src/decomp/scripts/semantic_filter_esqiff2_read_rbf_bytes_with_xor.awk "$ORIG_NORM" > "$ORIG_SEM"
awk -f src/decomp/scripts/semantic_filter_esqiff2_read_rbf_bytes_with_xor.awk "$GEN_NORM" > "$GEN_SEM"
