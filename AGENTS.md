...
- [x] Replace `src/decomp/replacements/modules/groups/a/n/esqfunc.s` passthrough include with the direct module body for the ESQFUNC status/clock/UI-service helper cluster, and update carry-forward docs so ESQFUNC is tracked as an active object-level hybrid replacement instead of a seeded passthrough boundary.
...
- [x] Replace `src/decomp/replacements/modules/groups/a/n/esqiff.s` passthrough include with the direct module body for the ESQIFF external-asset/weather/copper helper cluster, and update carry-forward docs so ESQIFF is tracked as an active object-level hybrid replacement instead of a seeded passthrough boundary.
...
- [x] Replace `src/decomp/replacements/modules/groups/a/p/esqshared.s` passthrough include with the direct module body for the ESQSHARED entry/default/title-filter helper cluster, and update carry-forward docs so ESQSHARED is tracked as an active object-level hybrid replacement instead of a seeded passthrough boundary.
...
- [x] Add `modules/groups/a/k/ed.s` to `src/decomp/replacements.map` and replace it with the direct module body now that the ED main editor / ESC-state helper module's restored SAS/C compare lanes are green in this checkout.
- [x] Add `modules/groups/a/a/bitmap.s` to `src/decomp/replacements.map` and replace it with the direct module body for `BITMAP_ProcessIlbmImage` now that the module's only direct export has a green restored SAS/C compare lane.
...
- [x] Replace `src/decomp/replacements/modules/groups/b/a/tliba3.s` passthrough include with the direct TLIBA3 module body now that the maintained strict `tliba3` SAS/C rerun is green across the direct and wrapper-backed view-mode helper lanes.
...
- [x] Replace `src/decomp/replacements/modules/groups/a/n/esqdisp.s` passthrough include with the direct module body for the ESQDISP display/status/highlight helper cluster, and update carry-forward docs so ESQDISP is tracked as an active object-level hybrid replacement instead of a seeded passthrough boundary.
...
- [x] Replace `src/decomp/replacements/modules/groups/a/o/esqpars.s` passthrough include with the direct module body for the ESQPARS parser/state-persistence helper cluster, and update carry-forward docs so ESQPARS is tracked as an active object-level hybrid replacement instead of a seeded passthrough boundary.
...
- [x] Replace `src/decomp/replacements/modules/groups/a/y/locavail.s` passthrough include with the direct module body for the LOCAVAIL local-availability filter helper cluster, and update carry-forward docs so LOCAVAIL is tracked as an active object-level hybrid replacement instead of a seeded passthrough boundary.
...
- [x] Add restored SAS/C coverage for `NEWGRID_ProcessGridMessages`, then replace `src/decomp/replacements/modules/groups/b/a/newgrid.s` passthrough include with the direct NEWGRID module body so the module is tracked as an active object-level hybrid replacement.
...
- [x] Add restored SAS/C coverage for `NEWGRID_ValidateSelectionCode`, then replace `src/decomp/replacements/modules/groups/b/a/newgrid1.s` passthrough include with the direct NEWGRID1 module body so the module is tracked as an active object-level hybrid replacement.
...
- [x] Replace `src/decomp/replacements/modules/groups/b/a/textdisp3.s` passthrough include with the direct module body for the TEXTDISP3 text/layout helper cluster, and update carry-forward docs so TEXTDISP3 is tracked as an active object-level hybrid replacement instead of a seeded passthrough boundary.
...
- [x] Add restored SAS/C coverage for `TLIBA1_DrawTextWithInsetSegments`, then replace `src/decomp/replacements/modules/groups/b/a/tliba1.s` passthrough include with the direct TLIBA1 module body so the module is tracked as an active object-level hybrid replacement.
...
- [x] Replace `src/decomp/replacements/modules/groups/a/z/locavail2.s` passthrough include with the direct module body for `LOCAVAIL2_AutoRequestNoOp`, `LOCAVAIL2_DisplayAlertDelayAndReboot`, and `OVERRIDE_INTUITION_FUNCS`, and update carry-forward docs so LOCAVAIL2 is tracked as an active object-level hybrid replacement instead of a seeded passthrough boundary.
...
- [x] Replace `src/decomp/replacements/modules/submodules/unknown40.s` passthrough include with the direct module body for `BATTCLOCK_GetSecondsFromBatteryBackedClock`, `BATTCLOCK_WriteSecondsToBatteryBackedClock`, `DOS_Delay`, `DOS_SystemTagList`, and `EXEC_CallVector_48`, and update carry-forward docs so UNKNOWN40 is tracked as an active object-level hybrid replacement instead of a seeded passthrough boundary.
...
- [x] Replace `src/decomp/replacements/modules/submodules/unknown5.s` passthrough include with the direct module body for the UNKNOWN5 string helper cluster, and update carry-forward docs so UNKNOWN5 is tracked as an active object-level hybrid replacement instead of a seeded passthrough boundary.
...
- [x] Replace `src/decomp/replacements/modules/submodules/unknown10.s` passthrough include with the direct module body for the UNKNOWN10 format/parse/open helper cluster, and update carry-forward docs so UNKNOWN10 is tracked as an active object-level hybrid replacement instead of a seeded passthrough boundary.
...
- [x] Replace `src/decomp/replacements/modules/submodules/unknown15.s` passthrough include with the direct module body for `STREAM_ReadLineWithLimit`, and update carry-forward docs so UNKNOWN15 is tracked as an active object-level hybrid replacement instead of a seeded passthrough boundary.
...
- [x] Replace `src/decomp/replacements/modules/submodules/unknown33.s` passthrough include with the direct module body for `ALLOC_InsertFreeBlock` and `STRING_FindSubstring`, and update compare-lane metadata so UNKNOWN33 coverage reports credit both helpers directly.
- [x] Replace `src/decomp/replacements/modules/submodules/unknown13.s` and `src/decomp/replacements/modules/submodules/unknown36.s` passthrough includes with their direct module bodies now that the restored compare lanes for `FORMAT_CallbackWriteChar` / `FORMAT_FormatToCallbackBuffer` and `UNKNOWN36_FinalizeRequest` / `UNKNOWN36_ShowAbortRequester` are green in this checkout.
...
- [x] Replace `src/decomp/replacements/modules/submodules/unknown6.s`, `unknown8.s`, and `unknown39.s` passthrough includes with the direct module bodies for `STRING_AppendAtNull`, `FORMAT_U32ToDecimalString`, and `GRAPHICS_BltBitMapRastPort`, and update carry-forward docs so those single-helper modules are tracked as active object-level hybrid replacements instead of seeded passthrough boundaries.
- [x] Replace `src/decomp/replacements/modules/submodules/unknown23.s` passthrough include with the direct module body for `HANDLE_GetEntryByIndex`, and update carry-forward docs so UNKNOWN23 is tracked as an active object-level hybrid replacement instead of a seeded passthrough boundary.
...
- [x] Replace `src/decomp/replacements/modules/submodules/unknown16.s` passthrough include with the direct module body for `BUFFER_FlushAllAndCloseWithCode`, and update carry-forward docs so UNKNOWN16 is tracked as an active object-level hybrid replacement instead of a seeded passthrough boundary.
- [x] Replace `src/decomp/replacements/modules/submodules/unknown18.s`, `unknown19.s`, and `unknown20.s` passthrough includes with direct module bodies for `DOS_SeekWithErrorState`, `DOS_ReadWithErrorState`, and `DOS_OpenWithErrorState`, and update carry-forward docs so this DOS error-wrapper cluster is tracked as active object-level hybrid replacement coverage instead of include-only boundaries.
- [x] Replace `src/decomp/replacements/modules/submodules/unknown26.s` passthrough include with the direct module body for `DOS_WriteByIndex`, and update carry-forward docs so UNKNOWN26 is tracked as an active object-level hybrid replacement instead of a seeded passthrough boundary.
...
- [x] Replace `src/decomp/replacements/modules/submodules/unknown22.s` passthrough include with the direct module body for the UNKNOWN22 arithmetic/msgport helper cluster, and update carry-forward docs so UNKNOWN22 is tracked as an active object-level hybrid replacement instead of a seeded passthrough boundary.
...
- [x] Replace `src/decomp/replacements/modules/submodules/unknown21.s` passthrough include with the direct module body for the UNKNOWN21 DOS/file-create and IOStdReq cleanup helper cluster, and update carry-forward docs so UNKNOWN21 is tracked as an active object-level hybrid replacement instead of a seeded passthrough boundary.
...
- [x] Replace `src/decomp/replacements/modules/submodules/unknown24.s` passthrough include with the direct module body for the UNKNOWN24 memlist/signed-long helper cluster, and update carry-forward docs so UNKNOWN24 is tracked as an active object-level hybrid replacement instead of a seeded passthrough boundary.
...
- [x] Replace `src/decomp/replacements/modules/submodules/unknown29.s` passthrough include with the direct module body for `ESQ_ParseCommandLineAndRun`, and update carry-forward docs so UNKNOWN29 is tracked as an active object-level hybrid replacement while preserving `UNKNOWN29_JMPTBL_ESQ_MainInitAndRun` as build glue.
...
- [x] Replace `src/decomp/replacements/modules/submodules/unknown34.s` passthrough include with the direct module body for the UNKNOWN34 list/memmove/read helper cluster, and update carry-forward docs so UNKNOWN34 is tracked as an active object-level hybrid replacement instead of a seeded passthrough boundary.
...
- [x] Add restored SAS/C coverage for `ED1_EnterEscMenu`, then replace `src/decomp/replacements/modules/groups/a/k/ed1.s` passthrough include with the direct ED1 module body so the module is tracked as an active object-level hybrid replacement.
- [x] Add `modules/groups/a/k/ed2.s` to `src/decomp/replacements.map` and replace it with the direct module body now that the ED2 module's restored entry-summary / diagnostics helper coverage is green across its full non-`JMPTBL` export set.
- [x] Add `modules/groups/a/j/disptext2.s` to `src/decomp/replacements.map` and seed the matching hybrid replacement boundary now that the DATETIME conversion/format helper module's restored non-`JMPTBL` SAS/C compare lanes are green in the maintained sweep.
- [x] Add restored SAS/C coverage for `DATETIME_ParseString` and `DATETIME_UpdateSelectionField`, then replace `src/decomp/replacements/modules/groups/a/j/disptext2.s` passthrough include with the direct DATETIME helper module body so the module is tracked as an active object-level hybrid replacement.
...
- [x] Replace `src/decomp/replacements/modules/groups/b/a/newgrid2.s` passthrough include with the direct module body for the NEWGRID2 state-dispatch / buffer-management helper cluster, and update carry-forward docs so NEWGRID2 is tracked as an active object-level hybrid replacement rather than a seeded passthrough boundary.
- [x] Add `src/decomp/sas_c/wdisp_update_selection_preview_panel.c`, wire `compare_sasc_wdisp_update_selection_preview_panel_trial.sh`, and replace `src/decomp/replacements/modules/groups/b/a/wdisp.s` passthrough include with the direct WDISP module body now that the weather-status helper module's full non-`JMPTBL` export set is green.
- [x] Add direct SAS/C compare lanes for `TLIBA3_BuildDisplayContextForViewMode`, `TLIBA3_DrawCenteredWrappedTextLines`, `TLIBA3_DrawViewModeGuides`, and `TLIBA3_InitPatternTable`, then include the direct `TLIBA3_GetViewModeHeight`, `TLIBA3_GetViewModeRastPort`, and `TLIBA3_SelectNextViewMode` checks in `run_sasc_core_sweep.sh` now that the expanded TLIBA3 lane is green in this checkout.
- [x] Replace `src/decomp/replacements/modules/groups/a/f/xjump.s` passthrough include with the direct module body for `GROUP_AF_JMPTBL_GCOMMAND_SaveBrushResult`, and update carry-forward docs so this wrapper-only brush-result xjump module is tracked as an active object-level hybrid replacement instead of a seeded passthrough boundary.
- [x] Replace `src/decomp/replacements/modules/groups/a/b/xjump.s` passthrough include with the direct module body for `GROUP_AB_JMPTBL_ESQFUNC_FreeLineTextBuffers`, `GROUP_AB_JMPTBL_ESQIFF2_ClearLineHeadTailByMode`, `GROUP_AB_JMPTBL_ESQIFF_DeallocateAdsAndLogoLstData`, `GROUP_AB_JMPTBL_ESQPARS_RemoveGroupEntryAndReleaseStrings`, `GROUP_AB_JMPTBL_IOSTDREQ_Free`, `GROUP_AB_JMPTBL_LADFUNC_FreeBannerRectEntries`, `GROUP_AB_JMPTBL_LOCAVAIL_FreeResourceChain`, `GROUP_AB_JMPTBL_NEWGRID_ShutdownGridResources`, `GROUP_AB_JMPTBL_UNKNOWN2A_Stub0`, and `GROUP_AB_JMPTBL_GRAPHICS_FreeRaster`, and update carry-forward docs so this wrapper-only cleanup/free-resource xjump module is tracked as an active object-level hybrid replacement instead of a seeded passthrough boundary.
- [x] Replace `src/decomp/replacements/modules/groups/a/i/xjump.s` passthrough include with the direct module body for `GROUP_AI_JMPTBL_FORMAT_FormatToBuffer2`, `GROUP_AI_JMPTBL_NEWGRID_SetSelectionMarkers`, `GROUP_AI_JMPTBL_STRING_AppendAtNull`, `GROUP_AI_JMPTBL_TLIBA1_DrawTextWithInsetSegments`, `GROUP_AI_JMPTBL_STR_CopyUntilAnyDelimN`, `GROUP_AI_JMPTBL_STR_FindCharPtr`, and `GROUP_AI_JMPTBL_STR_SkipClass3Chars`, and update carry-forward docs so this wrapper-only text/layout xjump module is tracked as an active object-level hybrid replacement instead of a seeded passthrough boundary.
- [x] Replace `src/decomp/replacements/modules/groups/a/r/xjump.s` passthrough include with the direct module body for `GROUP_AR_JMPTBL_PARSEINI_WriteErrorLogEntry` and `GROUP_AR_JMPTBL_STRING_AppendAtNull`, and update carry-forward docs so this wrapper-only log/string xjump module is tracked as an active object-level hybrid replacement instead of a seeded passthrough boundary.
- [x] Replace `src/decomp/replacements/modules/groups/a/t/xjump.s` passthrough include with the direct module body for `GROUP_AT_JMPTBL_DOS_SystemTagList`, `GROUP_AT_JMPTBL_ED1_WaitForFlagAndClearBit0`, and `GROUP_AT_JMPTBL_ED1_WaitForFlagAndClearBit1`, and update carry-forward docs so this wrapper-only DOS/ED1 xjump module is tracked as an active object-level hybrid replacement instead of a seeded passthrough boundary.
- [x] Replace `src/decomp/replacements/modules/groups/a/d/xjump.s` passthrough include with the direct module body for the full `GROUP_AD_JMPTBL_*` wrapper set, tighten the dedicated wrapper semantic filters, and update carry-forward docs so this wrapper-only display/copper/view-mode xjump module is tracked as an active object-level hybrid replacement instead of a seeded passthrough boundary.
- [x] Replace `src/decomp/replacements/modules/groups/a/g/xjump.s` passthrough include with the direct module body for the full `GROUP_AG_JMPTBL_*` wrapper set, tighten the dedicated wrapper semantic filters, and update carry-forward docs so this wrapper-only memory/parse/UI-service xjump module is tracked as an active object-level hybrid replacement instead of a seeded passthrough boundary.
- [x] Replace `src/decomp/replacements/modules/groups/a/m/xjump.s` passthrough include with the direct module body for the full `GROUP_AM_JMPTBL_*` wrapper set, and update carry-forward docs so this wrapper-only startup/init/config xjump module is tracked as an active object-level hybrid replacement instead of a seeded passthrough boundary.
- [x] Split the Unknown42 `PARALLEL_*` SAS/C helper coverage into role-based source files (`parallel_check_ready`, `parallel_check_ready_stub`, `parallel_wait_ready`, `parallel_write_char_d0`, `parallel_string_and_raw_dofmt`, `parallel_write_char_hw_and_raw_dofmt`), wire the maintained strict sweep to the direct alias compare lanes, and keep the direct `unknown42.s` hybrid replacement state reflected in the live docs.
- [x] Tighten `PARSE_ReadSignedLong` validation by splitting the SAS/C internal-label coverage into dedicated `parse_done`, `parse_loop_entry`, `parse_loop`, and `store_result` micro-lanes, and align the semantic filters so the callable-label wrappers validate the intended parse roles instead of raw wrapper shape.
- [x] Update `src/decomp/scripts/report_hybrid_module_promotion_coverage.py` so SAS/C coverage attribution uses canonicalized compare targets / entry metadata instead of raw compare-script filenames, preventing wrapper-backed direct exports from being undercounted in hybrid coverage reports.
- [x] Add repo-scope remaining-work reporters (`report_remaining_decomp_scope.py`, `report_replacement_only_modules.py`), wire the non-callable `interrupts/constants.s` include mirror into `src/decomp/replacements.map`, and document that the passthrough integration queue is now exhausted with all code modules mapped in this checkout.
- [x] Replace `src/decomp/replacements/modules/groups/a/y/xjump.s` passthrough include with the direct module body for the full `GROUP_AY_JMPTBL_*` wrapper set, and update carry-forward docs so this wrapper-only diskio/math/script/string xjump module is tracked as an active object-level hybrid replacement instead of a seeded passthrough boundary.
- [x] Replace `src/decomp/replacements/modules/groups/a/z/xjump.s` passthrough include with the direct module body for `GROUP_AZ_JMPTBL_ESQ_ColdReboot`, and update carry-forward docs so this wrapper-only cold-reboot xjump module is tracked as an active object-level hybrid replacement instead of a seeded passthrough boundary.
...
- [x] Restore the two remaining stubbed ESQSHARED functions from ASM (`ESQSHARED_CreateGroupEntryAndTitle`, `ESQSHARED_UpdateMatchingEntriesByTitle`) as real SAS/C in `src/decomp/sas_c/`, `.dis`-verify against the original, add maintained compare lanes + semantic filters, and wire both into `run_sasc_core_sweep.sh`. Both now link into `ESQ_fullc` as real code (no longer `_missing_stubs.s` no-ops).
- [x] Root-cause and fix the "vamos-uncompilable" compile failures: `--cwd esq:` made the emulated `sc` CPU-crash on ~a dozen sources; `compile_all_sasc_far.sh` now references `esq:<file>` with no `--cwd`, so all restorations (incl. the 3 NEWGRID `*WithOffset`/`FindNextEntryWithAltMarkers`) compile FAR and link as real code. `_missing_stubs.s` now holds only the real `_AddIntVector` LVO shim — zero no-op placeholder stubs remain in the whole-program C link.
- [x] Close the whole-program C link to 0 undefined / 0 multiply-defined (IDLEN=80 recompile for two long-symbol format/path-string references); `ESQ_fullc` loads + runs under vamos to the graphics.library open.
- [x] Extend the differential audit harness (`gen_leaf_diff.py`) to the buffer/pointer-arg tier (seeded 64-byte buffer + post-call digest); audited 11 more leaf functions (8 PASS, 2 explained non-bugs, 1 inconclusive crash). Cumulative: 25 audited, 21 PASS, 1 real bug (ESQDISP, fixed), 3 explained non-bugs.
- [x] Prove the A4-global (writable near-data) differential tier: touched globals in a writable `SECTION __MERGED,DATA` object XDEF'd as both `Name`/`_Name`, runner `run_global_diff.sh`, fixtures under `src/decomp/sas_c/_gtest/`. Verified `FORMAT_Buffer2WriteChar` and `UNKNOWN10_PrintfPutcToBuffer` PASS.
- [x] Systemic `NEG.B` booleanize-sign scan (COMPLETE, both directions): fixed 6 functions that returned/stored -1 where the original's `Scc D0; NEG.B D0` yields +1 (`DISPTEXT_AppendToBuffer`, `DISPTEXT_LayoutSourceToLines`, `DISPTEXT_LayoutAndAppendToBuffer`, `WDISP_UpdateSelectionPreviewPanel`, `NEWGRID2_DispatchGridOperation`, and `DATETIME_SecondsToStruct`'s raw-leap-flag store); recompiled + relinked (ESQ_fullc stays 0/0). All suspects resolved: `stream_buffered_getc` traced and confirmed CORRECT (maps to the late ReadReject branch, not the early FlushReject); the `ESQDISP_GetEntryAuxPointerByMode` SGT/SMI/SLT+NEG.B booleanizers are `; Unreferenced Code` (dead, after the live RTS). Mirror direction (0/-1, no NEG.B) has one site, `ESQ_TestBit1Based`, verified correct. See memory `booleanize-neg-b-sign-bug-class`.
