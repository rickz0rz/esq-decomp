...
SRC_C="src/decomp/c/replacements/newgrid_fill_grid_rects_gcc.c"
ORIG_ASM="src/modules/groups/b/a/newgrid.s"
OUT_DIR="build/decomp/c_trial_gcc"
GEN_ASM="${OUT_DIR}/newgrid_fill_grid_rects.generated.gcc.s"
ORIG_SLICE="${OUT_DIR}/newgrid_fill_grid_rects.original_slice.s"
GEN_SLICE="${OUT_DIR}/newgrid_fill_grid_rects.generated_slice.s"
ORIG_NORM="${OUT_DIR}/newgrid_fill_grid_rects.original_slice.norm.s"
GEN_NORM="${OUT_DIR}/newgrid_fill_grid_rects.generated_slice.norm.s"
...
awk '
$0 ~ /^NEWGRID_FillGridRects:?$/ {in_func=1}
in_func {
    if ($0 ~ /^;!======/) {
        exit
    }
...
awk '
$0 ~ /^_?NEWGRID_FillGridRects:?$/ {in_func=1}
in_func { print }
' "$GEN_ASM" > "$GEN_SLICE"
...
awk -f src/decomp/scripts/semantic_filter_newgrid_fill_grid_rects.awk "$ORIG_NORM" > "$ORIG_SEM"
awk -f src/decomp/scripts/semantic_filter_newgrid_fill_grid_rects.awk "$GEN_NORM" > "$GEN_SEM"
