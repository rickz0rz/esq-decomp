...
cd "$ROOT_DIR"
SASC_SRC="esqdisp_jmptbl_stubs.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/a/n/esqdisp.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="esqdisp_jmptbl_newgrid_and_graphics_alloc_raster"
ENTRY_ORIG="ESQDISP_JMPTBL_NEWGRID_ProcessGridMessages"
ENTRY_SASC="ESQDISP_JMPTBL_NEWGRID_ProcessGr"
mkdir -p "$OUT_DIR"
./sc-build-with-dis.sh "$SASC_SRC" >"${OUT_DIR}/sc_build_${BASE}.log" 2>&1
awk '$0 ~ /^ESQDISP_JMPTBL_NEWGRID_ProcessGridMessages:?$/ {inf=1} inf { if ($0 ~ /^;!======/) exit; print }' "$ORIG_ASM" >"${OUT_DIR}/${BASE}.original.s"
...
awk -f src/decomp/scripts/semantic_filter_sasc_esqdisp_jmptbl_newgrid_and_graphics_alloc_raster.awk "${OUT_DIR}/${BASE}.original.norm.s" >"${OUT_DIR}/${BASE}.original.semantic.txt"
awk -f src/decomp/scripts/semantic_filter_sasc_esqdisp_jmptbl_newgrid_and_graphics_alloc_raster.awk "${OUT_DIR}/${BASE}.sasc.norm.s" >"${OUT_DIR}/${BASE}.sasc.semantic.txt"
