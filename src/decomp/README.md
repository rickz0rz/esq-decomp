...
- Follow-up verification note (March 13, 2026): a full local rerun of the current `compare_sasc_*` script set likewise produced no non-empty `build/decomp/sasc_trial/*.semantic.diff` files. Treat the current SAS/C lane as semantically green by default and spend new effort on genuinely unported behavior or non-sweep compare lanes, not repeated core-sweep revalidation. A currently uncovered behavior-heavy GCC-only example is `ESQIFF_ShowExternalAssetWithCopperFx`; its `_return` tail already has SAS/C coverage, but the main body still does not.
