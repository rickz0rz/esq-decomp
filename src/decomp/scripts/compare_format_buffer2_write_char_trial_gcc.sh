...
SRC_C="src/decomp/c/replacements/format_buffer2_write_char_gcc.c"
ORIG_ASM="src/modules/submodules/unknown27.s"
OUT_DIR="build/decomp/c_trial_gcc"
GEN_ASM="${OUT_DIR}/format_buffer2_write_char.generated.gcc.s"
ORIG_SLICE="${OUT_DIR}/format_buffer2_write_char.original_slice.s"
GEN_SLICE="${OUT_DIR}/format_buffer2_write_char.generated_slice.s"
ORIG_NORM="${OUT_DIR}/format_buffer2_write_char.original_slice.norm.s"
GEN_NORM="${OUT_DIR}/format_buffer2_write_char.generated_slice.norm.s"
...
awk '$0 ~ /^FORMAT_Buffer2WriteChar:?$/ {in_func=1} in_func { if ($0 ~ /^;!======/) exit; print }' "$ORIG_ASM" > "$ORIG_SLICE"
awk '$0 ~ /^_?FORMAT_Buffer2WriteChar:?$/ {in_func=1} in_func { print }' "$GEN_ASM" > "$GEN_SLICE"
...
awk -f src/decomp/scripts/semantic_filter_format_buffer2_write_char.awk "$ORIG_NORM" > "$ORIG_SEM"
awk -f src/decomp/scripts/semantic_filter_format_buffer2_write_char.awk "$GEN_NORM" > "$GEN_SEM"
