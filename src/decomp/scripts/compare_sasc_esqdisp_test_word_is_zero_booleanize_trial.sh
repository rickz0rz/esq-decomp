...
SASC_SRC="esqdisp_test_word_is_zero_booleanize.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/a/n/esqdisp.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="esqdisp_test_word_is_zero_booleanize"
ENTRY="ESQDISP_TestWordIsZeroBooleanize"
ENTRY_SASC_REGEX="^ESQDISP_TestWordIsZeroBooleanize[A-Za-z0-9_]*:$"
ENTRY_SEM="ESQDISP_TESTWORDISZEROBOOLEANIZE:"
ENTRY_ALT_SEM="ESQDISP_TESTWORDISZEROBOOLEANIZE"
...
awk -v ENTRY_PREFIX="$ENTRY_SEM" -v ENTRY_ALT_PREFIX="$ENTRY_ALT_SEM" -f src/decomp/scripts/semantic_filter_sasc_esqdisp_test_word_is_zero_booleanize.awk "${OUT_DIR}/${BASE}.original.norm.s" >"${OUT_DIR}/${BASE}.original.semantic.txt"
awk -v ENTRY_PREFIX="$ENTRY_SEM" -v ENTRY_ALT_PREFIX="$ENTRY_ALT_SEM" -f src/decomp/scripts/semantic_filter_sasc_esqdisp_test_word_is_zero_booleanize.awk "${OUT_DIR}/${BASE}.sasc.norm.s" >"${OUT_DIR}/${BASE}.sasc.semantic.txt"
