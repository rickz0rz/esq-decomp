...
cd "$ROOT_DIR"
SASC_SRC="tliba3_jmptbl_stubs.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/b/a/tliba3.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="tliba3_jmptbl_gcommand_applyhighlightflag"
ENTRY_ORIG="TLIBA3_JMPTBL_GCOMMAND_ApplyHighlightFlag"
ENTRY_SASC="TLIBA3_JMPTBL_GCOMMAND_ApplyHigh"
TARGET="GCOMMAND_ApplyHighlightFlag"
mkdir -p "$OUT_DIR"
./sc-build-with-dis.sh "$SASC_SRC" >"${OUT_DIR}/sc_build_${BASE}.log" 2>&1
awk -v e="^${ENTRY_ORIG}:$" '$0 ~ e {inf=1} inf { if ($0 ~ /^TLIBA3_JMPTBL_/ && $0 !~ e) exit; if ($0 ~ /^;!======/) exit; print }' "$ORIG_ASM" >"${OUT_DIR}/${BASE}.original.s"
