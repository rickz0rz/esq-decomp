...
BASE="group_ai_jmptbl_string_append_at_null"
ENTRY_ORIG="GROUP_AI_JMPTBL_STRING_AppendAtNull"
ENTRY_SASC="GROUP_AI_JMPTBL_STRING_AppendAtN"
TARGET="STRING_AppendAtNull"
mkdir -p "$OUT_DIR"
./sc-build-with-dis.sh "$SASC_SRC" >"${OUT_DIR}/sc_build_${BASE}.log" 2>&1
awk -v e="^${ENTRY_ORIG}:$" '$0 ~ e {inf=1} inf { if ($0 ~ /^GROUP_AI_JMPTBL_STR_CopyUntilAnyDelimN:$/) exit; print }' "$ORIG_ASM" >"${OUT_DIR}/${BASE}.original.s"
awk -v e="^${ENTRY_SASC}:$" '$0 ~ e {inf=1} inf { if ($0 ~ /^GROUP_AI_JMPTBL_STR_CopyUntilAny:$/) exit; print }' "$SASC_DIS" >"${OUT_DIR}/${BASE}.sasc.dis.s"
