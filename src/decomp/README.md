...
- Practical workflow: once the maintained sweep is green, use `src/decomp/scripts/report_sasc_semantic_audit_candidates.py` to rank lanes whose semantic diff is already empty but whose raw diff is still large. Treat those rows as the current filter-hardening / deeper-audit queue rather than relying on older mismatch notes.
