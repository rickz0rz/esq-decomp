...
BASE="diskio_write_buffered_bytes_return"
ENTRY="DISKIO_WriteBufferedBytes_Return"
ENTRY_SASC_REGEX="^DISKIO_WriteBufferedBytes_R[A-Za-z0-9_]*:$"
ENTRY_SEM="DISKIO_WRITEBUFFEREDBYTES_RETURN:"
...
awk -v ENTRY_PREFIX="$ENTRY_SEM" -f src/decomp/scripts/semantic_filter_sasc_diskio_return_stub.awk "${OUT_DIR}/${BASE}.original.norm.s" >"${OUT_DIR}/${BASE}.original.semantic.txt"
awk -v ENTRY_PREFIX="$ENTRY_SEM" -f src/decomp/scripts/semantic_filter_sasc_diskio_return_stub.awk "${OUT_DIR}/${BASE}.sasc.norm.s" >"${OUT_DIR}/${BASE}.sasc.semantic.txt"
