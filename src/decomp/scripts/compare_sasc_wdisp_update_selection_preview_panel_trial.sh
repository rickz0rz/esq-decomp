...
SASC_SRC="wdisp_update_selection_preview_panel.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/b/a/wdisp.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="wdisp_update_selection_preview_panel"
ENTRY_ORIG="WDISP_UpdateSelectionPreviewPanel"
ENTRY_SASC_REGEX="^WDISP_UpdateSelectionPreviewPane[A-Za-z0-9_]*:$"

mkdir -p "$OUT_DIR"

./sc-build-with-dis.sh "$SASC_SRC" >"${OUT_DIR}/sc_build_${BASE}.log" 2>&1

awk -v start="^${ENTRY_ORIG}:$" '
  $0 ~ start {in_func=1}
  in_func {
    if ($0 ~ /^;!======/) exit
    print
...
awk -f src/decomp/scripts/semantic_filter_sasc_wdisp_update_selection_preview_panel.awk "${OUT_DIR}/${BASE}.original.norm.s" >"${OUT_DIR}/${BASE}.original.semantic.txt"
awk -f src/decomp/scripts/semantic_filter_sasc_wdisp_update_selection_preview_panel.awk "${OUT_DIR}/${BASE}.sasc.norm.s" >"${OUT_DIR}/${BASE}.sasc.semantic.txt"
