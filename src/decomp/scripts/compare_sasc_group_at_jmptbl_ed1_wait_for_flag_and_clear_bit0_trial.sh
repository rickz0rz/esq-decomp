...
SASC_SRC="group_at_jmptbl_stubs_core.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/a/t/xjump.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="group_at_jmptbl_ed1_wait_for_flag_and_clear_bit0"
ENTRY_ORIG="GROUP_AT_JMPTBL_ED1_WaitForFlagAndClearBit0"
ENTRY_SASC="GROUP_AT_JMPTBL_ED1_WaitForFlagA"
TARGET="ED1_WaitForFlagAndClearBit0"

mkdir -p "$OUT_DIR"
./sc-build-with-dis.sh "$SASC_SRC" >"${OUT_DIR}/sc_build_${BASE}.log" 2>&1

awk -v e="^${ENTRY_ORIG}:$" '$0 ~ e {inf=1} inf { if ($0 ~ /^GROUP_AT_JMPTBL_DOS_SystemTagList:$/) exit; print }' "$ORIG_ASM" >"${OUT_DIR}/${BASE}.original.s"
awk -v e="^${ENTRY_SASC}:$" '$0 ~ e {inf=1} inf { if ($0 ~ /^GROUP_AT_JMPTBL_DOS_SystemTagLis:$/) exit; print }' "$SASC_DIS" >"${OUT_DIR}/${BASE}.sasc.dis.s"
