...
cd "$ROOT_DIR"
SASC_SRC="ctasks_close_task_teardown.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/a/f/ctasks.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="ctasks_close_task_teardown"
ENTRY="CTASKS_CloseTaskTeardown"
ENTRY_SASC_REGEX="^CTASKS_CloseTaskTeardown[A-Za-z0-9_]*:$"
mkdir -p "$OUT_DIR"
...
awk -f src/decomp/scripts/semantic_filter_sasc_ctasks_close_task_teardown.awk "${OUT_DIR}/${BASE}.original.norm.s" >"${OUT_DIR}/${BASE}.original.semantic.txt"
awk -f src/decomp/scripts/semantic_filter_sasc_ctasks_close_task_teardown.awk "${OUT_DIR}/${BASE}.sasc.norm.s" >"${OUT_DIR}/${BASE}.sasc.semantic.txt"
