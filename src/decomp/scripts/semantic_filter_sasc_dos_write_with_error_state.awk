BEGIN {h_entry=0; h_sig=0; h_clear=0; h_write=0; h_cmp=0; h_ioerr=0; h_set5=0; h_ret=0; h_rts=0}
function t(s){sub(/;.*/,"",s);sub(/^[ \t]+/,"",s);sub(/[ \t]+$/,"",s);gsub(/[ \t]+/," ",s);return toupper(s)}
{
  l=t($0)
  if(l=="") next
  if(l~/^DOS_WRITEWITHERRORSTATE:/) h_entry=1
  ...
  if(l~/^(JSR|BSR(\.W)?) _LVOWRITE(\(A[0-7]\))?$/) h_write=1
