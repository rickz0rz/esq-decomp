...
awk -v e="^${ENTRY}:$" '$0 ~ e {in_func=1} in_func { if ($0 ~ /^;!======/) exit; print }' "$ORIG_ASM" >"${OUT_DIR}/iostdreq_free.original.s"
awk -v e="^${ENTRY}:$" '$0 ~ e {in_func=1} in_func { if ($0 ~ /^IOSTDREQ_CleanupSignalAndMsgport:$/) exit; print }' "$SASC_DIS" >"${OUT_DIR}/iostdreq_free.sasc.dis.s"
