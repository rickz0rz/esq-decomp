...
BASE="str_find_char_ptr"
ENTRY="STR_FindCharPtr"
ENTRY_SASC_REGEX="^STR_FindCharPtr[A-Za-z0-9_]*:$"
...
awk -v e="^${ENTRY}:$" -v e2="$ENTRY_SASC_REGEX" '
    $0 ~ e || $0 ~ e2 { in_func=1 }
    in_func {
        if (($0 ~ /^STR_[A-Za-z0-9_]+:$/ || $0 ~ /^_?STR_[A-Za-z0-9_]+:$/) && $0 !~ e && $0 !~ e2) exit
...
awk -f src/decomp/scripts/semantic_filter_str_find_char_ptr.awk \
