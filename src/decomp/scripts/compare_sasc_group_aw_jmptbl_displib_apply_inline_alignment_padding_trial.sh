...
BASE="group_aw_jmptbl_displib_apply_inline_alignment_padding"
ENTRY_ORIG="GROUP_AW_JMPTBL_DISPLIB_ApplyInlineAlignmentPadding"
ENTRY_SASC="GROUP_AW_JMPTBL_DISPLIB_ApplyInl"
TARGET="DISPLIB_ApplyInlineAlignmentPadd"

mkdir -p "$OUT_DIR"
./sc-build-with-dis.sh "$SASC_SRC" >"${OUT_DIR}/sc_build_${BASE}.log" 2>&1
awk -v e="^${ENTRY_ORIG}:$" '$0 ~ e {inf=1} inf { if ($0 ~ /^GROUP_AW_JMPTBL_ESQIFF_RunCopperRiseTransition:$/) exit; print }' "$ORIG_ASM" >"${OUT_DIR}/${BASE}.original.s"
awk -v e="^${ENTRY_SASC}:$" '$0 ~ e {inf=1} inf { if ($0 ~ /^GROUP_AW_JMPTBL_ESQIFF_RunCopper:$/) exit; print }' "$SASC_DIS" >"${OUT_DIR}/${BASE}.sasc.dis.s"
