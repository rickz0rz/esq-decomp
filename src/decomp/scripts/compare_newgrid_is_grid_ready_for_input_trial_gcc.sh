...
SRC_C="src/decomp/c/replacements/newgrid_is_grid_ready_for_input_gcc.c"
ORIG_ASM="src/modules/groups/b/a/newgrid.s"
OUT_DIR="build/decomp/c_trial_gcc"
GEN_ASM="${OUT_DIR}/newgrid_is_grid_ready_for_input.generated.gcc.s"
ORIG_SLICE="${OUT_DIR}/newgrid_is_grid_ready_for_input.original_slice.s"
GEN_SLICE="${OUT_DIR}/newgrid_is_grid_ready_for_input.generated_slice.s"
ORIG_NORM="${OUT_DIR}/newgrid_is_grid_ready_for_input.original_slice.norm.s"
GEN_NORM="${OUT_DIR}/newgrid_is_grid_ready_for_input.generated_slice.norm.s"
...
awk '
$0 ~ /^NEWGRID_IsGridReadyForInput:?$/ {in_func=1}
in_func {
    if ($0 ~ /^;!======/) {
        exit
    }
...
awk '
$0 ~ /^_?NEWGRID_IsGridReadyForInput:?$/ {in_func=1}
in_func { print }
' "$GEN_ASM" > "$GEN_SLICE"
...
awk -f src/decomp/scripts/semantic_filter_newgrid_is_grid_ready_for_input.awk "$ORIG_NORM" > "$ORIG_SEM"
awk -f src/decomp/scripts/semantic_filter_newgrid_is_grid_ready_for_input.awk "$GEN_NORM" > "$GEN_SEM"
