...
cd "$ROOT_DIR"
SASC_SRC="newgrid_adjust_clock_string_by_slot.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/b/a/newgrid.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="newgrid_adjust_clock_string_by_slot"
ENTRY="NEWGRID_AdjustClockStringBySlot"
ENTRY_SASC_REGEX="^NEWGRID_AdjustClockStringBySlo[A-Za-z0-9_]*:$"
mkdir -p "$OUT_DIR"
./sc-build-with-dis.sh "$SASC_SRC" >"${OUT_DIR}/sc_build_${BASE}.log" 2>&1
awk -v start="^${ENTRY}:$" -v next_label="^NEWGRID_AdjustClockStringBySlotWithOffset:$" '$0 ~ start {in_func=1} in_func {if ($0 ~ next_label) exit; print}' "$ORIG_ASM" >"${OUT_DIR}/${BASE}.original.s"
...
awk -f src/decomp/scripts/semantic_filter_sasc_newgrid_adjust_clock_string_by_slot.awk "${OUT_DIR}/${BASE}.original.norm.s" >"${OUT_DIR}/${BASE}.original.semantic.txt"
awk -f src/decomp/scripts/semantic_filter_sasc_newgrid_adjust_clock_string_by_slot.awk "${OUT_DIR}/${BASE}.sasc.norm.s" >"${OUT_DIR}/${BASE}.sasc.semantic.txt"
