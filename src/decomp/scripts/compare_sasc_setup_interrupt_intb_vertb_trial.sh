...
SASC_SRC="setup_interrupt_intb_vertb.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/a/n/esqfunc.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="setup_interrupt_intb_vertb"
ENTRY="SETUP_INTERRUPT_INTB_VERTB"
ENTRY_SASC_REGEX="^SETUP_INTERRUPT_INTB_VER[A-Za-z0-9_]*:$"

mkdir -p "$OUT_DIR"

./sc-build-with-dis.sh "$SASC_SRC" >"${OUT_DIR}/sc_build_${BASE}.log" 2>&1

awk -v start="^${ENTRY}:$" -v next_label="^SETUP_INTERRUPT_INTB_AUD1:$" '$0 ~ start {in_func=1} in_func {if ($0 ~ next_label) exit; print}' "$ORIG_ASM" >"${OUT_DIR}/${BASE}.original.s"
...
awk -f src/decomp/scripts/semantic_filter_sasc_setup_interrupt_intb_vertb.awk "${OUT_DIR}/${BASE}.original.norm.s" >"${OUT_DIR}/${BASE}.original.semantic.txt"
awk -f src/decomp/scripts/semantic_filter_sasc_setup_interrupt_intb_vertb.awk "${OUT_DIR}/${BASE}.sasc.norm.s" >"${OUT_DIR}/${BASE}.sasc.semantic.txt"
