...
SRC_C="src/decomp/c/replacements/esqfunc_jmptbl_cleanup_processalerts_gcc.c"
ORIG_ASM="src/modules/groups/a/n/esqfunc.s"
OUT_DIR="build/decomp/c_trial_gcc"
GEN_ASM="${OUT_DIR}/esqfunc_jmptbl_cleanup_processalerts.generated.gcc.s"
ORIG_SLICE="${OUT_DIR}/esqfunc_jmptbl_cleanup_processalerts.original_slice.s"
GEN_SLICE="${OUT_DIR}/esqfunc_jmptbl_cleanup_processalerts.generated_slice.s"
ORIG_NORM="${OUT_DIR}/esqfunc_jmptbl_cleanup_processalerts.original_slice.norm.s"
GEN_NORM="${OUT_DIR}/esqfunc_jmptbl_cleanup_processalerts.generated_slice.norm.s"
...
awk -v start_label="ESQFUNC_JMPTBL_CLEANUP_ProcessAlerts" -v next_label="ESQFUNC_JMPTBL_ESQ_GetHalfHourSlotIndex" '
...
awk '$0 ~ /^_?ESQFUNC_JMPTBL_CLEANUP_ProcessAlerts:?$/ {in_func=1} in_func { print }' "$GEN_ASM" > "$GEN_SLICE"
...
awk -f "src/decomp/scripts/semantic_filter_esqfunc_jmptbl_cleanup_processalerts.awk" "$ORIG_NORM" > "$ORIG_SEM"
awk -f "src/decomp/scripts/semantic_filter_esqfunc_jmptbl_cleanup_processalerts.awk" "$GEN_NORM" > "$GEN_SEM"
