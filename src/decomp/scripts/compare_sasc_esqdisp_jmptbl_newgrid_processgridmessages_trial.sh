...
BASE="esqdisp_jmptbl_newgrid_processgridmessages"
ENTRY_ORIG="ESQDISP_JMPTBL_NEWGRID_ProcessGridMessages"
ENTRY_SASC="ESQDISP_JMPTBL_NEWGRID_ProcessGr"
TARGET="NEWGRID_ProcessGridMessages"
mkdir -p "$OUT_DIR"
./sc-build-with-dis.sh "$SASC_SRC" >"${OUT_DIR}/sc_build_${BASE}.log" 2>&1
awk -v e="^${ENTRY_ORIG}:$" '$0 ~ e {inf=1} inf { if ($0 ~ /^ESQDISP_JMPTBL_/ && $0 !~ e) exit; if ($0 ~ /^;!======/) exit; print }' "$ORIG_ASM" >"${OUT_DIR}/${BASE}.original.s"
