...

{
    ...
    if (u ~ /^SUBQ\.L #\$?1,[$]?[0-9A-F]+\((A[0-7]|A0)\)$/ || u ~ /^SUBI?\.L #\$?1,[$]?[0-9A-F]+\((A[0-7]|A0)\)$/ || u ~ /^MOVE\.L \([$]?[0-9A-F]+,A[0-7]\),D[0-7]$/ || u ~ /^MOVE\.L D[0-7],\([$]?[0-9A-F]+,A[0-7]\)$/) has_stream_count_decrement = 1
    if (u ~ /STREAM_BUFFEREDGETC/ || u ~ /^JSR \(A[0-7]\)$/ || u ~ /^JSR \(A[0-7]\)\+$/) has_refill_call = 1
    if (u ~ /^MOVE\.B \(A[0-7]\),D[0-7]$/ || u ~ /^MOVE\.B \(A[0-7]\)\+,D[0-7]$/ || u ~ /^MOVE\.B [$]?[0-9A-F]+\((A[0-7]|A0)\),D[0-7]$/) has_readptr_byte_load = 1

    if (u ~ /^MOVEQ(\.L)? #\-1,D[0-7]$/ || u ~ /^CMPI?\.L #\-1,D[0-7]$/ || u ~ /^CMP\.L D[0-7],D[0-7]$/) has_eof_check = 1
    if (u ~ /^MOVE\.B D[0-7],\(A[0-7]\)\+$/ || u ~ /^MOVE\.B D[0-7],[$]?[0-9A-F]*\(A[0-7](,D[0-7]\.L)?\)$/) has_dst_store = 1
    if (u ~ /^MOVEQ(\.L)? #\$?A,D[0-7]$/ || u ~ /^MOVEQ(\.L)? #10,D[0-7]$/ || u ~ /^CMPI?\.B #10,D[0-7]$/ || u ~ /^CMPI?\.L #10,D[0-7]$/) has_newline_check = 1
    if (u ~ /^CLR\.B \(A[0-7]\)$/ || u ~ /^CLR\.B [$]?[0-9A-F]*\(A[0-7](,D[0-7]\.L)?\)$/ || u ~ /^MOVE\.B #\$?0,\(A[0-7]\)$/) has_nul_terminate = 1

    if (u ~ /^MOVEQ(\.L)? #\$?0,D0$/ || u ~ /^CLR\.L D0$/) has_zero_return_path = 1
