...
BASE="esqiff_show_external_asset_with_copper_fx"
...
awk '
  /^ESQIFF_ShowExternalAssetWithCopperFx:$/ {in_func=1}
  !in_func && /^ESQIFF_ShowExternalAssetWithC[^_]*:$/ && $0 !~ /RETURN/ {in_func=1}
  in_func {
    if (($0 ~ /^ESQIFF_[A-Za-z0-9_]+:$/ || $0 ~ /^_?ESQIFF_[A-Za-z0-9_]+:$/) &&
        $0 !~ /^ESQIFF_ShowExternalAssetWithCopperFx:$/ &&
        $0 !~ /^ESQIFF_ShowExternalAssetWithC[^_]*:$/) exit
...
awk -f src/decomp/scripts/semantic_filter_sasc_esqiff_show_external_asset_with_copper_fx.awk "${OUT_DIR}/${BASE}.original.norm.s" >"${OUT_DIR}/${BASE}.original.semantic.txt"
awk -f src/decomp/scripts/semantic_filter_sasc_esqiff_show_external_asset_with_copper_fx.awk "${OUT_DIR}/${BASE}.sasc.norm.s" >"${OUT_DIR}/${BASE}.sasc.semantic.txt"
