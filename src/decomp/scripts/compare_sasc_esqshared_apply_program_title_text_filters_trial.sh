...
SASC_SRC="esqshared_apply_program_title_text_filters.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/a/p/esqshared.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="esqshared_apply_program_title_text_filters"
ENTRY="ESQSHARED_ApplyProgramTitleTextFilters"
ENTRY_SASC_REGEX="^ESQSHARED_ApplyProgramTitleTextF[A-Za-z0-9_]*:$"
...
awk -v e="^${ENTRY}:$" -v e2="$ENTRY_SASC_REGEX" '
  $0 ~ e || $0 ~ e2 {in_func=1}
  in_func {
    if (($0 ~ /^ESQSHARED_[A-Za-z0-9_]+:$/ || $0 ~ /^_?ESQSHARED_[A-Za-z0-9_]+:$/) && $0 !~ e && $0 !~ e2) exit
...
awk -f src/decomp/scripts/semantic_filter_sasc_esqshared_apply_program_title_text_filters.awk \
