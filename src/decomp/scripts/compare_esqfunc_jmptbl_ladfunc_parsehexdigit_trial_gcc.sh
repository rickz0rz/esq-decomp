...
SRC_C="src/decomp/c/replacements/esqfunc_jmptbl_ladfunc_parsehexdigit_gcc.c"
ORIG_ASM="src/modules/groups/a/n/esqfunc.s"
OUT_DIR="build/decomp/c_trial_gcc"
GEN_ASM="${OUT_DIR}/esqfunc_jmptbl_ladfunc_parsehexdigit.generated.gcc.s"
ORIG_SLICE="${OUT_DIR}/esqfunc_jmptbl_ladfunc_parsehexdigit.original_slice.s"
GEN_SLICE="${OUT_DIR}/esqfunc_jmptbl_ladfunc_parsehexdigit.generated_slice.s"
ORIG_NORM="${OUT_DIR}/esqfunc_jmptbl_ladfunc_parsehexdigit.original_slice.norm.s"
GEN_NORM="${OUT_DIR}/esqfunc_jmptbl_ladfunc_parsehexdigit.generated_slice.norm.s"
...
awk -v start_label="ESQFUNC_JMPTBL_LADFUNC_ParseHexDigit" -v next_label="ESQFUNC_JMPTBL_CLEANUP_ProcessAlerts" '
...
awk '$0 ~ /^_?ESQFUNC_JMPTBL_LADFUNC_ParseHexDigit:?$/ {in_func=1} in_func { print }' "$GEN_ASM" > "$GEN_SLICE"
...
awk -f "src/decomp/scripts/semantic_filter_esqfunc_jmptbl_ladfunc_parsehexdigit.awk" "$ORIG_NORM" > "$ORIG_SEM"
awk -f "src/decomp/scripts/semantic_filter_esqfunc_jmptbl_ladfunc_parsehexdigit.awk" "$GEN_NORM" > "$GEN_SEM"
