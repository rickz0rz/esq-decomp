...
SRC_C="src/decomp/c/replacements/gcommand_get_banner_char_gcc.c"
ORIG_ASM="src/modules/groups/a/u/gcommand3.s"
OUT_DIR="build/decomp/c_trial_gcc"
GEN_ASM="${OUT_DIR}/gcommand_get_banner_char.generated.gcc.s"
ORIG_SLICE="${OUT_DIR}/gcommand_get_banner_char.original_slice.s"
GEN_SLICE="${OUT_DIR}/gcommand_get_banner_char.generated_slice.s"
ORIG_NORM="${OUT_DIR}/gcommand_get_banner_char.original_slice.norm.s"
GEN_NORM="${OUT_DIR}/gcommand_get_banner_char.generated_slice.norm.s"
...
awk '
$0 ~ /^GCOMMAND_GetBannerChar:?$/ {in_func=1}
in_func {
    if ($0 ~ /^;!======/) exit
    print
}
...
awk '
$0 ~ /^_?GCOMMAND_GetBannerChar:?$/ {in_func=1}
in_func { print }
' "$GEN_ASM" > "$GEN_SLICE"
...
awk -f src/decomp/scripts/semantic_filter_gcommand_get_banner_char.awk "$ORIG_NORM" > "$ORIG_SEM"
awk -f src/decomp/scripts/semantic_filter_gcommand_get_banner_char.awk "$GEN_NORM" > "$GEN_SEM"
