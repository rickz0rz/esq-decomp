...
SRC_C="src/decomp/c/replacements/coi_process_entry_selection_state_gcc.c"
ORIG_ASM="src/modules/groups/a/e/coi.s"
OUT_DIR="build/decomp/c_trial_gcc"
GEN_ASM="${OUT_DIR}/coi_process_entry_selection_state.generated.gcc.s"
ORIG_SLICE="${OUT_DIR}/coi_process_entry_selection_state.original_slice.s"
GEN_SLICE="${OUT_DIR}/coi_process_entry_selection_state.generated_slice.s"
ORIG_NORM="${OUT_DIR}/coi_process_entry_selection_state.original_slice.norm.s"
GEN_NORM="${OUT_DIR}/coi_process_entry_selection_state.generated_slice.norm.s"
...
awk '
$0 ~ /^COI_ProcessEntrySelectionState:?$/ {print; exit}
' "$ORIG_ASM" > "$ORIG_SLICE"

awk '
$0 ~ /^_?COI_ProcessEntrySelectionState:?$/ {in_func=1}
in_func { print }
' "$GEN_ASM" > "$GEN_SLICE"
...
awk -f src/decomp/scripts/semantic_filter_coi_process_entry_selection_state.awk "$ORIG_NORM" > "$ORIG_SEM"
awk -f src/decomp/scripts/semantic_filter_coi_process_entry_selection_state.awk "$GEN_NORM" > "$GEN_SEM"
