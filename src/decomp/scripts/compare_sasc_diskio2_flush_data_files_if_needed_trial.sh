...
BASE="diskio2_flush_data_files_if_needed"
ENTRY="DISKIO2_FlushDataFilesIfNeeded"
ENTRY_SASC_REGEX="^DISKIO2_FlushDataFilesIfNee[A-Za-z0-9_]*:$"
ENTRY_SEM="DISKIO2_FLUSHDATAFILESIFNEEDE"
ENTRY_ALT_SEM="DISKIO2_FLUSHDATAFILESIFNEE"
...
awk -v ENTRY_PREFIX="$ENTRY_SEM" -v ENTRY_ALT_PREFIX="$ENTRY_ALT_SEM" -f src/decomp/scripts/semantic_filter_sasc_diskio2_flush_helper.awk "${OUT_DIR}/${BASE}.original.norm.s" >"${OUT_DIR}/${BASE}.original.semantic.txt"
awk -v ENTRY_PREFIX="$ENTRY_SEM" -v ENTRY_ALT_PREFIX="$ENTRY_ALT_SEM" -f src/decomp/scripts/semantic_filter_sasc_diskio2_flush_helper.awk "${OUT_DIR}/${BASE}.sasc.norm.s" >"${OUT_DIR}/${BASE}.sasc.semantic.txt"
