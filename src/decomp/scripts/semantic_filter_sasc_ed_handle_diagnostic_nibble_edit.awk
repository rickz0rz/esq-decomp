...
function t(s, x){x=s;sub(/;.*/,"",x);sub(/^[ \t]+/,"",x);sub(/[ \t]+$/,"",x);gsub(/[ \t]+/," ",x);return toupper(x)}
{
    ...
    if(l~/(JSR|BSR).*ED1_JMPTBL_ESQSHARED4_LOADDEFAULTPALETTETOCOPPER_NO_OP/ || l~/(JSR|BSR).*ED1_JMPTBL_ESQSHARED4_LOADDEFAULT/ || l~/(JSR|BSR).*ED1_JMPTBL_ESQSHARED4_LOADDEFAUL/)h_palette=1
