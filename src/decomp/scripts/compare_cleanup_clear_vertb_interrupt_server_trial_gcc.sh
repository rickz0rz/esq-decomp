...
SRC_C="src/decomp/c/replacements/cleanup_clear_vertb_interrupt_server_gcc.c"
ORIG_ASM="src/modules/groups/a/b/cleanup.s"
OUT_DIR="build/decomp/c_trial_gcc"
GEN_ASM="${OUT_DIR}/cleanup_clear_vertb_interrupt_server.generated.gcc.s"
ORIG_SLICE="${OUT_DIR}/cleanup_clear_vertb_interrupt_server.original_slice.s"
GEN_SLICE="${OUT_DIR}/cleanup_clear_vertb_interrupt_server.generated_slice.s"
ORIG_NORM="${OUT_DIR}/cleanup_clear_vertb_interrupt_server.original_slice.norm.s"
GEN_NORM="${OUT_DIR}/cleanup_clear_vertb_interrupt_server.generated_slice.norm.s"
...
awk '$0 ~ /^CLEANUP_ClearVertbInterruptServer:?$/ {in_func=1} in_func { if ($0 ~ /^;!======/) exit; print }' "$ORIG_ASM" > "$ORIG_SLICE"
awk '$0 ~ /^_?CLEANUP_ClearVertbInterruptServer:?$/ {in_func=1} in_func { print }' "$GEN_ASM" > "$GEN_SLICE"
...
awk -f src/decomp/scripts/semantic_filter_cleanup_clear_vertb_interrupt_server.awk "$ORIG_NORM" > "$ORIG_SEM"
awk -f src/decomp/scripts/semantic_filter_cleanup_clear_vertb_interrupt_server.awk "$GEN_NORM" > "$GEN_SEM"
