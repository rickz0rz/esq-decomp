...
- Current audit note (March 19, 2026): the compare scripts are only as trustworthy as their `semantic_filter_sasc_*` companions. When a lane shows an empty semantic diff but the raw `.diff` is still very large, inspect the filter before assuming the implementation is complete. This is now a proven source of false greens in this repo.
- Current ESQPARS note (March 19, 2026): `src/decomp/sas_c/esqpars_consume_rbf_byte_and_dispatch_command.c` has moved well beyond the earlier stub and now restores the dispatcher families around `!`, `P/p`, `g`, banner/config/font/status handling, and related parser-state updates. The remaining work in that file is micro-semantic, especially exact reject-path byte consumption and precise title/attribute mutation inside the `!` and lower-`p` branches.
