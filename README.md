...
- Current repo state: the maintained SAS/C lane is semantically green. A full local rerun of the current `compare_sasc_*` set on March 13, 2026 produced no non-empty `build/decomp/sasc_trial/*.semantic.diff` files, so do not assume an existing SAS/C target still needs first-pass recovery just because its raw asm diff is large or because an older GCC trial still exists.
- Current triage state (March 13, 2026): the maintained `run_sasc_core_sweep.sh` set, the remaining out-of-sweep `compare_sasc_memory_allocate_trial.sh` / `compare_sasc_esqfunc_draw_esc_menu_version_trial.sh` checks, and a full local rerun of the current `compare_sasc_*` script set all currently produce zero-byte semantic diffs. Future triage should start from genuinely GCC-only behavior targets rather than re-checking the already-green SAS/C lane.
- Additional triage note (March 13, 2026): an exact-word scan of the current non-`JMPTBL` GCC replacement exports against `src/decomp/sas_c/` now also comes back empty. Use `src/decomp/scripts/list_missing_sasc_non_jmptbl_exports.py` before assuming a non-wrapper GCC trial still lacks a restored SAS/C definition.
