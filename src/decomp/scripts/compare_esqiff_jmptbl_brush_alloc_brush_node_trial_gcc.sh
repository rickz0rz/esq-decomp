...
SRC_C="src/decomp/c/replacements/esqiff_jmptbl_brush_alloc_brush_node_gcc.c"
ORIG_ASM="src/modules/groups/a/n/esqiff.s"
OUT_DIR="build/decomp/c_trial_gcc"
GEN_ASM="${OUT_DIR}/esqiff_jmptbl_brush_alloc_brush_node.generated.gcc.s"
ORIG_SLICE="${OUT_DIR}/esqiff_jmptbl_brush_alloc_brush_node.original_slice.s"
GEN_SLICE="${OUT_DIR}/esqiff_jmptbl_brush_alloc_brush_node.generated_slice.s"
ORIG_NORM="${OUT_DIR}/esqiff_jmptbl_brush_alloc_brush_node.original_slice.norm.s"
GEN_NORM="${OUT_DIR}/esqiff_jmptbl_brush_alloc_brush_node.generated_slice.norm.s"
...
awk '
$0 ~ /^ESQIFF_JMPTBL_BRUSH_AllocBrushNode:?$/ {in_func=1}
in_func {
    if ($0 ~ /^ESQIFF_JMPTBL_ESQ_NoOp_006A:?$/) {
        exit
    }
    print
}
...
awk '
$0 ~ /^_?ESQIFF_JMPTBL_BRUSH_AllocBrushNode:?$/ {in_func=1}
in_func { print }
' "$GEN_ASM" > "$GEN_SLICE"
...
awk -f src/decomp/scripts/semantic_filter_esqiff_jmptbl_brush_alloc_brush_node.awk "$ORIG_NORM" > "$ORIG_SEM"
awk -f src/decomp/scripts/semantic_filter_esqiff_jmptbl_brush_alloc_brush_node.awk "$GEN_NORM" > "$GEN_SEM"
