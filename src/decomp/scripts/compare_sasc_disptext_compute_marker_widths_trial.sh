...
SASC_SRC="disptext_marker_helpers.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/a/i/disptext.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="disptext_compute_marker_widths"
ENTRY="DISPTEXT_ComputeMarkerWidths"
ENTRY_SASC_REGEX="^DISPTEXT_ComputeMarkerWidths[A-Za-z0-9_]*:$"
ENTRY_SEM="DISPTEXT_COMPUTEMARKERWIDTHS:"
ENTRY_ALT_SEM="DISPTEXT_COMPUTEMARKERWIDTH"
...
awk -v ENTRY_PREFIX="$ENTRY_SEM" -v ENTRY_ALT_PREFIX="$ENTRY_ALT_SEM" -f src/decomp/scripts/semantic_filter_sasc_disptext_compute_marker_widths.awk "${OUT_DIR}/${BASE}.original.norm.s" >"${OUT_DIR}/${BASE}.original.semantic.txt"
awk -v ENTRY_PREFIX="$ENTRY_SEM" -v ENTRY_ALT_PREFIX="$ENTRY_ALT_SEM" -f src/decomp/scripts/semantic_filter_sasc_disptext_compute_marker_widths.awk "${OUT_DIR}/${BASE}.sasc.norm.s" >"${OUT_DIR}/${BASE}.sasc.semantic.txt"
