...
SASC_SRC="stream_buffered_write_string.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/submodules/unknown2b.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="stream_buffered_write_string"
ENTRY="STREAM_BufferedWriteString"
ENTRY_SASC_REGEX="^STREAM_BufferedWriteString[A-Za-z0-9_]*:$"
...
awk -v e="^${ENTRY}:$" -v e2="$ENTRY_SASC_REGEX" '$0 ~ e || $0 ~ e2 {inf=1} inf { if (($0 ~ /^STREAM_[A-Za-z0-9_]+:$/) && $0 !~ e && $0 !~ e2) exit; if ($0 ~ /^XREF / || $0 ~ /^XDEF / || $0 ~ /^ END$/ || $0 ~ /^END$/ || $0 ~ /^__const:$/) exit; print }' "$SASC_DIS" >"${OUT_DIR}/${BASE}.sasc.dis.s"
...
awk -f src/decomp/scripts/semantic_filter_sasc_stream_buffered_write_string.awk "${OUT_DIR}/${BASE}.original.norm.s" >"${OUT_DIR}/${BASE}.original.semantic.txt"
awk -f src/decomp/scripts/semantic_filter_sasc_stream_buffered_write_string.awk "${OUT_DIR}/${BASE}.sasc.norm.s" >"${OUT_DIR}/${BASE}.sasc.semantic.txt"
