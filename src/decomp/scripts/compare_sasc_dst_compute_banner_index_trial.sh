...
awk '$0 ~ /^DST_ComputeBannerIndex:$/ {in_func=1} in_func { if ($0 ~ /^;!======/) exit; print }' "$ORIG_ASM" >"${OUT_DIR}/dst_compute_banner_index.original.s"
awk '$0 ~ /^[[:space:]]*DST_ComputeBannerIndex:/ {in_func=1} in_func { if ($0 ~ /^[[:space:]]*__const:$/) exit; print }' "$SASC_DIS" >"${OUT_DIR}/dst_compute_banner_index.sasc.dis.s"
