...
SASC_SRC="kybd_initialize_input_devices.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/a/v/kybd.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="kybd_initialize_input_devices"
ENTRY="KYBD_InitializeInputDevices"
ENTRY_SASC_REGEX="^KYBD_InitializeInputDev[A-Za-z0-9_]*:$"
...
awk -v e="^${ENTRY}:$" -v e2="$ENTRY_SASC_REGEX" '$0 ~ e || $0 ~ e2 {in_func=1} in_func { if (($0 ~ /^KYBD_[A-Za-z0-9_]+:$/ || $0 ~ /^_?KYBD_[A-Za-z0-9_]+:$/) && $0 !~ e && $0 !~ e2) exit; if ($0 ~ /^XREF / || $0 ~ /^XDEF / || $0 ~ /^ END$/ || $0 ~ /^END$/ || $0 ~ /^__const:$/) exit; print }' \
...
awk -f src/decomp/scripts/semantic_filter_sasc_kybd_initialize_input_devices.awk \
