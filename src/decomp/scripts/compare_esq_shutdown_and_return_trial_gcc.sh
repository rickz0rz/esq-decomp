...
SRC_C="src/decomp/c/replacements/esq_shutdown_and_return_gcc.c"
ORIG_ASM="src/modules/groups/_main/a/a.s"
OUT_DIR="build/decomp/c_trial_gcc"
GEN_ASM="${OUT_DIR}/esq_shutdown_and_return.generated.gcc.s"
ORIG_SLICE="${OUT_DIR}/esq_shutdown_and_return.original_slice.s"
GEN_SLICE="${OUT_DIR}/esq_shutdown_and_return.generated_slice.s"
ORIG_NORM="${OUT_DIR}/esq_shutdown_and_return.original_slice.norm.s"
GEN_NORM="${OUT_DIR}/esq_shutdown_and_return.generated_slice.norm.s"
...
awk -v start_label="ESQ_ShutdownAndReturn" -v next_label="ESQ_STR_DosLibrary" '
...
awk '$0 ~ /^_?ESQ_ShutdownAndReturn:?$/ {in_func=1} in_func { print }' "$GEN_ASM" > "$GEN_SLICE"
...
awk -f "src/decomp/scripts/semantic_filter_esq_shutdown_and_return.awk" "$ORIG_NORM" > "$ORIG_SEM"
awk -f "src/decomp/scripts/semantic_filter_esq_shutdown_and_return.awk" "$GEN_NORM" > "$GEN_SEM"
