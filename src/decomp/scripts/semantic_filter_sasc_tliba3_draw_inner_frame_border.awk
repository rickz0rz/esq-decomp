...

{
    ...
    if (u ~ /^TLIBA3_DRAWINNERFRAMEBORDER:/ || u ~ /^TLIBA3_DRAWINNERFRAMEBORDE[A-Z0-9_]*:/) has_entry=1
    if (n ~ /LVOMOVE/) move_calls++
...
END {
    ...
    print "HAS_MOVE_CALLS_GE_2="(move_calls >= 2 ? 1 : 0)
    print "HAS_DRAW_CALLS_GE_2="(draw_calls >= 2 ? 1 : 0)
