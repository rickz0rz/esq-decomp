...
BASE="diskio1_append_attr_flag_hilite_src_state_flow"
ENTRY="DISKIO1_AppendAttrFlagHiliteSrc"
ENTRY_SASC_REGEX="^DISKIO1_AppendAttrFlagHiliteSrc[A-Za-z0-9_]*:$"
...
awk -v e="^${ENTRY}:$" -v e2="$ENTRY_SASC_REGEX" '
  $0 ~ e || $0 ~ e2 {in_func=1}
  in_func {
    if ($0 ~ /^DISKIO1_AppendAttrFlagSummarySrc:$/) exit
...
awk -f src/decomp/scripts/semantic_filter_sasc_diskio1_append_attr_flag_hilite_src_state_flow.awk "${OUT_DIR}/${BASE}.original.norm.s" >"${OUT_DIR}/${BASE}.original.semantic.txt"
awk -f src/decomp/scripts/semantic_filter_sasc_diskio1_append_attr_flag_hilite_src_state_flow.awk "${OUT_DIR}/${BASE}.sasc.norm.s" >"${OUT_DIR}/${BASE}.sasc.semantic.txt"
