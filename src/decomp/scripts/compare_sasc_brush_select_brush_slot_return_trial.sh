...
cd "$ROOT_DIR"
SASC_SRC="brush_select_brush_slot_return.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/a/a/brush.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="brush_select_brush_slot_return"
ENTRY="BRUSH_SelectBrushSlot_Return"
ENTRY_SASC_REGEX="^BRUSH_SelectBrushSlot_Return[A-Za-z0-9_]*:$"
mkdir -p "$OUT_DIR"
...
awk -v ENTRY="$ENTRY" -v ENTRY_REGEX="$ENTRY_SASC_REGEX" -f src/decomp/scripts/semantic_filter_sasc_brush_select_brush_slot_return.awk "${OUT_DIR}/${BASE}.original.norm.s" >"${OUT_DIR}/${BASE}.original.semantic.txt"
awk -v ENTRY="$ENTRY" -v ENTRY_REGEX="$ENTRY_SASC_REGEX" -f src/decomp/scripts/semantic_filter_sasc_brush_select_brush_slot_return.awk "${OUT_DIR}/${BASE}.sasc.norm.s" >"${OUT_DIR}/${BASE}.sasc.semantic.txt"
