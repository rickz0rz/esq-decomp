...
cd "$ROOT_DIR"
SASC_SRC="script_jmptbl_stubs.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/b/a/script.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="script_jmptbl_diskio_closebufferedfileandflush"
ENTRY_ORIG="SCRIPT_JMPTBL_DISKIO_CloseBufferedFileAndFlush"
ENTRY_SASC="SCRIPT_JMPTBL_DISKIO_CloseBuffer"
TARGET="DISKIO_CloseBufferedFileAndFlush"
mkdir -p "$OUT_DIR"
./sc-build-with-dis.sh "$SASC_SRC" >"${OUT_DIR}/sc_build_${BASE}.log" 2>&1
awk -v e="^${ENTRY_ORIG}:$" '$0 ~ e {inf=1} inf { if ($0 ~ /^SCRIPT_JMPTBL_/ && $0 !~ e) exit; if ($0 ~ /^;!======/) exit; print }' "$ORIG_ASM" >"${OUT_DIR}/${BASE}.original.s"
