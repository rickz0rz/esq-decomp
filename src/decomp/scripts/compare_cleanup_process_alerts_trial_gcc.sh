...
SRC_C="src/decomp/c/replacements/cleanup_process_alerts_gcc.c"
ORIG_ASM="src/modules/groups/a/c/cleanup2.s"
OUT_DIR="build/decomp/c_trial_gcc"
GEN_ASM="${OUT_DIR}/cleanup_process_alerts.generated.gcc.s"
ORIG_SLICE="${OUT_DIR}/cleanup_process_alerts.original_slice.s"
GEN_SLICE="${OUT_DIR}/cleanup_process_alerts.generated_slice.s"
ORIG_NORM="${OUT_DIR}/cleanup_process_alerts.original_slice.norm.s"
GEN_NORM="${OUT_DIR}/cleanup_process_alerts.generated_slice.norm.s"
ORIG_SEM="${OUT_DIR}/cleanup_process_alerts.original_slice.semantic.txt"
GEN_SEM="${OUT_DIR}/cleanup_process_alerts.generated_slice.semantic.txt"
...
awk '$0 ~ /^CLEANUP_ProcessAlerts:?$/ {in_func=1} in_func { if ($0 ~ /^;!======/) exit; print }' "$ORIG_ASM" > "$ORIG_SLICE"
awk '$0 ~ /^_?CLEANUP_ProcessAlerts:?$/ {in_func=1} in_func { print }' "$GEN_ASM" > "$GEN_SLICE"
...
awk -f src/decomp/scripts/semantic_filter_cleanup_process_alerts.awk "$ORIG_NORM" > "$ORIG_SEM"
awk -f src/decomp/scripts/semantic_filter_cleanup_process_alerts.awk "$GEN_NORM" > "$GEN_SEM"
