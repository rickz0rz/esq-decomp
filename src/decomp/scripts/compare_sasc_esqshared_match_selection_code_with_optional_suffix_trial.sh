...
SASC_SRC="esqshared_match_selection_code_with_optional_suffix.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/a/p/esqshared.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="esqshared_match_selection_code_with_optional_suffix"
ENTRY="ESQSHARED_MatchSelectionCodeWithOptionalSuffix"
...
awk -v e="^${ENTRY}:$" -v e2="$ENTRY_SASC_REGEX" '$0 ~ e || $0 ~ e2 {in_func=1} in_func { if (($0 ~ /^ESQSHARED_[A-Za-z0-9_]+:$/ || $0 ~ /^_?ESQSHARED_[A-Za-z0-9_]+:$/) && $0 !~ e && $0 !~ e2) exit; if ($0 ~ /^XREF / || $0 ~ /^XDEF / || $0 ~ /^ END$/ || $0 ~ /^END$/ || $0 ~ /^__const:$/) exit; print }' \
...
awk -f src/decomp/scripts/semantic_filter_sasc_esqshared_match_selection_code_with_optional_suffix.awk \
