...
BASE="esqdisp_get_entry_pointer_by_mode"
ENTRY="ESQDISP_GetEntryPointerByMode"
ENTRY_SASC_REGEX="^ESQDISP_GetEntryPointerByMode[A-Za-z0-9_]*:$"
ENTRY_SEM="ESQDISP_GETENTRYPOINTERBYMODE:"
ENTRY_ALT_SEM="ESQDISP_GETENTRYPOINTERBYMOD"
...
awk -v ENTRY_PREFIX="$ENTRY_SEM" -v ENTRY_ALT_PREFIX="$ENTRY_ALT_SEM" -f src/decomp/scripts/semantic_filter_sasc_esqdisp_get_entry_pointer_by_mode.awk "${OUT_DIR}/${BASE}.original.norm.s" >"${OUT_DIR}/${BASE}.original.semantic.txt"
awk -v ENTRY_PREFIX="$ENTRY_SEM" -v ENTRY_ALT_PREFIX="$ENTRY_ALT_SEM" -f src/decomp/scripts/semantic_filter_sasc_esqdisp_get_entry_pointer_by_mode.awk "${OUT_DIR}/${BASE}.sasc.norm.s" >"${OUT_DIR}/${BASE}.sasc.semantic.txt"
