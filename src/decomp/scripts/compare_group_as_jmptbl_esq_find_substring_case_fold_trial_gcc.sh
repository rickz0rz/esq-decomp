...
SRC_C="src/decomp/c/replacements/group_as_jmptbl_esq_find_substring_case_fold_gcc.c"
ORIG_ASM="src/modules/groups/a/s/xjump.s"
OUT_DIR="build/decomp/c_trial_gcc"
GEN_ASM="${OUT_DIR}/group_as_jmptbl_esq_find_substring_case_fold.generated.gcc.s"
ORIG_SLICE="${OUT_DIR}/group_as_jmptbl_esq_find_substring_case_fold.original_slice.s"
GEN_SLICE="${OUT_DIR}/group_as_jmptbl_esq_find_substring_case_fold.generated_slice.s"
ORIG_NORM="${OUT_DIR}/group_as_jmptbl_esq_find_substring_case_fold.original_slice.norm.s"
GEN_NORM="${OUT_DIR}/group_as_jmptbl_esq_find_substring_case_fold.generated_slice.norm.s"
...
awk '
$0 ~ /^GROUP_AS_JMPTBL_ESQ_FindSubstringCaseFold:?$/ {in_func=1}
in_func { print }
' "$ORIG_ASM" > "$ORIG_SLICE"

awk '
$0 ~ /^_?GROUP_AS_JMPTBL_ESQ_FindSubstringCaseFold:?$/ {in_func=1}
in_func { print }
' "$GEN_ASM" > "$GEN_SLICE"
...
awk -f src/decomp/scripts/semantic_filter_group_as_jmptbl_esq_find_substring_case_fold.awk "$ORIG_NORM" > "$ORIG_SEM"
awk -f src/decomp/scripts/semantic_filter_group_as_jmptbl_esq_find_substring_case_fold.awk "$GEN_NORM" > "$GEN_SEM"
