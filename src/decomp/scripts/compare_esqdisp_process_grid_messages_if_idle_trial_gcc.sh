...
SRC_C="src/decomp/c/replacements/esqdisp_process_grid_messages_if_idle_gcc.c"
ORIG_ASM="src/modules/groups/a/n/esqdisp.s"
OUT_DIR="build/decomp/c_trial_gcc"
GEN_ASM="${OUT_DIR}/esqdisp_process_grid_messages_if_idle.generated.gcc.s"
ORIG_SLICE="${OUT_DIR}/esqdisp_process_grid_messages_if_idle.original_slice.s"
GEN_SLICE="${OUT_DIR}/esqdisp_process_grid_messages_if_idle.generated_slice.s"
ORIG_NORM="${OUT_DIR}/esqdisp_process_grid_messages_if_idle.original_slice.norm.s"
GEN_NORM="${OUT_DIR}/esqdisp_process_grid_messages_if_idle.generated_slice.norm.s"
...
awk '$0 ~ /^ESQDISP_ProcessGridMessagesIfIdle:?$/ {in_func=1} in_func { if ($0 ~ /^;!======/) exit; print }' "$ORIG_ASM" > "$ORIG_SLICE"
awk '$0 ~ /^_?ESQDISP_ProcessGridMessagesIfIdle:?$/ {in_func=1} in_func { print }' "$GEN_ASM" > "$GEN_SLICE"
...
awk -f src/decomp/scripts/semantic_filter_esqdisp_process_grid_messages_if_idle.awk "$ORIG_NORM" > "$ORIG_SEM"
awk -f src/decomp/scripts/semantic_filter_esqdisp_process_grid_messages_if_idle.awk "$GEN_NORM" > "$GEN_SEM"
