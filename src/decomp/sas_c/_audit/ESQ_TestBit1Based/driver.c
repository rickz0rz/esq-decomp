...
#include <stdio.h>
extern LONG __stdargs TESTFN(UBYTE *, ULONG);
int main(void) {
    ...
    static ULONG a1[] = { 0x00000000UL, 0x00000002UL, 0x00000004UL, 0x00000008UL, 0x00000010UL, 0x00000018UL, 0x00000020UL, 0x00000028UL, 0x00000030UL, 0x00000000UL, 0x00000002UL, 0x00000004UL, 0x00000008UL, 0x00000010UL, 0x00000018UL, 0x00000020UL, 0x00000028UL, 0x00000030UL };
    ...
    for (i = 0; i < n; i++) {
        ...
        r = TESTFN((UBYTE *)buf0, (ULONG)a1[i]);
        for (j = 0; j < 48; j++) d0 = d0 * 33 + buf0[j];
        printf("%08lx %08lx -> %08lx\n", (ULONG)d0, (ULONG)a1[i], (ULONG)r);
    }
    return 0;
}
