BEGIN {entry=0; scan=0; alloc=0; link=0; clr=0; open=0; size34=0; nullret=0; ret=0; rts=0}
function t(s){sub(/;.*/,"",s);sub(/^[ \t]+/,"",s);sub(/[ \t]+$/,"",s);gsub(/[ \t]+/," ",s);return toupper(s)}
{
  l=t($0)
  if(l=="") next
  if(l~/^HANDLE_OPENWITHMODE:/ || l~/^HANDLE_OPENWITHMOD[A-Z0-9_]*:/) entry=1
  ...
  if(l~/#34([^0-9]|$)/ || l~/#\$22/ || l~/PEA 34\.W/) size34=1
  if(l~/MOVEQ(\.L)? #\$?0,D0/ || l~/MOVEQ(\.L)? #0,D0/) nullret=1
...
END {
  ...
  print "HAS_SIZE_34=" size34
  print "HAS_NULL_RETURN=" nullret
