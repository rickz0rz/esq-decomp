...
BASE="group_ai_jmptbl_str_copy_until_any_delim_n"
ENTRY_ORIG="GROUP_AI_JMPTBL_STR_CopyUntilAnyDelimN"
ENTRY_SASC="GROUP_AI_JMPTBL_STR_CopyUntilAny"
TARGET="STR_CopyUntilAnyDelimN"
mkdir -p "$OUT_DIR"
./sc-build-with-dis.sh "$SASC_SRC" >"${OUT_DIR}/sc_build_${BASE}.log" 2>&1
awk -v e="^${ENTRY_ORIG}:$" '$0 ~ e {inf=1} inf { if ($0 ~ /^MOVEQ   #97,D0:$/) exit; print }' "$ORIG_ASM" >"${OUT_DIR}/${BASE}.original.s"
