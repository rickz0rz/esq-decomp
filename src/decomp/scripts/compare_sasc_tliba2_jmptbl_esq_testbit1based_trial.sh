...
cd "$ROOT_DIR"
SASC_SRC="tliba2_jmptbl_stubs.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/b/a/tliba2.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="tliba2_jmptbl_esq_testbit1based"
ENTRY_ORIG="TLIBA2_JMPTBL_ESQ_TestBit1Based"
ENTRY_SASC="TLIBA2_JMPTBL_ESQ_TestBit1Based"
TARGET="ESQ_TestBit1Based"
mkdir -p "$OUT_DIR"
./sc-build-with-dis.sh "$SASC_SRC" >"${OUT_DIR}/sc_build_${BASE}.log" 2>&1
awk -v e="^${ENTRY_ORIG}:$" '$0 ~ e {inf=1} inf { if ($0 ~ /^TLIBA2_JMPTBL_/ && $0 !~ e) exit; if ($0 ~ /^;!======/) exit; print }' "$ORIG_ASM" >"${OUT_DIR}/${BASE}.original.s"
