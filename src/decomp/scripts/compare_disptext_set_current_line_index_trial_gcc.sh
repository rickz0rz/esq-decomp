...
SRC_C="src/decomp/c/replacements/disptext_set_current_line_index_gcc.c"
ORIG_ASM="src/modules/groups/a/i/disptext.s"
OUT_DIR="build/decomp/c_trial_gcc"
GEN_ASM="${OUT_DIR}/disptext_set_current_line_index.generated.gcc.s"
ORIG_SLICE="${OUT_DIR}/disptext_set_current_line_index.original_slice.s"
GEN_SLICE="${OUT_DIR}/disptext_set_current_line_index.generated_slice.s"
ORIG_NORM="${OUT_DIR}/disptext_set_current_line_index.original_slice.norm.s"
GEN_NORM="${OUT_DIR}/disptext_set_current_line_index.generated_slice.norm.s"
...
awk '
$0 ~ /^DISPTEXT_SetCurrentLineIndex:?$/ {in_func=1}
in_func {
    if ($0 ~ /^;!======/) exit
    print
}
...
awk '
$0 ~ /^_?DISPTEXT_SetCurrentLineIndex:?$/ {in_func=1}
in_func { print }
' "$GEN_ASM" > "$GEN_SLICE"
...
awk -f src/decomp/scripts/semantic_filter_disptext_set_current_line_index.awk "$ORIG_NORM" > "$ORIG_SEM"
awk -f src/decomp/scripts/semantic_filter_disptext_set_current_line_index.awk "$GEN_NORM" > "$GEN_SEM"
