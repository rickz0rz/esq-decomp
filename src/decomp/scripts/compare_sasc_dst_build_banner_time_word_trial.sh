...
awk '$0 ~ /^DST_BuildBannerTimeWord:$/ {in_func=1} in_func { if ($0 ~ /^;!======/) exit; print }' "$ORIG_ASM" >"${OUT_DIR}/dst_build_banner_time_word.original.s"
awk '$0 ~ /^[[:space:]]*DST_BuildBannerTimeWord:/ {in_func=1} in_func { if ($0 ~ /^[[:space:]]*__const:$/) exit; print }' "$SASC_DIS" >"${OUT_DIR}/dst_build_banner_time_word.sasc.dis.s"
