...

{
    ...
    if (uline ~ /^DISKIO1_APPENDTIMESLOTMASKVALUEHEADER:/) has_entry = 1
    if (uline ~ /^PEA DISKIO_STR_TIMESLOTLISTOPENPAREN$/) has_pea = 1
    if (uline ~ /^JSR GROUP_AJ_JMPTBL_FORMAT_RAWDOFMTWITHSCRATCHBUFFER(\(PC\))?$/) has_jsr = 1
    if (uline ~ /^ADDQ\.W #4,A7$/) has_addq = 1
    if (uline ~ /^MOVEQ #1,D4$/) has_moveq = 1
}
