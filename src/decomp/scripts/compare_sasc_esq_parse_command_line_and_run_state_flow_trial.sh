...
SASC_SRC="esq_parse_command_line_and_run.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/submodules/unknown29.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="esq_parse_command_line_and_run_state_flow"
ENTRY="ESQ_ParseCommandLineAndRun"
ENTRY_SASC_REGEX="^ESQ_ParseCommandLineAndRun[A-Za-z0-9_]*:$"
...
awk -v e="^${ENTRY}:$" -v e2="$ENTRY_SASC_REGEX" '$0 ~ e || $0 ~ e2 {in_func=1} in_func { if ($0 ~ /^XREF / || $0 ~ /^XDEF / || $0 ~ /^ END$/ || $0 ~ /^END$/ || $0 ~ /^__const:$/) exit; print }' \
...
awk -f src/decomp/scripts/semantic_filter_sasc_esq_parse_command_line_and_run_state_flow.awk \
