...
SASC_SRC="p_type_load_promo_id_data_file.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/b/a/p_type.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="p_type_load_promo_id_data_file"
ENTRY_ORIG="P_TYPE_LoadPromoIdDataFile"
ENTRY_SASC_REGEX="^P_TYPE_LoadPromoIdDataFile[A-Za-z0-9_]*:$"

mkdir -p "$OUT_DIR"

./sc-build-with-dis.sh "$SASC_SRC" >"${OUT_DIR}/sc_build_${BASE}.log" 2>&1

awk -v start="^${ENTRY_ORIG}:$" -v next_label="^P_TYPE_JMPTBL_STRING_FindSubstring:$" '$0 ~ start {in_func=1} in_func {if ($0 ~ next_label) exit; print}' "$ORIG_ASM" >"${OUT_DIR}/${BASE}.original.s"
...
awk -f src/decomp/scripts/semantic_filter_sasc_p_type_load_promo_id_data_file.awk "${OUT_DIR}/${BASE}.original.norm.s" >"${OUT_DIR}/${BASE}.original.semantic.txt"
awk -f src/decomp/scripts/semantic_filter_sasc_p_type_load_promo_id_data_file.awk "${OUT_DIR}/${BASE}.sasc.norm.s" >"${OUT_DIR}/${BASE}.sasc.semantic.txt"
