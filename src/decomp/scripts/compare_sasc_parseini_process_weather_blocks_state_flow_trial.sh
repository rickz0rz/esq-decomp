...
awk -v start="^${ENTRY}:$" -v next_label="^PARSEINI_LoadWeatherStrings:$" '
    $0 ~ start { in_func = 1 }
    in_func {
...
awk -f src/decomp/scripts/semantic_filter_sasc_parseini_process_weather_blocks_state_flow.awk "${OUT_DIR}/${BASE}.original.norm.s" >"${OUT_DIR}/${BASE}.original.semantic.txt"
awk -f src/decomp/scripts/semantic_filter_sasc_parseini_process_weather_blocks_state_flow.awk "${OUT_DIR}/${BASE}.sasc.norm.s" >"${OUT_DIR}/${BASE}.sasc.semantic.txt"
