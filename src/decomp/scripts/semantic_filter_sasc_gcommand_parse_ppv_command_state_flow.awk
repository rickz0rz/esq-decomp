BEGIN {
    h_entry = 0
    h_defaults = 0
    h_empty_guard = 0
    h_prefix_parse = 0
    h_three_digit_parse = 0
    h_pen_parse = 0
    h_hex_parse = 0
    h_fold_upper = 0
    h_enable_flag = 0
    h_mode_cycle = 0
    h_window_minutes = 0
    h_tolerance_minutes = 0
    h_message_text_pen = 0
    h_message_frame_pen = 0
    h_editor_layout_pen = 0
    h_editor_row_pen = 0
    h_showtimes_layout_pen = 0
    h_showtimes_initial_line = 0
    h_showtimes_row_pen = 0
    h_workflow_mode = 0
    h_detail_flag = 0
    h_rowspan_parse = 0
    h_tail_find = 0
    h_tail_split_nul = 0
    h_tail_clamp = 0
    h_period_replace = 0
    h_listings_replace = 0
    h_tail_fallback_replace = 0
    h_load_template = 0
    h_rts = 0

    seed_seen = 0
    seed_copy_count = 0
    parse_long_count = 0
    parse_valid_digits_count = 0
    parse_pen_count = 0
    parse_hex_count = 0
    fold_upper_count = 0
    replace_seen = 0
    saw_find_delim = 0
    saw_find_call = 0
    saw_fallback_period_base = 0
    phase_enable_flag = 0
    phase_mode_cycle = 0
    phase_window_minutes = 0
    phase_tolerance_minutes = 0
    phase_message_text_pen = 0
    phase_message_frame_pen = 0
    phase_editor_layout_pen = 0
    phase_editor_row_pen = 0
    phase_showtimes_layout_pen = 0
    phase_showtimes_initial_line = 0
    phase_showtimes_row_pen = 0
    phase_workflow_mode = 0
    phase_detail_flag = 0
    phase_rowspan = 0
    phase_tail_find = 0
    phase_tail_split_nul = 0
    phase_tail_clamp = 0
    phase_period_replace = 0
    phase_listings_replace = 0
    phase_tail_fallback = 0
    prev = ""
    prev2 = ""
    prev3 = ""
    prev4 = ""
}

function norm(s, t) {
    t = s
    sub(/;.*/, "", t)
    sub(/^[ \t]+/, "", t)
    sub(/[ \t]+$/, "", t)
    gsub(/[ \t]+/, " ", t)
    return toupper(t)
}

{
    l = norm($0)
    if (l == "") {
        next
    }

    if (l ~ /^GCOMMAND_PARSEPPVCOMMAND:/ || l ~ /^GCOMMAND_PARSEPPVCOMMAND[A-Z0-9_]*:/) {
        h_entry = 1
    }

    if (l ~ /GCOMMAND_PPVPARSESCRATCHSEEDLONG/) {
        seed_seen = 1
    }
    if ((l ~ /^MOVE\.B / && l ~ /\(A0\)\+/ && l ~ /\(A1\)\+/) ||
        (seed_seen && l ~ /MOVE\.B D0,\$2[4-7]\(A7\)/)) {
        seed_copy_count++
    }
    if ((seed_seen && seed_copy_count >= 4) || l ~ /FLIB2_LOADDIGITALPPVDEFAULTS/) {
        h_defaults = 1
    }

    if (l ~ /STRING_COPYPADNUL/) {
        h_prefix_parse = 1
    }
    if (l ~ /PARSE_READSIGNEDLONGSKIPCLASS3_ALT/ || l ~ /PARSE_READSIGNEDLONGSKIPCLASS3_A/) {
        parse_long_count++
    }
    if (l ~ /PARSE_VALID_DIGITS_VALUE/) {
        parse_valid_digits_count++
    }
    if (l ~ /PARSE_PEN_1_TO_3/) {
        parse_pen_count++
    }
    if (l ~ /LADFUNC_PARSEHEXDIGIT/) {
        parse_hex_count++
    }
    if (l ~ /FOLD_UPPER_IF_ALPHA/) {
        fold_upper_count++
    }
    if (parse_valid_digits_count >= 2 ||
        (parse_long_count >= 4 && h_window_minutes && h_tolerance_minutes)) {
        h_three_digit_parse = 1
    }
    if (parse_pen_count >= 4 ||
        (h_message_text_pen && h_editor_layout_pen && h_showtimes_layout_pen && h_showtimes_initial_line)) {
        h_pen_parse = 1
    }
    if (parse_hex_count >= 3) {
        h_hex_parse = 1
    }
    if (fold_upper_count >= 3 ||
        ((h_enable_flag + h_workflow_mode + h_detail_flag) >= 3 && (l ~ /WDISP_CHARCLASSTABLE/ || prev ~ /WDISP_CHARCLASSTABLE/ || prev2 ~ /WDISP_CHARCLASSTABLE/)) ||
        (h_enable_flag && h_workflow_mode && h_detail_flag)) {
        h_fold_upper = 1
    }

    if ((l ~ /^MOVE\.L A[35],D0$/ || l ~ /^TST\.B \(A[35]\)$/) &&
        prev ~ /FLIB2_LOADDIGITALPPVDEFAULTS/) {
        h_empty_guard = 1
    }
    if (l ~ /BEQ\.[A-Z]* .*RETURN/ || l ~ /BEQ\.[A-Z]* .*__3/ || l ~ /GCOMMAND_LOADPPVTEMPLATE/) {
        if (prev ~ /^MOVE\.L A[35],D0$/ || prev2 ~ /^MOVE\.L A[35],D0$/ ||
            prev ~ /^TST\.B \(A[35]\)$/ || prev2 ~ /^TST\.B \(A[35]\)$/) {
            h_empty_guard = 1
        }
    }

    if (l ~ /GCOMMAND_DIGITALPPVENABLEDFLAG/ || l ~ /GCOMMAND_DIGITALPPVENABL/) {
        h_enable_flag = 1
        if (!phase_enable_flag) {
            print "PHASE_ENABLE_FLAG_STORE"
            phase_enable_flag = 1
        }
    }
    if (l ~ /GCOMMAND_PPVMODECYCLECOUNT/ || l ~ /GCOMMAND_PPVMODECYCLEC/) {
        h_mode_cycle = 1
        if (!phase_mode_cycle) {
            print "PHASE_MODE_CYCLE_STORE"
            phase_mode_cycle = 1
        }
    }
    if (l ~ /GCOMMAND_PPVSELECTIONWINDOWMINUTES/ || l ~ /GCOMMAND_PPVSELECTIONWINDOWMINUT/) {
        h_window_minutes = 1
        if (!phase_window_minutes) {
            print "PHASE_SELECTION_WINDOW_STORE"
            phase_window_minutes = 1
        }
    }
    if (l ~ /GCOMMAND_PPVSELECTIONTOLERANCEMINUTES/ || l ~ /GCOMMAND_PPVSELECTIONTOLERANCEMI/) {
        h_tolerance_minutes = 1
        if (!phase_tolerance_minutes) {
            print "PHASE_SELECTION_TOLERANCE_STORE"
            phase_tolerance_minutes = 1
        }
    }
    if (l ~ /GCOMMAND_PPVMESSAGETEXTPEN/ || l ~ /GCOMMAND_PPVMESSAGETEXTP/) {
        h_message_text_pen = 1
        if (!phase_message_text_pen) {
            print "PHASE_MESSAGE_TEXT_PEN_STORE"
            phase_message_text_pen = 1
        }
    }
    if (l ~ /GCOMMAND_PPVMESSAGEFRAMEPEN/ || l ~ /GCOMMAND_PPVMESSAGEFRAMEP/) {
        h_message_frame_pen = 1
        if (!phase_message_frame_pen) {
            print "PHASE_MESSAGE_FRAME_PEN_STORE"
            phase_message_frame_pen = 1
        }
    }
    if (l ~ /GCOMMAND_PPVEDITORLAYOUTPEN/ || l ~ /GCOMMAND_PPVEDITORLAYOUTP/) {
        h_editor_layout_pen = 1
        if (!phase_editor_layout_pen) {
            print "PHASE_EDITOR_LAYOUT_PEN_STORE"
            phase_editor_layout_pen = 1
        }
    }
    if (l ~ /GCOMMAND_PPVEDITORROWPEN/ || l ~ /GCOMMAND_PPVEDITORROWP/) {
        h_editor_row_pen = 1
        if (!phase_editor_row_pen) {
            print "PHASE_EDITOR_ROW_PEN_STORE"
            phase_editor_row_pen = 1
        }
    }
    if (l ~ /GCOMMAND_PPVSHOWTIMESLAYOUTPEN/ || l ~ /GCOMMAND_PPVSHOWTIMESLAYOUTP/) {
        h_showtimes_layout_pen = 1
        if (!phase_showtimes_layout_pen) {
            print "PHASE_SHOWTIMES_LAYOUT_PEN_STORE"
            phase_showtimes_layout_pen = 1
        }
    }
    if (l ~ /GCOMMAND_PPVSHOWTIMESINITIALLINEINDEX/ || l ~ /GCOMMAND_PPVSHOWTIMESINITIALLINE/) {
        h_showtimes_initial_line = 1
        if (!phase_showtimes_initial_line) {
            print "PHASE_SHOWTIMES_INITIAL_LINE_STORE"
            phase_showtimes_initial_line = 1
        }
    }
    if (l ~ /GCOMMAND_PPVSHOWTIMESROWPEN/ || l ~ /GCOMMAND_PPVSHOWTIMESROWP/) {
        h_showtimes_row_pen = 1
        if (!phase_showtimes_row_pen) {
            print "PHASE_SHOWTIMES_ROW_PEN_STORE"
            phase_showtimes_row_pen = 1
        }
    }
    if (l ~ /GCOMMAND_PPVSHOWTIMESWORKFLOWMODE/ || l ~ /GCOMMAND_PPVSHOWTIMESWORKFLOWMOD/) {
        h_workflow_mode = 1
        if (!phase_workflow_mode) {
            print "PHASE_WORKFLOW_MODE_STORE"
            phase_workflow_mode = 1
        }
    }
    if (l ~ /GCOMMAND_PPVDETAILLAYOUTFLAG/ || l ~ /GCOMMAND_PPVDETAILLAYOUTF/) {
        h_detail_flag = 1
        if (!phase_detail_flag) {
            print "PHASE_DETAIL_FLAG_STORE"
            phase_detail_flag = 1
        }
    }
    if (l ~ /GCOMMAND_PPVSHOWTIMESROWSPAN/ || l ~ /GCOMMAND_PPVSHOWTIMESROWSPAN/) {
        h_rowspan_parse = 1
        if (!phase_rowspan) {
            print "PHASE_ROWSPAN_STORE"
            phase_rowspan = 1
        }
    }

    if (l ~ /#\$12/ || l ~ /#18/ || l ~ /\(\$12\)\.W/ || l ~ /\(18\)\.W/ ||
        l ~ /MOVE\.B #\$12,-19\(A5\)/) {
        saw_find_delim = 1
    }
    if (l ~ /STR_FINDCHARPTR/ || l ~ /GROUP_AS_JMPTBL_STR_FINDCHARPTR/) {
        saw_find_call = 1
    }
    if (saw_find_delim && saw_find_call) {
        h_tail_find = 1
        if (!phase_tail_find) {
            print "PHASE_TAIL_FIND_SPLIT"
            phase_tail_find = 1
        }
    }
    if (l ~ /^CLR\.B \(A[0-7]\)\+$/ || l ~ /^CLR\.B \(A2\)\+$/) {
        h_tail_split_nul = 1
        if (!phase_tail_split_nul) {
            print "PHASE_TAIL_SPLIT_NUL"
            phase_tail_split_nul = 1
        }
    }
    if (l ~ /CLR\.B 127\(A3,D7\.L\)/ || l ~ /CLR\.B \$7F\(A3,D7\.L\)/ ||
        l ~ /CLR\.B \$7F\(A3\)/ || l ~ /TAIL\[127\] = 0/ || l ~ /#\$7F/ || l ~ /#127/) {
        if (prev ~ /CMP\.L D[0-7],D[0-7]/ || prev2 ~ /CMP\.L D[0-7],D[0-7]/ ||
            l ~ /CLR\.B \$7F\(A3\)/ || l ~ /CLR\.B 127\(A3,D7\.L\)/) {
            h_tail_clamp = 1
            if (!phase_tail_clamp) {
                print "PHASE_TAIL_CLAMP_127"
                phase_tail_clamp = 1
            }
        }
    }

    if (l ~ /ESQPARS_REPLACEOWNEDSTRING/) {
        replace_seen = 1
    }
    if ((l ~ /MOVEA?\.L A3,A0/ || l ~ /MOVE\.L A5,A0/) &&
        (prev ~ /TST\.B 0\(A3,D7\.L\)/ || prev ~ /BEQ\.[A-Z]* .*APPEND_TAIL_FALLBACK/ ||
         prev2 ~ /TST\.B 0\(A3,D7\.L\)/ || prev2 ~ /BNE\.[A-Z]* .*APPEND_TAIL_FALLBACK/ ||
         prev ~ /BEQ\.[A-Z]* .*__85/ || prev2 ~ /BEQ\.[A-Z]* .*__85/)) {
        saw_fallback_period_base = 1
    }
    if ((l ~ /ADDA?\.L D6,A0/ || l ~ /ADD\.L D5,A0/) && (prev ~ /MOVEA?\.L A3,A0/ || prev ~ /MOVE\.L A5,A0/)) {
        saw_fallback_period_base = 1
    }

    if (replace_seen && (l ~ /GCOMMAND_PPVPERIODTEMPLATEPTR/ || l ~ /GCOMMAND_PPVPERIODTEMPL/)) {
        h_period_replace = 1
        if (saw_fallback_period_base) {
            h_tail_fallback_replace = 1
            if (!phase_tail_fallback) {
                print "PHASE_TAIL_FALLBACK_PERIOD_REPLACE"
                phase_tail_fallback = 1
            }
        }
        if (!phase_period_replace) {
            print "PHASE_PERIOD_TEMPLATE_REPLACE"
            phase_period_replace = 1
        }
        replace_seen = 0
        saw_fallback_period_base = 0
    } else if (replace_seen && (l ~ /GCOMMAND_PPVLISTINGSTEMPLATEPTR/ || l ~ /GCOMMAND_PPVLISTINGSTEMPL/)) {
        h_listings_replace = 1
        if (!phase_listings_replace) {
            print "PHASE_LISTINGS_TEMPLATE_REPLACE"
            phase_listings_replace = 1
        }
        replace_seen = 0
    }

    if (l ~ /GCOMMAND_LOADPPVTEMPLATE/) {
        h_load_template = 1
    }

    if (l == "RTS") {
        h_rts = 1
    }

    prev4 = prev3
    prev3 = prev2
    prev2 = prev
    prev = l
}

END {
    print "HAS_ENTRY=" h_entry
    print "HAS_DEFAULTS_RESET=" h_defaults
    print "HAS_EMPTY_GUARD=" h_empty_guard
    print "HAS_PREFIX_PARSE=" h_prefix_parse
    print "HAS_THREE_DIGIT_PARSE=" h_three_digit_parse
    print "HAS_PEN_PARSE=" h_pen_parse
    print "HAS_HEX_PARSE=" h_hex_parse
    print "HAS_FOLD_UPPER=" h_fold_upper
    print "HAS_ENABLE_FLAG_STORE=" h_enable_flag
    print "HAS_MODE_CYCLE_STORE=" h_mode_cycle
    print "HAS_WINDOW_MINUTES_STORE=" h_window_minutes
    print "HAS_TOLERANCE_MINUTES_STORE=" h_tolerance_minutes
    print "HAS_MESSAGE_TEXT_PEN_STORE=" h_message_text_pen
    print "HAS_MESSAGE_FRAME_PEN_STORE=" h_message_frame_pen
    print "HAS_EDITOR_LAYOUT_PEN_STORE=" h_editor_layout_pen
    print "HAS_EDITOR_ROW_PEN_STORE=" h_editor_row_pen
    print "HAS_SHOWTIMES_LAYOUT_PEN_STORE=" h_showtimes_layout_pen
    print "HAS_SHOWTIMES_INITIAL_LINE_STORE=" h_showtimes_initial_line
    print "HAS_SHOWTIMES_ROW_PEN_STORE=" h_showtimes_row_pen
    print "HAS_WORKFLOW_MODE_STORE=" h_workflow_mode
    print "HAS_DETAIL_FLAG_STORE=" h_detail_flag
    print "HAS_ROWSPAN_STORE=" h_rowspan_parse
    print "HAS_TAIL_FIND=" h_tail_find
    print "HAS_TAIL_SPLIT_NUL=" h_tail_split_nul
    print "HAS_TAIL_CLAMP=" h_tail_clamp
    print "HAS_PERIOD_TEMPLATE_REPLACE=" h_period_replace
    print "HAS_LISTINGS_TEMPLATE_REPLACE=" h_listings_replace
    print "HAS_TAIL_FALLBACK_REPLACE=" h_tail_fallback_replace
    print "HAS_LOAD_TEMPLATE_CALL=" h_load_template
    print "HAS_RTS=" h_rts
}
