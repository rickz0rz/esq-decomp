...
awk '$0 ~ /^MEM_Move:$/ {in_func=1} in_func { if ($0 ~ /^;!======/) exit; print }' "$ORIG_ASM" >"${OUT_DIR}/${BASE}.original.s"
awk '$0 ~ /^MEM_Move:$/ {in_func=1} in_func { if ($0 ~ /^ END$/ || $0 ~ /^END$/) exit; print }' "$SASC_DIS" >"${OUT_DIR}/${BASE}.sasc.dis.s"
...
awk -f src/decomp/scripts/semantic_filter_sasc_mem_move.awk "${OUT_DIR}/${BASE}.original.norm.s" >"${OUT_DIR}/${BASE}.original.semantic.txt"
awk -f src/decomp/scripts/semantic_filter_sasc_mem_move.awk "${OUT_DIR}/${BASE}.sasc.norm.s" >"${OUT_DIR}/${BASE}.sasc.semantic.txt"
