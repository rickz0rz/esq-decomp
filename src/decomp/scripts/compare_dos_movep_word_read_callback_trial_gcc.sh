...
SRC_C="src/decomp/c/replacements/dos_movep_word_read_callback_gcc.c"
ORIG_ASM="src/modules/submodules/unknown2b.s"
OUT_DIR="build/decomp/c_trial_gcc"
GEN_ASM="${OUT_DIR}/dos_movep_word_read_callback.generated.gcc.s"
ORIG_SLICE="${OUT_DIR}/dos_movep_word_read_callback.original_slice.s"
GEN_SLICE="${OUT_DIR}/dos_movep_word_read_callback.generated_slice.s"
ORIG_NORM="${OUT_DIR}/dos_movep_word_read_callback.original_slice.norm.s"
GEN_NORM="${OUT_DIR}/dos_movep_word_read_callback.generated_slice.norm.s"
...
awk '$0 ~ /^DOS_MovepWordReadCallback:?$/ {in_func=1} in_func { if ($0 ~ /^;!======/) exit; print }' "$ORIG_ASM" > "$ORIG_SLICE"
awk '$0 ~ /^_?DOS_MovepWordReadCallback:?$/ {in_func=1} in_func { print; if ($0 ~ /\.word[[:space:]]+0x?0+$/) exit }' "$GEN_ASM" > "$GEN_SLICE"
...
awk -f src/decomp/scripts/semantic_filter_dos_movep_word_read_callback.awk "$ORIG_NORM" > "$ORIG_SEM"
awk -f src/decomp/scripts/semantic_filter_dos_movep_word_read_callback.awk "$GEN_NORM" > "$GEN_SEM"
