...
awk -v start="^${ENTRY}:$" -v next_label="^PARSEINI_UpdateCtrlHDeltaMax:$" '
    $0 ~ start { in_func=1 }
    in_func {
...
awk -v e="^${ENTRY}:$" -v e2="$ENTRY_SASC_REGEX" '
    $0 ~ e || $0 ~ e2 { in_func=1 }
    in_func {
        if (($0 ~ /^PARSEINI_[A-Za-z0-9_]+:$/ || $0 ~ /^_?PARSEINI_[A-Za-z0-9_]+:$/) && $0 !~ e && $0 !~ e2) exit
...
awk -f src/decomp/scripts/semantic_filter_sasc_parseini3_monitor_clock_change.awk \
