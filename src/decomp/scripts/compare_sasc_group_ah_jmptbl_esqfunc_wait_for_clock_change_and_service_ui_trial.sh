...
cd "$ROOT_DIR"
SASC_SRC="group_ah_jmptbl_stubs.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/a/h/xjump.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="group_ah_jmptbl_esqfunc_wait_for_clock_change_and_service_ui"
ENTRY_ORIG="GROUP_AH_JMPTBL_ESQFUNC_WaitForClockChangeAndServiceUi"
ENTRY_SASC="GROUP_AH_JMPTBL_ESQFUNC_WaitForC"
TARGET="ESQFUNC_WaitForClockChangeAndServiceUi"
mkdir -p "$OUT_DIR"
./sc-build-with-dis.sh "$SASC_SRC" >"${OUT_DIR}/sc_build_${BASE}.log" 2>&1
awk -v e="^${ENTRY_ORIG}:$" '
  $0 ~ e {inf=1}
  inf {
    if ($0 ~ /^GROUP_AH_JMPTBL_/ && $0 !~ e) exit
    if ($0 ~ /^;!======/) exit
    print
  }
