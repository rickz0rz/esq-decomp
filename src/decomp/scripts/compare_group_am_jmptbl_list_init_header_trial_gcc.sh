...
SRC_C="src/decomp/c/replacements/group_am_jmptbl_list_init_header_gcc.c"
ORIG_ASM="src/modules/groups/a/m/xjump.s"
OUT_DIR="build/decomp/c_trial_gcc"
GEN_ASM="${OUT_DIR}/group_am_jmptbl_list_init_header.generated.gcc.s"
ORIG_SLICE="${OUT_DIR}/group_am_jmptbl_list_init_header.original_slice.s"
GEN_SLICE="${OUT_DIR}/group_am_jmptbl_list_init_header.generated_slice.s"
ORIG_NORM="${OUT_DIR}/group_am_jmptbl_list_init_header.original_slice.norm.s"
GEN_NORM="${OUT_DIR}/group_am_jmptbl_list_init_header.generated_slice.norm.s"
...
awk '
$0 ~ /^GROUP_AM_JMPTBL_LIST_InitHeader:?$/ {in_func=1}
in_func {
    if ($0 ~ /^GROUP_AM_JMPTBL_ESQ_SetCopperEffect_OnEnableHighlight:?$/) {
        exit
    }
    print
}
...
awk '
$0 ~ /^_?GROUP_AM_JMPTBL_LIST_InitHeader:?$/ {in_func=1}
in_func { print }
' "$GEN_ASM" > "$GEN_SLICE"
...
awk -f src/decomp/scripts/semantic_filter_group_am_jmptbl_list_init_header.awk "$ORIG_NORM" > "$ORIG_SEM"
awk -f src/decomp/scripts/semantic_filter_group_am_jmptbl_list_init_header.awk "$GEN_NORM" > "$GEN_SEM"
