...

{
    ...
    if (u ~ /^TLIBA3_GETVIEWMODERASTPORT:/ || u ~ /^TLIBA3_GETVIEWMODERAST[A-Z0-9_]*:/) has_entry=1
    if (n ~ /MULU32/) has_mulu=1
    if (u ~ /#77/ || u ~ /#\$4D/ || u ~ /#154/ || u ~ /#\$9A/ || u ~ /\(\$9A\)/) has_const_154=1
    if (n ~ /TLIBA3VMARRAYRUNTIMETABLE/) has_vm_table=1
    if (u ~ /10\(A0\)/ || u ~ /\$A\(A[0-7]\)/ || u ~ /MOVE\.L \$A\(A5\),D0/ || u ~ /MOVE\.L \$A\(A0\),D0/ || n ~ /LEA10A0A1/) has_offset_10=1
    if (u == "RTS") has_rts=1
}
