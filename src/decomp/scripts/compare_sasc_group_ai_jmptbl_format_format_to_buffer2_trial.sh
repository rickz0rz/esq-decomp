...
BASE="group_ai_jmptbl_format_format_to_buffer2"
ENTRY_ORIG="GROUP_AI_JMPTBL_FORMAT_FormatToBuffer2"
ENTRY_SASC="GROUP_AI_JMPTBL_FORMAT_FormatToB"
TARGET="FORMAT_FormatToBuffer2"
mkdir -p "$OUT_DIR"
./sc-build-with-dis.sh "$SASC_SRC" >"${OUT_DIR}/sc_build_${BASE}.log" 2>&1
awk -v e="^${ENTRY_ORIG}:$" '$0 ~ e {inf=1} inf { if ($0 ~ /^GROUP_AI_JMPTBL_STR_SkipClass3Chars:$/) exit; print }' "$ORIG_ASM" >"${OUT_DIR}/${BASE}.original.s"
awk -v e="^${ENTRY_SASC}:$" '$0 ~ e {inf=1} inf { if ($0 ~ /^GROUP_AI_JMPTBL_STR_SkipClass3Ch:$/) exit; print }' "$SASC_DIS" >"${OUT_DIR}/${BASE}.sasc.dis.s"
