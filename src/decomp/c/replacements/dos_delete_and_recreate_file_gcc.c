...
/*
 * Target 045 GCC trial function.
 * Delete file if it exists, then open MODE_NEWFILE.
 */
...
s32 DOS_DeleteFile(const char *name);
s32 DOS_Open(const char *name, s32 mode);
s32 DOS_IoErr(void);

s32 DOS_DeleteAndRecreateFile(const char *filename) __attribute__((noinline, used));

s32 DOS_DeleteAndRecreateFile(const char *filename)
{
    s32 lock;
    s32 fh;
