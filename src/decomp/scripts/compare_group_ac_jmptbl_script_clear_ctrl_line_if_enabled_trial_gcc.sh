...
SRC_C="src/decomp/c/replacements/group_ac_jmptbl_script_clear_ctrl_line_if_enabled_gcc.c"
ORIG_ASM="src/modules/groups/a/c/xjump.s"
OUT_DIR="build/decomp/c_trial_gcc"
GEN_ASM="${OUT_DIR}/group_ac_jmptbl_script_clear_ctrl_line_if_enabled.generated.gcc.s"
ORIG_SLICE="${OUT_DIR}/group_ac_jmptbl_script_clear_ctrl_line_if_enabled.original_slice.s"
GEN_SLICE="${OUT_DIR}/group_ac_jmptbl_script_clear_ctrl_line_if_enabled.generated_slice.s"
ORIG_NORM="${OUT_DIR}/group_ac_jmptbl_script_clear_ctrl_line_if_enabled.original_slice.norm.s"
GEN_NORM="${OUT_DIR}/group_ac_jmptbl_script_clear_ctrl_line_if_enabled.generated_slice.norm.s"
...
awk '
$0 ~ /^GROUP_AC_JMPTBL_SCRIPT_ClearCtrlLineIfEnabled:?$/ {in_func=1}
in_func {
    if ($0 ~ /^GROUP_AC_JMPTBL_ESQFUNC_FreeExtraTitleTextPointers:?$/) {
        exit
    }
    print
}
...
awk '
$0 ~ /^_?GROUP_AC_JMPTBL_SCRIPT_ClearCtrlLineIfEnabled:?$/ {in_func=1}
in_func { print }
' "$GEN_ASM" > "$GEN_SLICE"
...
awk -f "src/decomp/scripts/semantic_filter_group_ac_jmptbl_script_clear_ctrl_line_if_enabled.awk" "$ORIG_NORM" > "$ORIG_SEM"
awk -f "src/decomp/scripts/semantic_filter_group_ac_jmptbl_script_clear_ctrl_line_if_enabled.awk" "$GEN_NORM" > "$GEN_SEM"
