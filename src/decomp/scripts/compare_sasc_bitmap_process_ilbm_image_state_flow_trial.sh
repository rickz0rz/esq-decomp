...
BASE="bitmap_process_ilbm_image_state_flow"
ENTRY="BITMAP_ProcessIlbmImage"
...
awk -v start="^${ENTRY}:$" -v next_label="^BITMAP_" '
    $0 ~ start { in_func=1 }
    in_func {
        if ($0 ~ next_label && $0 !~ start) exit
        print
    }
...
awk -f src/decomp/scripts/semantic_filter_sasc_bitmap_process_ilbm_image_state_flow.awk "${OUT_DIR}/${BASE}.original.norm.s" >"${OUT_DIR}/${BASE}.original.semantic.txt"
awk -f src/decomp/scripts/semantic_filter_sasc_bitmap_process_ilbm_image_state_flow.awk "${OUT_DIR}/${BASE}.sasc.norm.s" >"${OUT_DIR}/${BASE}.sasc.semantic.txt"
