...
SRC_C="src/decomp/c/replacements/group_at_jmptbl_dos_system_taglist_gcc.c"
ORIG_ASM="src/modules/groups/a/t/xjump.s"
OUT_DIR="build/decomp/c_trial_gcc"
GEN_ASM="${OUT_DIR}/group_at_jmptbl_dos_system_taglist.generated.gcc.s"
ORIG_SLICE="${OUT_DIR}/group_at_jmptbl_dos_system_taglist.original_slice.s"
GEN_SLICE="${OUT_DIR}/group_at_jmptbl_dos_system_taglist.generated_slice.s"
ORIG_NORM="${OUT_DIR}/group_at_jmptbl_dos_system_taglist.original_slice.norm.s"
GEN_NORM="${OUT_DIR}/group_at_jmptbl_dos_system_taglist.generated_slice.norm.s"
...
awk '
$0 ~ /^GROUP_AT_JMPTBL_DOS_SystemTagList:?$/ {in_func=1}
in_func {
    if ($0 ~ /^GROUP_AT_JMPTBL_ED1_WaitForFlagAndClearBit1:?$/) {
        exit
    }
    print
}
...
awk '
$0 ~ /^_?GROUP_AT_JMPTBL_DOS_SystemTagList:?$/ {in_func=1}
in_func { print }
' "$GEN_ASM" > "$GEN_SLICE"
...
awk -f src/decomp/scripts/semantic_filter_group_at_jmptbl_dos_system_taglist.awk "$ORIG_NORM" > "$ORIG_SEM"
awk -f src/decomp/scripts/semantic_filter_group_at_jmptbl_dos_system_taglist.awk "$GEN_NORM" > "$GEN_SEM"
