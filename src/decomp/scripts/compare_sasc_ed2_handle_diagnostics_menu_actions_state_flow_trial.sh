...
BASE="ed2_handle_diagnostics_menu_actions_state_flow"
...
awk -v start="^${ENTRY_ORIG}:$" -v next_label="^ED2_HandleScrollSpeedSelection:$" '
    $0 ~ start {in_func=1}
    in_func {
...
awk -f src/decomp/scripts/semantic_filter_sasc_ed2_handle_diagnostics_menu_actions_state_flow.awk \
