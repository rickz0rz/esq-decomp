...
SRC_C="src/decomp/c/replacements/cleanup_shutdown_input_devices_gcc.c"
ORIG_ASM="src/modules/groups/a/b/cleanup.s"
OUT_DIR="build/decomp/c_trial_gcc"
GEN_ASM="${OUT_DIR}/cleanup_shutdown_input_devices.generated.gcc.s"
ORIG_SLICE="${OUT_DIR}/cleanup_shutdown_input_devices.original_slice.s"
GEN_SLICE="${OUT_DIR}/cleanup_shutdown_input_devices.generated_slice.s"
ORIG_NORM="${OUT_DIR}/cleanup_shutdown_input_devices.original_slice.norm.s"
GEN_NORM="${OUT_DIR}/cleanup_shutdown_input_devices.generated_slice.norm.s"
...
awk '$0 ~ /^CLEANUP_ShutdownInputDevices:?$/ {in_func=1} in_func { if ($0 ~ /^;!======/) exit; print }' "$ORIG_ASM" > "$ORIG_SLICE"
awk '$0 ~ /^_?CLEANUP_ShutdownInputDevices:?$/ {in_func=1} in_func { print }' "$GEN_ASM" > "$GEN_SLICE"
...
awk -f src/decomp/scripts/semantic_filter_cleanup_shutdown_input_devices.awk "$ORIG_NORM" > "$ORIG_SEM"
awk -f src/decomp/scripts/semantic_filter_cleanup_shutdown_input_devices.awk "$GEN_NORM" > "$GEN_SEM"
