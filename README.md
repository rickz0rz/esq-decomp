...
- Current hybrid-integration note (March 13, 2026, later re-validated): `modules/groups/a/a/app3.s` is now seeded in `src/decomp/replacements.map` as a passthrough hybrid boundary. Treat the APP3 reboot/init helper cluster as integration-ready at the module boundary, even though the replacement file still includes the canonical asm module verbatim for now.
- Current hybrid-integration note (March 13, 2026, later re-validated): `modules/groups/a/d/cleanup3.s` is now seeded in `src/decomp/replacements.map` as a passthrough hybrid boundary. Treat the CLEANUP3 aligned-status banner/screen helper cluster as integration-ready at the module boundary, even though the replacement file still includes the canonical asm module verbatim for now.
...
- Current hybrid-integration note (March 13, 2026): `modules/submodules/unknown38.s` is now seeded in `src/decomp/replacements.map` as a passthrough hybrid boundary. Treat the UNKNOWN38 signal/exit helper module (`SIGNAL_PollAndDispatch`) as integration-ready at the module boundary, even though the replacement file still includes the canonical asm module verbatim for now.
...
- Seeded `modules/groups/_main/a/a.s` in `src/decomp/replacements.map` with a passthrough hybrid boundary now that the primary `_main` startup/shutdown helper module has green SAS/C compare coverage for `ESQ_StartupEntry`, `ESQ_ReturnWithStackCode`, and `ESQ_ShutdownAndReturn`.
...
- Seeded `modules/submodules/unknown38.s` in `src/decomp/replacements.map` with a passthrough hybrid boundary now that the UNKNOWN38 signal/exit helper module has green restored SAS/C compare coverage in the current checkout (`SIGNAL_PollAndDispatch`).
