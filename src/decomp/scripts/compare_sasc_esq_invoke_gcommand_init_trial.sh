...
SASC_SRC="esq_invoke_gcommand_init.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/a/a/app3.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="esq_invoke_gcommand_init"
ENTRY="ESQ_InvokeGcommandInit"
ENTRY_SASC_REGEX="^ESQ_InvokeGcommandInit[A-Za-z0-9_]*:$"
ENTRY_SEM="ESQ_INVOKEGCOMMANDINIT:"
ENTRY_ALT_SEM="ESQ_INVOKEGCOMMANDINIT"
...
awk -v ENTRY_PREFIX="$ENTRY_SEM" -v ENTRY_ALT_PREFIX="$ENTRY_ALT_SEM" -f src/decomp/scripts/semantic_filter_sasc_esq_invoke_gcommand_init.awk "${OUT_DIR}/${BASE}.original.norm.s" >"${OUT_DIR}/${BASE}.original.semantic.txt"
awk -v ENTRY_PREFIX="$ENTRY_SEM" -v ENTRY_ALT_PREFIX="$ENTRY_ALT_SEM" -f src/decomp/scripts/semantic_filter_sasc_esq_invoke_gcommand_init.awk "${OUT_DIR}/${BASE}.sasc.norm.s" >"${OUT_DIR}/${BASE}.sasc.semantic.txt"
