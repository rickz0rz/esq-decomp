...
SRC_C="src/decomp/c/replacements/esq_try_rom_write_test_gcc.c"
ORIG_ASM="src/modules/groups/a/a/app3.s"
OUT_DIR="build/decomp/c_trial_gcc"
GEN_ASM="${OUT_DIR}/esq_try_rom_write_test.generated.gcc.s"
ORIG_SLICE="${OUT_DIR}/esq_try_rom_write_test.original_slice.s"
GEN_SLICE="${OUT_DIR}/esq_try_rom_write_test.generated_slice.s"
ORIG_NORM="${OUT_DIR}/esq_try_rom_write_test.original_slice.norm.s"
GEN_NORM="${OUT_DIR}/esq_try_rom_write_test.generated_slice.norm.s"
ORIG_SEM="${OUT_DIR}/esq_try_rom_write_test.original_slice.semantic.txt"
GEN_SEM="${OUT_DIR}/esq_try_rom_write_test.generated_slice.semantic.txt"
...
awk '$0 ~ /^ESQ_TryRomWriteTest:?$/ {in_func=1} in_func { if ($0 ~ /^;!======/) exit; print }' "$ORIG_ASM" > "$ORIG_SLICE"
awk '$0 ~ /^_?ESQ_TryRomWriteTest:?$/ {in_func=1} in_func { print }' "$GEN_ASM" > "$GEN_SLICE"
...
awk -f src/decomp/scripts/semantic_filter_esq_try_rom_write_test.awk "$ORIG_NORM" > "$ORIG_SEM"
awk -f src/decomp/scripts/semantic_filter_esq_try_rom_write_test.awk "$GEN_NORM" > "$GEN_SEM"
