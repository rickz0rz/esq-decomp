...
SRC_C="src/decomp/c/replacements/group_ae_jmptbl_script_build_token_index_map_gcc.c"
ORIG_ASM="src/modules/groups/a/e/xjump.s"
OUT_DIR="build/decomp/c_trial_gcc"
GEN_ASM="${OUT_DIR}/group_ae_jmptbl_script_build_token_index_map.generated.gcc.s"
ORIG_SLICE="${OUT_DIR}/group_ae_jmptbl_script_build_token_index_map.original_slice.s"
GEN_SLICE="${OUT_DIR}/group_ae_jmptbl_script_build_token_index_map.generated_slice.s"
ORIG_NORM="${OUT_DIR}/group_ae_jmptbl_script_build_token_index_map.original_slice.norm.s"
GEN_NORM="${OUT_DIR}/group_ae_jmptbl_script_build_token_index_map.generated_slice.norm.s"
...
awk '
$0 ~ /^GROUP_AE_JMPTBL_SCRIPT_BuildTokenIndexMap:?$/ {in_func=1}
in_func {
    if ($0 ~ /^GROUP_AE_JMPTBL_ESQDISP_GetEntryAuxPointerByMode:?$/) {
        exit
    }
    print
}
...
awk '
$0 ~ /^_?GROUP_AE_JMPTBL_SCRIPT_BuildTokenIndexMap:?$/ {in_func=1}
in_func { print }
' "$GEN_ASM" > "$GEN_SLICE"
...
awk -f "src/decomp/scripts/semantic_filter_group_ae_jmptbl_script_build_token_index_map.awk" "$ORIG_NORM" > "$ORIG_SEM"
awk -f "src/decomp/scripts/semantic_filter_group_ae_jmptbl_script_build_token_index_map.awk" "$GEN_NORM" > "$GEN_SEM"
