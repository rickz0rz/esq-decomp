...
SRC_C="src/decomp/c/replacements/group_aw_jmptbl_mem_move_gcc.c"
ORIG_ASM="src/modules/groups/a/w/xjump.s"
OUT_DIR="build/decomp/c_trial_gcc"
GEN_ASM="${OUT_DIR}/group_aw_jmptbl_mem_move.generated.gcc.s"
ORIG_SLICE="${OUT_DIR}/group_aw_jmptbl_mem_move.original_slice.s"
GEN_SLICE="${OUT_DIR}/group_aw_jmptbl_mem_move.generated_slice.s"
ORIG_NORM="${OUT_DIR}/group_aw_jmptbl_mem_move.original_slice.norm.s"
GEN_NORM="${OUT_DIR}/group_aw_jmptbl_mem_move.generated_slice.norm.s"
...
awk '
$0 ~ /^GROUP_AW_JMPTBL_MEM_Move:?$/ {in_func=1}
in_func {
    if ($0 ~ /^GROUP_AW_JMPTBL_WDISP_SPrintf:?$/) {
        exit
    }
    print
}
...
awk '
$0 ~ /^_?GROUP_AW_JMPTBL_MEM_Move:?$/ {in_func=1}
in_func { print }
' "$GEN_ASM" > "$GEN_SLICE"
...
awk -f src/decomp/scripts/semantic_filter_group_aw_jmptbl_mem_move.awk "$ORIG_NORM" > "$ORIG_SEM"
awk -f src/decomp/scripts/semantic_filter_group_aw_jmptbl_mem_move.awk "$GEN_NORM" > "$GEN_SEM"
