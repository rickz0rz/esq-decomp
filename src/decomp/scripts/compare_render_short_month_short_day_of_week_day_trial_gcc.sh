...
SRC_C="src/decomp/c/replacements/render_short_month_short_day_of_week_day_gcc.c"
ORIG_ASM="src/modules/groups/a/c/cleanup2.s"
OUT_DIR="build/decomp/c_trial_gcc"
GEN_ASM="${OUT_DIR}/render_short_month_short_day_of_week_day.generated.gcc.s"
ORIG_SLICE="${OUT_DIR}/render_short_month_short_day_of_week_day.original_slice.s"
GEN_SLICE="${OUT_DIR}/render_short_month_short_day_of_week_day.generated_slice.s"
ORIG_NORM="${OUT_DIR}/render_short_month_short_day_of_week_day.original_slice.norm.s"
GEN_NORM="${OUT_DIR}/render_short_month_short_day_of_week_day.generated_slice.norm.s"
...
awk '$0 ~ /^RENDER_SHORT_MONTH_SHORT_DAY_OF_WEEK_DAY:?$/ {in_func=1} in_func { if ($0 ~ /^;!======/) exit; print }' "$ORIG_ASM" > "$ORIG_SLICE"
awk '$0 ~ /^_?RENDER_SHORT_MONTH_SHORT_DAY_OF_WEEK_DAY:?$/ {in_func=1} in_func { print }' "$GEN_ASM" > "$GEN_SLICE"
...
awk -f src/decomp/scripts/semantic_filter_render_short_month_short_day_of_week_day.awk "$ORIG_NORM" > "$ORIG_SEM"
awk -f src/decomp/scripts/semantic_filter_render_short_month_short_day_of_week_day.awk "$GEN_NORM" > "$GEN_SEM"
