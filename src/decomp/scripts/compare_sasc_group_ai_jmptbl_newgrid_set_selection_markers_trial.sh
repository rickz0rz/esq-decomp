...
BASE="group_ai_jmptbl_newgrid_set_selection_markers"
ENTRY_ORIG="GROUP_AI_JMPTBL_NEWGRID_SetSelectionMarkers"
ENTRY_SASC="GROUP_AI_JMPTBL_NEWGRID_SetSelec"
TARGET="NEWGRID_SetSelectionMarkers"
mkdir -p "$OUT_DIR"
./sc-build-with-dis.sh "$SASC_SRC" >"${OUT_DIR}/sc_build_${BASE}.log" 2>&1
awk -v e="^${ENTRY_ORIG}:$" '$0 ~ e {inf=1} inf { if ($0 ~ /^GROUP_AI_JMPTBL_STR_FindCharPtr:$/) exit; print }' "$ORIG_ASM" >"${OUT_DIR}/${BASE}.original.s"
awk -v e="^${ENTRY_SASC}:$" '$0 ~ e {inf=1} inf { if ($0 ~ /^GROUP_AI_JMPTBL_STR_FindCharPtr:$/) exit; print }' "$SASC_DIS" >"${OUT_DIR}/${BASE}.sasc.dis.s"
