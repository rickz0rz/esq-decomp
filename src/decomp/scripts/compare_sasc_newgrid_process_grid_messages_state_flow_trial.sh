...
SASC_SRC="newgrid_process_grid_messages.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/b/a/newgrid.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="newgrid_process_grid_messages_state_flow"
ENTRY_ORIG="NEWGRID_ProcessGridMessages"
ENTRY_SASC_REGEX="^NEWGRID_ProcessGridMessag[A-Za-z0-9_]*:$"
...
awk -v e="^${ENTRY_ORIG}:$" '
  $0 ~ e {in_func=1}
  in_func {
    if ($0 ~ /^NEWGRID_JMPTBL_MATH_DivS32:$/) exit
    print
  }
...
awk -f src/decomp/scripts/semantic_filter_sasc_newgrid_process_grid_messages_state_flow.awk "${OUT_DIR}/${BASE}.original.norm.s" >"${OUT_DIR}/${BASE}.original.semantic.txt"
awk -f src/decomp/scripts/semantic_filter_sasc_newgrid_process_grid_messages_state_flow.awk "${OUT_DIR}/${BASE}.sasc.norm.s" >"${OUT_DIR}/${BASE}.sasc.semantic.txt"
