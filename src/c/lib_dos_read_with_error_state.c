/* RESTORES: _DOS_ReadWithErrorState
 * MODULE:   modules/submodules/unknown19.s
 * STATUS:   behavioural
 *
 * SAS/C library code. Reads through a DOS file handle and records the failure
 ...
 * read is not an error here: it returns the count and leaves Global_DosIoErr
 * zero. A caller wanting short-read detection has to compare against the
 * length it asked for.
 *
...
long DOS_ReadWithErrorState(long fh, char *buf, long len)
{
    long n;
