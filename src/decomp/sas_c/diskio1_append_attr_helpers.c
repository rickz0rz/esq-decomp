...
enum {
    ...
    ATTRFLAG_VIDEO_TAG_DISABLE_MASK = 0x08,
    ATTRFLAG_PPV_SRC_MASK = 0x10,
    ATTRFLAG_DITTO_MASK = 0x20,
    ATTRFLAG_ALT_HILITE_SRC_MASK = 0x40,
    ATTRFLAG_BIT7_MASK = 0x80
};
...
void DISKIO1_AppendAttrFlagHiliteSrc(void)
{
    if (gDiskio1AttrFlags & ATTRFLAG_HILITE_SRC_MASK) {
        FORMAT_RawDoFmtWithScratchBuffer(DISKIO_STR_HILITE_SRC_CompactSourceAttrFlags);
    }
...
void DISKIO1_AppendAttrFlagSummarySrc(void)
{
    if (gDiskio1AttrFlags & ATTRFLAG_SUMMARY_SRC_MASK) {
        FORMAT_RawDoFmtWithScratchBuffer(DISKIO_STR_SUM_SRC_CompactSourceAttrFlags);
    }
...
void DISKIO1_AppendAttrFlagVideoTagDisable(void)
{
    if (gDiskio1AttrFlags & ATTRFLAG_VIDEO_TAG_DISABLE_MASK) {
        FORMAT_RawDoFmtWithScratchBuffer(DISKIO_STR_VIDEO_TAG_DISABLE_CompactSourceAttrFlags);
    }
...
void DISKIO1_AppendAttrFlagPpvSrc(void)
{
    if (gDiskio1AttrFlags & ATTRFLAG_PPV_SRC_MASK) {
        FORMAT_RawDoFmtWithScratchBuffer(DISKIO_STR_PPV_SRC_CompactSourceAttrFlags);
    }
...
void DISKIO1_AppendAttrFlagDitto(void)
{
    if (gDiskio1AttrFlags & ATTRFLAG_DITTO_MASK) {
        FORMAT_RawDoFmtWithScratchBuffer(DISKIO_STR_DITTO_CompactSourceAttrFlags);
    }
...
void DISKIO1_AppendAttrFlagAltHiliteSrc(void)
{
    if (gDiskio1AttrFlags & ATTRFLAG_ALT_HILITE_SRC_MASK) {
        FORMAT_RawDoFmtWithScratchBuffer(DISKIO_STR_ALTHILITESRC_CompactSourceAttrFlags);
    }
