...
SASC_SRC="esq_startup_entry.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/_main/a/a.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="esq_startup_entry"
ENTRY="ESQ_StartupEntry"
ENTRY_SASC_REGEX="^ESQ_StartupEntry[A-Za-z0-9_]*:$"
...
awk '
  {
    if ($0 ~ /^ESQ_ReturnWithStackCode:$/) exit
    if ($0 ~ /^;!======/) next
    print
  }
...
awk -f src/decomp/scripts/semantic_filter_sasc_esq_startup_entry.awk "${OUT_DIR}/${BASE}.original.norm.s" >"${OUT_DIR}/${BASE}.original.semantic.txt"
awk -f src/decomp/scripts/semantic_filter_sasc_esq_startup_entry.awk "${OUT_DIR}/${BASE}.sasc.norm.s" >"${OUT_DIR}/${BASE}.sasc.semantic.txt"
