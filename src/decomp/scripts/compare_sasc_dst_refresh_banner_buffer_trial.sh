...
awk '$0 ~ /^DST_RefreshBannerBuffer:$/ {in_func=1} in_func { if ($0 ~ /^;!======/) exit; print }' "$ORIG_ASM" >"${OUT_DIR}/dst_refresh_banner_buffer.original.s"
awk '$0 ~ /^[[:space:]]*DST_RefreshBannerBuffer:/ {in_func=1} in_func { if ($0 ~ /^[[:space:]]*__const:$/) exit; print }' "$SASC_DIS" >"${OUT_DIR}/dst_refresh_banner_buffer.sasc.dis.s"
