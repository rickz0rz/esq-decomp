...
cd "$ROOT_DIR"
SASC_SRC="tliba1_jmptbl_stubs.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/b/a/tliba1.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="tliba1_jmptbl_esqdisp_computescheduleoffsetforrow"
ENTRY_ORIG="TLIBA1_JMPTBL_ESQDISP_ComputeScheduleOffsetForRow"
ENTRY_SASC="TLIBA1_JMPTBL_ESQDISP_ComputeSch"
TARGET="ESQDISP_ComputeScheduleOffsetForRow"
mkdir -p "$OUT_DIR"
./sc-build-with-dis.sh "$SASC_SRC" >"${OUT_DIR}/sc_build_${BASE}.log" 2>&1
awk -v e="^${ENTRY_ORIG}:$" '$0 ~ e {inf=1} inf { if ($0 ~ /^TLIBA1_JMPTBL_/ && $0 !~ e) exit; if ($0 ~ /^;!======/) exit; print }' "$ORIG_ASM" >"${OUT_DIR}/${BASE}.original.s"
