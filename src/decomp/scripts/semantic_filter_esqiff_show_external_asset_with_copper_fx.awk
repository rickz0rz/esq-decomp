...

{
    ...
    if (uline ~ /^ESQIFF_SHOWEXTERNALASSETWITHCOPPERFX:/) has_entry = 1
    if (uline ~ /ESQIFF_JMPTBL_BRUSH_SELECTBRUSHSLOT/) has_select_slot = 1
    if (uline ~ /ESQIFF_JMPTBL_TLIBA3_BUILDDISPLAYCONTEXTFORVIEWMODE/) has_build_ctx = 1
    if (uline ~ /ESQIFF_JMPTBL_MATH_DIVS32/) has_divs = 1
    if (uline ~ /ESQIFF_JMPTBL_SCRIPT_BEGINBANNERCHARTRANSITION/) has_begin_banner = 1
    if (uline ~ /ESQPARS_JMPTBL_BRUSH_PLANEMASKFORINDEX/) has_plane_mask = 1
    if (uline ~ /ESQIFF_RUNCOPPERRISETRANSITION/) has_run_rise = 1
    if (uline ~ /ESQIFF_RUNCOPPERDROPTRANSITION/) has_run_drop = 1
    if (uline ~ /_LVOSETAPEN\(A6\)/) has_set_apen = 1
    if (uline ~ /_LVOCOPYMEM\(A6\)/) has_copy_mem = 1
    if (uline ~ /ESQIFF_SHOWEXTERNALASSETWITHCOPPERFX_RETURN/) has_return_ref = 1
}
...
END {
    ...
    print "HAS_RETURN_REF=" has_return_ref
}
