    XDEF    _ED1_HandleEscMenuInput


;------------------------------------------------------------------------------
; FUNC: _ED1_HandleEscMenuInput   (Handle ESC menu command selectionuncertain)
; ARGS:
;   (none)
; RET:
;   D0: result/status
...
; CALLS:
;   _ED_GetEscMenuActionCode, _ED_DrawAdNumberPrompt, _ED_DrawDiagnosticModeHelpText, _ED_DrawMenuSelectionHighlight, _ED_DrawScrollSpeedMenuText, _ED_DrawBottomHelpBarBackground, _ED_DrawEscMainMenuText,
...
; target as a hand-computed offset, which resolves to _ED1_ExitEscMenu only while
...
; above _ED1_ExitEscMenu and the menu never closed. The symbolic form assembles
; to the same bytes and cannot drift. See AGENTS.md, "ESQ_FARCALLS=1 lifts the
; 16-bit ceiling".
    BSR.W   _ED1_ExitEscMenu
...
.done:
    ...
    PEA     _ED2_STR_LOCAL_EDIT_NOT_AVAILABLE
    PEA     270.W
    PEA     145.W
