...
BASE="esqpars_consume_rbf_byte_and_dispatch_command_state_flow"
...
awk -v start="^${ENTRY_ORIG}:$" -v next_label="^ESQPARS_ReadLengthWordWithChecksumXor:$" '
    $0 ~ start { in_func=1 }
    in_func {
...
awk -f src/decomp/scripts/semantic_filter_sasc_esqpars_consume_rbf_byte_and_dispatch_command_state_flow.awk "${OUT_DIR}/${BASE}.original.norm.s" >"${OUT_DIR}/${BASE}.original.semantic.txt"
awk -f src/decomp/scripts/semantic_filter_sasc_esqpars_consume_rbf_byte_and_dispatch_command_state_flow.awk "${OUT_DIR}/${BASE}.sasc.norm.s" >"${OUT_DIR}/${BASE}.sasc.semantic.txt"
