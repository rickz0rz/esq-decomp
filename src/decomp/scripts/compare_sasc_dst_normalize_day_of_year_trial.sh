...
awk '$0 ~ /^DST_NormalizeDayOfYear:$/ {in_func=1} in_func { if ($0 ~ /^;!======/) exit; print }' "$ORIG_ASM" >"${OUT_DIR}/dst_normalize_day_of_year.original.s"
awk '$0 ~ /^[[:space:]]*DST_NormalizeDayOfYear:/ {in_func=1} in_func { if ($0 ~ /^[[:space:]]*__const:$/) exit; print }' "$SASC_DIS" >"${OUT_DIR}/dst_normalize_day_of_year.sasc.dis.s"
