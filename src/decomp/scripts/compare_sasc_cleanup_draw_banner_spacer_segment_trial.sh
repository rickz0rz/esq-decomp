...
cd "$ROOT_DIR"
SASC_SRC="cleanup_draw_banner_spacer_segment.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/a/c/cleanup2.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="cleanup_draw_banner_spacer_segment"
ENTRY="CLEANUP_DrawBannerSpacerSegment"
ENTRY_SASC_REGEX="^CLEANUP_DrawBannerSpacerSegment[A-Za-z0-9_]*:$"
mkdir -p "$OUT_DIR"
...
awk -f src/decomp/scripts/semantic_filter_sasc_cleanup_draw_banner_spacer_segment.awk "${OUT_DIR}/${BASE}.original.norm.s" >"${OUT_DIR}/${BASE}.original.semantic.txt"
awk -f src/decomp/scripts/semantic_filter_sasc_cleanup_draw_banner_spacer_segment.awk "${OUT_DIR}/${BASE}.sasc.norm.s" >"${OUT_DIR}/${BASE}.sasc.semantic.txt"
