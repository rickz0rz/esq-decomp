...
cd "$ROOT_DIR"
SASC_SRC="ed1_jmptbl_stubs.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/a/k/ed1.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="ed1_jmptbl_esqshared4_loaddefaultpalettetocopper_noop"
ENTRY_ORIG="ED1_JMPTBL_ESQSHARED4_LoadDefaultPaletteToCopper_NoOp"
ENTRY_SASC="ED1_JMPTBL_ESQSHARED4_LoadDefaul"
TARGET="ESQSHARED4_LoadDefaultPaletteToCopper_NoOp"
mkdir -p "$OUT_DIR"
./sc-build-with-dis.sh "$SASC_SRC" >"${OUT_DIR}/sc_build_${BASE}.log" 2>&1
awk -v e="^${ENTRY_ORIG}:$" '
  $0 ~ e {inf=1}
  inf {
    if ($0 ~ /^ED1_JMPTBL_/ && $0 !~ e) exit
    if ($0 ~ /^;!======/) exit
    print
  }
