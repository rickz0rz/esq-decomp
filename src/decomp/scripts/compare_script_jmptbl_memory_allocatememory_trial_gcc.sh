...
SRC_C="src/decomp/c/replacements/script_jmptbl_memory_allocatememory_gcc.c"
ORIG_ASM="src/modules/groups/b/a/script.s"
OUT_DIR="build/decomp/c_trial_gcc"
GEN_ASM="${OUT_DIR}/script_jmptbl_memory_allocatememory.generated.gcc.s"
ORIG_SLICE="${OUT_DIR}/script_jmptbl_memory_allocatememory.original_slice.s"
GEN_SLICE="${OUT_DIR}/script_jmptbl_memory_allocatememory.generated_slice.s"
ORIG_NORM="${OUT_DIR}/script_jmptbl_memory_allocatememory.original_slice.norm.s"
GEN_NORM="${OUT_DIR}/script_jmptbl_memory_allocatememory.generated_slice.norm.s"
...
awk -v start_label="SCRIPT_JMPTBL_MEMORY_AllocateMemory" -v next_label="SCRIPT_JMPTBL_DISKIO_OpenFileWithBuffer" '
...
awk '$0 ~ /^_?SCRIPT_JMPTBL_MEMORY_AllocateMemory:?$/ {in_func=1} in_func { print }' "$GEN_ASM" > "$GEN_SLICE"
...
awk -f "src/decomp/scripts/semantic_filter_script_jmptbl_memory_allocatememory.awk" "$ORIG_NORM" > "$ORIG_SEM"
awk -f "src/decomp/scripts/semantic_filter_script_jmptbl_memory_allocatememory.awk" "$GEN_NORM" > "$GEN_SEM"
