...
SASC_SRC="tliba3_set_font_for_all_view_modes.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/b/a/tliba3.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="tliba3_set_font_for_all_view_modes"
ENTRY="TLIBA3_SetFontForAllViewModes"
ENTRY_SASC_REGEX="^TLIBA3_SetFontForAllViewMode[A-Za-z0-9_]*:$"

mkdir -p "$OUT_DIR"

./sc-build-with-dis.sh "$SASC_SRC" >"${OUT_DIR}/sc_build_${BASE}.log" 2>&1

awk -v start="^${ENTRY}:$" -v next_label="^TLIBA3_JMPTBL_GCOMMAND_ApplyHighlightFlag:$" '$0 ~ start {in_func=1} in_func {if ($0 ~ next_label) exit; print}' "$ORIG_ASM" >"${OUT_DIR}/${BASE}.original.s"
...
awk -f src/decomp/scripts/semantic_filter_sasc_tliba3_set_font_for_all_view_modes.awk "${OUT_DIR}/${BASE}.original.norm.s" >"${OUT_DIR}/${BASE}.original.semantic.txt"
awk -f src/decomp/scripts/semantic_filter_sasc_tliba3_set_font_for_all_view_modes.awk "${OUT_DIR}/${BASE}.sasc.norm.s" >"${OUT_DIR}/${BASE}.sasc.semantic.txt"
