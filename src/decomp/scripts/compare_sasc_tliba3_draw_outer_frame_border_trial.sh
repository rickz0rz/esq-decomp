...
SASC_SRC="tliba3_draw_outer_frame_border.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/b/a/tliba3.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="tliba3_draw_outer_frame_border"
ENTRY="TLIBA3_DrawOuterFrameBorder"
ENTRY_SASC_REGEX="^TLIBA3_DrawOuterFrameBorde[A-Za-z0-9_]*:$"

mkdir -p "$OUT_DIR"

./sc-build-with-dis.sh "$SASC_SRC" >"${OUT_DIR}/sc_build_${BASE}.log" 2>&1

awk -v start="^${ENTRY}:$" -v next_label="^TLIBA3_DrawInnerFrameBorder:$" '$0 ~ start {in_func=1} in_func {if ($0 ~ next_label) exit; print}' "$ORIG_ASM" >"${OUT_DIR}/${BASE}.original.s"
...
awk -f src/decomp/scripts/semantic_filter_sasc_tliba3_draw_outer_frame_border.awk "${OUT_DIR}/${BASE}.original.norm.s" >"${OUT_DIR}/${BASE}.original.semantic.txt"
awk -f src/decomp/scripts/semantic_filter_sasc_tliba3_draw_outer_frame_border.awk "${OUT_DIR}/${BASE}.sasc.norm.s" >"${OUT_DIR}/${BASE}.sasc.semantic.txt"
