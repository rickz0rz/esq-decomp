...
BASE="esqfunc_jmptbl_locavail_rebuildfilterstatefromcurrentgroup"
ENTRY_ORIG="ESQFUNC_JMPTBL_LOCAVAIL_RebuildFilterStateFromCurrentGroup"
ENTRY_SASC="ESQFUNC_JMPTBL_LOCAVAIL_RebuildF"
TARGET="LOCAVAIL_RebuildFilterStateFromCurrentGroup"
mkdir -p "$OUT_DIR"
./sc-build-with-dis.sh "$SASC_SRC" >"${OUT_DIR}/sc_build_${BASE}.log" 2>&1
awk -v e="^${ENTRY_ORIG}:$" '
  $0 ~ e {inf=1}
  inf {
    if ($0 ~ /^ESQFUNC_JMPTBL_/ && $0 !~ e) exit
    if ($0 ~ /^;!======/) exit
    print
  }
