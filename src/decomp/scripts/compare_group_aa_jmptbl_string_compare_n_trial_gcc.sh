...
SRC_C="src/decomp/c/replacements/group_aa_jmptbl_string_compare_n_gcc.c"
ORIG_ASM="src/modules/groups/a/a/xjump.s"
OUT_DIR="build/decomp/c_trial_gcc"
GEN_ASM="${OUT_DIR}/group_aa_jmptbl_string_compare_n.generated.gcc.s"
ORIG_SLICE="${OUT_DIR}/group_aa_jmptbl_string_compare_n.original_slice.s"
GEN_SLICE="${OUT_DIR}/group_aa_jmptbl_string_compare_n.generated_slice.s"
ORIG_NORM="${OUT_DIR}/group_aa_jmptbl_string_compare_n.original_slice.norm.s"
GEN_NORM="${OUT_DIR}/group_aa_jmptbl_string_compare_n.generated_slice.norm.s"
...
awk '
$0 ~ /^GROUP_AA_JMPTBL_STRING_CompareN:?$/ {in_func=1}
in_func {
    if ($0 ~ /^GROUP_AA_JMPTBL_GCOMMAND_FindPathSeparator:?$/) {
        exit
    }
    print
}
...
awk '
$0 ~ /^_?GROUP_AA_JMPTBL_STRING_CompareN:?$/ {in_func=1}
in_func { print }
' "$GEN_ASM" > "$GEN_SLICE"
...
awk -f src/decomp/scripts/semantic_filter_group_aa_jmptbl_string_compare_n.awk "$ORIG_NORM" > "$ORIG_SEM"
awk -f src/decomp/scripts/semantic_filter_group_aa_jmptbl_string_compare_n.awk "$GEN_NORM" > "$GEN_SEM"
