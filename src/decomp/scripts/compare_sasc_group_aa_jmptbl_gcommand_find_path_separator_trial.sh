...
BASE="group_aa_jmptbl_gcommand_find_path_separator"
ENTRY_ORIG="GROUP_AA_JMPTBL_GCOMMAND_FindPathSeparator"
ENTRY_SASC="GROUP_AA_JMPTBL_GCOMMAND_FindPat"
TARGET="GCOMMAND_FindPathSeparator"
mkdir -p "$OUT_DIR"
./sc-build-with-dis.sh "$SASC_SRC" >"${OUT_DIR}/sc_build_${BASE}.log" 2>&1
awk -v e="^${ENTRY_ORIG}:$" '$0 ~ e {inf=1} inf { if ($0 ~ /^GROUP_AA_JMPTBL_GRAPHICS_AllocRaster:$/) exit; print }' "$ORIG_ASM" >"${OUT_DIR}/${BASE}.original.s"
awk -v e="^${ENTRY_SASC}:$" '$0 ~ e {inf=1} inf { if ($0 ~ /^GROUP_AA_JMPTBL_GRAPHICS_AllocRa:$/) exit; print }' "$SASC_DIS" >"${OUT_DIR}/${BASE}.sasc.dis.s"
