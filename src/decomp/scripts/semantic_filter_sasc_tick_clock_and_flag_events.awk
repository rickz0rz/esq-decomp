...

{
    ...
    if (u ~ /12\(A0\)/ || u ~ /\(12,A0\)/ || u ~ /\$C\(A[05]\)/) has_second_field = 1
    if (u ~ /10\(A0\)/ || u ~ /\(10,A0\)/ || u ~ /\$A\(A[05]\)/) has_minute_field = 1
    if (u ~ /8\(A0\)/ || u ~ /\(8,A0\)/ || u ~ /\$8\(A[05]\)/) has_hour_field = 1
    if (u ~ /16\(A0\)/ || u ~ /\(16,A0\)/ || u ~ /\$10\(A[05]\)/) has_doy_field = 1
    if (u ~ /20\(A0\)/ || u ~ /\(20,A0\)/ || u ~ /\$14\(A[05]\)/) has_leap_field = 1

    if (u ~ /CLOCK_MINUTETRIGGER30MINUSBASE/ || u ~ /CLOCK_MINUTETRIGGER60MINUSBASE/ || u ~ /CLOCK_MINUTETRIGGERBASEOFFSETPLUS30/ || u ~ /CLOCK_MINUTETRIGGERBASEOFFSET/ || u ~ /CLOCK_MINUTETRIGGERBASEOFFSETPLU/) has_trigger_globals = 1
    if (u ~ /ESQ_UPDATEMONTHDAYFROMDAYOFYEAR/) has_update_month_day_call = 1
    if (u ~ /#3,D[0-7]/ || u ~ /MOVEQ #3,D[0-7]/ || u ~ /#\$?3,D[0-7]/) has_status3 = 1
    if (u ~ /#4,D[0-7]/ || u ~ /MOVEQ #4,D[0-7]/ || u ~ /#\$?4,D[0-7]/) has_status4 = 1
    if (u ~ /#5,D[0-7]/ || u ~ /MOVEQ #5,D[0-7]/ || u ~ /#\$?5,D[0-7]/) has_status5 = 1
    if (u ~ /EORI\.W #\$?FFFF,18\(A0\)/ || u ~ /EOR\.W #\$?FFFF,18\(A0\)/ || u ~ /EOR\.W #-1,18\(A0\)/ || u ~ /NOT\.W D[0-7]/ || u ~ /NOT\.W 18\(A0\)/ || u ~ /NOT\.W \$12\(A[05]\)/) has_toggle_ampm = 1
