...
SRC_C="src/decomp/c/replacements/textdisp_format_entry_time_gcc.c"
ORIG_ASM="src/modules/groups/b/a/textdisp3.s"
OUT_DIR="build/decomp/c_trial_gcc"
GEN_ASM="${OUT_DIR}/textdisp_format_entry_time.generated.gcc.s"
ORIG_SLICE="${OUT_DIR}/textdisp_format_entry_time.original_slice.s"
GEN_SLICE="${OUT_DIR}/textdisp_format_entry_time.generated_slice.s"
ORIG_NORM="${OUT_DIR}/textdisp_format_entry_time.original_slice.norm.s"
GEN_NORM="${OUT_DIR}/textdisp_format_entry_time.generated_slice.norm.s"
...
awk '
$0 ~ /^TEXTDISP_FormatEntryTime:?$/ {in_func=1}
in_func { if ($0 ~ /^;!======/) exit; print }
' "$ORIG_ASM" > "$ORIG_SLICE"

awk '
$0 ~ /^_?TEXTDISP_FormatEntryTime:?$/ {in_func=1}
in_func { print }
' "$GEN_ASM" > "$GEN_SLICE"
...
awk -f src/decomp/scripts/semantic_filter_textdisp_format_entry_time.awk "$ORIG_NORM" > "$ORIG_SEM"
awk -f src/decomp/scripts/semantic_filter_textdisp_format_entry_time.awk "$GEN_NORM" > "$GEN_SEM"
