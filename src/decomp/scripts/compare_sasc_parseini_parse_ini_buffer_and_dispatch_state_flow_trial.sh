...
BASE="parseini_parse_ini_buffer_and_dispatch_state_flow"
...
awk -v start="^${ENTRY}:$" -v next_label="^PARSEINI_ParseHexValueFromString:$" '
    $0 ~ start { in_func = 1 }
    in_func {
...
awk -v e="^${ENTRY}:$" -v e2="$ENTRY_SASC_REGEX" '
    $0 ~ e || $0 ~ e2 { in_func = 1 }
    in_func {
        if (($0 ~ /^PARSEINI_[A-Za-z0-9_]+:$/ || $0 ~ /^_?PARSEINI_[A-Za-z0-9_]+:$/) &&
            $0 !~ e && $0 !~ e2) exit
...
awk -f src/decomp/scripts/semantic_filter_sasc_parseini_parse_ini_buffer_and_dispatch_state_flow.awk "${OUT_DIR}/${BASE}.original.norm.s" >"${OUT_DIR}/${BASE}.original.semantic.txt"
awk -f src/decomp/scripts/semantic_filter_sasc_parseini_parse_ini_buffer_and_dispatch_state_flow.awk "${OUT_DIR}/${BASE}.sasc.norm.s" >"${OUT_DIR}/${BASE}.sasc.semantic.txt"
