...

{
    ...
    if (u ~ /WDISP_STATUSDAYENTRY0/) has_status_entry_ref = 1
    if (u ~ /CLOCK_CURRENTDAYOFYEAR/) has_clock_day_ref = 1
    if (u ~ /CLOCK_CURRENTYEARVALUE/) has_clock_year_ref = 1

    if (u ~ /#43/ || u ~ /#\$/ || u ~ /#\+'\''/ || u ~ /CMP.B D0,D7/) has_plus_const = 1
    if (u ~ /#63/ || u ~ /#\$3F/ || u ~ /#\?'\''/) has_question_const = 1
    if (u ~ /-999/ || u ~ /#-999/ || u ~ /#\$FFFFFC19/) has_neg999_const = 1
    if (u ~ /#20/ || u ~ /#\$14/ || has_mulu_call) has_stride20_const = 1
