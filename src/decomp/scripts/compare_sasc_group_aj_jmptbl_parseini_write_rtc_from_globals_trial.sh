...
cd "$ROOT_DIR"
SASC_SRC="group_aj_jmptbl_stubs.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/a/j/xjump.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="group_aj_jmptbl_parseini_write_rtc_from_globals"
ENTRY_ORIG="GROUP_AJ_JMPTBL_PARSEINI_WriteRtcFromGlobals"
ENTRY_SASC="GROUP_AJ_JMPTBL_PARSEINI_WriteRt"
TARGET="PARSEINI_WriteRtcFromGlobals"
mkdir -p "$OUT_DIR"
./sc-build-with-dis.sh "$SASC_SRC" >"${OUT_DIR}/sc_build_${BASE}.log" 2>&1
awk -v e="^${ENTRY_ORIG}:$" '$0 ~ e {inf=1} inf { if ($0 ~ /^GROUP_AJ_JMPTBL_MATH_Mulu32:$/) exit; print }' "$ORIG_ASM" >"${OUT_DIR}/${BASE}.original.s"
awk -v e="^${ENTRY_SASC}:$" '$0 ~ e {inf=1} inf { if ($0 ~ /^GROUP_AJ_JMPTBL_MATH_Mulu32:$/) exit; print }' "$SASC_DIS" >"${OUT_DIR}/${BASE}.sasc.dis.s"
