...
ENTRY_ORIG="UNKNOWN10_PrintfPutcToBuffer"

mkdir -p "$OUT_DIR"

./sc-build-with-dis.sh "$SASC_SRC" >"${OUT_DIR}/sc_build_unknown10_printf_putc_to_buffer.log" 2>&1

awk -v entry="^${ENTRY_ORIG}:$" '$0 ~ entry {in_func=1} in_func { if ($0 ~ /^;!======/) exit; print }' "$ORIG_ASM" >"${OUT_DIR}/unknown10_printf_putc_to_buffer.original.s"
awk -v entry="^${ENTRY_ORIG}:$" '$0 ~ entry {in_func=1} in_func { if ($0 ~ /^WDISP_SPrintf:$/) exit; print }' "$SASC_DIS" >"${OUT_DIR}/unknown10_printf_putc_to_buffer.sasc.dis.s"
