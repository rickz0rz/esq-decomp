...

{
    ...
    if (uline ~ /TST\.W ED_DIAGNOSTICSSCREENACTIVE/ || uline ~ /BEQ\.W \.RETURN/) has_active_guard = 1
    ...
    if (uline ~ /^PEA 40\.W$/) x40_count++
    if (uline ~ /^PEA 112\.W$/) y112_count++
    if (uline ~ /^PEA 142\.W$/) y142_count++
    if (uline ~ /^PEA 172\.W$/) y172_count++
    if (uline ~ /^PEA 202\.W$/) y202_count++
    if (uline ~ /^PEA 232\.W$/) y232_count++
    if (uline ~ /^PEA 262\.W$/) y262_count++
    if (uline ~ /^PEA 292\.W$/) y292_count++
    ...
    if (uline ~ /GLOBAL_STR_DATA_OVERRUNS_FORMATTED/) has_data_overruns_fmt = 1
    if (uline ~ /GLOBAL_STR_DATA_H_T_C_MAX_FORMATTED/) has_data_htc_fmt = 1
    if (uline ~ /GLOBAL_STR_CTRL_H_T_C_MAX_FORMATTED/) has_ctrl_htc_fmt = 1
    if (uline ~ /GLOBAL_STR_JULIAN_DAY_NEXT_FORMATTED/) has_julian_fmt = 1
    if (uline ~ /GLOBAL_STR_JDAY1_JDAY2_FORMATTED/) has_jday_fmt = 1
    if (uline ~ /GLOBAL_STR_CURCLU_NXTCLU_FORMATTED/) has_curclu_fmt = 1
    if (uline ~ /GLOBAL_STR_C_DATE_C_MONTH_LP_YR_FORMATTED/) has_cache_date_fmt = 1
