...
SRC_C="src/decomp/c/replacements/coi_load_oi_data_file_gcc.c"
ORIG_ASM="src/modules/groups/a/e/coi.s"
OUT_DIR="build/decomp/c_trial_gcc"
GEN_ASM="${OUT_DIR}/coi_load_oi_data_file.generated.gcc.s"
ORIG_SLICE="${OUT_DIR}/coi_load_oi_data_file.original_slice.s"
GEN_SLICE="${OUT_DIR}/coi_load_oi_data_file.generated_slice.s"
ORIG_NORM="${OUT_DIR}/coi_load_oi_data_file.original_slice.norm.s"
GEN_NORM="${OUT_DIR}/coi_load_oi_data_file.generated_slice.norm.s"
...
awk '
$0 ~ /^COI_LoadOiDataFile:?$/ {in_func=1}
in_func {
    if ($0 ~ /^;!======/) exit
    print
}
...
awk '
$0 ~ /^_?COI_LoadOiDataFile:?$/ {in_func=1}
in_func { print }
' "$GEN_ASM" > "$GEN_SLICE"
...
awk -f src/decomp/scripts/semantic_filter_coi_load_oi_data_file.awk "$ORIG_NORM" > "$ORIG_SEM"
awk -f src/decomp/scripts/semantic_filter_coi_load_oi_data_file.awk "$GEN_NORM" > "$GEN_SEM"
