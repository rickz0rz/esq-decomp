...
cd "$ROOT_DIR"
SASC_SRC="esq_reverse_bits_in6_bytes.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/a/a/app2.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="esq_reverse_bits_in6_bytes"
ENTRY="ESQ_ReverseBitsIn6Bytes"
mkdir -p "$OUT_DIR"
...
awk -v ENTRY="$ENTRY" -f src/decomp/scripts/semantic_filter_sasc_reverse_bits_in6_bytes.awk "${OUT_DIR}/${BASE}.original.norm.s" >"${OUT_DIR}/${BASE}.original.semantic.txt"
awk -v ENTRY="$ENTRY" -f src/decomp/scripts/semantic_filter_sasc_reverse_bits_in6_bytes.awk "${OUT_DIR}/${BASE}.sasc.norm.s" >"${OUT_DIR}/${BASE}.sasc.semantic.txt"
