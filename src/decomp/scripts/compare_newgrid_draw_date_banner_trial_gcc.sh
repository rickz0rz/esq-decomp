...
SRC_C="src/decomp/c/replacements/newgrid_draw_date_banner_gcc.c"
ORIG_ASM="src/modules/groups/b/a/newgrid.s"
OUT_DIR="build/decomp/c_trial_gcc"
GEN_ASM="${OUT_DIR}/newgrid_draw_date_banner.generated.gcc.s"
ORIG_SLICE="${OUT_DIR}/newgrid_draw_date_banner.original_slice.s"
GEN_SLICE="${OUT_DIR}/newgrid_draw_date_banner.generated_slice.s"
ORIG_NORM="${OUT_DIR}/newgrid_draw_date_banner.original_slice.norm.s"
GEN_NORM="${OUT_DIR}/newgrid_draw_date_banner.generated_slice.norm.s"
...
awk '
$0 ~ /^NEWGRID_DrawDateBanner:?$/ {in_func=1}
in_func {
    if ($0 ~ /^;!======/) {
        exit
    }
...
awk '
$0 ~ /^_?NEWGRID_DrawDateBanner:?$/ {in_func=1}
in_func { print }
' "$GEN_ASM" > "$GEN_SLICE"
...
awk -f src/decomp/scripts/semantic_filter_newgrid_draw_date_banner.awk "$ORIG_NORM" > "$ORIG_SEM"
awk -f src/decomp/scripts/semantic_filter_newgrid_draw_date_banner.awk "$GEN_NORM" > "$GEN_SEM"
