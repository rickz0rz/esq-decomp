...
SASC_SRC="disptext_append_helpers.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/a/i/disptext.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="disptext_append_to_buffer"
ENTRY="DISPTEXT_AppendToBuffer"
ENTRY_SASC_REGEX="^DISPTEXT_AppendToBuff[A-Za-z0-9_]*:$"
ENTRY_SEM="DISPTEXT_APPENDTOBUFFER:"
ENTRY_ALT_SEM="DISPTEXT_APPENDTOBUFF"
...
awk -v ENTRY_PREFIX="$ENTRY_SEM" -v ENTRY_ALT_PREFIX="$ENTRY_ALT_SEM" -f src/decomp/scripts/semantic_filter_sasc_disptext_append_to_buffer.awk "${OUT_DIR}/${BASE}.original.norm.s" >"${OUT_DIR}/${BASE}.original.semantic.txt"
awk -v ENTRY_PREFIX="$ENTRY_SEM" -v ENTRY_ALT_PREFIX="$ENTRY_ALT_SEM" -f src/decomp/scripts/semantic_filter_sasc_disptext_append_to_buffer.awk "${OUT_DIR}/${BASE}.sasc.norm.s" >"${OUT_DIR}/${BASE}.sasc.semantic.txt"
