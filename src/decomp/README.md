...
- Current hybrid-integration note (March 13, 2026, later re-validated): `modules/groups/_main/a/a.s` is now wired into `src/decomp/replacements.map` as a passthrough hybrid boundary. Its restored SAS/C coverage already spans the primary `_main` startup/shutdown helpers (`ESQ_StartupEntry`, `ESQ_ReturnWithStackCode`, `ESQ_ShutdownAndReturn`), so future object-level replacement work can land there without another root include-graph edit.
- Current hybrid-integration note (March 13, 2026, later re-validated): `modules/groups/a/a/app3.s` is now wired into `src/decomp/replacements.map` as a passthrough hybrid boundary. Its restored SAS/C coverage already spans the non-`JMPTBL` reboot/init helpers in that module, so future object-level replacement work can land there without another root include-graph edit.
- Current hybrid-integration note (March 13, 2026, later re-validated): `modules/groups/a/d/cleanup3.s` is now wired into `src/decomp/replacements.map` as a passthrough hybrid boundary. Its restored SAS/C coverage already spans the aligned-status banner/screen helper cluster in that module, so future object-level replacement work can land there without another root include-graph edit.
...
- Current hybrid-integration note (March 13, 2026, later re-validated): `modules/groups/a/m/esq.s` is now wired into `src/decomp/replacements.map` as a passthrough hybrid boundary. Its restored SAS/C coverage currently includes `ESQ_MainInitAndRun`, so future object-level replacement work can land in that main startup/init module without another root include-graph edit.
...
- Current hybrid-integration note (March 13, 2026): `modules/groups/a/v/gcommand5.s` is now wired into `src/decomp/replacements.map` as a passthrough hybrid boundary. Its restored SAS/C coverage currently includes `GCOMMAND_ProcessCtrlCommand`, so future object-level replacement work can land in that control-command helper module without another root include-graph edit.
- Current hybrid-integration note (March 13, 2026): `modules/groups/a/v/kybd.s` is now wired into `src/decomp/replacements.map` as a passthrough hybrid boundary. Its restored SAS/C coverage currently includes `KYBD_InitializeInputDevices`, so future object-level replacement work can land in that input-device initialization module without another root include-graph edit.
...
- Current hybrid-integration note (March 13, 2026): `modules/submodules/unknown4.s`, `modules/submodules/unknown11.s`, and `modules/submodules/unknown14.s` are now wired into `src/decomp/replacements.map` as passthrough hybrid boundaries. Their restored SAS/C coverage already spans `STRING_ToUpperInPlace`, `DOS_SeekByIndex`, and `HANDLE_OpenFromModeString`, so future object-level replacement work can land in those helper modules without another root include-graph edit.
...
- Current hybrid-integration note (March 13, 2026): `modules/submodules/unknown38.s` is now wired into `src/decomp/replacements.map` as a passthrough hybrid boundary. Its restored SAS/C coverage already spans the current non-`JMPTBL` signal/exit helper (`SIGNAL_PollAndDispatch`), so future object-level replacement work can land there without another root include-graph edit.
