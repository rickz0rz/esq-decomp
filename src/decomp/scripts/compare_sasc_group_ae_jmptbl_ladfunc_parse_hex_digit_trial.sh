...
BASE="group_ae_jmptbl_ladfunc_parse_hex_digit"
ENTRY_ORIG="GROUP_AE_JMPTBL_LADFUNC_ParseHexDigit"
ENTRY_SASC="GROUP_AE_JMPTBL_LADFUNC_ParseHex"
TARGET="LADFUNC_ParseHexDigit"
mkdir -p "$OUT_DIR"
./sc-build-with-dis.sh "$SASC_SRC" >"${OUT_DIR}/sc_build_${BASE}.log" 2>&1
awk -v e="^${ENTRY_ORIG}:$" '$0 ~ e {inf=1} inf { if ($0 ~ /^GROUP_AE_JMPTBL_SCRIPT_DeallocateBufferArray:$/) exit; print }' "$ORIG_ASM" >"${OUT_DIR}/${BASE}.original.s"
awk -v e="^${ENTRY_SASC}:$" '$0 ~ e {inf=1} inf { if ($0 ~ /^GROUP_AE_JMPTBL_SCRIPT_Deallocat:$/) exit; print }' "$SASC_DIS" >"${OUT_DIR}/${BASE}.sasc.dis.s"
