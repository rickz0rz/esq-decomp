...
BASE="esqshared4_copy_banner_rows_with_byte_offset_state_flow"
...
awk -v e="^${ENTRY}:$" -v e2="$ENTRY_SASC_REGEX" '
  $0 ~ e || $0 ~ e2 {in_func=1}
  in_func {
    if (($0 ~ /^ESQSHARED4_[A-Za-z0-9_]+:$/ || $0 ~ /^_?ESQSHARED4_[A-Za-z0-9_]+:$/) &&
        $0 !~ /^___/ && $0 !~ e && $0 !~ e2) exit
...
awk -f src/decomp/scripts/semantic_filter_sasc_esqshared4_copy_banner_rows_with_byte_offset_state_flow.awk \
