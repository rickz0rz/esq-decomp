...
SASC_SRC="disptext_layout_helpers.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/a/i/disptext.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="disptext_finalize_line_table"
ENTRY="DISPTEXT_FinalizeLineTable"
ENTRY_SASC_REGEX="^DISPTEXT_FinalizeLineTable[A-Za-z0-9_]*:$"
ENTRY_SEM="DISPTEXT_FINALIZELINETABLE:"
ENTRY_ALT_SEM="DISPTEXT_FINALIZELINETABL"
...
awk -v ENTRY_PREFIX="$ENTRY_SEM" -v ENTRY_ALT_PREFIX="$ENTRY_ALT_SEM" -f src/decomp/scripts/semantic_filter_sasc_disptext_finalize_line_table.awk "${OUT_DIR}/${BASE}.original.norm.s" >"${OUT_DIR}/${BASE}.original.semantic.txt"
awk -v ENTRY_PREFIX="$ENTRY_SEM" -v ENTRY_ALT_PREFIX="$ENTRY_ALT_SEM" -f src/decomp/scripts/semantic_filter_sasc_disptext_finalize_line_table.awk "${OUT_DIR}/${BASE}.sasc.norm.s" >"${OUT_DIR}/${BASE}.sasc.semantic.txt"
