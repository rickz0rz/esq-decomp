...
cd "$ROOT_DIR"
SASC_SRC="group_ad_jmptbl_stubs.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/a/d/xjump.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="group_ad_jmptbl_esqfunc_select_and_apply_brush_for_current_entry"
ENTRY_ORIG="GROUP_AD_JMPTBL_ESQFUNC_SelectAndApplyBrushForCurrentEntry"
ENTRY_SASC="GROUP_AD_JMPTBL_ESQFUNC_SelectAn"
TARGET="ESQFUNC_SelectAndApplyBrushForCurrentEntry"
mkdir -p "$OUT_DIR"
./sc-build-with-dis.sh "$SASC_SRC" >"${OUT_DIR}/sc_build_${BASE}.log" 2>&1
awk -v e="^${ENTRY_ORIG}:$" '
  $0 ~ e {inf=1}
  inf {
    if ($0 ~ /^GROUP_AD_JMPTBL_/ && $0 !~ e) exit
    if ($0 ~ /^;!======/) exit
    print
  }
