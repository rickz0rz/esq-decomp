...
SRC_C="src/decomp/c/replacements/ctasks_close_task_teardown_gcc.c"
ORIG_ASM="src/modules/groups/a/f/ctasks.s"
OUT_DIR="build/decomp/c_trial_gcc"
GEN_ASM="${OUT_DIR}/ctasks_close_task_teardown.generated.gcc.s"
ORIG_SLICE="${OUT_DIR}/ctasks_close_task_teardown.original_slice.s"
GEN_SLICE="${OUT_DIR}/ctasks_close_task_teardown.generated_slice.s"
ORIG_NORM="${OUT_DIR}/ctasks_close_task_teardown.original_slice.norm.s"
GEN_NORM="${OUT_DIR}/ctasks_close_task_teardown.generated_slice.norm.s"
...
awk '
$0 ~ /^CTASKS_CloseTaskTeardown:?$/ {in_func=1}
in_func {
    if ($0 ~ /^;!======/) exit
    print
}
...
awk '
$0 ~ /^_?CTASKS_CloseTaskTeardown:?$/ {in_func=1}
in_func { print }
' "$GEN_ASM" > "$GEN_SLICE"
...
awk -f src/decomp/scripts/semantic_filter_ctasks_close_task_teardown.awk "$ORIG_NORM" > "$ORIG_SEM"
awk -f src/decomp/scripts/semantic_filter_ctasks_close_task_teardown.awk "$GEN_NORM" > "$GEN_SEM"
