...
BASE="disptext_is_last_line_selected"
ENTRY="DISPTEXT_IsLastLineSelected"
ENTRY_SASC_REGEX="^DISPTEXT_IsLastLineSelecte[A-Za-z0-9_]*:$"
ENTRY_SEM="DISPTEXT_ISLASTLINESELECTED:"
ENTRY_ALT_SEM="DISPTEXT_ISLASTLINESELECTE"
...
awk -v ENTRY_PREFIX="$ENTRY_SEM" -v ENTRY_ALT_PREFIX="$ENTRY_ALT_SEM" -f src/decomp/scripts/semantic_filter_sasc_disptext_is_last_line_selected.awk "${OUT_DIR}/${BASE}.original.norm.s" >"${OUT_DIR}/${BASE}.original.semantic.txt"
awk -v ENTRY_PREFIX="$ENTRY_SEM" -v ENTRY_ALT_PREFIX="$ENTRY_ALT_SEM" -f src/decomp/scripts/semantic_filter_sasc_disptext_is_last_line_selected.awk "${OUT_DIR}/${BASE}.sasc.norm.s" >"${OUT_DIR}/${BASE}.sasc.semantic.txt"
