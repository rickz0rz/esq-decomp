...
cd "$ROOT_DIR"
SASC_SRC="esq_format_time_stamp.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/a/a/app2.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="esq_format_time_stamp"
ENTRY="ESQ_FormatTimeStamp"
mkdir -p "$OUT_DIR"
...
awk -v ENTRY="$ENTRY" -f src/decomp/scripts/semantic_filter_sasc_format_time_stamp.awk "${OUT_DIR}/${BASE}.original.norm.s" >"${OUT_DIR}/${BASE}.original.semantic.txt"
awk -v ENTRY="$ENTRY" -f src/decomp/scripts/semantic_filter_sasc_format_time_stamp.awk "${OUT_DIR}/${BASE}.sasc.norm.s" >"${OUT_DIR}/${BASE}.sasc.semantic.txt"
