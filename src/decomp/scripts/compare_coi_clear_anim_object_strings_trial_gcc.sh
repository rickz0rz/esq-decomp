...
SRC_C="src/decomp/c/replacements/coi_clear_anim_object_strings_gcc.c"
ORIG_ASM="src/modules/groups/a/e/coi.s"
OUT_DIR="build/decomp/c_trial_gcc"
GEN_ASM="${OUT_DIR}/coi_clear_anim_object_strings.generated.gcc.s"
ORIG_SLICE="${OUT_DIR}/coi_clear_anim_object_strings.original_slice.s"
GEN_SLICE="${OUT_DIR}/coi_clear_anim_object_strings.generated_slice.s"
ORIG_NORM="${OUT_DIR}/coi_clear_anim_object_strings.original_slice.norm.s"
GEN_NORM="${OUT_DIR}/coi_clear_anim_object_strings.generated_slice.norm.s"
...
awk '
$0 ~ /^COI_ClearAnimObjectStrings:?$/ {in_func=1}
in_func {
    if ($0 ~ /^COI_ClearAnimObjectStrings_Return:?$/) exit
    print
}
' "$ORIG_ASM" > "$ORIG_SLICE"

awk '
$0 ~ /^_?COI_ClearAnimObjectStrings:?$/ {in_func=1}
in_func { print }
' "$GEN_ASM" > "$GEN_SLICE"
...
awk -f src/decomp/scripts/semantic_filter_coi_clear_anim_object_strings.awk "$ORIG_NORM" > "$ORIG_SEM"
awk -f src/decomp/scripts/semantic_filter_coi_clear_anim_object_strings.awk "$GEN_NORM" > "$GEN_SEM"
