...
SASC_SRC="group_aw_jmptbl_stubs_core.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/a/w/xjump.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="group_aw_jmptbl_tliba3_build_display_context_for_view_mode"
ENTRY_ORIG="GROUP_AW_JMPTBL_TLIBA3_BuildDisplayContextForViewMode"
ENTRY_SASC="GROUP_AW_JMPTBL_TLIBA3_BuildDisp"
TARGET="TLIBA3_BuildDisplayContextForVie"

mkdir -p "$OUT_DIR"
./sc-build-with-dis.sh "$SASC_SRC" >"${OUT_DIR}/sc_build_${BASE}.log" 2>&1
awk -v e="^${ENTRY_ORIG}:$" '$0 ~ e {inf=1} inf { if ($0 ~ /^GROUP_AW_JMPTBL_DISPLIB_ApplyInlineAlignmentPadding:$/) exit; print }' "$ORIG_ASM" >"${OUT_DIR}/${BASE}.original.s"
awk -v e="^${ENTRY_SASC}:$" '$0 ~ e {inf=1} inf { if ($0 ~ /^GROUP_AW_JMPTBL_DISPLIB_ApplyInl:$/) exit; print }' "$SASC_DIS" >"${OUT_DIR}/${BASE}.sasc.dis.s"
