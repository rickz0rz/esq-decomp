...
- Current hybrid-integration note (March 13, 2026): `modules/groups/a/t/gcommand2.s` is now wired into `src/decomp/replacements.map` as a passthrough hybrid boundary. Its restored SAS/C coverage already spans the current non-`JMPTBL` helper foothold (`GCOMMAND_CopyGfxToWorkIfAvailable`, `GCOMMAND_FindPathSeparator`), so future object-level replacement work can land there without another root include-graph edit.
...
- Current hybrid-integration note (March 13, 2026): `modules/groups/a/r/flib.s` is now wired into `src/decomp/replacements.map` as a passthrough hybrid boundary. Its restored SAS/C coverage already spans the current non-`JMPTBL` log-append helper exports in that module, so future object-level replacement work can land there without another root include-graph edit.
...
- Current hybrid-integration note (March 13, 2026, later re-validated): `modules/groups/b/a/script.s` is now wired into `src/decomp/replacements.map` as a passthrough hybrid boundary. Its restored SAS/C coverage already spans the current non-`JMPTBL` buffer allocation, deallocation, and token-index helpers in that module, so future object-level replacement work can land there without another root include-graph edit.
...
- Current hybrid-integration note (March 13, 2026): `modules/submodules/unknown4.s` is now wired into `src/decomp/replacements.map` as a passthrough hybrid boundary. Its restored SAS/C coverage already spans `STRING_ToUpperInPlace`, so future object-level replacement work can land in that helper module without another root include-graph edit.
