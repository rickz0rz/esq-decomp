...
SASC_SRC="group_aw_jmptbl_esqiff_run_copper_drop_transition.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/a/w/xjump.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="group_aw_jmptbl_esqiff_run_copper_drop_transition"
ENTRY_ORIG="GROUP_AW_JMPTBL_ESQIFF_RunCopperDropTransition"
ENTRY_SASC="GROUP_AW_JMPTBL_ESQIFF_RunCopper"
TARGET="ESQIFF_RunCopperDropTransition"

mkdir -p "$OUT_DIR"
./sc-build-with-dis.sh "$SASC_SRC" >"${OUT_DIR}/sc_build_${BASE}.log" 2>&1
awk -v e="^${ENTRY_ORIG}:$" '$0 ~ e {inf=1} inf { if ($0 ~ /^GROUP_AW_JMPTBL_DISPLIB_DisplayTextAtPosition:$/) exit; print }' "$ORIG_ASM" >"${OUT_DIR}/${BASE}.original.s"
