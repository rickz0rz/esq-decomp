...
- Current hybrid-integration note (March 13, 2026): `modules/groups/a/s/gcommand.s` is now wired into `src/decomp/replacements.map` as a passthrough hybrid boundary. Its restored SAS/C coverage already spans the current non-`JMPTBL` parser and template-loading helpers in that module, so future object-level replacement work can land there without another root include-graph edit.
