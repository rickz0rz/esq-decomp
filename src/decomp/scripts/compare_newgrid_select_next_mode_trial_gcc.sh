...
SRC_C="src/decomp/c/replacements/newgrid_select_next_mode_gcc.c"
ORIG_ASM="src/modules/groups/b/a/newgrid.s"
OUT_DIR="build/decomp/c_trial_gcc"
GEN_ASM="${OUT_DIR}/newgrid_select_next_mode.generated.gcc.s"
ORIG_SLICE="${OUT_DIR}/newgrid_select_next_mode.original_slice.s"
GEN_SLICE="${OUT_DIR}/newgrid_select_next_mode.generated_slice.s"
ORIG_NORM="${OUT_DIR}/newgrid_select_next_mode.original_slice.norm.s"
GEN_NORM="${OUT_DIR}/newgrid_select_next_mode.generated_slice.norm.s"
...
awk '
$0 ~ /^NEWGRID_SelectNextMode:?$/ {in_func=1}
in_func {
    if ($0 ~ /^;!======/) {
        exit
    }
...
awk '
$0 ~ /^_?NEWGRID_SelectNextMode:?$/ {in_func=1}
in_func { print }
' "$GEN_ASM" > "$GEN_SLICE"
...
awk -f src/decomp/scripts/semantic_filter_newgrid_select_next_mode.awk "$ORIG_NORM" > "$ORIG_SEM"
awk -f src/decomp/scripts/semantic_filter_newgrid_select_next_mode.awk "$GEN_NORM" > "$GEN_SEM"
