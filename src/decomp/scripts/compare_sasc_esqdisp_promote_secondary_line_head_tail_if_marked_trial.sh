...
cd "$ROOT_DIR"
SASC_SRC="esqdisp_promote_secondary_line_head_tail_if_marked.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/a/n/esqdisp.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="esqdisp_promote_secondary_line_head_tail_if_marked"
ENTRY="ESQDISP_PromoteSecondaryLineHeadTailIfMarked"
ENTRY_SASC_REGEX="^ESQDISP_PromoteSecondaryLineHe[A-Za-z0-9_]*:$"
mkdir -p "$OUT_DIR"
...
awk -f src/decomp/scripts/semantic_filter_sasc_esqdisp_promote_secondary_line_head_tail_if_marked.awk "${OUT_DIR}/${BASE}.original.norm.s" >"${OUT_DIR}/${BASE}.original.semantic.txt"
awk -f src/decomp/scripts/semantic_filter_sasc_esqdisp_promote_secondary_line_head_tail_if_marked.awk "${OUT_DIR}/${BASE}.sasc.norm.s" >"${OUT_DIR}/${BASE}.sasc.semantic.txt"
