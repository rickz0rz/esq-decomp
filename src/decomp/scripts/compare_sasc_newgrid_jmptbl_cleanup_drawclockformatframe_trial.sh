...
cd "$ROOT_DIR"
SASC_SRC="newgrid_jmptbl_cleanup_draw_clock_format_frame.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/b/a/newgrid.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="newgrid_jmptbl_cleanup_drawclockformatframe"
ENTRY_ORIG="NEWGRID_JMPTBL_CLEANUP_DrawClockFormatFrame"
ENTRY_SASC="NEWGRID_JMPTBL_CLEANUP_DrawClock"
TARGET="CLEANUP_DrawClockFormatFrame"
mkdir -p "$OUT_DIR"
./sc-build-with-dis.sh "$SASC_SRC" >"${OUT_DIR}/sc_build_${BASE}.log" 2>&1
awk -v e="^${ENTRY_ORIG}:$" '$0 ~ e {inf=1} inf { if ($0 ~ /^NEWGRID_JMPTBL_/ && $0 !~ e) exit; if ($0 ~ /^;!======/) exit; print }' "$ORIG_ASM" >"${OUT_DIR}/${BASE}.original.s"
