...
BASE="group_av_jmptbl_signal_create_msgport_with_signal"
ENTRY_ORIG="GROUP_AV_JMPTBL_SIGNAL_CreateMsgPortWithSignal"
ENTRY_SASC="GROUP_AV_JMPTBL_SIGNAL_CreateMsg"
TARGET="SIGNAL_CreateMsgPortWithSignal"
mkdir -p "$OUT_DIR"
./sc-build-with-dis.sh "$SASC_SRC" >"${OUT_DIR}/sc_build_${BASE}.log" 2>&1
awk -v e="^${ENTRY_ORIG}:$" '$0 ~ e {inf=1} inf { if ($0 ~ /^GROUP_AV_JMPTBL_DISKIO_ProbeDrivesAndAssignPaths:$/) exit; print }' "$ORIG_ASM" >"${OUT_DIR}/${BASE}.original.s"
awk -v e="^${ENTRY_SASC}:$" '$0 ~ e {inf=1} inf { if ($0 ~ /^GROUP_AV_JMPTBL_DISKIO_ProbeDriv:$/) exit; print }' "$SASC_DIS" >"${OUT_DIR}/${BASE}.sasc.dis.s"
