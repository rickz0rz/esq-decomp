...
BASE="group_ab_jmptbl_newgrid_shutdown_grid_resources"
ENTRY_ORIG="GROUP_AB_JMPTBL_NEWGRID_ShutdownGridResources"
ENTRY_SASC="GROUP_AB_JMPTBL_NEWGRID_Shutdown"
TARGET="NEWGRID_ShutdownGridResources"
mkdir -p "$OUT_DIR"
./sc-build-with-dis.sh "$SASC_SRC" >"${OUT_DIR}/sc_build_${BASE}.log" 2>&1
awk -v e="^${ENTRY_ORIG}:$" '$0 ~ e {inf=1} inf { if ($0 ~ /^GROUP_AB_JMPTBL_LOCAVAIL_FreeResourceChain:$/) exit; print }' "$ORIG_ASM" >"${OUT_DIR}/${BASE}.original.s"
awk -v e="^${ENTRY_SASC}:$" '$0 ~ e {inf=1} inf { if ($0 ~ /^GROUP_AB_JMPTBL_LOCAVAIL_FreeRes:$/) exit; print }' "$SASC_DIS" >"${OUT_DIR}/${BASE}.sasc.dis.s"
