...
BASE="group_aj_jmptbl_math_div_u32"
ENTRY_ORIG="GROUP_AJ_JMPTBL_MATH_DivU32"
ENTRY_SASC="GROUP_AJ_JMPTBL_MATH_DivU32"
TARGET="MATH_DivU32"
mkdir -p "$OUT_DIR"
./sc-build-with-dis.sh "$SASC_SRC" >"${OUT_DIR}/sc_build_${BASE}.log" 2>&1
awk -v e="^${ENTRY_ORIG}:$" '$0 ~ e {inf=1} inf { if ($0 ~ /^GROUP_AJ_JMPTBL_PARSEINI_WriteRtcFromGlobals:$/) exit; print }' "$ORIG_ASM" >"${OUT_DIR}/${BASE}.original.s"
awk -v e="^${ENTRY_SASC}:$" '$0 ~ e {inf=1} inf { if ($0 ~ /^GROUP_AJ_JMPTBL_PARSEINI_WriteRt:$/) exit; print }' "$SASC_DIS" >"${OUT_DIR}/${BASE}.sasc.dis.s"
