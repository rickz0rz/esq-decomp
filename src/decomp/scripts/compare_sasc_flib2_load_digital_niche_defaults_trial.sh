...
ENTRY="FLIB2_LoadDigitalNicheDefaults"
ENTRY_SASC_REGEX="^FLIB2_LoadDigitalNicheDefaults[A-Za-z0-9_]*:$"
...
awk -v e="^${ENTRY}:$" -v e2="$ENTRY_SASC_REGEX" '$0 ~ e || $0 ~ e2 {in_func=1} in_func { if (($0 ~ /^FLIB2_[A-Za-z0-9_]+:$/ || $0 ~ /^_?FLIB2_[A-Za-z0-9_]+:$/) && $0 !~ e && $0 !~ e2) exit; if ($0 ~ /^XREF / || $0 ~ /^XDEF / || $0 ~ /^ END$/ || $0 ~ /^END$/) exit; print }' \
...
awk -f src/decomp/scripts/semantic_filter_sasc_flib2_defaults.awk \
