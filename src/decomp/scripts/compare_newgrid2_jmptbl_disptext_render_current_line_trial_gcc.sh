...
SRC_C="src/decomp/c/replacements/newgrid2_jmptbl_disptext_render_current_line_gcc.c"
ORIG_ASM="src/modules/groups/b/a/newgrid2.s"
OUT_DIR="build/decomp/c_trial_gcc"
GEN_ASM="${OUT_DIR}/newgrid2_jmptbl_disptext_render_current_line.generated.gcc.s"
ORIG_SLICE="${OUT_DIR}/newgrid2_jmptbl_disptext_render_current_line.original_slice.s"
GEN_SLICE="${OUT_DIR}/newgrid2_jmptbl_disptext_render_current_line.generated_slice.s"
ORIG_NORM="${OUT_DIR}/newgrid2_jmptbl_disptext_render_current_line.original_slice.norm.s"
GEN_NORM="${OUT_DIR}/newgrid2_jmptbl_disptext_render_current_line.generated_slice.norm.s"
...
awk -v start_label="NEWGRID2_JMPTBL_DISPTEXT_RenderCurrentLine" -v next_label="NEWGRID2_JMPTBL_COI_ProcessEntrySelectionState" '
...
awk '$0 ~ /^_?NEWGRID2_JMPTBL_DISPTEXT_RenderCurrentLine:?$/ {in_func=1} in_func { print }' "$GEN_ASM" > "$GEN_SLICE"
...
awk -f "src/decomp/scripts/semantic_filter_newgrid2_jmptbl_disptext_render_current_line.awk" "$ORIG_NORM" > "$ORIG_SEM"
awk -f "src/decomp/scripts/semantic_filter_newgrid2_jmptbl_disptext_render_current_line.awk" "$GEN_NORM" > "$GEN_SEM"
