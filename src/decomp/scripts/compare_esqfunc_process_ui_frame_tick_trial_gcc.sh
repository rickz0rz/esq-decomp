...
SRC_C="src/decomp/c/replacements/esqfunc_process_ui_frame_tick_gcc.c"
ORIG_ASM="src/modules/groups/a/n/esqfunc.s"
OUT_DIR="build/decomp/c_trial_gcc"
GEN_ASM="${OUT_DIR}/esqfunc_process_ui_frame_tick.generated.gcc.s"
ORIG_SLICE="${OUT_DIR}/esqfunc_process_ui_frame_tick.original_slice.s"
GEN_SLICE="${OUT_DIR}/esqfunc_process_ui_frame_tick.generated_slice.s"
ORIG_NORM="${OUT_DIR}/esqfunc_process_ui_frame_tick.original_slice.norm.s"
GEN_NORM="${OUT_DIR}/esqfunc_process_ui_frame_tick.generated_slice.norm.s"
...
awk '$0 ~ /^ESQFUNC_ProcessUiFrameTick:?$/ {in_func=1} in_func { if ($0 ~ /^;!======/) exit; print }' "$ORIG_ASM" > "$ORIG_SLICE"
awk '$0 ~ /^_?ESQFUNC_ProcessUiFrameTick:?$/ {in_func=1} in_func { print }' "$GEN_ASM" > "$GEN_SLICE"
...
awk -f src/decomp/scripts/semantic_filter_esqfunc_process_ui_frame_tick.awk "$ORIG_NORM" > "$ORIG_SEM"
awk -f src/decomp/scripts/semantic_filter_esqfunc_process_ui_frame_tick.awk "$GEN_NORM" > "$GEN_SEM"
