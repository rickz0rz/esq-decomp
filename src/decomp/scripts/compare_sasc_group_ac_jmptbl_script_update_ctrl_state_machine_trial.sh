...
cd "$ROOT_DIR"
SASC_SRC="group_ac_jmptbl_script_update_ctrl_state_machine.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/a/c/xjump.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="group_ac_jmptbl_script_update_ctrl_state_machine"
ENTRY_ORIG="GROUP_AC_JMPTBL_SCRIPT_UpdateCtrlStateMachine"
ENTRY_SASC="GROUP_AC_JMPTBL_SCRIPT_UpdateCtr"
TARGET="SCRIPT_UpdateCtrlStateMachine"
mkdir -p "$OUT_DIR"
./sc-build-with-dis.sh "$SASC_SRC" >"${OUT_DIR}/sc_build_${BASE}.log" 2>&1
awk -v e="^${ENTRY_ORIG}:$" '
  $0 ~ e {inf=1}
  inf {
    if ($0 ~ /^GROUP_AC_JMPTBL_/ && $0 !~ e) exit
    if ($0 ~ /^;!======/) exit
    print
  }
