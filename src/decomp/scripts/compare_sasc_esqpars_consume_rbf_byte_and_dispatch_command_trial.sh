...
SASC_SRC="esqpars_consume_rbf_byte_and_dispatch_command.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/a/o/esqpars.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="esqpars_consume_rbf_byte_and_dispatch_command"
ENTRY_ORIG="ESQPARS_ConsumeRbfByteAndDispatchCommand"
ENTRY_SASC_REGEX="^ESQPARS_ConsumeRbfByteAndDispatc[A-Za-z0-9_]*:$"

mkdir -p "$OUT_DIR"

./sc-build-with-dis.sh "$SASC_SRC" >"${OUT_DIR}/sc_build_${BASE}.log" 2>&1

awk -v start="^${ENTRY_ORIG}:$" -v next_label="^ESQPARS_ReadLengthWordWithChecksumXor:$" '$0 ~ start {in_func=1} in_func {if ($0 ~ next_label) exit; print}' "$ORIG_ASM" >"${OUT_DIR}/${BASE}.original.s"
...
awk -f src/decomp/scripts/semantic_filter_sasc_esqpars_consume_rbf_byte_and_dispatch_command.awk "${OUT_DIR}/${BASE}.original.norm.s" >"${OUT_DIR}/${BASE}.original.semantic.txt"
awk -f src/decomp/scripts/semantic_filter_sasc_esqpars_consume_rbf_byte_and_dispatch_command.awk "${OUT_DIR}/${BASE}.sasc.norm.s" >"${OUT_DIR}/${BASE}.sasc.semantic.txt"
