...
char *GROUP_AI_JMPTBL_STR_FindCharPtr(const char *s, long ch){return STR_FindCharPtr(s, ch);}
void GROUP_AI_JMPTBL_TLIBA1_DrawTextWithInsetSegments(char *rp, LONG x, LONG y, LONG insetSecondary, LONG insetPrimary, const char *text){TLIBA1_DrawTextWithInsetSegments(rp, x, y, insetSecondary, insetPrimary, text);}
...
char *GROUP_AI_JMPTBL_STR_SkipClass3Chars(const char *s){return STR_SkipClass3Chars(s);}
char *GROUP_AI_JMPTBL_STRING_AppendAtNull(char *dst, const char *src){return STRING_AppendAtNull(dst, src);}
char *GROUP_AI_JMPTBL_STR_CopyUntilAnyDelimN(const char *src, char *dst, LONG maxLen, const char *delims){return STR_CopyUntilAnyDelimN(src, dst, maxLen, delims);}
