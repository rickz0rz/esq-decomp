...
BASE="wdisp_handle_weather_status_command_state_flow"
ENTRY="WDISP_HandleWeatherStatusCommand"
ENTRY_SASC_REGEX="^WDISP_HandleWeatherStatusComman[A-Za-z0-9_]*:$"
NEXT_LABEL="^WDISP_UpdateSelectionPreviewPanel:$"
...
awk -v e="^${ENTRY}:$" -v e2="$ENTRY_SASC_REGEX" '
    $0 ~ e || $0 ~ e2 {in_func=1}
    in_func {
        if (($0 ~ /^WDISP_[A-Za-z0-9_]+:$/ || $0 ~ /^_?WDISP_[A-Za-z0-9_]+:$/) &&
            $0 !~ e && $0 !~ e2) exit
...
awk -f src/decomp/scripts/semantic_filter_sasc_wdisp_handle_weather_status_command_state_flow.awk \
