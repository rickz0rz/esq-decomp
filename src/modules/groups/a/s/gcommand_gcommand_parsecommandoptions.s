    XDEF    _GCOMMAND_ParseCommandOptions

;------------------------------------------------------------------------------
; FUNC: _GCOMMAND_ParseCommandOptions   (Parse digital-niche command options string)
; ARGS:
...
; CALLS:
;   _ESQPARS_JMPTBL_PARSE_ReadSignedLongSkipClass3_Alt, _GCOMMAND_LoadCommandFile, _GROUP_AW_JMPTBL_STRING_CopyPadNul, _ESQPARS_ReplaceOwnedString, _FLIB2_LoadDigitalNicheDefaults, _LADFUNC_ParseHexDigit
; READS:
;   _GCOMMAND_NicheParseScratchSeedWord, _WDISP_CharClassTable, _GCOMMAND_DigitalNicheListingsTemplatePtr, return
; WRITES:
...
;------------------------------------------------------------------------------
_GCOMMAND_ParseCommandOptions:
    LINK.W  A5,#-20
    MOVEM.L D4-D7/A3,-(A7)
    MOVEA.L 8(A5),A3
    MOVEQ   #0,D6
    LEA     _GCOMMAND_NicheParseScratchSeedWord,A0
    LEA     -12(A5),A1
    MOVE.B  (A0)+,(A1)+
    MOVE.B  (A0)+,(A1)+
    MOVE.B  (A0)+,(A1)+
...
.return:
    BSR.W   _GCOMMAND_LoadCommandFile
