...

{
    ...
    if (u ~ /^TST\.B 0?\(A[0-7](,D[0-7]\.L)?\)$/ || u ~ /^TST\.B [$]?[0-9A-F]+\((A[0-7]|A0)\)$/ || u ~ /^MOVE\.B 0?\(A[0-7](,D[0-7]\.L)?\),D[0-7]$/ || u ~ /^MOVE\.B \(A[0-7]\)\+,D[0-7]$/) has_src_guard = 1
    if (u ~ /^TST\.B 0?\(A[0-7](,D[0-7]\.L)?\)$/ || u ~ /^TST\.B [$]?[0-9A-F]+\((A[0-7]|A0)\)$/ || u ~ /^MOVE\.B \(A[0-7]\)\+,D[0-7]$/) has_delim_guard = 1
    if (u ~ /^(BEQ|BEQ\.S|BEQ\.B|JEQ|JEQ\.S) /) jeq_count++

    if (u ~ /^CMP\.B 0?\(A[0-7](,D[0-7]\.L)?\),D[0-7]$/ || u ~ /^CMP\.B D[0-7],0?\(A[0-7](,D[0-7]\.L)?\)$/ || u ~ /^CMP\.B D[0-7],[$]?[0-9A-F]+\((A[0-7]|A0)\)$/ || u ~ /^CMP\.B D[0-7],D[0-7]$/) has_delim_compare = 1

    if (u ~ /^ADDQ\.L #\$?1,D[0-7]$/ || u ~ /^ADDQ\.L #\$?1,A[0-7]$/ || u ~ /^LEA \([0-9]+,A[0-7]\),A[0-7]$/ || u ~ /^LEA [$]?[0-9A-F]+\((A[0-7]|A0)\),A[0-7]$/ || u ~ /^MOVE\.B \(A[0-7]\)\+,D[0-7]$/) has_delim_advance = 1
    if (u ~ /^MOVE\.B D[0-7],[$]?0?\(A[0-7](,D[0-7]\.L)?\)$/ || u ~ /^MOVE\.B [$]?0?\(A[0-7](,D[0-7]\.L)?\),[$]?0?\(A[0-7](,D[0-7]\.L)?\)$/ || u ~ /^MOVE\.B D[0-7],\(A[0-7]\)\+$/) has_copy_store = 1
    if (u ~ /^ADDQ\.L #\$?1,D[0-7]$/ || u ~ /^ADDQ\.L #\$?1,A[0-7]$/ || u ~ /^LEA \([0-9]+,A[0-7]\),A[0-7]$/ || u ~ /^LEA [$]?[0-9A-F]+\((A[0-7]|A0)\),A[0-7]$/ || u ~ /^MOVE\.B \(A[0-7]\)\+,D[0-7]$/) has_copy_advance = 1
