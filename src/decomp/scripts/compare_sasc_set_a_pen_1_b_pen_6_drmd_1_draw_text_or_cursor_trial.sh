...
SASC_SRC="set_a_pen_1_b_pen_6_drmd_1_draw_text_or_cursor.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/a/l/ed3.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="set_a_pen_1_b_pen_6_drmd_1_draw_text_or_cursor"
ENTRY="SET_A_PEN_1_B_PEN_6_DRMD_1_DRAW_TEXT_OR_CURSOR"
ENTRY_SASC_REGEX="^SET_A_PEN_1_B_PEN_6_DRMD_1_DRAW_:$"

mkdir -p "$OUT_DIR"

./sc-build-with-dis.sh "$SASC_SRC" >"${OUT_DIR}/sc_build_${BASE}.log" 2>&1

awk -v start="^${ENTRY}:$" -v next_label="^SET_A_PEN_1_B_PEN_6_DRMD_1_DRAW_LINE_OR_PAGE:$" '$0 ~ start {in_func=1} in_func {if ($0 ~ next_label) exit; print}' "$ORIG_ASM" >"${OUT_DIR}/${BASE}.original.s"
...
awk -f src/decomp/scripts/semantic_filter_sasc_set_a_pen_1_b_pen_6_drmd_1_draw_text_or_cursor.awk "${OUT_DIR}/${BASE}.original.norm.s" >"${OUT_DIR}/${BASE}.original.semantic.txt"
awk -f src/decomp/scripts/semantic_filter_sasc_set_a_pen_1_b_pen_6_drmd_1_draw_text_or_cursor.awk "${OUT_DIR}/${BASE}.sasc.norm.s" >"${OUT_DIR}/${BASE}.sasc.semantic.txt"
