...
SASC_SRC="script2_clear_ctrl_line_if_enabled.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/b/a/script2.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="script2_clear_ctrl_line_if_enabled"
ENTRY="SCRIPT_ClearCtrlLineIfEnabled"
ENTRY_SASC_REGEX="^SCRIPT_ClearCtrlLineIfEnable[A-Za-z0-9_]*:$"

mkdir -p "$OUT_DIR"

./sc-build-with-dis.sh "$SASC_SRC" >"${OUT_DIR}/sc_build_${BASE}.log" 2>&1

awk -v start="^${ENTRY}:$" -v next_label="^SCRIPT_AssertCtrlLineNow:$" '
    $0 ~ start { in_func=1 }
    in_func {
...
awk -f src/decomp/scripts/semantic_filter_sasc_script2_clear_ctrl_line_if_enabled.awk "${OUT_DIR}/${BASE}.original.norm.s" >"${OUT_DIR}/${BASE}.original.semantic.txt"
awk -f src/decomp/scripts/semantic_filter_sasc_script2_clear_ctrl_line_if_enabled.awk "${OUT_DIR}/${BASE}.sasc.norm.s" >"${OUT_DIR}/${BASE}.sasc.semantic.txt"
