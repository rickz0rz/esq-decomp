...
SASC_SRC="group_as_jmptbl_stubs.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/a/s/xjump.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="group_as_jmptbl_str_find_char_ptr"
ENTRY_ORIG="GROUP_AS_JMPTBL_STR_FindCharPtr"
ENTRY_SASC="GROUP_AS_JMPTBL_STR_FindCharPtr"
TARGET="STR_FindCharPtr"

mkdir -p "$OUT_DIR"
./sc-build-with-dis.sh "$SASC_SRC" >"${OUT_DIR}/sc_build_${BASE}.log" 2>&1

awk -v e="^${ENTRY_ORIG}:$" '$0 ~ e {inf=1} inf { if ($0 ~ /^GROUP_AS_JMPTBL_ESQ_FindSubstringCaseFold:$/) exit; print }' "$ORIG_ASM" >"${OUT_DIR}/${BASE}.original.s"
awk -v e="^${ENTRY_SASC}:$" '$0 ~ e {inf=1} inf { if ($0 ~ /^GROUP_AS_JMPTBL_ESQ_FindSubstrin:$/) exit; print }' "$SASC_DIS" >"${OUT_DIR}/${BASE}.sasc.dis.s"
