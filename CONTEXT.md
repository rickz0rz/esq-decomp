...
- `src/data/script.s` now aliases `SCRIPT_CommandTextPtr` as `SCRIPT_CommandTextPtr`; `script3.s` callsites in the text-display command/context paths now use the alias, and `textdisp.s` cmd-`C` notes include parser-bound budget context for `"xx%s"` into the 200-byte local buffer.
