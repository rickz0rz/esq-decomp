...
SRC_C="src/decomp/c/replacements/esqiff_jmptbl_memory_deallocate_memory_gcc.c"
ORIG_ASM="src/modules/groups/a/n/esqiff.s"
OUT_DIR="build/decomp/c_trial_gcc"
GEN_ASM="${OUT_DIR}/esqiff_jmptbl_memory_deallocate_memory.generated.gcc.s"
ORIG_SLICE="${OUT_DIR}/esqiff_jmptbl_memory_deallocate_memory.original_slice.s"
GEN_SLICE="${OUT_DIR}/esqiff_jmptbl_memory_deallocate_memory.generated_slice.s"
ORIG_NORM="${OUT_DIR}/esqiff_jmptbl_memory_deallocate_memory.original_slice.norm.s"
GEN_NORM="${OUT_DIR}/esqiff_jmptbl_memory_deallocate_memory.generated_slice.norm.s"
...
awk '
$0 ~ /^ESQIFF_JMPTBL_MEMORY_DeallocateMemory:?$/ {in_func=1}
in_func {
    if ($0 ~ /^ESQIFF_JMPTBL_DISKIO_ForceUiRefreshIfIdle:?$/) {
        exit
    }
    print
}
...
awk '
$0 ~ /^_?ESQIFF_JMPTBL_MEMORY_DeallocateMemory:?$/ {in_func=1}
in_func { print }
' "$GEN_ASM" > "$GEN_SLICE"
...
awk -f src/decomp/scripts/semantic_filter_esqiff_jmptbl_memory_deallocate_memory.awk "$ORIG_NORM" > "$ORIG_SEM"
awk -f src/decomp/scripts/semantic_filter_esqiff_jmptbl_memory_deallocate_memory.awk "$GEN_NORM" > "$GEN_SEM"
