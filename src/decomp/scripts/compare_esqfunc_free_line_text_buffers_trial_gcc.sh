...
SRC_C="src/decomp/c/replacements/esqfunc_free_line_text_buffers_gcc.c"
ORIG_ASM="src/modules/groups/a/n/esqfunc.s"
OUT_DIR="build/decomp/c_trial_gcc"
GEN_ASM="${OUT_DIR}/esqfunc_free_line_text_buffers.generated.gcc.s"
ORIG_SLICE="${OUT_DIR}/esqfunc_free_line_text_buffers.original_slice.s"
GEN_SLICE="${OUT_DIR}/esqfunc_free_line_text_buffers.generated_slice.s"
ORIG_NORM="${OUT_DIR}/esqfunc_free_line_text_buffers.original_slice.norm.s"
GEN_NORM="${OUT_DIR}/esqfunc_free_line_text_buffers.generated_slice.norm.s"
...
awk '$0 ~ /^ESQFUNC_FreeLineTextBuffers:?$/ {in_func=1} in_func { if ($0 ~ /^;!======/) exit; print }' "$ORIG_ASM" > "$ORIG_SLICE"
awk '$0 ~ /^_?ESQFUNC_FreeLineTextBuffers:?$/ {in_func=1} in_func { print }' "$GEN_ASM" > "$GEN_SLICE"
...
awk -f src/decomp/scripts/semantic_filter_esqfunc_free_line_text_buffers.awk "$ORIG_NORM" > "$ORIG_SEM"
awk -f src/decomp/scripts/semantic_filter_esqfunc_free_line_text_buffers.awk "$GEN_NORM" > "$GEN_SEM"
