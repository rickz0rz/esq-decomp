...
SRC_C="src/decomp/c/replacements/esqproto_copy_label_to_global_gcc.c"
ORIG_ASM="src/modules/submodules/unknown.s"
OUT_DIR="build/decomp/c_trial_gcc"
GEN_ASM="${OUT_DIR}/esqproto_copy_label_to_global.generated.gcc.s"
ORIG_SLICE="${OUT_DIR}/esqproto_copy_label_to_global.original_slice.s"
GEN_SLICE="${OUT_DIR}/esqproto_copy_label_to_global.generated_slice.s"
ORIG_NORM="${OUT_DIR}/esqproto_copy_label_to_global.original_slice.norm.s"
GEN_NORM="${OUT_DIR}/esqproto_copy_label_to_global.generated_slice.norm.s"
...
awk '
$0 ~ /^ESQPROTO_CopyLabelToGlobal:?$/ {in_func=1}
in_func {
    if ($0 ~ /^;!======/) exit
    print
}
...
awk '
$0 ~ /^_?ESQPROTO_CopyLabelToGlobal:?$/ {in_func=1}
in_func { print }
' "$GEN_ASM" > "$GEN_SLICE"
...
awk -f src/decomp/scripts/semantic_filter_esqproto_copy_label_to_global.awk "$ORIG_NORM" > "$ORIG_SEM"
awk -f src/decomp/scripts/semantic_filter_esqproto_copy_label_to_global.awk "$GEN_NORM" > "$GEN_SEM"
