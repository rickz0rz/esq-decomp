...
extern WORD ESQ_GetHalfHourSlotIndex(LONG *slot);
...
LONG NEWGRID_HandleAltGridState(char *ctx, LONG keyIndex, WORD rowIndex)
{
    ...
    if (NEWGRID_AltGridStateLatch == 4) {
        ...
        if (aux != 0) {
            if (rowIndex == 1 || (WORD)(ESQ_GetHalfHourSlotIndex(&CLOCK_DaySlotIndex) - 1) == 0) {
                keyIndex = TLIBA_FindFirstWildcardMatchIndex((char *)aux);
        ...
        DISPTEXT_SetLayoutParams((LONG)NEWGRID_ColumnWidthPx * 3 - 12, 20, 1);
