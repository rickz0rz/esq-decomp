...
SASC_SRC="group_ar_jmptbl_stubs.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/a/r/xjump.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="group_ar_jmptbl_parseini_write_error_log_entry"
ENTRY_ORIG="GROUP_AR_JMPTBL_PARSEINI_WriteErrorLogEntry"
ENTRY_SASC="GROUP_AR_JMPTBL_PARSEINI_WriteEr"
TARGET="PARSEINI_WriteErrorLogEntry"

mkdir -p "$OUT_DIR"
./sc-build-with-dis.sh "$SASC_SRC" >"${OUT_DIR}/sc_build_${BASE}.log" 2>&1

awk -v e="^${ENTRY_ORIG}:$" '$0 ~ e {inf=1} inf { if ($0 ~ /^GROUP_AR_JMPTBL_STRING_AppendAtNull:$/) exit; print }' "$ORIG_ASM" >"${OUT_DIR}/${BASE}.original.s"
awk -v e="^${ENTRY_SASC}:$" '$0 ~ e {inf=1} inf { if ($0 ~ /^GROUP_AR_JMPTBL_STRING_AppendAtN:$/) exit; print }' "$SASC_DIS" >"${OUT_DIR}/${BASE}.sasc.dis.s"
