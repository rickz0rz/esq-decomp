...
cd "$ROOT_DIR"
SASC_SRC="coi_select_anim_field_pointer.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/a/e/coi.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="coi_select_anim_field_pointer"
ENTRY="COI_SelectAnimFieldPointer"
ENTRY_SASC_REGEX="^COI_SelectAnimFieldPointer[A-Za-z0-9_]*:$"
mkdir -p "$OUT_DIR"
./sc-build-with-dis.sh "$SASC_SRC" >"${OUT_DIR}/sc_build_${BASE}.log" 2>&1
awk -v e="^${ENTRY}:$" -v target="^COI_GetAnimFieldPointerByMode:$" '
  $0 ~ e {in_func=1}
  in_func {
    print
    if ($0 ~ target) exit
...
awk -f src/decomp/scripts/semantic_filter_sasc_coi_select_anim_field_pointer.awk "${OUT_DIR}/${BASE}.original.norm.s" >"${OUT_DIR}/${BASE}.original.semantic.txt"
awk -f src/decomp/scripts/semantic_filter_sasc_coi_select_anim_field_pointer.awk "${OUT_DIR}/${BASE}.sasc.norm.s" >"${OUT_DIR}/${BASE}.sasc.semantic.txt"
