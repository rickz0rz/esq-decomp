...
SRC_C="src/decomp/c/replacements/stream_buffered_write_string_gcc.c"
ORIG_ASM="src/modules/submodules/unknown2b.s"
OUT_DIR="build/decomp/c_trial_gcc"
GEN_ASM="${OUT_DIR}/stream_buffered_write_string.generated.gcc.s"
ORIG_SLICE="${OUT_DIR}/stream_buffered_write_string.original_slice.s"
GEN_SLICE="${OUT_DIR}/stream_buffered_write_string.generated_slice.s"
ORIG_NORM="${OUT_DIR}/stream_buffered_write_string.original_slice.norm.s"
GEN_NORM="${OUT_DIR}/stream_buffered_write_string.generated_slice.norm.s"
...
awk '$0 ~ /^STREAM_BufferedWriteString:?$/ {in_func=1} in_func { if ($0 ~ /^;!======/) exit; print }' "$ORIG_ASM" > "$ORIG_SLICE"
awk '$0 ~ /^_?STREAM_BufferedWriteString:?$/ {in_func=1} in_func { print }' "$GEN_ASM" > "$GEN_SLICE"
...
awk -f src/decomp/scripts/semantic_filter_stream_buffered_write_string.awk "$ORIG_NORM" > "$ORIG_SEM"
awk -f src/decomp/scripts/semantic_filter_stream_buffered_write_string.awk "$GEN_NORM" > "$GEN_SEM"
