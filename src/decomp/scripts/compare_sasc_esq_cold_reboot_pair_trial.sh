...
cd "$ROOT_DIR"
SASC_SRC="esq_cold_reboot_pair.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/a/a/app2.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="esq_cold_reboot_pair"
ENTRY="ESQ_ColdReboot"
ENTRY_VIA="ESQ_ColdRebootViaSupervisor"
ENTRY_SASC_REGEX="^ESQ_ColdReboot[A-Za-z0-9_]*:$"
mkdir -p "$OUT_DIR"
...
awk -v e="^${ENTRY}:$" -v e2="^${ENTRY_VIA}:$" -v e3="$ENTRY_SASC_REGEX" '
  $0 ~ e || $0 ~ e2 || $0 ~ e3 {inf=1}
  inf {
    if (($0 ~ /^ESQ_[A-Za-z0-9_]+:$/ || $0 ~ /^_?ESQ_[A-Za-z0-9_]+:$/) && $0 !~ e && $0 !~ e2 && $0 !~ e3) exit
...
awk -f src/decomp/scripts/semantic_filter_sasc_cold_reboot_pair.awk "${OUT_DIR}/${BASE}.original.norm.s" >"${OUT_DIR}/${BASE}.original.semantic.txt"
awk -f src/decomp/scripts/semantic_filter_sasc_cold_reboot_pair.awk "${OUT_DIR}/${BASE}.sasc.norm.s" >"${OUT_DIR}/${BASE}.sasc.semantic.txt"
