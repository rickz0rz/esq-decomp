...
SASC_SRC="ladfunc2_emit_escaped_string_with_limit.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/a/x/ladfunc2.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="ladfunc2_emit_escaped_string_with_limit"
ENTRY="LADFUNC2_EmitEscapedStringWithLimit"
...
awk -v e="^${ENTRY}:$" -v e2="$ENTRY_SASC_REGEX" '$0 ~ e || $0 ~ e2 {in_func=1} in_func { if (($0 ~ /^LADFUNC2_[A-Za-z0-9_]+:$/ || $0 ~ /^_?LADFUNC2_[A-Za-z0-9_]+:$/) && $0 !~ e && $0 !~ e2) exit; if ($0 ~ /^XREF / || $0 ~ /^XDEF / || $0 ~ /^ END$/ || $0 ~ /^END$/ || $0 ~ /^__const:$/) exit; print }' \
...
awk -f src/decomp/scripts/semantic_filter_sasc_ladfunc2_emit_escaped_string_with_limit.awk \
