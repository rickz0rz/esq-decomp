...
SRC_C="src/decomp/c/replacements/disptext_init_buffers_gcc.c"
ORIG_ASM="src/modules/groups/a/i/disptext.s"
OUT_DIR="build/decomp/c_trial_gcc"
GEN_ASM="${OUT_DIR}/disptext_init_buffers.generated.gcc.s"
ORIG_SLICE="${OUT_DIR}/disptext_init_buffers.original_slice.s"
GEN_SLICE="${OUT_DIR}/disptext_init_buffers.generated_slice.s"
ORIG_NORM="${OUT_DIR}/disptext_init_buffers.original_slice.norm.s"
GEN_NORM="${OUT_DIR}/disptext_init_buffers.generated_slice.norm.s"
...
awk '$0 ~ /^DISPTEXT_InitBuffers:?$/ {in_func=1} in_func { if ($0 ~ /^;!======/) exit; print }' "$ORIG_ASM" > "$ORIG_SLICE"
awk '$0 ~ /^_?DISPTEXT_InitBuffers:?$/ {in_func=1} in_func { print }' "$GEN_ASM" > "$GEN_SLICE"
...
awk -f src/decomp/scripts/semantic_filter_disptext_init_buffers.awk "$ORIG_NORM" > "$ORIG_SEM"
awk -f src/decomp/scripts/semantic_filter_disptext_init_buffers.awk "$GEN_NORM" > "$GEN_SEM"
