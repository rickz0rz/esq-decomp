...
SASC_SRC="script_setup_highlight_effect.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/b/a/script4.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="script_setup_highlight_effect_state_flow"
ENTRY="SCRIPT_SetupHighlightEffect"
ENTRY_SASC_REGEX="^SCRIPT_SetupHighlightEffe[A-Za-z0-9_]*:$"

mkdir -p "$OUT_DIR"

./sc-build-with-dis.sh "$SASC_SRC" >"${OUT_DIR}/sc_build_${BASE}.log" 2>&1

awk -v e="^${ENTRY}:$" '$0 ~ e {in_func=1} in_func { print }' \
...
awk -f src/decomp/scripts/semantic_filter_sasc_script_setup_highlight_effect_state_flow.awk \
