...
SASC_SRC="script_reset_banner_char_defaults.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/b/a/script4.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="script_reset_banner_char_defaults"
ENTRY="SCRIPT_ResetBannerCharDefaults"
ENTRY_SASC_REGEX="^SCRIPT_ResetBannerCharDefaul[A-Za-z0-9_]*:$"

mkdir -p "$OUT_DIR"

./sc-build-with-dis.sh "$SASC_SRC" >"${OUT_DIR}/sc_build_${BASE}.log" 2>&1

awk -v start="^${ENTRY}:$" -v next_label="^SCRIPT_GetBannerCharOrFallback:$" '
    $0 ~ start { in_func=1 }
    in_func {
...
awk -f src/decomp/scripts/semantic_filter_sasc_script_reset_banner_char_defaults.awk \
