BEGIN {
    ...
    has_blit_guard_selected_node = 0
    has_blit_guard_found_match = 0
    setrast_calls = 0
...

{
    ...
    if (uline ~ /MOVE\.L -24\(A5\),-4\(A5\)/ || uline ~ /MOVE\.L A3,A5/) has_script_selection_hit = 1
    ...
    if (uline ~ /TST\.L BRUSH_SELECTEDNODE/ || uline ~ /BRUSH_SELECTEDNODE != \(ESQFUNC_BRUSHNODE \*\)0/) has_blit_guard_selected_node = 1
    if ((uline ~ /TST\.L D[56]/ && has_default_selected_brush) || uline ~ /FOUNDBRUSH != ESQFUNC_FALSE/) has_blit_guard_found_match = 1
...
END {
    ...
    print "HAS_BLIT_GUARD_SELECTED_NODE=" has_blit_guard_selected_node
    print "HAS_BLIT_GUARD_FOUND_MATCH=" has_blit_guard_found_match
