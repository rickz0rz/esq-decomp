...

{
    ...
    if (u ~ /^MOVE\.B D0,\(A0\)\+$/ || u ~ /^MOVE\.B D[0-7],\(A0\)\+$/ || u ~ /^MOVE\.B D[0-7],\(A5\)\+$/) h_store_char = 1
    if (u ~ /^MOVE\.L A0,GLOBAL_PRINTFBUFFERPTR\(A4\)$/ || u ~ /^MOVE\.L A5,GLOBAL_PRINTFBUFFERPTR\(A4\)$/) h_store_ptr = 1
