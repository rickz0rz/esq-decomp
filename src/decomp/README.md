...
- Follow-up verification note (March 13, 2026): do not treat the whole SAS/C lane as universally green. Current non-empty semantic diffs still exist outside the maintained sweep in `disptext_is_current_line_last`, `disptext_is_last_line_selected`, and `newgrid_update_grid_state`, so broader mismatch reduction still remains after the maintained baseline.
