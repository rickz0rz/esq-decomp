...

{
    ...
    if (u ~ /^SUBQ\.L #\$?1,[$]?[0-9A-F]+\((A[0-7]|A0)\)$/ || u ~ /^SUBQ\.L #\$?1,\([$]?[0-9A-F]+,A[0-7]\)$/ || u ~ /^SUBQ\.L #\$?1,D[0-7]$/) has_space_decrement = 1
    if (u ~ /^MOVE\.L D[0-7],\([0-9]+,A[0-7]\)$/ || u ~ /^MOVE\.L D[0-7],[0-9]+\((A[0-7]|A0)\)$/) has_space_decrement = 1
    if (u ~ /^(BLT|BLT\.S|BLT\.B|JLT|JLT\.S) / || u ~ /^(BMI|BMI\.S|BMI\.B) /) has_overflow_branch = 1
    if (u ~ /^LEA [\$]?[0-9]+\(A[0-7]\),A[0-7]$/ || u ~ /^LEA \([0-9]+,A[0-7]\),A[0-7]$/ || u ~ /^ADDQ\.L #\$?1,A[0-7]$/ || u ~ /^MOVE\.L A[0-7],[0-9]+\((A[0-7]|A0)\)$/ || u ~ /^MOVE\.L A[0-7],\([0-9]+,A[0-7]\)$/) has_write_ptr_advance = 1
