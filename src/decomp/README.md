...
- Current hybrid-integration note (March 13, 2026): `modules/submodules/unknown4.s` and `modules/submodules/unknown14.s` are now wired into `src/decomp/replacements.map` as passthrough hybrid boundaries. Their restored SAS/C coverage already spans `STRING_ToUpperInPlace` and `HANDLE_OpenFromModeString`, so future object-level replacement work can land in those helper modules without another root include-graph edit.
