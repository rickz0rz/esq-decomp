...

{
    ...
    if (n ~ /SCRIPT3JMPTBLGCOMMANDADJUSTBANNERCOPPEROFFSET/) has_adjust = 1
    if (n ~ /SCRIPTBANNERTRANSITIONACTIVE/) has_active = 1
    if (n ~ /SCRIPTBANNERTRANSITIONSTEPCURSOR/) has_cursor = 1
    ...
    if (u ~ /CMP|BGT|BLT|BEQ|BNE|TST/) has_compare = 1
