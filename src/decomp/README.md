...
- Current hybrid-integration note (March 13, 2026, later re-validated): `modules/submodules/unknown10.s` is now wired into `src/decomp/replacements.map` as a passthrough hybrid boundary. Its restored SAS/C coverage already spans a useful non-`JMPTBL` foothold for the hex formatter, signed-long parser helpers, open-with-flags helper, and `WDISP_SPrintf` callback path (`FORMAT_U32ToHexString`, `PARSE_ReadSignedLong`, `PARSE_ReadSignedLong_NoBranch`, `HANDLE_OpenEntryWithFlags`, `UNKNOWN10_PrintfPutcToBuffer`, and `WDISP_SPrintf`), so future object-level replacement work can land there without another root include-graph edit.
...
- Current hybrid-integration note (March 13, 2026): `modules/submodules/unknown21.s` is now wired into `src/decomp/replacements.map` as a passthrough hybrid boundary. Its restored SAS/C coverage already spans the current non-`JMPTBL` DOS/file-create and IOStdReq cleanup helpers (`IOSTDREQ_Free`, `IOSTDREQ_CleanupSignalAndMsgport`, `DOS_OpenNewFileIfMissing`, and `DOS_DeleteAndRecreateFile`), so future object-level replacement work can land there without another root include-graph edit.
...
- Current hybrid-integration note (March 13, 2026): `modules/submodules/unknown34.s` is now wired into `src/decomp/replacements.map` as a passthrough hybrid boundary. Its restored SAS/C coverage already spans the current non-`JMPTBL` list/memory/read helper cluster (`LIST_InitHeader`, `MEM_Move`, and `DOS_ReadByIndex`), so future object-level replacement work can land there without another root include-graph edit.
