...
cd "$ROOT_DIR"
SASC_SRC="brush_alloc_brush_node.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/a/a/brush.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="brush_alloc_brush_node"
ENTRY="BRUSH_AllocBrushNode"
ENTRY_SASC_REGEX="^BRUSH_AllocBrushNode[A-Za-z0-9_]*:$"
mkdir -p "$OUT_DIR"
...
awk -v ENTRY="$ENTRY" -v ENTRY_REGEX="$ENTRY_SASC_REGEX" -f src/decomp/scripts/semantic_filter_sasc_brush_alloc_brush_node.awk "${OUT_DIR}/${BASE}.original.norm.s" >"${OUT_DIR}/${BASE}.original.semantic.txt"
awk -v ENTRY="$ENTRY" -v ENTRY_REGEX="$ENTRY_SASC_REGEX" -f src/decomp/scripts/semantic_filter_sasc_brush_alloc_brush_node.awk "${OUT_DIR}/${BASE}.sasc.norm.s" >"${OUT_DIR}/${BASE}.sasc.semantic.txt"
