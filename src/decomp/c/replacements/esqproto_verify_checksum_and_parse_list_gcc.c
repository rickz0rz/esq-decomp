...
/*
 * Target 594 GCC trial function.
 * Read list payload, verify XOR checksum, and dispatch list parser.
 */
...
s32 UNKNOWN_ParseListAndUpdateEntries(const u8 *buf) __attribute__((noinline));

s32 ESQPROTO_VerifyChecksumAndParseList(u8 seed) __attribute__((noinline, used));

s32 ESQPROTO_VerifyChecksumAndParseList(u8 seed)
{
    s32 checksum;
