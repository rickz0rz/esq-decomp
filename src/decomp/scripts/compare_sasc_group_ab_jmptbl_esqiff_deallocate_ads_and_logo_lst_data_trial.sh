...
BASE="group_ab_jmptbl_esqiff_deallocate_ads_and_logo_lst_data"
ENTRY_ORIG="GROUP_AB_JMPTBL_ESQIFF_DeallocateAdsAndLogoLstData"
ENTRY_SASC="GROUP_AB_JMPTBL_ESQIFF_Deallocat"
TARGET="ESQIFF_DeallocateAdsAndLogoLstDa"
mkdir -p "$OUT_DIR"
./sc-build-with-dis.sh "$SASC_SRC" >"${OUT_DIR}/sc_build_${BASE}.log" 2>&1
awk -v e="^${ENTRY_ORIG}:$" '$0 ~ e {inf=1} inf { if ($0 ~ /^GROUP_AB_JMPTBL_LADFUNC_FreeBannerRectEntries:$/) exit; print }' "$ORIG_ASM" >"${OUT_DIR}/${BASE}.original.s"
awk -v e="^${ENTRY_SASC}:$" '$0 ~ e {inf=1} inf { if ($0 ~ /^GROUP_AB_JMPTBL_LADFUNC_FreeBann:$/) exit; print }' "$SASC_DIS" >"${OUT_DIR}/${BASE}.sasc.dis.s"
