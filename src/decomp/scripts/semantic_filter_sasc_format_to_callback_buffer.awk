...

{
    ...
    if (u ~ /GLOBAL_FORMATCALLBACKBYTECOUNT/ && (u ~ /^CLR\.L / || u ~ /^MOVEQ\.L? #\$?0,/ || u ~ /^MOVE\.L #\$?0,/)) has_counter_clear = 1
    if (u ~ /GLOBAL_FORMATCALLBACKBUFFERPTR/ && (u ~ /^MOVE\.L A[0-7],/ || u ~ /^MOVE\.L D[0-7],/ || u ~ /^MOVEA?\.L /)) has_buffer_ptr_set = 1

    if (u ~ /FORMAT_CALLBACKWRITECHAR/ || u ~ /^PEA .*\(PC\)$/) has_writer_ref = 1
    if (u ~ /WDISP_FORMATWITHCALLBACK/) has_format_call = 1
    if (u ~ /STREAM_BUFFEREDPUTCORFLUSH/) has_flush_call = 1
    if (u ~ /PEA -1\.W/ || u ~ /^PEA \(\$?FFFFFFFF\)\.W$/ || u ~ /^MOVEQ\.L? #\-1,D[0-7]$/ || u ~ /^MOVE\.L #\-1,/) has_minus_one = 1
