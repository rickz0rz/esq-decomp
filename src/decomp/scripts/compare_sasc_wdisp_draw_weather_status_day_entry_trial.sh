...
SASC_SRC="wdisp_draw_weather_status_day_entry.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/b/a/wdisp.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="wdisp_draw_weather_status_day_entry"
ENTRY_ORIG="WDISP_DrawWeatherStatusDayEntry"
ENTRY_SASC_REGEX="^WDISP_DrawWeatherStatusDayEntr[A-Za-z0-9_]*:$"

mkdir -p "$OUT_DIR"

./sc-build-with-dis.sh "$SASC_SRC" >"${OUT_DIR}/sc_build_${BASE}.log" 2>&1

awk -v start="^${ENTRY_ORIG}:$" -v next_label="^WDISP_DrawWeatherStatusSummary:$" '$0 ~ start {in_func=1} in_func {if ($0 ~ next_label) exit; print}' "$ORIG_ASM" >"${OUT_DIR}/${BASE}.original.s"
...
awk -f src/decomp/scripts/semantic_filter_sasc_wdisp_draw_weather_status_day_entry.awk "${OUT_DIR}/${BASE}.original.norm.s" >"${OUT_DIR}/${BASE}.original.semantic.txt"
awk -f src/decomp/scripts/semantic_filter_sasc_wdisp_draw_weather_status_day_entry.awk "${OUT_DIR}/${BASE}.sasc.norm.s" >"${OUT_DIR}/${BASE}.sasc.semantic.txt"
