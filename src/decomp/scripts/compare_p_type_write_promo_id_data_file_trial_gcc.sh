...
SRC_C="src/decomp/c/replacements/p_type_write_promo_id_data_file_gcc.c"
ORIG_ASM="src/modules/groups/b/a/p_type.s"
OUT_DIR="build/decomp/c_trial_gcc"
GEN_ASM="${OUT_DIR}/p_type_write_promo_id_data_file.generated.gcc.s"
ORIG_SLICE="${OUT_DIR}/p_type_write_promo_id_data_file.original_slice.s"
GEN_SLICE="${OUT_DIR}/p_type_write_promo_id_data_file.generated_slice.s"
ORIG_NORM="${OUT_DIR}/p_type_write_promo_id_data_file.original_slice.norm.s"
GEN_NORM="${OUT_DIR}/p_type_write_promo_id_data_file.generated_slice.norm.s"
...
awk '
$0 ~ /^P_TYPE_WritePromoIdDataFile:?$/ {in_func=1}
in_func {
    if ($0 ~ /^;!======/) {
        exit
    }
...
awk '
$0 ~ /^_?P_TYPE_WritePromoIdDataFile:?$/ {in_func=1}
in_func { print }
' "$GEN_ASM" > "$GEN_SLICE"
...
awk -f src/decomp/scripts/semantic_filter_p_type_write_promo_id_data_file.awk "$ORIG_NORM" > "$ORIG_SEM"
awk -f src/decomp/scripts/semantic_filter_p_type_write_promo_id_data_file.awk "$GEN_NORM" > "$GEN_SEM"
