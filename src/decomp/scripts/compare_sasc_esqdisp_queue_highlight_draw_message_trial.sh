...
SASC_SRC="esqdisp_queue_highlight_draw_message.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/a/n/esqdisp.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="esqdisp_queue_highlight_draw_message"
ENTRY="ESQDISP_QueueHighlightDrawMessage"
ENTRY_SASC_REGEX="^ESQDISP_QueueHighlightDrawM[A-Za-z0-9_]*:$"
ENTRY_SEM="ESQDISP_QUEUEHIGHLIGHTDRAWMESSAGE:"
ENTRY_ALT_SEM="ESQDISP_QUEUEHIGHLIGHTDRAWM"
...
awk -v ENTRY_PREFIX="$ENTRY_SEM" -v ENTRY_ALT_PREFIX="$ENTRY_ALT_SEM" -f src/decomp/scripts/semantic_filter_sasc_esqdisp_queue_highlight_draw_message.awk "${OUT_DIR}/${BASE}.original.norm.s" >"${OUT_DIR}/${BASE}.original.semantic.txt"
awk -v ENTRY_PREFIX="$ENTRY_SEM" -v ENTRY_ALT_PREFIX="$ENTRY_ALT_SEM" -f src/decomp/scripts/semantic_filter_sasc_esqdisp_queue_highlight_draw_message.awk "${OUT_DIR}/${BASE}.sasc.norm.s" >"${OUT_DIR}/${BASE}.sasc.semantic.txt"
