...
- After you have a short list of complete candidates, use `src/decomp/scripts/report_passthrough_integration_candidates.py` to narrow that set to module boundaries whose replacement file still only `include`s the canonical asm module verbatim. Treat the `ready` rows from that report as the current object-integration queue.
...
- `src/decomp/scripts/report_passthrough_integration_candidates.py`: filters the mapped-module candidate list down to complete module boundaries whose replacement file is still a pure passthrough `include` of the canonical asm source, making the current object-integration queue explicit.
