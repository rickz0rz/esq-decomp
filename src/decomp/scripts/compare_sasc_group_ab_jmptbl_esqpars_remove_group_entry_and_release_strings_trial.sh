...
BASE="group_ab_jmptbl_esqpars_remove_group_entry_and_release_strings"
ENTRY_ORIG="GROUP_AB_JMPTBL_ESQPARS_RemoveGroupEntryAndReleaseStrings"
ENTRY_SASC="GROUP_AB_JMPTBL_ESQPARS_RemoveGr"
TARGET="ESQPARS_RemoveGroupEntryAndRelea"
mkdir -p "$OUT_DIR"
./sc-build-with-dis.sh "$SASC_SRC" >"${OUT_DIR}/sc_build_${BASE}.log" 2>&1
awk -v e="^${ENTRY_ORIG}:$" '$0 ~ e {inf=1} inf { if ($0 ~ /^GROUP_AB_JMPTBL_ESQFUNC_FreeLineTextBuffers:$/) exit; print }' "$ORIG_ASM" >"${OUT_DIR}/${BASE}.original.s"
awk -v e="^${ENTRY_SASC}:$" '$0 ~ e {inf=1} inf { if ($0 ~ /^GROUP_AB_JMPTBL_ESQFUNC_FreeLine:$/) exit; print }' "$SASC_DIS" >"${OUT_DIR}/${BASE}.sasc.dis.s"
