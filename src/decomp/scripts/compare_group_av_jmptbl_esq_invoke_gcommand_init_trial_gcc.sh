...
SRC_C="src/decomp/c/replacements/group_av_jmptbl_esq_invoke_gcommand_init_gcc.c"
ORIG_ASM="src/modules/groups/a/v/xjump.s"
OUT_DIR="build/decomp/c_trial_gcc"
GEN_ASM="${OUT_DIR}/group_av_jmptbl_esq_invoke_gcommand_init.generated.gcc.s"
ORIG_SLICE="${OUT_DIR}/group_av_jmptbl_esq_invoke_gcommand_init.original_slice.s"
GEN_SLICE="${OUT_DIR}/group_av_jmptbl_esq_invoke_gcommand_init.generated_slice.s"
ORIG_NORM="${OUT_DIR}/group_av_jmptbl_esq_invoke_gcommand_init.original_slice.norm.s"
GEN_NORM="${OUT_DIR}/group_av_jmptbl_esq_invoke_gcommand_init.generated_slice.norm.s"
...
awk -v start_label="GROUP_AV_JMPTBL_ESQ_InvokeGcommandInit" -v next_label="GROUP_AV_JMPTBL_EXEC_CallVector_48" '
...
awk '$0 ~ /^_?GROUP_AV_JMPTBL_ESQ_InvokeGcommandInit:?$/ {in_func=1} in_func { print }' "$GEN_ASM" > "$GEN_SLICE"
...
awk -f "src/decomp/scripts/semantic_filter_group_av_jmptbl_esq_invoke_gcommand_init.awk" "$ORIG_NORM" > "$ORIG_SEM"
awk -f "src/decomp/scripts/semantic_filter_group_av_jmptbl_esq_invoke_gcommand_init.awk" "$GEN_NORM" > "$GEN_SEM"
