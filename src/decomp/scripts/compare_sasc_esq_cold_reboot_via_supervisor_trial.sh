...
BASE="esq_cold_reboot_via_supervisor"
ENTRY="ESQ_ColdRebootViaSupervisor"
ENTRY_SASC_REGEX="^ESQ_ColdRebootViaSuper[A-Za-z0-9_]*:$"
mkdir -p "$OUT_DIR"
...
awk -f src/decomp/scripts/semantic_filter_sasc_cold_reboot_via_supervisor.awk "${OUT_DIR}/${BASE}.original.norm.s" >"${OUT_DIR}/${BASE}.original.semantic.txt"
awk -f src/decomp/scripts/semantic_filter_sasc_cold_reboot_via_supervisor.awk "${OUT_DIR}/${BASE}.sasc.norm.s" >"${OUT_DIR}/${BASE}.sasc.semantic.txt"
