BEGIN {
    ...
    align_mode_write_count = 0
    plane_mask_count = 0
    has_accumulator_capture_ref = 0
    has_accumulator_flush_ref = 0
    has_copymem = 0
    saw_accumulator_row_count = 0
    saw_accumulator_row_size = 0
    has_select_brush = 0
...

{
    ...
    if (n ~ /FINDBRUSHBYPREDICATE/) brush_find_count++
    ...
    if (n ~ /LVOSETFONT/) set_font_count++
    if (n ~ /LVOSETFONT/ || n ~ /HANDLEPREVUECFONT/) has_set_font = 1
    if (n ~ /LVOSETAPEN/) set_apen_count++
    if (n ~ /LVOSETDRMD/) set_drmd_count++
    if (u ~ /20\(A0\)/ || u ~ /\$14,A0/) has_font_height_ref = 1
    if (u ~ /26\(A0\)/ || u ~ /\$1A,A0/) has_font_baseline_ref = 1
    if (u ~ /356\(A0\)/ || u ~ /360\(A0\)/ || u ~ /\$164\(A3\)/ || u ~ /\$168\(A3\)/) align_mode_write_count++
    ...
    if (u ~ /#4([^0-9]|$)/ || u ~ /#\$4([^A-Z0-9]|$)/) saw_accumulator_row_count = 1
    if (u ~ /#8([^0-9]|$)/ || u ~ /#\$8([^A-Z0-9]|$)/) saw_accumulator_row_size = 1
    if (n ~ /SELECTBRUSHSLOT/) {
        has_select_brush = 1
        brush_select_count++
    }
...
END {
    ...
    print "HAS_BRUSH_FIND_PAIR=" (brush_find_count == 2)
    ...
    print "HAS_SET_RAST_CALL=" (set_rast_count == 1)
    print "HAS_SET_FONT=" has_set_font
    print "HAS_SET_FONT_CALL=" (set_font_count == 1)
    print "HAS_FONT_HEIGHT_REF=" has_font_height_ref
    print "HAS_FONT_BASELINE_REF=" has_font_baseline_ref
    print "HAS_BRUSH_ALIGN_MODE_WRITES=" (align_mode_write_count >= 2)
    ...
    print "HAS_ACCUMULATOR_COPY_GEOMETRY=" (saw_accumulator_row_count && saw_accumulator_row_size)
    ...
    print "HAS_LAYOUT_MUL_PAIR=" (math_mul_count == 2)
    ...
    print "HAS_SETAPEN_COUNT=" (set_apen_count == 4)
    print "HAS_SETDRMD_COUNT=" (set_drmd_count == 4)
    print "HAS_MOVES=" (move_count >= 4)
    print "HAS_TEXT_DRAWS=" (text_count >= 4)
    print "HAS_CLEANUP_DEALLOC=" has_cleanup_dealloc
    print "HAS_DEALLOC_CALL=" (dealloc_count == 1)
