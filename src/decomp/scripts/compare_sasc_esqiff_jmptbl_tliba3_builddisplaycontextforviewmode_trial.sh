...
cd "$ROOT_DIR"
SASC_SRC="esqiff_jmptbl_stubs_a.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/a/n/esqiff.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="esqiff_jmptbl_tliba3_builddisplaycontextforviewmode"
ENTRY_ORIG="ESQIFF_JMPTBL_TLIBA3_BuildDisplayContextForViewMode"
ENTRY_SASC="ESQIFF_JMPTBL_TLIBA3_BuildDispla"
TARGET="TLIBA3_BuildDisplayContextForViewMode"
mkdir -p "$OUT_DIR"
./sc-build-with-dis.sh "$SASC_SRC" >"${OUT_DIR}/sc_build_${BASE}.log" 2>&1
awk -v e="^${ENTRY_ORIG}:$" '$0 ~ e {inf=1} inf { if ($0 ~ /^ESQIFF_JMPTBL_/ && $0 !~ e) exit; if ($0 ~ /^;!======/) exit; print }' "$ORIG_ASM" >"${OUT_DIR}/${BASE}.original.s"
