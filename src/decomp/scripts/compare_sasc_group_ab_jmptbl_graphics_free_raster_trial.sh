...
BASE="group_ab_jmptbl_graphics_free_raster"
ENTRY_ORIG="GROUP_AB_JMPTBL_GRAPHICS_FreeRaster"
ENTRY_SASC="GROUP_AB_JMPTBL_GRAPHICS_FreeRas"
TARGET="GRAPHICS_FreeRaster"
mkdir -p "$OUT_DIR"
./sc-build-with-dis.sh "$SASC_SRC" >"${OUT_DIR}/sc_build_${BASE}.log" 2>&1
awk -v e="^${ENTRY_ORIG}:$" '$0 ~ e {inf=1} inf { if ($0 ~ /^GROUP_AB_JMPTBL_IOSTDREQ_Free:$/) exit; print }' "$ORIG_ASM" >"${OUT_DIR}/${BASE}.original.s"
awk -v e="^${ENTRY_SASC}:$" '$0 ~ e {inf=1} inf { if ($0 ~ /^GROUP_AB_JMPTBL_IOSTDREQ_Free:$/) exit; print }' "$SASC_DIS" >"${OUT_DIR}/${BASE}.sasc.dis.s"
