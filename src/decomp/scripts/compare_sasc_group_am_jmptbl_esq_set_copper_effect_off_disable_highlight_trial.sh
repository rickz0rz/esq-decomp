...
cd "$ROOT_DIR"
SASC_SRC="group_am_jmptbl_esq_set_copper_effect_off_disable_highlight.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/a/m/xjump.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="group_am_jmptbl_esq_set_copper_effect_off_disable_highlight"
ENTRY_ORIG="GROUP_AM_JMPTBL_ESQ_SetCopperEffect_OffDisableHighlight"
ENTRY_SASC="GROUP_AM_JMPTBL_ESQ_SetCopperEff"
TARGET="ESQ_SetCopperEffect_OffDisableHighlight"
mkdir -p "$OUT_DIR"
./sc-build-with-dis.sh "$SASC_SRC" >"${OUT_DIR}/sc_build_${BASE}.log" 2>&1
awk -v e="^${ENTRY_ORIG}:$" '
  $0 ~ e {inf=1}
  inf {
    if ($0 ~ /^GROUP_AM_JMPTBL_/ && $0 !~ e) exit
    if ($0 ~ /^;!======/) exit
    print
  }
