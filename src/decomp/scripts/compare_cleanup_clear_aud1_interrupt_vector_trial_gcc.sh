...
SRC_C="src/decomp/c/replacements/cleanup_clear_aud1_interrupt_vector_gcc.c"
ORIG_ASM="src/modules/groups/a/b/cleanup.s"
OUT_DIR="build/decomp/c_trial_gcc"
GEN_ASM="${OUT_DIR}/cleanup_clear_aud1_interrupt_vector.generated.gcc.s"
ORIG_SLICE="${OUT_DIR}/cleanup_clear_aud1_interrupt_vector.original_slice.s"
GEN_SLICE="${OUT_DIR}/cleanup_clear_aud1_interrupt_vector.generated_slice.s"
ORIG_NORM="${OUT_DIR}/cleanup_clear_aud1_interrupt_vector.original_slice.norm.s"
GEN_NORM="${OUT_DIR}/cleanup_clear_aud1_interrupt_vector.generated_slice.norm.s"
...
awk '$0 ~ /^CLEANUP_ClearAud1InterruptVector:?$/ {in_func=1} in_func { if ($0 ~ /^;!======/) exit; print }' "$ORIG_ASM" > "$ORIG_SLICE"
awk '$0 ~ /^_?CLEANUP_ClearAud1InterruptVector:?$/ {in_func=1} in_func { print }' "$GEN_ASM" > "$GEN_SLICE"
...
awk -f src/decomp/scripts/semantic_filter_cleanup_clear_aud1_interrupt_vector.awk "$ORIG_NORM" > "$ORIG_SEM"
awk -f src/decomp/scripts/semantic_filter_cleanup_clear_aud1_interrupt_vector.awk "$GEN_NORM" > "$GEN_SEM"
