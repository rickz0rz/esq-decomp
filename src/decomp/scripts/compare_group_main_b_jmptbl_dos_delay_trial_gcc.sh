...
SRC_C="src/decomp/c/replacements/group_main_b_jmptbl_dos_delay_gcc.c"
ORIG_ASM="src/modules/groups/_main/b/xjump.s"
OUT_DIR="build/decomp/c_trial_gcc"
GEN_ASM="${OUT_DIR}/group_main_b_jmptbl_dos_delay.generated.gcc.s"
ORIG_SLICE="${OUT_DIR}/group_main_b_jmptbl_dos_delay.original_slice.s"
GEN_SLICE="${OUT_DIR}/group_main_b_jmptbl_dos_delay.generated_slice.s"
ORIG_NORM="${OUT_DIR}/group_main_b_jmptbl_dos_delay.original_slice.norm.s"
GEN_NORM="${OUT_DIR}/group_main_b_jmptbl_dos_delay.generated_slice.norm.s"
...
awk '
$0 ~ /^GROUP_MAIN_B_JMPTBL_DOS_Delay:?$/ {in_func=1}
in_func {
    if ($0 ~ /^;!======/) {
        exit
    }
...
awk '
$0 ~ /^_?GROUP_MAIN_B_JMPTBL_DOS_Delay:?$/ {in_func=1}
in_func { print }
' "$GEN_ASM" > "$GEN_SLICE"
...
awk -f src/decomp/scripts/semantic_filter_group_main_b_jmptbl_dos_delay.awk "$ORIG_NORM" > "$ORIG_SEM"
awk -f src/decomp/scripts/semantic_filter_group_main_b_jmptbl_dos_delay.awk "$GEN_NORM" > "$GEN_SEM"
