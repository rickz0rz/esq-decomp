...
- `src/decomp/scripts/report_passthrough_integration_candidates.py`: filters the mapped-module candidate list down to complete module boundaries whose replacement file is still a pure passthrough `include` of the canonical asm source, making the current object-integration queue explicit. Pass `--details` to print the covered exports and compare-script backing for a selected candidate.
