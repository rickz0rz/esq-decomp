...
BASE="disptext_measure_current_line_length"
ENTRY="DISPTEXT_MeasureCurrentLineLength"
ENTRY_SASC_REGEX="^DISPTEXT_MeasureCurrentLineL[A-Za-z0-9_]*:$"
ENTRY_SEM="DISPTEXT_MEASURECURRENTLINELENGTH:"
ENTRY_ALT_SEM="DISPTEXT_MEASURECURRENTLINEL"
...
awk -v ENTRY_PREFIX="$ENTRY_SEM" -v ENTRY_ALT_PREFIX="$ENTRY_ALT_SEM" -f src/decomp/scripts/semantic_filter_sasc_disptext_measure_current_line_length.awk "${OUT_DIR}/${BASE}.original.norm.s" >"${OUT_DIR}/${BASE}.original.semantic.txt"
awk -v ENTRY_PREFIX="$ENTRY_SEM" -v ENTRY_ALT_PREFIX="$ENTRY_ALT_SEM" -f src/decomp/scripts/semantic_filter_sasc_disptext_measure_current_line_length.awk "${OUT_DIR}/${BASE}.sasc.norm.s" >"${OUT_DIR}/${BASE}.sasc.semantic.txt"
