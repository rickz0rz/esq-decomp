...
cd "$ROOT_DIR"
SASC_SRC="esqshared_jmptbl_stubs.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/a/p/esqshared.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="esqshared_jmptbl_esq_adjustbracketedhourinstring"
ENTRY_ORIG="ESQSHARED_JMPTBL_ESQ_AdjustBracketedHourInString"
ENTRY_SASC="ESQSHARED_JMPTBL_ESQ_AdjustBrack"
TARGET="ESQ_AdjustBracketedHourInString"
mkdir -p "$OUT_DIR"
./sc-build-with-dis.sh "$SASC_SRC" >"${OUT_DIR}/sc_build_${BASE}.log" 2>&1
awk -v e="^${ENTRY_ORIG}:$" '$0 ~ e {inf=1} inf { if ($0 ~ /^ESQSHARED_JMPTBL_/ && $0 !~ e) exit; if ($0 ~ /^;!======/) exit; print }' "$ORIG_ASM" >"${OUT_DIR}/${BASE}.original.s"
