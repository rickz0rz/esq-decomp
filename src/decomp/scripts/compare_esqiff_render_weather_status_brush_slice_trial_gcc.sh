...
SRC_C="src/decomp/c/replacements/esqiff_render_weather_status_brush_slice_gcc.c"
ORIG_ASM="src/modules/groups/a/n/esqiff.s"
OUT_DIR="build/decomp/c_trial_gcc"
GEN_ASM="${OUT_DIR}/esqiff_render_weather_status_brush_slice.generated.gcc.s"
ORIG_SLICE="${OUT_DIR}/esqiff_render_weather_status_brush_slice.original_slice.s"
GEN_SLICE="${OUT_DIR}/esqiff_render_weather_status_brush_slice.generated_slice.s"
ORIG_NORM="${OUT_DIR}/esqiff_render_weather_status_brush_slice.original_slice.norm.s"
GEN_NORM="${OUT_DIR}/esqiff_render_weather_status_brush_slice.generated_slice.norm.s"
...
awk '$0 ~ /^ESQIFF_RenderWeatherStatusBrushSlice:?$/ {in_func=1} in_func { if ($0 ~ /^;!======/) exit; print }' "$ORIG_ASM" > "$ORIG_SLICE"
awk '$0 ~ /^_?ESQIFF_RenderWeatherStatusBrushSlice:?$/ {in_func=1} in_func { print }' "$GEN_ASM" > "$GEN_SLICE"
...
awk -f src/decomp/scripts/semantic_filter_esqiff_render_weather_status_brush_slice.awk "$ORIG_NORM" > "$ORIG_SEM"
awk -f src/decomp/scripts/semantic_filter_esqiff_render_weather_status_brush_slice.awk "$GEN_NORM" > "$GEN_SEM"
