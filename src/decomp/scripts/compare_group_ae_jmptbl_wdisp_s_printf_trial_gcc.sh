...
SRC_C="src/decomp/c/replacements/group_ae_jmptbl_wdisp_s_printf_gcc.c"
ORIG_ASM="src/modules/groups/a/e/xjump.s"
OUT_DIR="build/decomp/c_trial_gcc"
GEN_ASM="${OUT_DIR}/group_ae_jmptbl_wdisp_s_printf.generated.gcc.s"
ORIG_SLICE="${OUT_DIR}/group_ae_jmptbl_wdisp_s_printf.original_slice.s"
GEN_SLICE="${OUT_DIR}/group_ae_jmptbl_wdisp_s_printf.generated_slice.s"
ORIG_NORM="${OUT_DIR}/group_ae_jmptbl_wdisp_s_printf.original_slice.norm.s"
GEN_NORM="${OUT_DIR}/group_ae_jmptbl_wdisp_s_printf.generated_slice.norm.s"
...
awk '
$0 ~ /^GROUP_AE_JMPTBL_WDISP_SPrintf:?$/ {in_func=1}
in_func {
    if ($0 ~ /^GROUP_AE_JMPTBL_SCRIPT_AllocateBufferArray:?$/) {
        exit
    }
    print
}
...
awk '
$0 ~ /^_?GROUP_AE_JMPTBL_WDISP_SPrintf:?$/ {in_func=1}
in_func { print }
' "$GEN_ASM" > "$GEN_SLICE"
...
awk -f "src/decomp/scripts/semantic_filter_group_ae_jmptbl_wdisp_s_printf.awk" "$ORIG_NORM" > "$ORIG_SEM"
awk -f "src/decomp/scripts/semantic_filter_group_ae_jmptbl_wdisp_s_printf.awk" "$GEN_NORM" > "$GEN_SEM"
