...
SRC_C="src/decomp/c/replacements/esqpars_jmptbl_diskio_parseconfigbuffer_gcc.c"
ORIG_ASM="src/modules/groups/a/o/esqpars.s"
OUT_DIR="build/decomp/c_trial_gcc"
GEN_ASM="${OUT_DIR}/esqpars_jmptbl_diskio_parseconfigbuffer.generated.gcc.s"
ORIG_SLICE="${OUT_DIR}/esqpars_jmptbl_diskio_parseconfigbuffer.original_slice.s"
GEN_SLICE="${OUT_DIR}/esqpars_jmptbl_diskio_parseconfigbuffer.generated_slice.s"
ORIG_NORM="${OUT_DIR}/esqpars_jmptbl_diskio_parseconfigbuffer.original_slice.norm.s"
GEN_NORM="${OUT_DIR}/esqpars_jmptbl_diskio_parseconfigbuffer.generated_slice.norm.s"
...
awk -v start_label="ESQPARS_JMPTBL_DISKIO_ParseConfigBuffer" -v next_label="ESQPARS_JMPTBL_CLEANUP_ParseAlignedListingBlock" '
...
awk '$0 ~ /^_?ESQPARS_JMPTBL_DISKIO_ParseConfigBuffer:?$/ {in_func=1} in_func { print }' "$GEN_ASM" > "$GEN_SLICE"
...
awk -f "src/decomp/scripts/semantic_filter_esqpars_jmptbl_diskio_parseconfigbuffer.awk" "$ORIG_NORM" > "$ORIG_SEM"
awk -f "src/decomp/scripts/semantic_filter_esqpars_jmptbl_diskio_parseconfigbuffer.awk" "$GEN_NORM" > "$GEN_SEM"
