...
BASE="group_aj_jmptbl_format_raw_dofmt_with_scratch_buffer"
ENTRY_ORIG="GROUP_AJ_JMPTBL_FORMAT_RawDoFmtWithScratchBuffer"
ENTRY_SASC="GROUP_AJ_JMPTBL_FORMAT_RawDoFmtW"
TARGET="FORMAT_RawDoFmtWithScratchBuffer"
mkdir -p "$OUT_DIR"
./sc-build-with-dis.sh "$SASC_SRC" >"${OUT_DIR}/sc_build_${BASE}.log" 2>&1
awk -v e="^${ENTRY_ORIG}:$" '$0 ~ e {inf=1} inf { if ($0 ~ /^GROUP_AJ_JMPTBL_MATH_DivU32:$/) exit; print }' "$ORIG_ASM" >"${OUT_DIR}/${BASE}.original.s"
awk -v e="^${ENTRY_SASC}:$" '$0 ~ e {inf=1} inf { if ($0 ~ /^GROUP_AJ_JMPTBL_MATH_DivU32:$/) exit; print }' "$SASC_DIS" >"${OUT_DIR}/${BASE}.sasc.dis.s"
