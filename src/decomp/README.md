...
- Current hybrid-integration note (March 13, 2026, later re-validated): `modules/groups/a/o/esqpars.s` is now wired into `src/decomp/replacements.map` as a passthrough hybrid boundary. Its restored SAS/C coverage already spans the current non-`JMPTBL` parser, owned-string, and state-persistence helpers in that module, so future object-level replacement work can land there without another root include-graph edit.
...
- Current hybrid-integration note (March 13, 2026): `modules/submodules/unknown29.s` is now wired into `src/decomp/replacements.map` as a passthrough hybrid boundary. Its restored SAS/C coverage already spans the current non-`JMPTBL` command-line/startup helper (`ESQ_ParseCommandLineAndRun`), so future object-level replacement work can land there without another root include-graph edit.
