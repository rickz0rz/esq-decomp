...
awk '$0 ~ /^DST_UpdateBannerQueue:$/ {in_func=1} in_func { if ($0 ~ /^;!======/) exit; print }' "$ORIG_ASM" >"${OUT_DIR}/dst_update_banner_queue.original.s"
awk '$0 ~ /^[[:space:]]*DST_UpdateBannerQueue:/ {in_func=1} in_func { if ($0 ~ /^[[:space:]]*__const:$/) exit; print }' "$SASC_DIS" >"${OUT_DIR}/dst_update_banner_queue.sasc.dis.s"
