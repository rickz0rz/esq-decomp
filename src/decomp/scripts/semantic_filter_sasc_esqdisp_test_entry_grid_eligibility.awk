...

{
    ...
    if (l ~ /MOVEM\.L D6-D7\/A3,-\(A7\)/ || l ~ /MOVEM\.L D6\/D7\/A3,-\(A7\)/ || l ~ /MOVEM\.L D7\/A5,-\(A7\)/ || l ~ /MOVE\.L A5,-\(A7\)/) has_save = 1
    ...
    if (l ~ /MOVEQ #0,D0/ || l ~ /MOVEQ\.L #\$?0,D0/ || l ~ /MOVEQ #1,D0/ || l ~ /MOVEQ\.L #\$?1,D0/) has_truefalse = 1
    ...
    if (l ~ /MOVEM\.L \(A7\)\+,D6-D7\/A3/ || l ~ /MOVEM\.L \(A7\)\+,D6\/D7\/A3/ || l ~ /MOVEM\.L \(A7\)\+,D7\/A5/ || l ~ /MOVE\.L \(A7\)\+,A5/) has_restore = 1
    if (l ~ /^RTS$/) has_rts = 1
}
