...
SRC_C="src/decomp/c/replacements/ed1_clear_esc_menu_mode_gcc.c"
ORIG_ASM="src/modules/groups/a/k/ed1.s"
OUT_DIR="build/decomp/c_trial_gcc"
GEN_ASM="${OUT_DIR}/ed1_clear_esc_menu_mode.generated.gcc.s"
ORIG_SLICE="${OUT_DIR}/ed1_clear_esc_menu_mode.original_slice.s"
GEN_SLICE="${OUT_DIR}/ed1_clear_esc_menu_mode.generated_slice.s"
ORIG_NORM="${OUT_DIR}/ed1_clear_esc_menu_mode.original_slice.norm.s"
GEN_NORM="${OUT_DIR}/ed1_clear_esc_menu_mode.generated_slice.norm.s"
...
awk '
$0 ~ /^ED1_ClearEscMenuMode:?$/ {in_func=1}
in_func {
    if ($0 ~ /^;!======/) exit
    print
}
...
awk '
$0 ~ /^_?ED1_ClearEscMenuMode:?$/ {in_func=1}
in_func { print }
' "$GEN_ASM" > "$GEN_SLICE"
...
awk -f src/decomp/scripts/semantic_filter_ed1_clear_esc_menu_mode.awk "$ORIG_NORM" > "$ORIG_SEM"
awk -f src/decomp/scripts/semantic_filter_ed1_clear_esc_menu_mode.awk "$GEN_NORM" > "$GEN_SEM"
