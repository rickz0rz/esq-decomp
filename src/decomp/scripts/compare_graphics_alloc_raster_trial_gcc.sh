...
SRC_C="src/decomp/c/replacements/graphics_alloc_raster_gcc.c"
ORIG_ASM="src/modules/submodules/unknown2b.s"
OUT_DIR="build/decomp/c_trial_gcc"
GEN_ASM="${OUT_DIR}/graphics_alloc_raster.generated.gcc.s"
ORIG_SLICE="${OUT_DIR}/graphics_alloc_raster.original_slice.s"
GEN_SLICE="${OUT_DIR}/graphics_alloc_raster.generated_slice.s"
ORIG_NORM="${OUT_DIR}/graphics_alloc_raster.original_slice.norm.s"
GEN_NORM="${OUT_DIR}/graphics_alloc_raster.generated_slice.norm.s"
...
awk '$0 ~ /^GRAPHICS_AllocRaster:?$/ {in_func=1} in_func { if ($0 ~ /^;!======/) exit; print }' "$ORIG_ASM" > "$ORIG_SLICE"
awk '$0 ~ /^_?GRAPHICS_AllocRaster:?$/ {in_func=1} in_func { print }' "$GEN_ASM" > "$GEN_SLICE"
...
awk -f src/decomp/scripts/semantic_filter_graphics_alloc_raster.awk "$ORIG_NORM" > "$ORIG_SEM"
awk -f src/decomp/scripts/semantic_filter_graphics_alloc_raster.awk "$GEN_NORM" > "$GEN_SEM"
