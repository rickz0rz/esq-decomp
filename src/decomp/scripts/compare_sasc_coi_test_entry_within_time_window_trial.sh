...
cd "$ROOT_DIR"
SASC_SRC="coi_test_entry_within_time_window.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/a/e/coi.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="coi_test_entry_within_time_window"
ENTRY="COI_TestEntryWithinTimeWindow"
ENTRY_SASC_REGEX="^COI_TestEntryWithinTimeWindow[A-Za-z0-9_]*:$"
mkdir -p "$OUT_DIR"
./sc-build-with-dis.sh "$SASC_SRC" >"${OUT_DIR}/sc_build_${BASE}.log" 2>&1
awk -v e="^${ENTRY}:$" '$0 ~ e {inf=1} inf { if ($0 ~ /^COI_TestEntryWithinTimeWindow_Return:/) exit; print }' "$ORIG_ASM" >"${OUT_DIR}/${BASE}.original.s"
...
awk -f src/decomp/scripts/semantic_filter_sasc_coi_test_entry_within_time_window.awk "${OUT_DIR}/${BASE}.original.norm.s" >"${OUT_DIR}/${BASE}.original.semantic.txt"
awk -f src/decomp/scripts/semantic_filter_sasc_coi_test_entry_within_time_window.awk "${OUT_DIR}/${BASE}.sasc.norm.s" >"${OUT_DIR}/${BASE}.sasc.semantic.txt"
