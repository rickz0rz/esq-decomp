...
function trim(s,t){t=s; sub(/;.*/,"",t); sub(/^[ \t]+/,"",t); sub(/[ \t]+$/,"",t); return t}
{
    ...
    if (u ~ /GROUP_AE_JMPTBL_SCRIPT_BUILDTOKENINDEXMAP/ || u ~ /GROUP_AE_JMPTBL_SCRIPT_BUILDTOKE/) has_token_map = 1
    ...
    if (u ~ /GROUP_AE_JMPTBL_ESQPARS_REPLACEOWNEDSTRING/ || u ~ /GROUP_AE_JMPTBL_ESQPARS_REPLACEO/) has_replace_owned = 1
    if (u ~ /CLEANUP_FORMATENTRYSTRINGTOKENS/) has_format_tokens = 1
    if (u ~ /GROUP_AG_JMPTBL_PARSE_READSIGNEDLONGSKIPCLASS3_ALT/ || u ~ /GROUP_AG_JMPTBL_PARSE_READSIGNEDLONGSKIPCLAS/ || u ~ /GROUP_AG_JMPTBL_PARSE_READSIGNED/) has_parse_signed = 1
