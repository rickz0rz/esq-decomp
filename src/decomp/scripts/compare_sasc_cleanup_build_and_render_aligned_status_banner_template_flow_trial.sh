...
BASE="cleanup_build_and_render_aligned_status_banner_template_flow"
...
diff -u \
    ...
    >"${OUT_DIR}/${BASE}.diff" || true

awk -f src/decomp/scripts/semantic_filter_sasc_cleanup_build_and_render_aligned_status_banner_template_flow.awk \
    "${OUT_DIR}/${BASE}.original.norm.s" \
    >"${OUT_DIR}/${BASE}.original.semantic.txt"

awk -f src/decomp/scripts/semantic_filter_sasc_cleanup_build_and_render_aligned_status_banner_template_flow.awk \
    "${OUT_DIR}/${BASE}.sasc.norm.s" \
    >"${OUT_DIR}/${BASE}.sasc.semantic.txt"

diff -u \
    "${OUT_DIR}/${BASE}.original.semantic.txt" \
    "${OUT_DIR}/${BASE}.sasc.semantic.txt" \
    >"${OUT_DIR}/${BASE}.semantic.diff" || true
