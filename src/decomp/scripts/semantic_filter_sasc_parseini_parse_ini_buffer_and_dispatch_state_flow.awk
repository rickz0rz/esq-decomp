...

{
    ...
    if (n ~ /WDISPCHARCLASSTABLE/ && (n ~ /BTST3/ || n ~ /AND8/)) has_skip_ws = 1
    if (n ~ /STRFINDCHARPTR/ && (n ~ /93/ || n ~ /5D/)) has_header_find_close = 1
    ...
    if (qtable_stage >= 1 && n ~ /PARSEINIDELIMSPACETABSECTION1/) qtable_stage = advance_stage(qtable_stage, 2)
    if (qtable_stage >= 2 && n ~ /TEXTDISPALIASPTRTABLE/ && n ~ /MEMORYALLOCATEMEMORY/) qtable_stage = advance_stage(qtable_stage, 3)
    if (qtable_stage >= 3 && n ~ /ESQPARSREPLACEOWNEDSTRING/) qtable_stage = advance_stage(qtable_stage, 4)
    if (qtable_stage >= 4 && n ~ /PEA22W/) qtable_quote_checks++
    ...
    if (n ~ /MEMORYDEALLOCATEMEMORY/ && n ~ /GLOBALSTRPARSEINIC2/) has_cleanup_free = 1
...
END {
    ...
    print "HAS_LINE_LOOP=" (has_line_loop && has_skip_ws && has_header_find_close ? 1 : 0)
    ...
    print "HAS_QTABLE_FLOW=" (qtable_stage >= 4 && qtable_quote_checks >= 2 && has_qtable_success ? 1 : 0)
    ...
    print "HAS_CLEANUP_FREE=" has_cleanup_free
