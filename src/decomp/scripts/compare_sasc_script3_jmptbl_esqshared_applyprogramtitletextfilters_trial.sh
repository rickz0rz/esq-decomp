...
cd "$ROOT_DIR"
SASC_SRC="script3_jmptbl_stubs.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/b/a/script3.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="script3_jmptbl_esqshared_applyprogramtitletextfilters"
ENTRY_ORIG="SCRIPT3_JMPTBL_ESQSHARED_ApplyProgramTitleTextFilters"
ENTRY_SASC="SCRIPT3_JMPTBL_ESQSHARED_ApplyPr"
TARGET="ESQSHARED_ApplyProgramTitleTextFilters"
mkdir -p "$OUT_DIR"
./sc-build-with-dis.sh "$SASC_SRC" >"${OUT_DIR}/sc_build_${BASE}.log" 2>&1
awk -v e="^${ENTRY_ORIG}:$" '$0 ~ e {inf=1} inf { if ($0 ~ /^SCRIPT3_JMPTBL_/ && $0 !~ e) exit; if ($0 ~ /^;!======/) exit; print }' "$ORIG_ASM" >"${OUT_DIR}/${BASE}.original.s"
