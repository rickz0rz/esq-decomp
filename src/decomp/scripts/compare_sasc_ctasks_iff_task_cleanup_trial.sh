...
cd "$ROOT_DIR"
SASC_SRC="ctasks_iff_task_cleanup.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/a/f/ctasks.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="ctasks_iff_task_cleanup"
ENTRY="CTASKS_IFFTaskCleanup"
ENTRY_SASC_REGEX="^CTASKS_IFFTaskCleanup[A-Za-z0-9_]*:$"
mkdir -p "$OUT_DIR"
...
awk -f src/decomp/scripts/semantic_filter_sasc_ctasks_iff_task_cleanup.awk "${OUT_DIR}/${BASE}.original.norm.s" >"${OUT_DIR}/${BASE}.original.semantic.txt"
awk -f src/decomp/scripts/semantic_filter_sasc_ctasks_iff_task_cleanup.awk "${OUT_DIR}/${BASE}.sasc.norm.s" >"${OUT_DIR}/${BASE}.sasc.semantic.txt"
