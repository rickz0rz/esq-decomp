...
SRC_C="src/decomp/c/replacements/clock_check_date_or_seconds_from_epoch_gcc.c"
ORIG_ASM="src/modules/submodules/unknown42.s"
OUT_DIR="build/decomp/c_trial_gcc"
GEN_ASM="${OUT_DIR}/clock_check_date_or_seconds_from_epoch.generated.gcc.s"
ORIG_SLICE="${OUT_DIR}/clock_check_date_or_seconds_from_epoch.original_slice.s"
GEN_SLICE="${OUT_DIR}/clock_check_date_or_seconds_from_epoch.generated_slice.s"
ORIG_NORM="${OUT_DIR}/clock_check_date_or_seconds_from_epoch.original_slice.norm.s"
GEN_NORM="${OUT_DIR}/clock_check_date_or_seconds_from_epoch.generated_slice.norm.s"
...
awk '$0 ~ /^CLOCK_CheckDateOrSecondsFromEpoch:?$/ {in_func=1} in_func { if ($0 ~ /^;!======/) exit; print }' "$ORIG_ASM" > "$ORIG_SLICE"
awk '$0 ~ /^_?CLOCK_CheckDateOrSecondsFromEpoch:?$/ {in_func=1} in_func { print }' "$GEN_ASM" > "$GEN_SLICE"
...
awk -f src/decomp/scripts/semantic_filter_clock_check_date_or_seconds_from_epoch.awk "$ORIG_NORM" > "$ORIG_SEM"
awk -f src/decomp/scripts/semantic_filter_clock_check_date_or_seconds_from_epoch.awk "$GEN_NORM" > "$GEN_SEM"
