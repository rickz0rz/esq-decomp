 ...
 *   summary: 316 got vs 294 ref, and most of the excess is the exec base. The original reaches it as MOVEA.L AbsExecBase,A6, four bytes, at both library call sites; the volatile esq-exec.h base is a data symbol, so 6.51 emits MOVEA.L _SysBase,A6, six bytes, and it also saves and restores A6 around the whole body (2f0e / 2c5f) which the original does not. ESQSHARED4_CopyPlanesFromContextToSnapshot takes its argument in A1 with nothing on the stack, so it is declared __asm register __a1 -- read its clobber list before linking this caller, per the copper-list note in AGENTS.md. The GetMsg gate, the negative preset-record test, the three saved fields, the keycode dispatch, the countdown decrement and the ReplyMsg teardown match in kind and order.
...
extern void __asm ESQSHARED4_CopyPlanesFromContextToSnapshot(
                register __a1 struct HighlightMsg *msg);
