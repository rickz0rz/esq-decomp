...

{
    ...
    if (uline ~ /^DISKIO1_APPENDTIMESLOTMASKNONEIFALLBITSSET:/) has_entry = 1
    if (uline ~ /^CMPI\.L #0X5FA,D5$/ || uline ~ /^CMPI\.L #\$5FA,D5$/) has_cmp = 1
    if (uline ~ /^PEA DISKIO_STR_NONE_TIMESLOTMASKALLSET$/) has_pea = 1
    if (uline ~ /^JSR GROUP_AJ_JMPTBL_FORMAT_RAWDOFMTWITHSCRATCHBUFFER(\(PC\))?$/) has_jsr = 1
    if (uline ~ /^ADDQ\.W #4,A7$/) has_addq = 1
    if (uline ~ /^BNE\.[SW] DISKIO1_APPENDTIMESLOTMASKOFFAIRIFEMPTY$/ || uline ~ /^JMP DISKIO1_APPENDTIMESLOTMASKOFFAIRIFEMPTY$/) has_branch = 1
}
