...
SRC_C="src/decomp/c/replacements/group_as_jmptbl_str_find_char_ptr_gcc.c"
ORIG_ASM="src/modules/groups/a/s/xjump.s"
OUT_DIR="build/decomp/c_trial_gcc"
GEN_ASM="${OUT_DIR}/group_as_jmptbl_str_find_char_ptr.generated.gcc.s"
ORIG_SLICE="${OUT_DIR}/group_as_jmptbl_str_find_char_ptr.original_slice.s"
GEN_SLICE="${OUT_DIR}/group_as_jmptbl_str_find_char_ptr.generated_slice.s"
ORIG_NORM="${OUT_DIR}/group_as_jmptbl_str_find_char_ptr.original_slice.norm.s"
GEN_NORM="${OUT_DIR}/group_as_jmptbl_str_find_char_ptr.generated_slice.norm.s"
...
awk '
$0 ~ /^GROUP_AS_JMPTBL_STR_FindCharPtr:?$/ {in_func=1}
in_func {
    if ($0 ~ /^GROUP_AS_JMPTBL_ESQ_FindSubstringCaseFold:?$/) {
        exit
    }
    print
}
...
awk '
$0 ~ /^_?GROUP_AS_JMPTBL_STR_FindCharPtr:?$/ {in_func=1}
in_func { print }
' "$GEN_ASM" > "$GEN_SLICE"
...
awk -f src/decomp/scripts/semantic_filter_group_as_jmptbl_str_find_char_ptr.awk "$ORIG_NORM" > "$ORIG_SEM"
awk -f src/decomp/scripts/semantic_filter_group_as_jmptbl_str_find_char_ptr.awk "$GEN_NORM" > "$GEN_SEM"
