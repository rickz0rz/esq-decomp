...
BASE="group_ai_jmptbl_str_skip_class3_chars"
ENTRY_ORIG="GROUP_AI_JMPTBL_STR_SkipClass3Chars"
ENTRY_SASC="GROUP_AI_JMPTBL_STR_SkipClass3Ch"
TARGET="STR_SkipClass3Chars"
mkdir -p "$OUT_DIR"
./sc-build-with-dis.sh "$SASC_SRC" >"${OUT_DIR}/sc_build_${BASE}.log" 2>&1
awk -v e="^${ENTRY_ORIG}:$" '$0 ~ e {inf=1} inf { if ($0 ~ /^GROUP_AI_JMPTBL_STRING_AppendAtNull:$/) exit; print }' "$ORIG_ASM" >"${OUT_DIR}/${BASE}.original.s"
awk -v e="^${ENTRY_SASC}:$" '$0 ~ e {inf=1} inf { if ($0 ~ /^GROUP_AI_JMPTBL_STRING_AppendAtN:$/) exit; print }' "$SASC_DIS" >"${OUT_DIR}/${BASE}.sasc.dis.s"
