...
awk '$0 ~ /^DST_LoadBannerPairFromFiles:$/ {in_func=1} in_func { if ($0 ~ /^;!======/) exit; print }' "$ORIG_ASM" >"${OUT_DIR}/dst_load_banner_pair_from_files.original.s"
awk '$0 ~ /^[[:space:]]*DST_LoadBannerPairFromFiles:/ {in_func=1} in_func { if ($0 ~ /^[[:space:]]*__const:$/) exit; print }' "$SASC_DIS" >"${OUT_DIR}/dst_load_banner_pair_from_files.sasc.dis.s"
