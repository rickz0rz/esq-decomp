BEGIN {
    ...
    has_appendn=0
    has_appendnull=0
    has_summary_prefix=0
    has_sports_prefix=0
    has_channel_suffix=0
    has_ravesc_check=0
    has_range_dash=0
    has_draw_frame=0
    has_bevel=0
    ...
    has_text=0
    has_validate=0
    has_selection_code_write=0
    has_selected_state_write=0
    has_const17=0
    has_const67=0
    has_rts=0
}
...

{
    ...
    if (n ~ /SCRIPTPTRSUMMARYOFPREFIX/) has_summary_prefix=1
    if (n ~ /SCRIPTPTRSPORTSONPREFIX/) has_sports_prefix=1
    if (n ~ /SCRIPTPTRCHANNELSUFFIX/) has_channel_suffix=1
    if (n ~ /GLOBALWORDSELECTCODEISRAVESC/ || n ~ /GLOBALWORDSELECTCODEISRAVES/) has_ravesc_check=1
    if (n ~ /NEWGRIDSHOWTIMERANGEDASH/) has_range_dash=1
    ...
    if ((n ~ /SELECTIONCODE/ || u ~ /MOVE\.W .*(\$36\(A2\)|52\(A3\))/) && (u ~ /#17([^0-9]|$)/ || u ~ /#\$11/ || u ~ /MOVE\.W D0/)) has_selection_code_write=1
    if ((n ~ /SELECTEDSTATE/ || u ~ /MOVE\.L .*(\$20\(A2\)|32\(A3\))/) && (u ~ /#17([^0-9]|$)/ || u ~ /#\$11/ || u ~ /MOVE\.L D[01]/)) has_selected_state_write=1
...
END {
    ...
    print "HAS_SUMMARY_PREFIX="has_summary_prefix
    print "HAS_SPORTS_PREFIX="has_sports_prefix
    print "HAS_CHANNEL_SUFFIX="has_channel_suffix
    print "HAS_RAVESC_CHECK="has_ravesc_check
    print "HAS_RANGE_DASH="has_range_dash
    ...
    print "HAS_SELECTION_CODE_WRITE="has_selection_code_write
    print "HAS_SELECTED_STATE_WRITE="has_selected_state_write
