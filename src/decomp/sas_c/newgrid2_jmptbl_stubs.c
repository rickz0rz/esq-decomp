...
extern LONG COI_GetAnimFieldPointerByMode(const void *entry, UWORD key, UWORD mode);
...
const char *NEWGRID2_JMPTBL_COI_SelectAnimFieldPointer(const char *entry, LONG key, LONG field){return (const char *)COI_GetAnimFieldPointerByMode(entry, (UWORD)key, (UWORD)field);}
