...
SRC_C="src/decomp/c/replacements/esq_get_half_hour_slot_index_gcc.c"
ORIG_ASM="src/modules/groups/a/a/app2.s"
OUT_DIR="build/decomp/c_trial_gcc"
GEN_ASM="${OUT_DIR}/esq_get_half_hour_slot_index.generated.gcc.s"
ORIG_SLICE="${OUT_DIR}/esq_get_half_hour_slot_index.original_slice.s"
GEN_SLICE="${OUT_DIR}/esq_get_half_hour_slot_index.generated_slice.s"
ORIG_NORM="${OUT_DIR}/esq_get_half_hour_slot_index.original_slice.norm.s"
GEN_NORM="${OUT_DIR}/esq_get_half_hour_slot_index.generated_slice.norm.s"
...
awk '$0 ~ /^ESQ_GetHalfHourSlotIndex:?$/ {in_func=1} in_func { if ($0 ~ /^;!======/) exit; print }' "$ORIG_ASM" > "$ORIG_SLICE"
awk '$0 ~ /^_?ESQ_GetHalfHourSlotIndex:?$/ {in_func=1} in_func { print }' "$GEN_ASM" > "$GEN_SLICE"
...
awk -f src/decomp/scripts/semantic_filter_esq_get_half_hour_slot_index.awk "$ORIG_NORM" > "$ORIG_SEM"
awk -f src/decomp/scripts/semantic_filter_esq_get_half_hour_slot_index.awk "$GEN_NORM" > "$GEN_SEM"
