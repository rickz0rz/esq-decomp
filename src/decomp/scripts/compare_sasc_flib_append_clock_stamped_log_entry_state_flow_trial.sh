...
BASE="flib_append_clock_stamped_log_entry_state_flow"
...
awk -v e="^${ENTRY}:$" -v e2="$ENTRY_SASC_REGEX" '
    $0 ~ e || $0 ~ e2 { in_func=1 }
    in_func {
        if (($0 ~ /^FLIB_[A-Za-z0-9_]+:$/ || $0 ~ /^_?FLIB_[A-Za-z0-9_]+:$/) && $0 !~ e && $0 !~ e2) exit
...
awk -f src/decomp/scripts/semantic_filter_sasc_flib_append_clock_stamped_log_entry_state_flow.awk \
