...
BASE="esqiff_queue_next_external_asset_iff_job_state_flow"
ENTRY="ESQIFF_QueueNextExternalAssetIffJob"
...
awk -v e="^${ENTRY}:$" -v e2="$ENTRY_SASC_REGEX" '
    $0 ~ e || $0 ~ e2 { inf=1 }
    inf {
        if (($0 ~ /^[A-Z][A-Z0-9_]*:$/ || $0 ~ /^_?[A-Z][A-Z0-9_]*:$/) && $0 !~ e && $0 !~ e2 && $0 !~ /^___/) exit
...
awk -f src/decomp/scripts/semantic_filter_sasc_esqiff_queue_next_external_asset_iff_job_state_flow.awk "${OUT_DIR}/${BASE}.original.norm.s" >"${OUT_DIR}/${BASE}.original.semantic.txt"
awk -f src/decomp/scripts/semantic_filter_sasc_esqiff_queue_next_external_asset_iff_job_state_flow.awk "${OUT_DIR}/${BASE}.sasc.norm.s" >"${OUT_DIR}/${BASE}.sasc.semantic.txt"
