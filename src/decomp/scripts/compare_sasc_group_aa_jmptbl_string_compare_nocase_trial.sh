...
BASE="group_aa_jmptbl_string_compare_nocase"
ENTRY_ORIG="GROUP_AA_JMPTBL_STRING_CompareNoCase"
ENTRY_SASC="GROUP_AA_JMPTBL_STRING_CompareNo"
TARGET="STRING_CompareNoCase"
mkdir -p "$OUT_DIR"
./sc-build-with-dis.sh "$SASC_SRC" >"${OUT_DIR}/sc_build_${BASE}.log" 2>&1
awk -v e="^${ENTRY_ORIG}:$" '$0 ~ e {inf=1} inf { if ($0 ~ /^GROUP_AA_JMPTBL_STRING_CompareN:$/) exit; print }' "$ORIG_ASM" >"${OUT_DIR}/${BASE}.original.s"
awk -v e="^${ENTRY_SASC}:$" '$0 ~ e {inf=1} inf { if ($0 ~ /^GROUP_AA_JMPTBL_STRING_CompareN:$/) exit; print }' "$SASC_DIS" >"${OUT_DIR}/${BASE}.sasc.dis.s"
