...
SRC_C="src/decomp/c/replacements/disptext_get_total_line_count_gcc.c"
ORIG_ASM="src/modules/groups/a/i/disptext.s"
OUT_DIR="build/decomp/c_trial_gcc"
GEN_ASM="${OUT_DIR}/disptext_get_total_line_count.generated.gcc.s"
ORIG_SLICE="${OUT_DIR}/disptext_get_total_line_count.original_slice.s"
GEN_SLICE="${OUT_DIR}/disptext_get_total_line_count.generated_slice.s"
ORIG_NORM="${OUT_DIR}/disptext_get_total_line_count.original_slice.norm.s"
GEN_NORM="${OUT_DIR}/disptext_get_total_line_count.generated_slice.norm.s"
...
awk '
$0 ~ /^DISPTEXT_GetTotalLineCount:?$/ {in_func=1}
in_func {
    if ($0 ~ /^;!======/) exit
    print
}
...
awk '
$0 ~ /^_?DISPTEXT_GetTotalLineCount:?$/ {in_func=1}
in_func { print }
' "$GEN_ASM" > "$GEN_SLICE"
...
awk -f src/decomp/scripts/semantic_filter_disptext_get_total_line_count.awk "$ORIG_NORM" > "$ORIG_SEM"
awk -f src/decomp/scripts/semantic_filter_disptext_get_total_line_count.awk "$GEN_NORM" > "$GEN_SEM"
