...
BASE="newgrid_select_next_mode_state_flow"
...
awk -v start="^${ENTRY}:$" -v next_label="^NEWGRID_MapSelectionToMode:$" '
    $0 ~ start {in_func=1}
    in_func {
...
awk -f src/decomp/scripts/semantic_filter_sasc_newgrid_select_next_mode_state_flow.awk \
