...
cd "$ROOT_DIR"
SASC_SRC="group_ak_jmptbl_stubs.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/a/k/xjump.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="group_ak_jmptbl_script_update_serial_shadow_from_ctrl_byte"
ENTRY_ORIG="GROUP_AK_JMPTBL_SCRIPT_UpdateSerialShadowFromCtrlByte"
ENTRY_SASC="GROUP_AK_JMPTBL_SCRIPT_UpdateSer"
TARGET="SCRIPT_UpdateSerialShadowFromCtrlByte"
mkdir -p "$OUT_DIR"
./sc-build-with-dis.sh "$SASC_SRC" >"${OUT_DIR}/sc_build_${BASE}.log" 2>&1
awk -v e="^${ENTRY_ORIG}:$" '
  $0 ~ e {inf=1}
  inf {
    if ($0 ~ /^GROUP_AK_JMPTBL_/ && $0 !~ e) exit
    print
  }
