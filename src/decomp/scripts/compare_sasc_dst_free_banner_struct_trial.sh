...
awk '$0 ~ /^DST_FreeBannerStruct:$/ {in_func=1} in_func { if ($0 ~ /^;!======/) exit; print }' "$ORIG_ASM" >"${OUT_DIR}/dst_free_banner_struct.original.s"
awk '$0 ~ /^[[:space:]]*DST_FreeBannerStruct:/ {in_func=1} in_func { if ($0 ~ /^[[:space:]]*__const:$/) exit; print }' "$SASC_DIS" >"${OUT_DIR}/dst_free_banner_struct.sasc.dis.s"
