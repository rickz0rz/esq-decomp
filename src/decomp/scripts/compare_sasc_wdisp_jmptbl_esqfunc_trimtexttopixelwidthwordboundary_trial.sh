...
cd "$ROOT_DIR"
SASC_SRC="wdisp_jmptbl_stubs.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/b/a/wdisp.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="wdisp_jmptbl_esqfunc_trimtexttopixelwidthwordboundary"
ENTRY_ORIG="WDISP_JMPTBL_ESQFUNC_TrimTextToPixelWidthWordBoundary"
ENTRY_SASC="WDISP_JMPTBL_ESQFUNC_TrimTextToP"
TARGET="ESQFUNC_TrimTextToPixelWidthWordBoundary"
mkdir -p "$OUT_DIR"
./sc-build-with-dis.sh "$SASC_SRC" >"${OUT_DIR}/sc_build_${BASE}.log" 2>&1
awk -v e="^${ENTRY_ORIG}:$" '$0 ~ e {inf=1} inf { if ($0 ~ /^WDISP_JMPTBL_/ && $0 !~ e) exit; if ($0 ~ /^;!======/) exit; print }' "$ORIG_ASM" >"${OUT_DIR}/${BASE}.original.s"
