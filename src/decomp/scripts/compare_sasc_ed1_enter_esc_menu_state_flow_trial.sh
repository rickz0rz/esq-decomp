...
SASC_SRC="ed1_enter_esc_menu.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/a/k/ed1.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="ed1_enter_esc_menu_state_flow"
ENTRY="ED1_EnterEscMenu"
ENTRY_SASC_REGEX="^ED1_EnterEscMenu[A-Za-z0-9_]*:$"
...
awk -v e="^${ENTRY}:$" '
    $0 ~ e { inf=1 }
    inf {
        if ($0 ~ /^ED1_EnterEscMenu_AfterVersionText:$/) exit
        print
    }
...
awk -f src/decomp/scripts/semantic_filter_sasc_ed1_enter_esc_menu_state_flow.awk "${OUT_DIR}/${BASE}.original.norm.s" >"${OUT_DIR}/${BASE}.original.semantic.txt"
awk -f src/decomp/scripts/semantic_filter_sasc_ed1_enter_esc_menu_state_flow.awk "${OUT_DIR}/${BASE}.sasc.norm.s" >"${OUT_DIR}/${BASE}.sasc.semantic.txt"
