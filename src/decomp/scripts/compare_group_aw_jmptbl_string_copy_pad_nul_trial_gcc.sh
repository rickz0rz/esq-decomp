...
SRC_C="src/decomp/c/replacements/group_aw_jmptbl_string_copy_pad_nul_gcc.c"
ORIG_ASM="src/modules/groups/a/w/xjump.s"
OUT_DIR="build/decomp/c_trial_gcc"
GEN_ASM="${OUT_DIR}/group_aw_jmptbl_string_copy_pad_nul.generated.gcc.s"
ORIG_SLICE="${OUT_DIR}/group_aw_jmptbl_string_copy_pad_nul.original_slice.s"
GEN_SLICE="${OUT_DIR}/group_aw_jmptbl_string_copy_pad_nul.generated_slice.s"
ORIG_NORM="${OUT_DIR}/group_aw_jmptbl_string_copy_pad_nul.original_slice.norm.s"
GEN_NORM="${OUT_DIR}/group_aw_jmptbl_string_copy_pad_nul.generated_slice.norm.s"
...
awk '
$0 ~ /^GROUP_AW_JMPTBL_STRING_CopyPadNul:?$/ {in_func=1}
in_func { print }
' "$ORIG_ASM" > "$ORIG_SLICE"

awk '
$0 ~ /^_?GROUP_AW_JMPTBL_STRING_CopyPadNul:?$/ {in_func=1}
in_func { print }
' "$GEN_ASM" > "$GEN_SLICE"
...
awk -f src/decomp/scripts/semantic_filter_group_aw_jmptbl_string_copy_pad_nul.awk "$ORIG_NORM" > "$ORIG_SEM"
awk -f src/decomp/scripts/semantic_filter_group_aw_jmptbl_string_copy_pad_nul.awk "$GEN_NORM" > "$GEN_SEM"
