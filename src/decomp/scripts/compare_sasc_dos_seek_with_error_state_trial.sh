...
awk '$0 ~ /^DOS_SeekWithErrorState:$/ {in_func=1} in_func { if ($0 ~ /^;!======/) exit; print }' "$ORIG_ASM" >"${OUT_DIR}/dos_seek_with_error_state.original.s"
awk '$0 ~ /^DOS_SeekWithErrorState:$/ {in_func=1} in_func { if ($0 ~ /^__const:$/) exit; print }' "$SASC_DIS" >"${OUT_DIR}/dos_seek_with_error_state.sasc.dis.s"
