...

{
    ...
    if ((u ~ /BUFFERCAPACITY/ && (u ~ /^TST\.L / || u ~ /^CMP\.L #\$?0,/)) || u ~ /^TST\.L \$1C\(A[0-7]\)$/) has_capacity_test = 1
    if (u ~ /^BTST #/ || u ~ /^ANDI?\.L #\$?8,D[0-7]/) has_force_realloc_test = 1
    if (u ~ /ALLOC_ALLOCFROMFREELIST/) has_alloc_call = 1
    if (u ~ /BUFFERCURSOR/ || u ~ /^MOVE\.L D[0-7],\$C\(A[0-7]\)$/) has_cursor_store = 1
    if (u ~ /BUFFERBASE/ || u ~ /^MOVE\.L D[0-7],\$8\(A[0-7]\)$/) has_base_store = 1
    ...
    if ((u ~ /BUFFERCAPACITY/ && u ~ /GLOBAL_STREAMBUFFERALLOCSIZE/) || u ~ /^MOVE\.L GLOBAL_STREAMBUFFERALLOCSIZE\(A4\),\$1C\(A[0-7]\)$/) has_capacity_store = 1
    ...
    if (u ~ /WRITEREMAINING/ || u ~ /READREMAINING/ || u ~ /^CLR\.L \$14\(A[0-7]\)$/ || u ~ /^CLR\.L \$10\(A[0-7]\)$/) has_read_write_clear = 1
