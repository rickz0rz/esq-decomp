...
cd "$ROOT_DIR"
SASC_SRC="cleanup_update_entry_flag_bytes.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/a/e/cleanup4.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="cleanup_update_entry_flag_bytes"
ENTRY="CLEANUP_UpdateEntryFlagBytes"
ENTRY_SASC_REGEX="^CLEANUP_UpdateEntryFlagBytes[A-Za-z0-9_]*:$"
mkdir -p "$OUT_DIR"
...
awk -f src/decomp/scripts/semantic_filter_sasc_cleanup_update_entry_flag_bytes.awk "${OUT_DIR}/${BASE}.original.norm.s" >"${OUT_DIR}/${BASE}.original.semantic.txt"
awk -f src/decomp/scripts/semantic_filter_sasc_cleanup_update_entry_flag_bytes.awk "${OUT_DIR}/${BASE}.sasc.norm.s" >"${OUT_DIR}/${BASE}.sasc.semantic.txt"
