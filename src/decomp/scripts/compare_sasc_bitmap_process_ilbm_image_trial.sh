...
SASC_SRC="bitmap_process_ilbm_image.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/a/a/bitmap.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="bitmap_process_ilbm_image"
ENTRY_ORIG="BITMAP_ProcessIlbmImage"
ENTRY_SASC_REGEX="^BITMAP_ProcessIlbmImag[A-Za-z0-9_]*:$"

mkdir -p "$OUT_DIR"

./sc-build-with-dis.sh "$SASC_SRC" >"${OUT_DIR}/sc_build_${BASE}.log" 2>&1

awk -v start="^${ENTRY_ORIG}:$" -v next_label="^BITMAP_" '$0 ~ start {in_func=1} in_func {if ($0 ~ next_label && $0 !~ start) exit; print}' "$ORIG_ASM" >"${OUT_DIR}/${BASE}.original.s"
...
awk -f src/decomp/scripts/semantic_filter_sasc_bitmap_process_ilbm_image.awk "${OUT_DIR}/${BASE}.original.norm.s" >"${OUT_DIR}/${BASE}.original.semantic.txt"
awk -f src/decomp/scripts/semantic_filter_sasc_bitmap_process_ilbm_image.awk "${OUT_DIR}/${BASE}.sasc.norm.s" >"${OUT_DIR}/${BASE}.sasc.semantic.txt"
