...
SRC_C="src/decomp/c/replacements/script_deallocate_buffer_array_gcc.c"
ORIG_ASM="src/modules/groups/b/a/script.s"
OUT_DIR="build/decomp/c_trial_gcc"
GEN_ASM="${OUT_DIR}/script_deallocate_buffer_array.generated.gcc.s"
ORIG_SLICE="${OUT_DIR}/script_deallocate_buffer_array.original_slice.s"
GEN_SLICE="${OUT_DIR}/script_deallocate_buffer_array.generated_slice.s"
ORIG_NORM="${OUT_DIR}/script_deallocate_buffer_array.original_slice.norm.s"
GEN_NORM="${OUT_DIR}/script_deallocate_buffer_array.generated_slice.norm.s"
...
awk '
$0 ~ /^SCRIPT_DeallocateBufferArray:?$/ {in_func=1}
in_func {
    if ($0 ~ /^;!======/) {
        exit
    }
...
awk '
$0 ~ /^_?SCRIPT_DeallocateBufferArray:?$/ {in_func=1}
in_func { print }
' "$GEN_ASM" > "$GEN_SLICE"
...
awk -f src/decomp/scripts/semantic_filter_script_deallocate_buffer_array.awk "$ORIG_NORM" > "$ORIG_SEM"
awk -f src/decomp/scripts/semantic_filter_script_deallocate_buffer_array.awk "$GEN_NORM" > "$GEN_SEM"
