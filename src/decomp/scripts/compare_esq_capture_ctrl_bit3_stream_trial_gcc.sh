...
SRC_C="src/decomp/c/replacements/esq_capture_ctrl_bit3_stream_gcc.c"
ORIG_ASM="src/modules/groups/a/a/app.s"
OUT_DIR="build/decomp/c_trial_gcc"
GEN_ASM="${OUT_DIR}/esq_capture_ctrl_bit3_stream.generated.gcc.s"
ORIG_SLICE="${OUT_DIR}/esq_capture_ctrl_bit3_stream.original_slice.s"
GEN_SLICE="${OUT_DIR}/esq_capture_ctrl_bit3_stream.generated_slice.s"
ORIG_NORM="${OUT_DIR}/esq_capture_ctrl_bit3_stream.original_slice.norm.s"
GEN_NORM="${OUT_DIR}/esq_capture_ctrl_bit3_stream.generated_slice.norm.s"
...
awk '$0 ~ /^ESQ_CaptureCtrlBit3Stream:?$/ {in_func=1} in_func { if ($0 ~ /^;!======/) exit; print }' "$ORIG_ASM" > "$ORIG_SLICE"
awk '$0 ~ /^_?ESQ_CaptureCtrlBit3Stream:?$/ {in_func=1} in_func { print }' "$GEN_ASM" > "$GEN_SLICE"
...
awk -f src/decomp/scripts/semantic_filter_esq_capture_ctrl_bit3_stream.awk "$ORIG_NORM" > "$ORIG_SEM"
awk -f src/decomp/scripts/semantic_filter_esq_capture_ctrl_bit3_stream.awk "$GEN_NORM" > "$GEN_SEM"
