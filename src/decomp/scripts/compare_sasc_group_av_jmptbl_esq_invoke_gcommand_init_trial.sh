...
BASE="group_av_jmptbl_esq_invoke_gcommand_init"
ENTRY_ORIG="GROUP_AV_JMPTBL_ESQ_InvokeGcommandInit"
ENTRY_SASC="GROUP_AV_JMPTBL_ESQ_InvokeGcomma"
TARGET="ESQ_InvokeGcommandInit"
mkdir -p "$OUT_DIR"
./sc-build-with-dis.sh "$SASC_SRC" >"${OUT_DIR}/sc_build_${BASE}.log" 2>&1
awk -v e="^${ENTRY_ORIG}:$" '$0 ~ e {inf=1} inf { if ($0 ~ /^GROUP_AV_JMPTBL_EXEC_CallVector_48:$/) exit; print }' "$ORIG_ASM" >"${OUT_DIR}/${BASE}.original.s"
awk -v e="^${ENTRY_SASC}:$" '$0 ~ e {inf=1} inf { if ($0 ~ /^GROUP_AV_JMPTBL_EXEC_CallVector_:$/) exit; print }' "$SASC_DIS" >"${OUT_DIR}/${BASE}.sasc.dis.s"
