...
SRC_C="src/decomp/c/replacements/group_ag_jmptbl_script_check_path_exists_gcc.c"
ORIG_ASM="src/modules/groups/a/g/xjump.s"
OUT_DIR="build/decomp/c_trial_gcc"
GEN_ASM="${OUT_DIR}/group_ag_jmptbl_script_check_path_exists.generated.gcc.s"
ORIG_SLICE="${OUT_DIR}/group_ag_jmptbl_script_check_path_exists.original_slice.s"
GEN_SLICE="${OUT_DIR}/group_ag_jmptbl_script_check_path_exists.generated_slice.s"
ORIG_NORM="${OUT_DIR}/group_ag_jmptbl_script_check_path_exists.original_slice.norm.s"
GEN_NORM="${OUT_DIR}/group_ag_jmptbl_script_check_path_exists.generated_slice.norm.s"
...
awk -v start_label="GROUP_AG_JMPTBL_SCRIPT_CheckPathExists" -v next_label="GROUP_AG_JMPTBL_MATH_Mulu32" '
...
awk '$0 ~ /^_?GROUP_AG_JMPTBL_SCRIPT_CheckPathExists:?$/ {in_func=1} in_func { print }' "$GEN_ASM" > "$GEN_SLICE"
...
awk -f "src/decomp/scripts/semantic_filter_group_ag_jmptbl_script_check_path_exists.awk" "$ORIG_NORM" > "$ORIG_SEM"
awk -f "src/decomp/scripts/semantic_filter_group_ag_jmptbl_script_check_path_exists.awk" "$GEN_NORM" > "$GEN_SEM"
