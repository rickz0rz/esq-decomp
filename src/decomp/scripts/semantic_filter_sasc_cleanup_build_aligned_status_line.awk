...
function trim(s,t){t=s; sub(/;.*/,"",t); sub(/^[ \t]+/,"",t); sub(/[ \t]+$/,"",t); return t}
{
    ...
    if (u ~ /GROUP_AE_JMPTBL_ESQDISP_GETENTRYPOINTERBYMODE/ || u ~ /GROUP_AE_JMPTBL_ESQDISP_GETENTRYPOINTER/ || u ~ /GROUP_AE_JMPTBL_ESQDISP_GETENTRY/ || u ~ /ESQDISP_GETENTRYPOINTERBYMODE/) has_get_entry_ptr = 1
    if (u ~ /CLEANUP_TESTENTRYFLAGYANDBIT1/) has_test_flag = 1
    if (u ~ /COI_GETANIMFIELDPOINTERBYMODE/) has_get_anim_ptr = 1
    if (u ~ /GROUP_AE_JMPTBL_WDISP_SPRINTF/ || u ~ /GROUP_AE_JMPTBL_WDISP_SPRIN/ || u ~ /WDISP_SPRINTF/) has_sprintf = 1
    if (u ~ /GROUP_AI_JMPTBL_STRING_APPENDATNULL/ || u ~ /GROUP_AI_JMPTBL_STRING_APPENDAT/ || u ~ /STRING_APPENDATNULL/) has_append = 1
