...
/* JMPTBL pass-through wrappers. Several targets take arguments, so the wrappers
   MUST declare and forward those args -- an arg-less `(void){X();}` stub silently
   drops the caller's arguments and reads garbage (notably WDISP_SPrintf's varargs
   and ESQPARS_ReplaceOwnedString's owned-string swap). */
...
/* WDISP_SPrintf is varargs (K&R, reads &fmt+1); forward a generous fixed arg set. */
