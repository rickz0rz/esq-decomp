...
cd "$ROOT_DIR"
SASC_SRC="esqpars_jmptbl_esqproto_verify_checksum_and_parse_record.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/a/o/esqpars.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="esqpars_jmptbl_esqproto_verifychecksumandparserecord"
ENTRY_ORIG="ESQPARS_JMPTBL_ESQPROTO_VerifyChecksumAndParseRecord"
ENTRY_SASC="ESQPARS_JMPTBL_ESQPROTO_VerifyCh"
TARGET="ESQPROTO_VerifyChecksumAndParseRecord"
mkdir -p "$OUT_DIR"
./sc-build-with-dis.sh "$SASC_SRC" >"${OUT_DIR}/sc_build_${BASE}.log" 2>&1
awk -v e="^${ENTRY_ORIG}:$" '$0 ~ e {inf=1} inf { if ($0 ~ /^ESQPARS_JMPTBL_/ && $0 !~ e) exit; if ($0 ~ /^;!======/) exit; print }' "$ORIG_ASM" >"${OUT_DIR}/${BASE}.original.s"
