...
SRC_C="src/decomp/c/replacements/handle_open_entry_with_flags_gcc.c"
ORIG_ASM="src/modules/submodules/unknown10.s"
OUT_DIR="build/decomp/c_trial_gcc"
GEN_ASM="${OUT_DIR}/handle_open_entry_with_flags.generated.gcc.s"
ORIG_SLICE="${OUT_DIR}/handle_open_entry_with_flags.original_slice.s"
GEN_SLICE="${OUT_DIR}/handle_open_entry_with_flags.generated_slice.s"
ORIG_NORM="${OUT_DIR}/handle_open_entry_with_flags.original_slice.norm.s"
GEN_NORM="${OUT_DIR}/handle_open_entry_with_flags.generated_slice.norm.s"
...
awk '
$0 ~ /^HANDLE_OpenEntryWithFlags:?$/ {in_func=1}
in_func {
    if ($0 ~ /^;!======/) exit
    print
}
...
awk '
$0 ~ /^_?HANDLE_OpenEntryWithFlags:?$/ {in_func=1}
in_func { print }
' "$GEN_ASM" > "$GEN_SLICE"
...
awk -f src/decomp/scripts/semantic_filter_handle_open_entry_with_flags.awk "$ORIG_NORM" > "$ORIG_SEM"
awk -f src/decomp/scripts/semantic_filter_handle_open_entry_with_flags.awk "$GEN_NORM" > "$GEN_SEM"
