...
SASC_SRC="esq_try_rom_write_test.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/a/a/app3.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="esq_try_rom_write_test"
ENTRY="ESQ_TryRomWriteTest"
ENTRY_SASC_REGEX="^ESQ_TryRomWriteTest[A-Za-z0-9_]*:$"
ENTRY_SEM="ESQ_TRYROMWRITETEST:"
ENTRY_ALT_SEM="ESQ_TRYROMWRITETEST"
...
awk -v ENTRY_PREFIX="$ENTRY_SEM" -v ENTRY_ALT_PREFIX="$ENTRY_ALT_SEM" -f src/decomp/scripts/semantic_filter_sasc_esq_try_rom_write_test.awk "${OUT_DIR}/${BASE}.original.norm.s" >"${OUT_DIR}/${BASE}.original.semantic.txt"
awk -v ENTRY_PREFIX="$ENTRY_SEM" -v ENTRY_ALT_PREFIX="$ENTRY_ALT_SEM" -f src/decomp/scripts/semantic_filter_sasc_esq_try_rom_write_test.awk "${OUT_DIR}/${BASE}.sasc.norm.s" >"${OUT_DIR}/${BASE}.sasc.semantic.txt"
