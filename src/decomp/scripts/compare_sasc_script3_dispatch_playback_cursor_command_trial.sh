...
SASC_SRC="script3_dispatch_playback_cursor_command.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/b/a/script3.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="script3_dispatch_playback_cursor_command"
ENTRY="SCRIPT_DispatchPlaybackCursorCommand"
ENTRY_SASC_REGEX="^SCRIPT_DispatchPlaybackCursorC[A-Za-z0-9_]*:$"

mkdir -p "$OUT_DIR"

./sc-build-with-dis.sh "$SASC_SRC" >"${OUT_DIR}/sc_build_${BASE}.log" 2>&1

awk -v start="^${ENTRY}:$" -v next_label="^SCRIPT_SetCtrlContextMode:$" '
    $0 ~ start { in_func=1 }
    in_func {
...
awk -f src/decomp/scripts/semantic_filter_sasc_script3_dispatch_playback_cursor_command.awk \
