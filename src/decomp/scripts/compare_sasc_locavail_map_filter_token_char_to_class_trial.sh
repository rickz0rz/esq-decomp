...
SASC_SRC="locavail_map_filter_token_char_to_class.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/a/y/locavail.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="locavail_map_filter_token_char_to_class"
ENTRY="LOCAVAIL_MapFilterTokenCharToClass"
...
awk -v e="^${ENTRY}:$" -v e2="$ENTRY_SASC_REGEX" '$0 ~ e || $0 ~ e2 {in_func=1} in_func { if (($0 ~ /^LOCAVAIL_[A-Za-z0-9_]+:$/ || $0 ~ /^_?LOCAVAIL_[A-Za-z0-9_]+:$/) && $0 !~ e && $0 !~ e2) exit; if ($0 ~ /^XREF / || $0 ~ /^XDEF / || $0 ~ /^ END$/ || $0 ~ /^END$/ || $0 ~ /^__const:$/) exit; print }' \
...
awk -f src/decomp/scripts/semantic_filter_sasc_locavail_map_filter_token_char_to_class.awk \
