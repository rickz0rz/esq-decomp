...
- Additional triage note (March 13, 2026): the full `run_sasc_core_sweep.sh` maintained lane set plus the two out-of-sweep `compare_sasc_memory_allocate_trial.sh` and `compare_sasc_esqfunc_draw_esc_menu_version_trial.sh` checks also resolve to zero-byte semantic diffs. Treat the current SAS/C lane as semantically green by default and spend new effort on genuinely unported behavior or non-sweep compare lanes, not repeated core-sweep revalidation.
