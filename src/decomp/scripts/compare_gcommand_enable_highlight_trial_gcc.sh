...
SRC_C="src/decomp/c/replacements/gcommand_enable_highlight_gcc.c"
ORIG_ASM="src/modules/groups/a/u/gcommand3.s"
OUT_DIR="build/decomp/c_trial_gcc"
GEN_ASM="${OUT_DIR}/gcommand_enable_highlight.generated.gcc.s"
ORIG_SLICE="${OUT_DIR}/gcommand_enable_highlight.original_slice.s"
GEN_SLICE="${OUT_DIR}/gcommand_enable_highlight.generated_slice.s"
ORIG_NORM="${OUT_DIR}/gcommand_enable_highlight.original_slice.norm.s"
GEN_NORM="${OUT_DIR}/gcommand_enable_highlight.generated_slice.norm.s"
...
awk '
$0 ~ /^GCOMMAND_EnableHighlight:?$/ {in_func=1}
in_func {
    if ($0 ~ /^;!======/) exit
    print
}
...
awk '
$0 ~ /^_?GCOMMAND_EnableHighlight:?$/ {in_func=1}
in_func { print }
' "$GEN_ASM" > "$GEN_SLICE"
...
awk -f src/decomp/scripts/semantic_filter_gcommand_enable_highlight.awk "$ORIG_NORM" > "$ORIG_SEM"
awk -f src/decomp/scripts/semantic_filter_gcommand_enable_highlight.awk "$GEN_NORM" > "$GEN_SEM"
