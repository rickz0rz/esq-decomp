...
cd "$ROOT_DIR"
SASC_SRC="esq_move_copper_entry_toward_start.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/a/a/app2.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="esq_move_copper_entry_toward_start"
ENTRY="ESQ_MoveCopperEntryTowardStart"
mkdir -p "$OUT_DIR"
...
awk -v e="^${ENTRY}:$" '$0 ~ e {inf=1} inf { if (($0 ~ /^ESQ_[A-Za-z0-9_]+:$/ || $0 ~ /^_?ESQ_[A-Za-z0-9_]+:$/) && $0 !~ e) exit; if ($0 ~ /^XREF / || $0 ~ /^XDEF / || $0 ~ /^ END$/ || $0 ~ /^END$/) exit; print }' "$SASC_DIS" >"${OUT_DIR}/${BASE}.sasc.dis.s"
...
awk -v ENTRY="$ENTRY" -f src/decomp/scripts/semantic_filter_sasc_move_copper_entry_toward_start.awk "${OUT_DIR}/${BASE}.original.norm.s" >"${OUT_DIR}/${BASE}.original.semantic.txt"
awk -v ENTRY="$ENTRY" -f src/decomp/scripts/semantic_filter_sasc_move_copper_entry_toward_start.awk "${OUT_DIR}/${BASE}.sasc.norm.s" >"${OUT_DIR}/${BASE}.sasc.semantic.txt"
