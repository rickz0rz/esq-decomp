...
{
    FORMAT_RawDoFmtWithScratchBuffer(DISKIO_STR_TimeSlotListOpenParen);
...
{
    FORMAT_RawDoFmtWithScratchBuffer(DISKIO_STR_TimeSlotListCloseParenNewline);
    DISKIO1_FormatBlackoutMaskFlags();
}

void DISKIO1_AppendBlackoutMaskValueHeader(void)
{
    FORMAT_RawDoFmtWithScratchBuffer(DISKIO_STR_BlackoutListOpenParen);
    gDiskio1MaskValueBitIndex = DISKIO1_MASK_VALUE_FIRST_BIT_INDEX;
...
{
    FORMAT_RawDoFmtWithScratchBuffer(DISKIO_STR_BlackoutListCloseParenNewline);
