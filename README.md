...
- Current triage state (March 13, 2026): do not restart from the older maintained-sweep blocker list. The maintained `run_sasc_core_sweep.sh` set is green in this checkout, but there are still out-of-sweep semantic diffs in `disptext_is_current_line_last`, `disptext_is_last_line_selected`, and `newgrid_update_grid_state`, so broader mismatch work is not fully finished.
