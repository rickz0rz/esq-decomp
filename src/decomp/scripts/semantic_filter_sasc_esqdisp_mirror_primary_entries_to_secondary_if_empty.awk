...

{
    ...
    if ((uline ~ /ESQDISP_PRIMARYSECONDARYMIRRORFLAG/ || uline ~ /ESQDISP_PRIMARYSECONDARYMIRRORFL/) && (uline ~ /#1/ || uline ~ /#\$1/ || uline ~ /MOVE\.W D6,/ || uline ~ /MOVE\.L D6,/)) has_flag_set = 1
    if (uline ~ /^CLR\.W ESQDISP_PRIMARYSECONDARYMIRRORFLAG/ || uline ~ /^CLR\.L ESQDISP_PRIMARYSECONDARYMIRRORFLAG/ || uline ~ /^CLR\.W ESQDISP_PRIMARYSECONDARYMIRRORFL/ || uline ~ /^CLR\.L ESQDISP_PRIMARYSECONDARYMIRRORFL/ || ((uline ~ /ESQDISP_PRIMARYSECONDARYMIRRORFLAG/ || uline ~ /ESQDISP_PRIMARYSECONDARYMIRRORFL/) && (uline ~ /#0/ || uline ~ /MOVE\.W D7,/ || uline ~ /MOVE\.L D7,/))) has_flag_clear = 1
