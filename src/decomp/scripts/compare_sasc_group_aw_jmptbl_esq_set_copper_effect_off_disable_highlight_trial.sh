...
BASE="group_aw_jmptbl_esq_set_copper_effect_off_disable_highlight"
ENTRY_ORIG="GROUP_AW_JMPTBL_ESQ_SetCopperEffect_OffDisableHighlight"
ENTRY_SASC="GROUP_AW_JMPTBL_ESQ_SetCopperEff"
TARGET="ESQ_SetCopperEffect_OffDisableHi"

mkdir -p "$OUT_DIR"
./sc-build-with-dis.sh "$SASC_SRC" >"${OUT_DIR}/sc_build_${BASE}.log" 2>&1
awk -v e="^${ENTRY_ORIG}:$" '$0 ~ e {inf=1} inf { if ($0 ~ /^GROUP_AW_JMPTBL_STRING_CopyPadNul:$/) exit; print }' "$ORIG_ASM" >"${OUT_DIR}/${BASE}.original.s"
awk -v e="^${ENTRY_SASC}:$" '$0 ~ e {inf=1} inf { if ($0 ~ /^GROUP_AW_JMPTBL_STRING_CopyPadNu:$/) exit; print }' "$SASC_DIS" >"${OUT_DIR}/${BASE}.sasc.dis.s"
