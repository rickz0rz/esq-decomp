...
BASE="disptext_set_current_line_index"
ENTRY="DISPTEXT_SetCurrentLineIndex"
ENTRY_SASC_REGEX="^DISPTEXT_SetCurrentLineInd[A-Za-z0-9_]*:$"
ENTRY_SEM="DISPTEXT_SETCURRENTLINEINDEX:"
ENTRY_ALT_SEM="DISPTEXT_SETCURRENTLINEIND"
...
normalize(){ awk -f src/decomp/scripts/normalize_sasc_compare.awk; }
...
awk -v ENTRY_PREFIX="$ENTRY_SEM" -v ENTRY_ALT_PREFIX="$ENTRY_ALT_SEM" -f src/decomp/scripts/semantic_filter_sasc_disptext_set_current_line_index.awk "${OUT_DIR}/${BASE}.original.norm.s" >"${OUT_DIR}/${BASE}.original.semantic.txt"
awk -v ENTRY_PREFIX="$ENTRY_SEM" -v ENTRY_ALT_PREFIX="$ENTRY_ALT_SEM" -f src/decomp/scripts/semantic_filter_sasc_disptext_set_current_line_index.awk "${OUT_DIR}/${BASE}.sasc.norm.s" >"${OUT_DIR}/${BASE}.sasc.semantic.txt"
