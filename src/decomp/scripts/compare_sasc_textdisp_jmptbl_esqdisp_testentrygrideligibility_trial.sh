...
cd "$ROOT_DIR"
SASC_SRC="textdisp_jmptbl_stubs.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/b/a/textdisp.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="textdisp_jmptbl_esqdisp_testentrygrideligibility"
ENTRY_ORIG="TEXTDISP_JMPTBL_ESQDISP_TestEntryGridEligibility"
ENTRY_SASC="TEXTDISP_JMPTBL_ESQDISP_TestEntr"
TARGET="ESQDISP_TestEntryGridEligibility"
mkdir -p "$OUT_DIR"
./sc-build-with-dis.sh "$SASC_SRC" >"${OUT_DIR}/sc_build_${BASE}.log" 2>&1
awk -v e="^${ENTRY_ORIG}:$" '$0 ~ e {inf=1} inf { if ($0 ~ /^TEXTDISP_JMPTBL_/ && $0 !~ e) exit; if ($0 ~ /^;!======/) exit; print }' "$ORIG_ASM" >"${OUT_DIR}/${BASE}.original.s"
