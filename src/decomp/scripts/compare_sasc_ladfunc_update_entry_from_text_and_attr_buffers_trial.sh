...
SASC_SRC="ladfunc_update_entry_from_text_and_attr_buffers.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/a/w/ladfunc.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="ladfunc_update_entry_from_text_and_attr_buffers"
ENTRY="LADFUNC_UpdateEntryFromTextAndAttrBuffers"
ENTRY_SASC_REGEX="^LADFUNC_UpdateEntryFromTextAndA[A-Za-z0-9_]*:$"
...
awk -v e="^${ENTRY}:$" -v e2="$ENTRY_SASC_REGEX" '
    $0 ~ e || $0 ~ e2 { in_func=1 }
    in_func {
        if (($0 ~ /^LADFUNC_[A-Za-z0-9_]+:$/ || $0 ~ /^_?LADFUNC_[A-Za-z0-9_]+:$/) && $0 !~ e && $0 !~ e2) exit
...
awk -f src/decomp/scripts/semantic_filter_sasc_ladfunc_update_entry_from_text_and_attr_buffers.awk \
