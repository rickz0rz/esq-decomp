...
SRC_C="src/decomp/c/replacements/coi_format_entry_display_text_gcc.c"
ORIG_ASM="src/modules/groups/a/e/coi.s"
OUT_DIR="build/decomp/c_trial_gcc"
GEN_ASM="${OUT_DIR}/coi_format_entry_display_text.generated.gcc.s"
ORIG_SLICE="${OUT_DIR}/coi_format_entry_display_text.original_slice.s"
GEN_SLICE="${OUT_DIR}/coi_format_entry_display_text.generated_slice.s"
ORIG_NORM="${OUT_DIR}/coi_format_entry_display_text.original_slice.norm.s"
GEN_NORM="${OUT_DIR}/coi_format_entry_display_text.generated_slice.norm.s"
...
awk '
$0 ~ /^COI_FormatEntryDisplayText:?$/ {in_func=1}
in_func {
    if ($0 ~ /^COI_FormatEntryDisplayText_Return:?$/) exit
    print
}
' "$ORIG_ASM" > "$ORIG_SLICE"

awk '
$0 ~ /^_?COI_FormatEntryDisplayText:?$/ {in_func=1}
in_func { print }
' "$GEN_ASM" > "$GEN_SLICE"
...
awk -f src/decomp/scripts/semantic_filter_coi_format_entry_display_text.awk "$ORIG_NORM" > "$ORIG_SEM"
awk -f src/decomp/scripts/semantic_filter_coi_format_entry_display_text.awk "$GEN_NORM" > "$GEN_SEM"
