...
SRC_C="src/decomp/c/replacements/battclock_write_seconds_gcc.c"
ORIG_ASM="src/modules/submodules/unknown40.s"
OUT_DIR="build/decomp/c_trial_gcc"
GEN_ASM="${OUT_DIR}/battclock_write_seconds.generated.gcc.s"
ORIG_SLICE="${OUT_DIR}/battclock_write_seconds.original_slice.s"
GEN_SLICE="${OUT_DIR}/battclock_write_seconds.generated_slice.s"
ORIG_NORM="${OUT_DIR}/battclock_write_seconds.original_slice.norm.s"
GEN_NORM="${OUT_DIR}/battclock_write_seconds.generated_slice.norm.s"
...
awk '
$0 ~ /^BATTCLOCK_WriteSecondsToBatteryBackedClock:?$/ {in_func=1}
in_func {
    if ($0 ~ /^;!======/) exit
    print
}
...
awk '
$0 ~ /^_?BATTCLOCK_WriteSecondsToBatteryBackedClock:?$/ {in_func=1}
in_func { print }
' "$GEN_ASM" > "$GEN_SLICE"
...
awk -f src/decomp/scripts/semantic_filter_battclock_write_seconds.awk "$ORIG_NORM" > "$ORIG_SEM"
awk -f src/decomp/scripts/semantic_filter_battclock_write_seconds.awk "$GEN_NORM" > "$GEN_SEM"
