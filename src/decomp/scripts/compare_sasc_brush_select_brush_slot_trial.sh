...
cd "$ROOT_DIR"
SASC_SRC="brush_select_brush_slot.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/a/a/brush.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="brush_select_brush_slot"
ENTRY="BRUSH_SelectBrushSlot"
ENTRY_SASC_REGEX="^BRUSH_SelectBrushSlot[A-Za-z0-9_]*:$"
mkdir -p "$OUT_DIR"
...
in_func {
  if ($0 ~ /^;------------------------------------------------------------------------------/ && seen_ret) exit
  if ($0 ~ /^BRUSH_SelectBrushSlot_Return:?$/) seen_ret=1
  print
}
...
awk -v ENTRY="$ENTRY" -v ENTRY_REGEX="$ENTRY_SASC_REGEX" -f src/decomp/scripts/semantic_filter_sasc_brush_select_brush_slot.awk "${OUT_DIR}/${BASE}.original.norm.s" >"${OUT_DIR}/${BASE}.original.semantic.txt"
awk -v ENTRY="$ENTRY" -v ENTRY_REGEX="$ENTRY_SASC_REGEX" -f src/decomp/scripts/semantic_filter_sasc_brush_select_brush_slot.awk "${OUT_DIR}/${BASE}.sasc.norm.s" >"${OUT_DIR}/${BASE}.sasc.semantic.txt"
