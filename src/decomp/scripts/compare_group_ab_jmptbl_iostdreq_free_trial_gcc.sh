...
SRC_C="src/decomp/c/replacements/group_ab_jmptbl_iostdreq_free_gcc.c"
ORIG_ASM="src/modules/groups/a/b/xjump.s"
OUT_DIR="build/decomp/c_trial_gcc"
GEN_ASM="${OUT_DIR}/group_ab_jmptbl_iostdreq_free.generated.gcc.s"
ORIG_SLICE="${OUT_DIR}/group_ab_jmptbl_iostdreq_free.original_slice.s"
GEN_SLICE="${OUT_DIR}/group_ab_jmptbl_iostdreq_free.generated_slice.s"
ORIG_NORM="${OUT_DIR}/group_ab_jmptbl_iostdreq_free.original_slice.norm.s"
GEN_NORM="${OUT_DIR}/group_ab_jmptbl_iostdreq_free.generated_slice.norm.s"
...
awk '
$0 ~ /^GROUP_AB_JMPTBL_IOSTDREQ_Free:?$/ {in_func=1}
in_func {
    if ($0 ~ /^GROUP_AB_JMPTBL_ESQIFF2_ClearLineHeadTailByMode:?$/) {
        exit
    }
    print
}
...
awk '
$0 ~ /^_?GROUP_AB_JMPTBL_IOSTDREQ_Free:?$/ {in_func=1}
in_func { print }
' "$GEN_ASM" > "$GEN_SLICE"
...
awk -f "src/decomp/scripts/semantic_filter_group_ab_jmptbl_iostdreq_free.awk" "$ORIG_NORM" > "$ORIG_SEM"
awk -f "src/decomp/scripts/semantic_filter_group_ab_jmptbl_iostdreq_free.awk" "$GEN_NORM" > "$GEN_SEM"
