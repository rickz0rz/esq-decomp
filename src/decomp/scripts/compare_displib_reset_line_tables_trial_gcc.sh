...
SRC_C="src/decomp/c/replacements/displib_reset_line_tables_gcc.c"
ORIG_ASM="src/modules/groups/a/i/displib.s"
OUT_DIR="build/decomp/c_trial_gcc"
GEN_ASM="${OUT_DIR}/displib_reset_line_tables.generated.gcc.s"
ORIG_SLICE="${OUT_DIR}/displib_reset_line_tables.original_slice.s"
GEN_SLICE="${OUT_DIR}/displib_reset_line_tables.generated_slice.s"
ORIG_NORM="${OUT_DIR}/displib_reset_line_tables.original_slice.norm.s"
GEN_NORM="${OUT_DIR}/displib_reset_line_tables.generated_slice.norm.s"
...
awk '$0 ~ /^DISPLIB_ResetLineTables:?$/ {in_func=1} in_func { if ($0 ~ /^;!======/) exit; print }' "$ORIG_ASM" > "$ORIG_SLICE"
awk '$0 ~ /^_?DISPLIB_ResetLineTables:?$/ {in_func=1} in_func { print }' "$GEN_ASM" > "$GEN_SLICE"
...
awk -f src/decomp/scripts/semantic_filter_displib_reset_line_tables.awk "$ORIG_NORM" > "$ORIG_SEM"
awk -f src/decomp/scripts/semantic_filter_displib_reset_line_tables.awk "$GEN_NORM" > "$GEN_SEM"
