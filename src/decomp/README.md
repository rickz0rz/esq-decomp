...
- The hybrid coverage reporter now canonicalizes restored SAS/C target names from compare metadata (`ENTRY`, `ENTRY_ORIG`, and friends) before tallying coverage, so wrapper-backed direct exports are credited to the underlying target instead of being split by compare-script alias.
...
- After you have a short list of complete candidates, use `src/decomp/scripts/report_passthrough_integration_candidates.py` to narrow that set to module boundaries whose replacement file still only `include`s the canonical asm module verbatim. Treat the `ready` rows from that report as the current object-integration queue; add `--details` to print the covered exports and `compare_sasc_*` scripts you will need when preparing a concrete module integration pass, and add `--verify` when you need the report to rerun those compare lanes and show which candidates are exact/raw-diff clean versus semantic-only matches.
- In the current March 21, 2026 checkout, that passthrough queue is exhausted: `report_passthrough_integration_candidates.py` reports zero ready rows, `report_remaining_decomp_scope.py` reports zero unmapped code modules, and `report_replacement_only_modules.py` shows only the non-callable `interrupts/constants.s` include mirror outside promotion/compare coverage. Treat those reports as the starting point before assuming another unmapped module still needs a first hybrid boundary.
...
- Do not end a decomp run with your work left only as local uncommitted edits. If you changed files, stage them, review `git status`, and make a commit before handoff unless the user explicitly told you not to or a concrete blocker makes committing unsafe.
...
- Prefer SAS/C system headers from `/Users/RJ/Downloads/SAS-C-hdd/sc/include` for canonical AmigaOS structs instead of re-declaring local stand-ins when the shipped layout matches the restored usage. Current preferred examples include `graphics/rastport.h`, `graphics/gfx.h`, `graphics/text.h`, and `exec/lists.h`.
- Current header-substitution note (March 13, 2026): the maintained SAS/C lane has already switched a large batch of files over to canonical headers for `struct RastPort`, `struct TextFont`, `struct BitMap`, `struct MinList`, `struct MsgPort`, `struct Message`, `struct IOStdReq`, and `struct Library`. Before adding any new local “approx” or pad-based duplicate for one of those types, check whether the corresponding SAS/C header already works for the target file.
- Current exception note (March 13, 2026): two files still intentionally use local overlay structs because they depend on nonstandard cached/overlaid fields rather than plain header members: `src/decomp/sas_c/cleanup_draw_grid_time_banner.c` and `src/decomp/sas_c/render_short_month_short_day_of_week_day.c`.
...
- Current audit note (March 19, 2026): a semantic-filter hardening sweep across behavior-heavy SAS/C lanes found several real partial ports that had been masked by overly permissive semantic filters. The concrete restored functions now landed in this checkout are `TLIBA1_DrawFormattedTextBlock`, `SCRIPT_HandleBrushCommand`, and `ESQPARS_ConsumeRbfByteAndDispatchCommand`.
- Current audit note (March 19, 2026): the compare scripts are only as trustworthy as their `semantic_filter_sasc_*` companions. When a lane shows an empty semantic diff but the raw `.diff` is still very large, inspect the filter before assuming the implementation is complete. This is now a proven source of false greens in this repo. The current concrete example beyond ESQPARS is `ED_HandleEditorInput`, where a large raw diff hid a real ESC-path control-flow mismatch until the semantic filter was tightened and the asm was re-audited directly.
- Current audit note (March 30, 2026): the maintained strict sweep now also carries dedicated state-flow lanes for `DISKIO2_LoadCurDayDataFile`, `NEWGRID_ProcessScheduleState`, and `WDISP_DrawWeatherStatusOverlay`. The current semantic-filter hardening pass for the COI, CLEANUP, DISKIO, ED, ESQ, ESQPARS, GCOMMAND, NEWGRID, PARSEINI, SCRIPT3, TEXTDISP, and WDISP audit set is green in this checkout under `run_sasc_core_sweep.sh --strict`.
- Current ESQPARS note (March 19, 2026): `src/decomp/sas_c/esqpars_consume_rbf_byte_and_dispatch_command.c` has moved well beyond the earlier stub and now restores the dispatcher families around `!`, `P/p`, `g`, banner/config/font/status handling, and related parser-state updates. Later March 19 follow-up passes also corrected several micro-semantic mismatches: lower-`p` reject paths no longer clear `ESQPARS_ResetArmedFlag` too early; `R` now restores the real reset overlay loop; `K` error accounting matches the asm; `g` subtype `'1'` now reads the first payload byte from the stream; `C` and `v` preserve armed state on zero-length exits; uppercase `I` and lowercase `i` now match the asm's actual roles; and lowercase `c` plus uppercase `X` now match the original data-error behavior.
- Current ESQPARS verification note (March 19, 2026): `bash src/decomp/scripts/compare_sasc_esqpars_consume_rbf_byte_and_dispatch_command_trial.sh` currently ends with an empty semantic diff under the tightened filter, and `./test-hash.sh` still matches the canonical hash.
- Current ED note (March 19, 2026): `src/decomp/sas_c/ed_handle_editor_input.c` was audited next because its semantic lane was green while the raw diff stayed large. One real mismatch is now fixed: the ESC case returns immediately like the asm instead of falling through into the final sync/update logic. A later March 19 follow-up also fixed the line/page toggle path, where the asm stores `D1` after `MATH_DivS32` and therefore uses the remainder of `(flag + 1) / 2` as the new toggle state rather than the quotient. Another follow-up removed an extra `ED_TextLimit > 0` guard from the page-down path so the restored C now matches the asm's unconditional `ED_TextLimit - 1` multiply there too. `semantic_filter_sasc_ed_handle_editor_input.awk` was tightened accordingly and now also asserts cursor-sync, row-shift, insert-char, mode-9-help, directional-nav, and page-down markers, and the lane is currently back to an empty semantic diff.
...
- Coverage helper note (March 14, 2026): the module-coverage reporters now treat `ENTRY`, `ENTRY_ORIG`, and `TARGET` metadata as valid restored-symbol sources when attributing `compare_sasc_*` lanes back to asm modules. Wrapper-backed lanes such as the `P_TYPE` parse/reset/write helpers now count toward the underlying direct-export module coverage without needing ad hoc report exceptions.
...
- Practical workflow: once the maintained sweep is green, use `src/decomp/scripts/report_sasc_semantic_audit_candidates.py` to rank lanes whose semantic diff is already empty but whose raw diff is still large. Treat those rows as the current filter-hardening / deeper-audit queue rather than relying on older mismatch notes. When the checkout is already dirty, add `--exclude-dirty` to hide lanes whose SAS/C source, compare script, or semantic filter already has local edits so you can pick a safe non-overlapping audit target first.
...
- Some canonical SAS/C compare scripts still carry the historical source-slice prefix (`parseini2_*`, `parseini3_*`, `script2_*`, `script3_*`) even when the exported function is now triaged by its direct behavior name. Prefer the direct `compare_sasc_<export>_trial.sh` alias when present; for example `compare_sasc_parseini_monitor_clock_change_trial.sh` delegates to the canonical `parseini3` lane and `compare_sasc_script_read_next_rbf_byte_trial.sh` delegates to the canonical `script2` lane.
...
- Exact-word hits are only a starting point: they can come from `extern` declarations or caller references without a local definition. `SIGNAL_PollAndDispatch` was one concrete example, so verify that a candidate symbol is actually defined before treating it as covered.
- Example of that mismatch: `SCRIPT_CopyWeatherUpdateForString` currently builds from `src/decomp/sas_c/script_copy_weather_update_text.c`, which also carries the paired `SCRIPT_CopyWeatherUpdateText` helper. Use `src/decomp/scripts/compare_sasc_script_copy_weather_update_for_string_trial.sh` rather than guessing the source filename from the exported symbol.
- Coverage helper note: `src/decomp/scripts/report_sasc_xdef_coverage.sh` now counts both compare scripts that declare `ENTRY*=` metadata and scripts that inline the original export label directly in an awk extractor. If that report flags an uncovered export, treat it as a stronger signal than the older March 13, 2026 output that incorrectly omitted `FORMAT_U32ToDecimalString` and `UNKNOWN10_PrintfPutcToBuffer`.
...
- Startup-entry note (March 13, 2026): `src/decomp/sas_c/esq_startup_entry.c` currently restores the original `_main/a/a.s` entry ABI (`ESQ_StartupEntry` taking the startup command pointer/length contract), not a hosted SAS/C `main()` signature. Keep that symbol/signature unchanged while the compare lane is anchored to the original export. When the project is ready to build a pure SAS/C executable, introduce a thin CRT-facing wrapper or custom startup stub at that time rather than renaming the restored function now.
- Additional triage note (March 13, 2026, re-validated later the same day): the maintained `run_sasc_core_sweep.sh` blocker set that had been called out in stale notes is green in this checkout. Targeted strict reruns for `string_append_at_null`, `newgrid_init_grid_resources`, `cleanup_build_and_render_aligned_status_banner`, `cleanup_render_aligned_status_screen`, and `diskio1_dump_default_coi_info_block` all currently produce zero-byte semantic diffs, and the `diskio1_dump_default_coi_info_block` SAS/C compare lane compiles successfully.
- Follow-up verification note (March 13, 2026, later re-validated): do not treat stale mismatch notes as authoritative. The previously called-out out-of-sweep semantic diffs in `disptext_is_current_line_last`, `disptext_is_last_line_selected`, and `newgrid_update_grid_state` now resolve to zero-byte semantic diffs in this checkout after the SAS/C semantic filters were tightened to accept normalized `MOVEQ #-1,D0` booleanization and the direct `DISPTEXT_ComputeVisibleLineCount` call shape. Re-triage broader mismatch work from fresh sweep output instead of reusing that older list.
...
- Current hybrid-integration note (March 14, 2026): `modules/groups/a/a/app.s` is now a direct object-level hybrid replacement rather than a passthrough include. Its replacement file carries the APP serial/control helper module body directly, making APP the current direct integration foothold for this serial/control helper cluster.
- Current hybrid-integration note (March 14, 2026): `modules/groups/a/a/app2.s` is now a direct object-level hybrid replacement rather than a passthrough include. Its replacement file carries the APP2 ESQ utility/helper module body directly, making APP2 the current direct integration foothold for this ESQ helper cluster.
- Current hybrid-integration note (March 14, 2026): `modules/groups/_main/a/a.s` is now a direct object-level hybrid replacement rather than a passthrough include. Its replacement file carries the `_main` startup/shutdown helper module body directly, so this `_main` foothold is no longer waiting on a first module-body integration pass.
- Current hybrid-integration note (March 14, 2026): `modules/groups/a/a/bitmap.s` is now a direct object-level hybrid replacement rather than remaining unmapped. Its replacement file carries `BITMAP_ProcessIlbmImage` directly, making BITMAP the current direct integration foothold for this ILBM parsing helper.
- Current hybrid-integration note (March 14, 2026): `modules/groups/a/a/app3.s` is now a direct object-level hybrid replacement rather than a passthrough include. Its replacement file carries `ESQ_InvokeGcommandInit`, `ESQ_SupervisorColdReboot`, and `ESQ_TryRomWriteTest` directly, making APP3 the current direct integration foothold for this reboot/init helper cluster.
- Current hybrid-integration note (March 14, 2026): `modules/groups/a/d/cleanup3.s` is now a direct object-level hybrid replacement rather than a passthrough include. Its replacement file carries the aligned-status banner/screen helper module body directly, making CLEANUP3 the current direct integration foothold for that cluster.
- Current hybrid-integration note (March 14, 2026): `modules/groups/a/e/coi.s` is now a direct object-level hybrid replacement rather than a passthrough include. Its replacement file carries the COI entry/text helper module body directly, making COI the current direct integration foothold for this entry/animation-field helper cluster.
- Current hybrid-integration note (March 14, 2026): `modules/groups/a/f/ctasks.s` is now a direct object-level hybrid replacement rather than a passthrough include. Its replacement file carries the CTASKS task-lifecycle helper module body directly, making CTASKS the current direct integration foothold for this asynchronous task helper cluster.
- Current hybrid-integration note (March 13, 2026, later re-validated): `modules/groups/a/k/ed1.s` is now wired into `src/decomp/replacements.map` as a passthrough hybrid boundary. Its restored SAS/C coverage currently includes `ED1_ClearEscMenuMode`, so future object-level replacement work can land in that ESC-menu / diagnostics helper module without another root include-graph edit.
- Current hybrid-integration note (March 14, 2026): `modules/groups/a/k/ed2.s` is now a direct object-level hybrid replacement rather than remaining unmapped. Its replacement file carries `ED2_DrawEntryDetailsPanel`, `ED2_DrawEntrySummaryPanel`, `ED2_HandleMenuActions`, `ED2_HandleDiagnosticsMenuActions`, and `ED2_HandleScrollSpeedSelection` directly, making ED2 the current direct integration foothold for this entry-summary / diagnostics helper cluster.
- Current hybrid-integration note (March 14, 2026): `modules/groups/a/l/ed3.s` is now a direct object-level hybrid replacement rather than a passthrough include. Its replacement file carries the ED3 drawing-helper module body directly, making ED3 the current direct integration foothold for that helper cluster.
- Current hybrid-integration note (March 14, 2026): `modules/groups/a/m/esq.s` is now a direct object-level hybrid replacement rather than a passthrough include. Its replacement file carries `ESQ_MainInitAndRun` directly, making ESQ the current direct integration foothold for this main startup/init module.
- Current hybrid-integration note (March 14, 2026): `modules/groups/a/a/brush.s` is now a direct object-level hybrid replacement rather than a passthrough include. Its replacement file carries the BRUSH resource/list helper module body directly, making BRUSH the current direct integration foothold for that helper cluster.
- Current hybrid-integration note (March 14, 2026): `modules/groups/b/a/script2.s` is now a direct object-level hybrid replacement rather than a passthrough include. Its replacement file carries the SCRIPT2 serial control / handshake helper module body directly, while retaining the module's `SCRIPT2_JMPTBL_*` wrappers as build glue.
- Current hybrid-integration note (March 14, 2026): `modules/groups/a/s/gcommand.s` is now a direct object-level hybrid replacement rather than a passthrough include. Its replacement file carries the main GCOMMAND parser/template helper module body directly, making GCOMMAND the current direct integration foothold for that helper cluster.
- Current hybrid-integration note (March 14, 2026): `modules/groups/a/t/gcommand2.s` is now a direct object-level hybrid replacement rather than a passthrough include. Its replacement file carries `GCOMMAND_CopyGfxToWorkIfAvailable` and `GCOMMAND_FindPathSeparator` directly, making GCOMMAND2 the current direct integration foothold for this helper cluster.
- Current hybrid-integration note (March 21, 2026): `modules/groups/a/t/xjump.s` is now a direct object-level hybrid replacement rather than a passthrough include. Its replacement file carries `GROUP_AT_JMPTBL_DOS_SystemTagList`, `GROUP_AT_JMPTBL_ED1_WaitForFlagAndClearBit0`, and `GROUP_AT_JMPTBL_ED1_WaitForFlagAndClearBit1` directly now that the maintained sweep still reports those wrapper-backed compare lanes green, making GROUP_AT xjump the current direct integration foothold for this DOS/ED1 wrapper module.
- Current hybrid-integration note (March 21, 2026): `modules/groups/a/d/xjump.s` is now a direct object-level hybrid replacement rather than a passthrough include. Its replacement file carries the full `GROUP_AD_JMPTBL_*` wrapper set directly now that the maintained strict sweep and the wrapper family's dedicated compare lanes still report all 18 semantic diffs clean under tightened wrapper filters, making GROUP_AD xjump the current direct integration foothold for this display/copper/view-mode wrapper module.
- Current hybrid-integration note (March 21, 2026): `modules/groups/a/g/xjump.s` is now a direct object-level hybrid replacement rather than a passthrough include. Its replacement file carries the full `GROUP_AG_JMPTBL_*` wrapper set directly now that the maintained strict sweep and the wrapper family's dedicated compare lanes still report all 17 semantic diffs clean under tightened wrapper filters, making GROUP_AG xjump the current direct integration foothold for this memory/parse/UI-service wrapper module.
- Current hybrid-integration note (March 21, 2026): `modules/groups/a/m/xjump.s` is now a direct object-level hybrid replacement rather than a passthrough include. Its replacement file carries the full `GROUP_AM_JMPTBL_*` wrapper set directly now that the maintained strict sweep and the wrapper family's dedicated compare lanes still report all 31 semantic diffs clean, making GROUP_AM xjump the current direct integration foothold for this startup/init/config wrapper module.
- Current hybrid-integration note (March 21, 2026): `modules/submodules/unknown42.s` remains a direct object-level hybrid replacement. Its replacement file already carries the clock conversion wrappers and parallel-port helper cluster directly, and the current checkout now backs that module with split SAS/C role files plus maintained strict-sweep coverage for the promoted `PARALLEL_*` direct exports.
- Current validation note (March 21, 2026): `PARSE_ReadSignedLong` now has dedicated SAS/C micro-lanes for the internal-label roles `ParseDone`, `ParseLoopEntry`, `ParseLoop`, and `StoreResult`. These use semantic-role filters so the alias-label helper file can validate the intended control-flow role without requiring SAS/C to emit the original in-function labels literally.
- Current validation note (March 21, 2026): the maintained `run_sasc_core_sweep.sh --strict` set now includes those `PARSE_ReadSignedLong` micro-lanes plus the restored Unknown42 `PARALLEL_*` SAS/C compare lanes (`CheckReady`, `CheckReadyStub`, `WaitReady`, `WriteCharD0`, `WriteStringLoop`, `RawDoFmtStackArgs`, `RawDoFmtCommon`, `WriteCharHw`, and `RawDoFmt`), so they are covered by baseline regression sweeps instead of only targeted reruns.
- Current hybrid-integration note (March 14, 2026): `modules/groups/a/u/gcommand3.s` is now a direct object-level hybrid replacement rather than a passthrough include. Its replacement file carries the GCOMMAND3 banner/highlight helper module body directly, making GCOMMAND3 the current direct integration foothold for that helper cluster.
- Current hybrid-integration note (March 14, 2026): `modules/groups/a/u/gcommand4.s` is now a direct object-level hybrid replacement rather than a passthrough include. Its replacement file carries `GCOMMAND_SaveBrushResult` directly, making GCOMMAND4 the current direct integration foothold for this brush-result helper module.
- Current hybrid-integration note (March 21, 2026): `modules/groups/a/f/xjump.s` is now a direct object-level hybrid replacement rather than a passthrough include. Its replacement file carries `GROUP_AF_JMPTBL_GCOMMAND_SaveBrushResult` directly now that the maintained sweep still reports the wrapper-backed compare lane green, making GROUP_AF xjump the current direct integration foothold for this brush-result wrapper module.
- Current hybrid-integration note (March 21, 2026): `modules/groups/a/b/xjump.s` is now a direct object-level hybrid replacement rather than a passthrough include. Its replacement file carries `GROUP_AB_JMPTBL_ESQFUNC_FreeLineTextBuffers`, `GROUP_AB_JMPTBL_ESQIFF2_ClearLineHeadTailByMode`, `GROUP_AB_JMPTBL_ESQIFF_DeallocateAdsAndLogoLstData`, `GROUP_AB_JMPTBL_ESQPARS_RemoveGroupEntryAndReleaseStrings`, `GROUP_AB_JMPTBL_IOSTDREQ_Free`, `GROUP_AB_JMPTBL_LADFUNC_FreeBannerRectEntries`, `GROUP_AB_JMPTBL_LOCAVAIL_FreeResourceChain`, `GROUP_AB_JMPTBL_NEWGRID_ShutdownGridResources`, `GROUP_AB_JMPTBL_UNKNOWN2A_Stub0`, and `GROUP_AB_JMPTBL_GRAPHICS_FreeRaster` directly now that the maintained sweep still reports the wrapper-backed compare set green, making GROUP_AB xjump the current direct integration foothold for this cleanup/free-resource wrapper module.
- Current hybrid-integration note (March 21, 2026): `modules/groups/a/i/xjump.s` is now a direct object-level hybrid replacement rather than a passthrough include. Its replacement file carries `GROUP_AI_JMPTBL_FORMAT_FormatToBuffer2`, `GROUP_AI_JMPTBL_NEWGRID_SetSelectionMarkers`, `GROUP_AI_JMPTBL_STRING_AppendAtNull`, `GROUP_AI_JMPTBL_TLIBA1_DrawTextWithInsetSegments`, `GROUP_AI_JMPTBL_STR_CopyUntilAnyDelimN`, `GROUP_AI_JMPTBL_STR_FindCharPtr`, and `GROUP_AI_JMPTBL_STR_SkipClass3Chars` directly now that the maintained sweep still reports those wrapper-backed compare lanes green, making GROUP_AI xjump the current direct integration foothold for this text/layout wrapper module.
- Current hybrid-integration note (March 21, 2026): `modules/groups/a/r/xjump.s` is now a direct object-level hybrid replacement rather than a passthrough include. Its replacement file carries `GROUP_AR_JMPTBL_PARSEINI_WriteErrorLogEntry` and `GROUP_AR_JMPTBL_STRING_AppendAtNull` directly now that the maintained sweep still reports both wrapper-backed compare lanes green, making GROUP_AR xjump the current direct integration foothold for this log/string wrapper module.
- Current hybrid-integration note (March 21, 2026): `modules/groups/a/y/xjump.s` is now a direct object-level hybrid replacement rather than a passthrough include. Its replacement file carries the full `GROUP_AY_JMPTBL_*` wrapper set directly now that the maintained sweep and the dedicated wrapper compare lanes still report `DISKIO_{CloseBufferedFileAndFlush,ConsumeCStringFromWorkBuffer,LoadFileToWorkBuffer,OpenFileWithBuffer,ParseLongFromWorkBuffer,WriteBufferedBytes,WriteDecimalField}`, `MATH_Mulu32`, `SCRIPT_ReadCiaBBit5Mask`, and `STRING_CompareNoCaseN` semantic-clean, making GROUP_AY xjump the current direct integration foothold for this diskio/math/script/string wrapper module.
- Current hybrid-integration note (March 14, 2026): `modules/groups/a/v/gcommand5.s` is no longer just a passthrough hybrid boundary. Its replacement file now carries the `GCOMMAND_ProcessCtrlCommand` module body directly, making GCOMMAND5 the current direct object-level integration foothold for this helper cluster.
- Current hybrid-integration note (March 14, 2026): `modules/groups/a/v/kybd.s` is no longer just a passthrough hybrid boundary. Its replacement file now carries `KYBD_InitializeInputDevices` directly, making KYBD the next direct object-level integration foothold for this helper cluster.
- Current hybrid-integration note (March 14, 2026): `modules/groups/a/g/diskio1.s` is now a direct object-level hybrid replacement rather than a passthrough include. Its replacement file carries the DISKIO1 compact dump, mask, and attribute-flag helper module body directly, making DISKIO1 the current direct integration foothold for that helper cluster.
- Current hybrid-integration note (March 14, 2026): `modules/groups/a/h/diskio2.s` is now a direct object-level hybrid replacement rather than a passthrough include. Its replacement file carries the DISKIO2 data-file transfer, sync, and ini/oinfo helper module body directly, making DISKIO2 the current direct integration foothold for that helper cluster.
- Current hybrid-integration note (March 14, 2026): `modules/groups/a/i/disptext.s` is now a direct object-level hybrid replacement rather than a passthrough include. Its replacement file carries the DISPTEXT layout/selection helper module body directly, making DISPTEXT the current direct integration foothold for that helper cluster.
- Current hybrid-integration note (March 14, 2026): `modules/groups/a/k/ed.s` is now a direct object-level hybrid replacement rather than remaining unmapped. Its replacement file carries the ED main editor / ESC-state helper module body directly, making ED the current direct integration foothold for that editor-state cluster.
- Current hybrid-integration note (March 20, 2026): `modules/groups/a/k/ed1.s` is now a direct object-level hybrid replacement rather than a passthrough include. Its replacement file carries the ED1 ESC-menu / diagnostics helper module body directly now that `ED1_EnterEscMenu` has restored SAS/C compare coverage, making ED1 the current direct integration foothold for that helper cluster.
- Current hybrid-integration note (March 14, 2026): `modules/groups/a/k/ed2.s` is now a direct object-level hybrid replacement rather than remaining unmapped. Its replacement file carries the ED2 entry-summary, details-panel, and diagnostics helper module body directly, making ED2 the current direct integration foothold for that editor-helper cluster.
- Current hybrid-integration note (March 14, 2026): `modules/groups/a/i/displib.s` is now a direct object-level hybrid replacement rather than a passthrough include. Its replacement file carries the DISPLIB text/layout helper module body directly, making DISPLIB the current direct integration foothold for that helper cluster.
- Current hybrid-integration note (March 14, 2026): `modules/groups/a/n/esqfunc.s` is now a direct object-level hybrid replacement rather than a passthrough include. Its replacement file carries the ESQFUNC status/clock/UI-service helper module body directly, making ESQFUNC the current direct integration foothold for that helper cluster.
- Current hybrid-integration note (March 20, 2026): `modules/groups/a/n/esqdisp.s` is now a direct object-level hybrid replacement rather than a passthrough include. Its replacement file carries the ESQDISP display/status/highlight helper module body directly now that the maintained strict sweep still reports the module's full non-`JMPTBL` compare set green, making ESQDISP the current direct integration foothold for that helper cluster.
- Current hybrid-integration note (March 14, 2026): `modules/groups/a/n/esqiff.s` is now a direct object-level hybrid replacement rather than a passthrough include. Its replacement file carries the ESQIFF external-asset, weather-overlay, and copper-animation helper module body directly, making ESQIFF the current direct integration foothold for that helper cluster.
- Current hybrid-integration note (March 14, 2026): `modules/groups/a/o/esqiff2.s` is now a direct object-level hybrid replacement rather than a passthrough include. Its replacement file carries the ESQIFF2 serial/status ingest helper module body directly, making ESQIFF2 the current direct integration foothold for that helper cluster.
- Current hybrid-integration note (March 14, 2026): `modules/groups/a/o/esqpars.s` is now a direct object-level hybrid replacement rather than a passthrough include. Its replacement file carries the ESQPARS parser/state-persistence helper module body directly, making ESQPARS the current direct integration foothold for that helper cluster.
- Current hybrid-integration note (March 21, 2026): `modules/groups/a/p/esqshared.s` is now a direct object-level hybrid replacement rather than a passthrough include. Its replacement file carries the ESQSHARED entry/default/title-filter helper module body directly now that the maintained strict sweep still reports the module's full non-`JMPTBL` compare set green, so this helper cluster is no longer waiting on a first module-body integration pass.
- Current hybrid-integration note (March 14, 2026): `modules/groups/a/r/flib.s` is now a direct object-level hybrid replacement rather than a passthrough include. Its replacement file carries `FLIB_AppendClockStampedLogEntry` and `FLIB_AppendClockStampedLogEntry_Return` directly, making FLIB the current direct integration foothold for this log-append helper module.
- Current hybrid-integration note (March 14, 2026): `modules/groups/a/s/flib2.s` is now a direct object-level hybrid replacement rather than remaining unmapped. Its replacement file carries the FLIB2 template-default helper module body directly, and the restored SAS/C compare lane now includes direct `FLIB2_LoadDigital{Niche,Mplex,Ppv}Defaults` plus `FLIB2_ResetAndLoadListingTemplates` coverage for this module.
- Current hybrid-integration note (March 14, 2026): `modules/groups/a/x/ladfunc2.s` is now a direct object-level hybrid replacement rather than a passthrough include. Its replacement file carries the LADFUNC2 escaped-string helper module body directly, so this foothold is no longer waiting on a first module-body integration pass.
- Current hybrid-integration note (March 14, 2026): `modules/groups/a/w/ladfunc.s` is now a direct object-level hybrid replacement rather than a passthrough include. Its replacement file carries the LADFUNC text-ad/highlight helper module body directly, so this foothold is no longer waiting on a first module-body integration pass.
- Current hybrid-integration note (March 21, 2026): `modules/groups/a/y/locavail.s` is now a direct object-level hybrid replacement rather than a passthrough include. Its replacement file carries the LOCAVAIL local-availability filter helper module body directly now that the maintained strict sweep still reports the module's full non-`JMPTBL` compare set green.
- Current hybrid-integration note (March 14, 2026): `modules/groups/a/z/locavail2.s` is now a direct object-level hybrid replacement rather than a passthrough include. Its replacement file carries the LOCAVAIL2 intuition override helper module body directly, so this helper cluster is no longer waiting on a first module-body integration pass.
- Current hybrid-integration note (March 21, 2026): `modules/groups/a/z/xjump.s` is now a direct object-level hybrid replacement rather than a passthrough include. Its replacement file carries `GROUP_AZ_JMPTBL_ESQ_ColdReboot` directly now that the maintained sweep still reports the wrapper-backed compare lane green, making GROUP_AZ xjump the current direct integration foothold for this cold-reboot wrapper module.
- Current hybrid-integration note (March 14, 2026): `modules/groups/b/a/parseini.s` is now a direct object-level hybrid replacement rather than a passthrough include. Its replacement file carries the PARSEINI config/weather/font helper module body directly, while retaining the module's `PARSEINI_JMPTBL_*` wrappers as build glue.
- Current hybrid-integration note (March 14, 2026): `modules/groups/b/a/parseini2.s` is now a direct object-level hybrid replacement rather than a passthrough include. Its replacement file carries the PARSEINI2 clock/date/RTC helper module body directly, while retaining the module's `PARSEINI2_JMPTBL_*` wrappers as build glue.
- Current hybrid-integration note (March 14, 2026): `modules/groups/b/a/parseini3.s` is now a direct object-level hybrid replacement rather than a passthrough include. Its replacement file carries the PARSEINI3 clock/error helper module body directly, making PARSEINI3 the current direct integration foothold for that helper cluster.
- Current hybrid-integration note (March 14, 2026): `modules/groups/b/a/p_type.s` is now a direct object-level hybrid replacement rather than a passthrough include. Its replacement file carries the P_TYPE promo-id/group-list helper module body directly, and the restored SAS/C compare coverage for its eleven direct exports now reports as complete in this checkout.
- Current hybrid-integration note (March 14, 2026): `modules/groups/b/a/script.s` is now a direct object-level hybrid replacement rather than a passthrough include. Its replacement file carries the SCRIPT buffer allocation, deallocation, and token-index helper module body directly, while retaining the module's `SCRIPT_JMPTBL_*` wrappers as build glue.
- Current hybrid-integration note (March 14, 2026): `modules/groups/b/a/script3.s` is now a direct object-level hybrid replacement rather than a passthrough include. Its replacement file carries the SCRIPT3 playback/search/banner helper module body directly, while retaining the module's `SCRIPT3_JMPTBL_*` wrappers as build glue.
- Current hybrid-integration note (March 14, 2026): `modules/groups/b/a/script4.s` is now a direct object-level hybrid replacement rather than a passthrough include. Its replacement file carries the SCRIPT4 banner-char/inset/highlight helper module body directly, making SCRIPT4 the current direct integration foothold for that helper cluster.
- Current hybrid-integration note (March 14, 2026): `modules/groups/b/a/wdisp.s` is now a direct object-level hybrid replacement rather than a passthrough include. Its replacement file carries the WDISP weather-status draw/preview helper module body directly, making WDISP the current direct integration foothold for that helper cluster.
- Current hybrid-integration note (March 14, 2026): `modules/groups/b/a/newgrid2.s` is now a direct object-level hybrid replacement rather than a passthrough include. Its replacement file carries the NEWGRID2 state-dispatch and buffer-management helper module body directly, while retaining the module's `NEWGRID2_JMPTBL_*` wrappers as build glue.
- Current hybrid-integration note (March 14, 2026): `modules/groups/b/a/textdisp2.s` is now a direct object-level hybrid replacement rather than a passthrough include. Its replacement file carries the TEXTDISP2 preview/display-state helper module body directly, making TEXTDISP2 the current direct integration foothold for that helper cluster.
- Current hybrid-integration note (March 13, 2026): `modules/groups/b/a/tliba2.s` is now wired into `src/decomp/replacements.map` as a passthrough hybrid boundary. Its restored SAS/C coverage already spans the current non-`JMPTBL` broadcast-window and entry-time helper cluster in that module, so future object-level replacement work can land there without another root include-graph edit.
- Current hybrid-integration note (March 20, 2026): `modules/groups/b/a/tliba1.s` is now a direct object-level hybrid replacement rather than a passthrough include. Its replacement file carries the TLIBA1 formatted-text, clock-format, and style parsing helper module body directly, so this helper cluster is no longer waiting on a first module-body integration pass.
- Current hybrid-integration note (March 20, 2026): `modules/groups/b/a/tliba3.s` is now a direct object-level hybrid replacement rather than a passthrough include. Its replacement file carries the TLIBA3 view-mode, pattern-table, and scale/overlay helper module body directly now that a strict `tliba3` SAS/C rerun is green across the direct and wrapper-backed helper lanes.
- Current TLIBA3 note (March 21, 2026): the restored SAS/C lane now also has direct compare/build coverage for `TLIBA3_BuildDisplayContextForViewMode`, `TLIBA3_DrawCenteredWrappedTextLines`, `TLIBA3_DrawViewModeGuides`, and `TLIBA3_InitPatternTable`, and the maintained strict sweep now runs the direct `TLIBA3_GetViewModeHeight`, `TLIBA3_GetViewModeRastPort`, and `TLIBA3_SelectNextViewMode` checks so TLIBA3 verification no longer depends only on wrapper-backed coverage.
- Current TLIBA3 note (March 22, 2026): the maintained strict sweep now also runs the direct `TLIBA3_ClearViewModeRastPort`, `TLIBA3_Draw{HorizontalScaleTicks,InnerFrameBorder,OuterFrameBorder,VerticalScaleTicks,ViewModeOverlay}`, and `TLIBA3_SetFontForAllViewModes` lanes after the shared `tliba3_view_mode_types.h` runtime-layout pass revalidated those helpers semantic-clean, so the broader TLIBA3 draw cluster now has baseline direct verification rather than relying on a smaller direct foothold plus module-level integration status alone.
- Current WDISP note (March 22, 2026): `WDISP_DrawWeatherStatusOverlay` now reruns semantic-clean under its dedicated SAS/C compare lane with a tightened overlay-specific semantic filter, so the weather-overlay replacement body is backed by an explicit behavior check instead of only the module-level hybrid mapping.
- Current hybrid-integration note (March 20, 2026): `modules/groups/b/a/newgrid.s` is now a direct object-level hybrid replacement rather than a passthrough include. Its replacement file carries the NEWGRID primary grid rendering/input module body directly now that `NEWGRID_ProcessGridMessages` has restored SAS/C compare coverage, making NEWGRID the current direct integration foothold for that helper cluster.
- Current hybrid-integration note (March 20, 2026): `modules/groups/b/a/newgrid1.s` is now a direct object-level hybrid replacement rather than a passthrough include. Its replacement file carries the NEWGRID1 grid drawing, entry scan, and schedule-state helper module body directly now that `NEWGRID_ValidateSelectionCode` has restored SAS/C compare coverage, making NEWGRID1 the current direct integration foothold for that helper cluster.
- Current NEWGRID1 note (March 22, 2026): the maintained strict sweep now also runs the direct `NEWGRID_RebuildIndexCache`, `NEWGRID_ResetRowTable`, `NEWGRID_SetSelectionMarkers`, and `NEWGRID_ValidateSelectionCode` lanes after a fresh rerun kept all four semantic-clean, so NEWGRID1 verification no longer depends only on wrapper-backed coverage plus the earlier single direct foothold.
- Current SCRIPT note (March 22, 2026): fresh direct-alias SAS/C reruns for `GENERATE_GRID_DATE_STRING`, `SCRIPT_BeginBannerCharTransition`, `SCRIPT_HandleSerialCtrlCmd`, `SCRIPT_UpdateCtrlStateMachine`, and `SCRIPT_ESQ_CaptureCtrlBit4StreamBufferByte` all remain semantic-clean in this checkout, so those SCRIPT2/SCRIPT3 helpers now have stable alias compare entry points alongside the canonical `script2_*` and `script3_*` lanes.
- Current Group AM xjump note (March 22, 2026): the restored `modules/groups/a/m/xjump.s` wrapper set now also has readable direct-alias SAS/C compare entry points for the remaining boot/config helper stubs (`BUFFER_FlushAllAndCloseWithCode`, `CLEANUP_ShutdownSystem`, `DISKIO{,2}` config loaders, `ESQ_InitAudio1Dma`, `LADFUNC_*BannerRect*`, `LADFUNC_LoadTextAdsFromFile`, `LOCAVAIL_*`, `P_TYPE_ResetListsAndLoadPromoIds`, `PARSEINI_UpdateClockFromRtc`, `TEXTDISP_LoadSourceConfig`, and `TLIBA3_InitPatternTable`), so Group AM wrapper verification no longer depends on the compressed historical compare-script spellings alone.
- Current PARSEINI xjump note (March 22, 2026): the restored `parseini_jmptbl` wrapper family now also has readable direct-alias SAS/C compare entry points for `ESQFUNC_DrawEscMenuVersion`, `ESQFUNC_RebuildPwBrushListFromTagTable`, `GCOMMAND_FindPathSeparator`, `GCOMMAND_InitPresetTableFromPalette`, and `GCOMMAND_ValidatePresetTable`, and the maintained strict sweep now runs both the readable aliases and the historical compressed spellings so wrapper verification no longer depends on the legacy filenames alone.
- Current hybrid-integration note (March 14, 2026): `modules/groups/b/a/textdisp.s` is now a direct object-level hybrid replacement rather than a passthrough include. Its replacement file carries the TEXTDISP source-config, selection, and text-format helper module body directly, making TEXTDISP the current direct integration foothold for that helper cluster.
- Current hybrid-integration note (March 13, 2026, later re-validated): `modules/groups/b/a/textdisp2.s` is now wired into `src/decomp/replacements.map` as a passthrough hybrid boundary. Its restored SAS/C coverage already spans the current non-`JMPTBL` preview, selection-refresh, and display-state helper cluster in that module, so future object-level replacement work can land there without another root include-graph edit.
- Current hybrid-integration note (March 13, 2026, later re-validated): `modules/groups/b/a/script4.s` is now wired into `src/decomp/replacements.map` as a passthrough hybrid boundary. Its restored SAS/C coverage already spans the non-`JMPTBL` banner-char, inset-text, and highlight-effect helpers in that module, so future object-level replacement work can land there without another root include-graph edit.
- Current hybrid-integration note (March 20, 2026): `modules/groups/b/a/textdisp3.s` is now a direct object-level hybrid replacement rather than a passthrough include. Its replacement file carries the TEXTDISP3 text/layout helper module body directly now that the maintained strict verification still reports the module's full non-`JMPTBL` compare set semantic-clean, making TEXTDISP3 the current direct integration foothold for that helper cluster.
- Current hybrid-integration note (March 20, 2026): `modules/groups/a/j/disptext2.s` is now a direct object-level hybrid replacement rather than a passthrough include. Its replacement file carries the DATETIME conversion/format helper module body directly now that `DATETIME_ParseString` and `DATETIME_UpdateSelectionField` have restored SAS/C compare coverage, making DISPText2 the current direct integration foothold for that helper cluster.
- Current hybrid-integration note (March 14, 2026): `modules/submodules/unknown40.s` is now a direct object-level hybrid replacement rather than a passthrough include. Its replacement file carries `BATTCLOCK_GetSecondsFromBatteryBackedClock`, `BATTCLOCK_WriteSecondsToBatteryBackedClock`, `DOS_Delay`, `DOS_SystemTagList`, and `EXEC_CallVector_48` directly, so this battery-clock / DOS wrapper helper cluster is no longer waiting on a first module-body integration pass.
- Current hybrid-integration note (March 20, 2026): `modules/submodules/unknown15.s` is now a direct object-level hybrid replacement rather than a passthrough include. Its replacement file carries `STREAM_ReadLineWithLimit` directly, making UNKNOWN15 the current direct integration foothold for this stream line-reader helper.
- Current hybrid-integration note (March 20, 2026): `modules/submodules/unknown5.s` is now a direct object-level hybrid replacement rather than a passthrough include. Its replacement file carries the UNKNOWN5 string helper module body directly, making UNKNOWN5 the current direct integration foothold for `STRING_AppendN`, `STRING_CompareN`, `STRING_CompareNoCase`, `STRING_CompareNoCaseN`, and `STRING_CopyPadNul`.
- Current hybrid-integration note (March 13, 2026, later re-validated): `modules/submodules/unknown9.s`, `unknown12.s`, `unknown16.s`-`unknown20.s`, `unknown23.s`, `unknown28.s`, `unknown31.s`, `unknown35.s`, and `unknown37.s` are now wired into `src/decomp/replacements.map` as passthrough or direct hybrid boundaries around their current single-helper footholds, covering the octal formatter, `ALLOC_AllocFromFreeList`, the buffered close and `DOS_*WithErrorState` wrappers, `HANDLE_GetEntryByIndex`, `WDISP_FormatWithCallback`, `BUFFER_EnsureAllocated`, `HANDLE_OpenWithMode`, and `HANDLE_CloseByIndex`.
- Current hybrid-integration note (March 20, 2026): `modules/submodules/unknown6.s`, `unknown8.s`, and `unknown39.s` are now direct object-level hybrid replacements rather than passthrough includes. Their replacement files carry `STRING_AppendAtNull`, `FORMAT_U32ToDecimalString`, and `GRAPHICS_BltBitMapRastPort` directly.
- Current hybrid-integration note (March 14, 2026): `modules/submodules/unknown9.s`, `unknown28.s`, `unknown31.s`, `unknown32.s`, `unknown35.s`, and `unknown37.s` are now direct object-level hybrid replacements rather than passthrough includes. Their replacement files carry `FORMAT_U32ToOctalString`, `WDISP_FormatWithCallback`, `BUFFER_EnsureAllocated`, `HANDLE_CloseAllAndReturnWithCode` (while retaining `UNKNOWN32_JMPTBL_ESQ_ReturnWithStackCode` as build glue), `HANDLE_OpenWithMode`, and `HANDLE_CloseByIndex` directly.
- Current hybrid-integration note (March 14, 2026): `modules/submodules/unknown23.s` is now a direct object-level hybrid replacement rather than a passthrough include. Its replacement file carries `HANDLE_GetEntryByIndex` directly, making UNKNOWN23 the current direct integration foothold for this handle-table helper.
- Current hybrid-integration note (March 14, 2026): `modules/submodules/unknown12.s` is now a direct object-level hybrid replacement rather than a passthrough include. Its replacement file carries `ALLOC_AllocFromFreeList` directly, making UNKNOWN12 the current direct integration foothold for this free-list allocator helper.
- Current hybrid-integration note (March 14, 2026): `modules/submodules/unknown16.s` is now a direct object-level hybrid replacement rather than a passthrough include. Its replacement file carries `BUFFER_FlushAllAndCloseWithCode` directly, making UNKNOWN16 the current direct integration foothold for this buffered-close helper.
- Current hybrid-integration note (March 14, 2026): `modules/submodules/unknown17.s` is now a direct object-level hybrid replacement rather than a passthrough include. Its replacement file carries `DOS_WriteWithErrorState` directly, making UNKNOWN17 the current direct integration foothold for this write-with-error helper.
- Current hybrid-integration note (March 14, 2026): `modules/submodules/unknown18.s`, `unknown19.s`, and `unknown20.s` are now direct object-level hybrid replacements rather than passthrough includes. Their replacement files carry `DOS_SeekWithErrorState`, `DOS_ReadWithErrorState`, and `DOS_OpenWithErrorState` directly, making UNKNOWN18/19/20 the current direct integration footholds for this DOS error-wrapper helper cluster.
- Current hybrid-integration note (March 14, 2026): `modules/submodules/unknown10.s` is now a direct object-level hybrid replacement rather than a passthrough include. Its replacement file carries the UNKNOWN10 format/parse/open helper module body directly, so this foothold is no longer waiting on a first module-body integration pass.
- Current hybrid-integration note (March 13, 2026, later re-validated): `modules/submodules/unknown13.s`, `modules/submodules/unknown25.s`, `modules/submodules/unknown27.s`, `modules/submodules/unknown33.s`, and `modules/submodules/unknown36.s` are now wired into `src/decomp/replacements.map` as passthrough hybrid boundaries. Their green GCC promotion lanes already cover the current non-`JMPTBL` helper sets for callback formatting (`FORMAT_CallbackWriteChar`, `FORMAT_FormatToCallbackBuffer`), owned-struct allocation (`STRUCT_AllocWithOwner`, `STRUCT_FreeWithSizeField`), secondary formatter buffering (`FORMAT_Buffer2WriteChar`, `FORMAT_FormatToBuffer2`), allocator/string search (`ALLOC_InsertFreeBlock`, `STRING_FindSubstring`), and abort-request handling (`UNKNOWN36_FinalizeRequest`, `UNKNOWN36_ShowAbortRequester`), so future object-level replacement work can land in those helper modules without another root include-graph edit.
- Current hybrid-integration note (March 14, 2026): `modules/submodules/unknown33.s` is now a direct object-level hybrid replacement rather than a passthrough include. Its replacement file carries `ALLOC_InsertFreeBlock` and `STRING_FindSubstring` directly after the SAS/C compare lane metadata was updated so coverage reports correctly attribute `compare_sasc_string_find_substring_trial.sh` back to the UNKNOWN33 module.
- Current hybrid-integration note (March 14, 2026): `modules/submodules/unknown13.s` is now a direct object-level hybrid replacement rather than a passthrough include. Its replacement file carries the callback formatter helper pair directly (`FORMAT_CallbackWriteChar`, `FORMAT_FormatToCallbackBuffer`), so this formatting foothold is no longer waiting on a first module-body integration pass.
- Current hybrid-integration note (March 14, 2026): `modules/submodules/unknown36.s` is now a direct object-level hybrid replacement rather than a passthrough include. Its replacement file carries the abort-request helper pair directly (`UNKNOWN36_FinalizeRequest`, `UNKNOWN36_ShowAbortRequester`), so this requester/abort foothold is no longer waiting on a first module-body integration pass.
- Current hybrid-integration note (March 14, 2026): `modules/submodules/unknown4.s` is now a direct object-level hybrid replacement rather than a passthrough include. Its replacement file carries `STRING_ToUpperInPlace` directly, making UNKNOWN4 the current direct integration foothold for this uppercase helper.
- Current hybrid-integration note (March 14, 2026): `modules/submodules/unknown11.s` is now a direct object-level hybrid replacement rather than a passthrough include. Its replacement file carries `DOS_SeekByIndex` directly, making UNKNOWN11 the current direct integration foothold for this DOS seek helper.
- Current hybrid-integration note (March 14, 2026): `modules/submodules/unknown14.s` is now a direct object-level hybrid replacement rather than a passthrough include. Its replacement file carries `HANDLE_OpenFromModeString` directly, making UNKNOWN14 the current direct integration foothold for this handle-open helper.
- Current hybrid-integration note (March 14, 2026): `modules/submodules/unknown26.s` is now a direct object-level hybrid replacement rather than a passthrough include. Its replacement file carries `DOS_WriteByIndex` directly, making UNKNOWN26 the current direct integration foothold for this indexed DOS write helper.
- Current hybrid-integration note (March 14, 2026): `modules/submodules/unknown21.s` is now a direct object-level hybrid replacement rather than a passthrough include. Its replacement file carries the UNKNOWN21 DOS/file-create and IOStdReq cleanup helper module body directly, so this helper cluster is no longer waiting on a first module-body integration pass.
- Current hybrid-integration note (March 20, 2026): `modules/submodules/unknown22.s` is now a direct object-level hybrid replacement rather than a passthrough include. Its replacement file carries the UNKNOWN22 arithmetic/msgport helper module body directly for `ALLOCATE_AllocAndInitializeIOStdReq`, `DOS_CloseWithSignalCheck`, `MATH_DivS32`, `MATH_DivU32`, `MATH_Mulu32`, and `SIGNAL_CreateMsgPortWithSignal`.
- Current hybrid-integration note (March 20, 2026): `modules/submodules/unknown24.s` is now a direct object-level hybrid replacement rather than a passthrough include. Its replacement file carries the UNKNOWN24 memlist/signed-long helper module body directly, making UNKNOWN24 the current direct integration foothold for `MEMLIST_AllocTracked`, `MEMLIST_FreeAll`, `PARSE_ReadSignedLongSkipClass3`, and `PARSE_ReadSignedLongSkipClass3_Alt`.
- Current hybrid-integration note (March 14, 2026): `modules/submodules/unknown29.s` is now a direct object-level hybrid replacement rather than a passthrough include. Its replacement file carries `ESQ_ParseCommandLineAndRun` directly and preserves `UNKNOWN29_JMPTBL_ESQ_MainInitAndRun` as build glue for the startup handoff.
- Current hybrid-integration note (March 13, 2026): `modules/submodules/unknown.s`, `modules/submodules/unknown2a.s`, and `modules/submodules/unknown30.s` are now wired into `src/decomp/replacements.map` as passthrough hybrid boundaries. Their restored SAS/C coverage already spans the current protocol/status-parse helper cluster (`ESQPROTO_CopyLabelToGlobal`, `ESQPROTO_ParseDigitLabelAndDisplay`, `ESQPROTO_VerifyChecksumAndParseList`, `ESQPROTO_VerifyChecksumAndParseRecord`, `UNKNOWN_ParseListAndUpdateEntries`, and `UNKNOWN_ParseRecordAndUpdateDisplay`), the formatter foothold (`FORMAT_RawDoFmtWithScratchBuffer`, `UNKNOWN2A_Stub0`), and the Exec helper foothold (`EXEC_CallVector348`), so future object-level replacement work can land in those helper modules without another root include-graph edit.
- Current hybrid-integration note (March 13, 2026): `modules/submodules/unknown32.s` is now wired into `src/decomp/replacements.map` as a passthrough hybrid boundary. Its restored SAS/C coverage already spans the current non-`JMPTBL` handle-close/return helper (`HANDLE_CloseAllAndReturnWithCode`), so future object-level replacement work can land there without another root include-graph edit.
- Current hybrid-integration note (March 14, 2026): `modules/submodules/unknown32.s` is now a direct object-level hybrid replacement rather than a passthrough include. Its replacement file carries `HANDLE_CloseAllAndReturnWithCode` directly and keeps the module's `UNKNOWN32_JMPTBL_ESQ_ReturnWithStackCode` wrapper as build glue.
- Current hybrid-integration note (March 14, 2026): `modules/submodules/unknown34.s` is now a direct object-level hybrid replacement rather than a passthrough include. Its replacement file carries the list/memory/read helper module body directly, so this UNKNOWN34 foothold is no longer waiting on a first module-body integration pass.
- Current hybrid-integration note (March 14, 2026): `modules/submodules/unknown38.s` is now a direct object-level hybrid replacement rather than a passthrough include. Its replacement file carries `SIGNAL_PollAndDispatch` directly, making UNKNOWN38 the current direct integration foothold for this signal/exit helper.
- Current hybrid-integration note (March 13, 2026, later re-validated): `modules/submodules/unknown7.s` is now wired into `src/decomp/replacements.map` as a passthrough hybrid boundary. Its restored SAS/C coverage already spans the current non-`JMPTBL` string delimiter/search helpers (`STR_CopyUntilAnyDelimN`, `STR_FindChar`, `STR_FindCharPtr`, `STR_FindAnyCharInSet`, `STR_FindAnyCharPtr`, and `STR_SkipClass3Chars`), so future object-level replacement work can land there without another root include-graph edit.
...
- `src/decomp/scripts/report_passthrough_integration_candidates.py`: filters the mapped-module candidate list down to complete module boundaries whose replacement file is still a pure passthrough `include` of the canonical asm source, making the current object-integration queue explicit. Pass `--details` to print the covered exports and compare-script backing for a selected candidate, or `--verify` to rerun the compare scripts and summarize exact/raw-diff-clean versus semantic-only readiness.
- `src/decomp/scripts/report_remaining_decomp_scope.py`: walks the asm tree directly and reports how much code-module scope is still outside `src/decomp/replacements.map`, separated from known infrastructure include files.
- `src/decomp/scripts/report_replacement_only_modules.py`: lists replacement-map rows that have no promotion or compare-lane backing, which is mainly useful for intentional include mirrors such as `interrupts/constants.s`.
- `src/decomp/scripts/report_sasc_semantic_audit_candidates.py`: ranks current `build/decomp/sasc_trial/*.diff` lanes whose semantic diff is already empty but raw diff size still suggests they deserve semantic-filter review or a deeper asm/C audit.
...
- `src/decomp/scripts/compare_sasc_memory_deallocate_trial.sh`: standalone SAS/C compare lane for `MEMORY_DeallocateMemory`.
...
- Newly added direct lane: `src/decomp/scripts/compare_sasc_override_intuition_funcs_trial.sh` (`OVERRIDE_INTUITION_FUNCS`) with semantic gate `src/decomp/scripts/semantic_filter_sasc_override_intuition_funcs.awk`.
- Newly added direct lane: `src/decomp/scripts/compare_sasc_kybd_initialize_input_devices_trial.sh` (`KYBD_InitializeInputDevices`) with semantic gate `src/decomp/scripts/semantic_filter_sasc_kybd_initialize_input_devices.awk`.
- Newly added direct lane: `src/decomp/scripts/compare_sasc_ladfunc2_emit_escaped_char_to_scratch_trial.sh` (`LADFUNC2_EmitEscapedCharToScratch`) with semantic gate `src/decomp/scripts/semantic_filter_sasc_ladfunc2_emit_escaped_char_to_scratch.awk`.
...
- Newly added direct lane: `src/decomp/scripts/compare_sasc_gcommand_find_path_separator_trial.sh` (`GCOMMAND_FindPathSeparator`) with semantic gate `src/decomp/scripts/semantic_filter_sasc_gcommand_find_path_separator.awk`.
- Newly added direct lane: `src/decomp/scripts/compare_sasc_gcommand_copy_gfx_to_work_if_available_trial.sh` (`GCOMMAND_CopyGfxToWorkIfAvailable`) with semantic gate `src/decomp/scripts/semantic_filter_sasc_gcommand_copy_gfx_to_work_if_available.awk`.
- Newly added direct lane: `src/decomp/scripts/compare_sasc_gcommand_process_ctrl_command_trial.sh` (`GCOMMAND_ProcessCtrlCommand`) with semantic gate `src/decomp/scripts/semantic_filter_sasc_gcommand_process_ctrl_command.awk`.
- Newly added direct lane: `src/decomp/scripts/compare_sasc_gcommand_load_command_file_trial.sh` (`GCOMMAND_LoadCommandFile`) with semantic gate `src/decomp/scripts/semantic_filter_sasc_gcommand_load_command_file.awk`.
- Newly added direct lane: `src/decomp/scripts/compare_sasc_gcommand_load_default_table_trial.sh` (`GCOMMAND_LoadDefaultTable`) with semantic gate `src/decomp/scripts/semantic_filter_sasc_gcommand_load_default_table.awk`.
- Newly added direct lane: `src/decomp/scripts/compare_sasc_gcommand_load_mplex_file_trial.sh` (`GCOMMAND_LoadMplexFile`) with semantic gate `src/decomp/scripts/semantic_filter_sasc_gcommand_load_mplex_file.awk`.
- Newly added direct lane: `src/decomp/scripts/compare_sasc_gcommand_load_mplex_template_trial.sh` (`GCOMMAND_LoadMplexTemplate`) with semantic gate `src/decomp/scripts/semantic_filter_sasc_gcommand_load_mplex_template.awk`.
- Newly added direct lane: `src/decomp/scripts/compare_sasc_gcommand_load_ppv3_template_trial.sh` (`GCOMMAND_LoadPPV3Template`) with semantic gate `src/decomp/scripts/semantic_filter_sasc_gcommand_load_ppv3_template.awk`.
- Newly added direct lane: `src/decomp/scripts/compare_sasc_gcommand_load_ppv_template_trial.sh` (`GCOMMAND_LoadPPVTemplate`) with semantic gate `src/decomp/scripts/semantic_filter_sasc_gcommand_load_ppv_template.awk`.
- Newly added direct lane: `src/decomp/scripts/compare_sasc_gcommand_parse_command_options_trial.sh` (`GCOMMAND_ParseCommandOptions`) with semantic gate `src/decomp/scripts/semantic_filter_sasc_gcommand_parse_command_options.awk`.
- Newly added direct lane: `src/decomp/scripts/compare_sasc_gcommand_parse_command_string_trial.sh` (`GCOMMAND_ParseCommandString`) with semantic gate `src/decomp/scripts/semantic_filter_sasc_gcommand_parse_command_string.awk`.
- Newly added direct lane: `src/decomp/scripts/compare_sasc_gcommand_parse_ppv_command_trial.sh` (`GCOMMAND_ParsePPVCommand`) with semantic gate `src/decomp/scripts/semantic_filter_sasc_gcommand_parse_ppv_command.awk`.
- Newly added direct lane: `src/decomp/scripts/compare_sasc_gcommand_save_brush_result_trial.sh` (`GCOMMAND_SaveBrushResult`) with semantic gate `src/decomp/scripts/semantic_filter_sasc_gcommand_save_brush_result.awk`.
- Newly added direct lane: `src/decomp/scripts/compare_sasc_gcommand_reset_preset_work_tables_trial.sh` (`GCOMMAND_ResetPresetWorkTables`) with semantic gate `src/decomp/scripts/semantic_filter_sasc_gcommand_reset_preset_work_tables.awk`.
...
- Newly added direct lane: `src/decomp/scripts/compare_sasc_gcommand_service_highlight_messages_trial.sh` (`GCOMMAND_ServiceHighlightMessages`) with semantic gate `src/decomp/scripts/semantic_filter_sasc_gcommand_service_highlight_messages.awk`.
...
- Standalone memory-deallocate SAS/C compare script: `src/decomp/scripts/compare_sasc_memory_deallocate_trial.sh`
...
- ESQFUNC `ESQFUNC_TrimTextToPixelWidthWordBoundary` SAS/C compare script: `src/decomp/scripts/compare_sasc_esqfunc_trim_text_to_pixel_width_word_boundary_trial.sh`
...
- ESQDISP `ESQDISP_TestEntryBits0And2` SAS/C compare script: `src/decomp/scripts/compare_sasc_esqdisp_test_entry_bits0_and2_trial.sh`
- ESQDISP `ESQDISP_TestEntryGridEligibility` SAS/C compare script: `src/decomp/scripts/compare_sasc_esqdisp_test_entry_grid_eligibility_trial.sh`
...
- Utility `STRING_FindSubstring` SAS/C compare script: `src/decomp/scripts/compare_sasc_string_find_substring_trial.sh`
...
- Utility `STRING_ToUpperInPlace` SAS/C compare script: `src/decomp/scripts/compare_sasc_string_toupper_inplace_trial.sh`
...
- Group AM `GROUP_AM_JMPTBL_LOCAVAIL_ResetFilterStateStruct` direct alias compare script: `src/decomp/scripts/compare_sasc_group_am_jmptbl_locavail_reset_filter_state_struct_trial.sh`
...
- Group AM `GROUP_AM_JMPTBL_ESQ_InitAudio1Dma` direct alias compare script: `src/decomp/scripts/compare_sasc_group_am_jmptbl_esq_init_audio1_dma_trial.sh`
...
- Group AM `GROUP_AM_JMPTBL_LOCAVAIL_LoadAvailabilityDataFile` direct alias compare script: `src/decomp/scripts/compare_sasc_group_am_jmptbl_locavail_load_availability_data_file_trial.sh`
...
- Group AM `GROUP_AM_JMPTBL_GCOMMAND_InitPresetDefaults` direct alias compare script: `src/decomp/scripts/compare_sasc_group_am_jmptbl_gcommand_init_preset_defaults_trial.sh`
...
- Group AM `GROUP_AM_JMPTBL_BUFFER_FlushAllAndCloseWithCode` direct alias compare script: `src/decomp/scripts/compare_sasc_group_am_jmptbl_buffer_flush_all_and_close_with_code_trial.sh`
...
- Group AM `GROUP_AM_JMPTBL_FLIB2_ResetAndLoadListingTemplates` direct alias compare script: `src/decomp/scripts/compare_sasc_group_am_jmptbl_flib2_reset_and_load_listing_templates_trial.sh`
...
- Group AM `GROUP_AM_JMPTBL_ESQ_SetCopperEffect_OffDisableHighlight` direct alias compare script: `src/decomp/scripts/compare_sasc_group_am_jmptbl_esq_set_copper_effect_off_disable_highlight_trial.sh`
...
- Group AM `GROUP_AM_JMPTBL_CLEANUP_ShutdownSystem` direct alias compare script: `src/decomp/scripts/compare_sasc_group_am_jmptbl_cleanup_shutdown_system_trial.sh`
...
- Group AM `GROUP_AM_JMPTBL_LADFUNC_AllocBannerRectEntries` direct alias compare script: `src/decomp/scripts/compare_sasc_group_am_jmptbl_ladfunc_alloc_banner_rect_entries_trial.sh`
- Group AM `GROUP_AM_JMPTBL_KYBD_InitializeInputDevices` direct alias compare script: `src/decomp/scripts/compare_sasc_group_am_jmptbl_kybd_initialize_input_devices_trial.sh`
- Group AM `GROUP_AM_JMPTBL_GCOMMAND_ResetBannerFadeState` direct alias compare script: `src/decomp/scripts/compare_sasc_group_am_jmptbl_gcommand_reset_banner_fade_state_trial.sh`
- Group AM `GROUP_AM_JMPTBL_ESQ_SetCopperEffect_OnEnableHighlight` direct alias compare script: `src/decomp/scripts/compare_sasc_group_am_jmptbl_esq_set_copper_effect_on_enable_highlight_trial.sh`
- Group AM `GROUP_AM_JMPTBL_PARSEINI_UpdateClockFromRtc` direct alias compare script: `src/decomp/scripts/compare_sasc_group_am_jmptbl_parseini_update_clock_from_rtc_trial.sh`
- Group AM `GROUP_AM_JMPTBL_P_TYPE_ResetListsAndLoadPromoIds` direct alias compare script: `src/decomp/scripts/compare_sasc_group_am_jmptbl_p_type_reset_lists_and_load_promo_ids_trial.sh`
- Group AM `GROUP_AM_JMPTBL_LADFUNC_LoadTextAdsFromFile` direct alias compare script: `src/decomp/scripts/compare_sasc_group_am_jmptbl_ladfunc_load_text_ads_from_file_trial.sh`
- Group AM `GROUP_AM_JMPTBL_DISKIO_LoadConfigFromDisk` direct alias compare script: `src/decomp/scripts/compare_sasc_group_am_jmptbl_diskio_load_config_from_disk_trial.sh`
- Group AM `GROUP_AM_JMPTBL_TEXTDISP_LoadSourceConfig` direct alias compare script: `src/decomp/scripts/compare_sasc_group_am_jmptbl_textdisp_load_source_config_trial.sh`
- Group AM `GROUP_AM_JMPTBL_TLIBA3_InitPatternTable` direct alias compare script: `src/decomp/scripts/compare_sasc_group_am_jmptbl_tliba3_init_pattern_table_trial.sh`
- Group AM `GROUP_AM_JMPTBL_DISKIO2_ParseIniFileFromDisk` direct alias compare script: `src/decomp/scripts/compare_sasc_group_am_jmptbl_diskio2_parse_ini_file_from_disk_trial.sh`
...
- TEXTDISP2 `TEXTDISP_TickDisplayState` SAS/C compare script: `src/decomp/scripts/compare_sasc_textdisp2_tick_display_state_trial.sh`
...
- PARSEINI2 `PARSEINI_WriteRtcFromGlobals` SAS/C compare script: `src/decomp/scripts/compare_sasc_parseini2_write_rtc_from_globals_trial.sh`
- PARSEINI3 `PARSEINI_CheckCtrlHChange` SAS/C compare script: `src/decomp/scripts/compare_sasc_parseini3_check_ctrl_h_change_trial.sh`
...
- SCRIPT3 `GENERATE_GRID_DATE_STRING` direct alias compare script: `src/decomp/scripts/compare_sasc_script_generate_grid_date_string_trial.sh`
...
- SCRIPT3 `SCRIPT_ApplyPendingBannerTarget` SAS/C compare script: `src/decomp/scripts/compare_sasc_script3_apply_pending_banner_target_trial.sh`
...
- SCRIPT3 `SCRIPT_SelectPlaybackCursorFromSearchText` SAS/C compare script: `src/decomp/scripts/compare_sasc_script3_select_playback_cursor_from_search_text_trial.sh`
- SCRIPT3 `SCRIPT_UpdateBannerCharTransition` SAS/C compare script: `src/decomp/scripts/compare_sasc_script3_update_banner_char_transition_trial.sh`
- SCRIPT3 `SCRIPT_BeginBannerCharTransition` SAS/C compare script: `src/decomp/scripts/compare_sasc_script3_begin_banner_char_transition_trial.sh`
- SCRIPT3 `SCRIPT_BeginBannerCharTransition` direct alias compare script: `src/decomp/scripts/compare_sasc_script_begin_banner_char_transition_trial.sh`
- SCRIPT3 `SCRIPT_UpdateCtrlStateMachine` SAS/C compare script: `src/decomp/scripts/compare_sasc_script3_update_ctrl_state_machine_trial.sh`
- SCRIPT3 `SCRIPT_UpdateCtrlStateMachine` direct alias compare script: `src/decomp/scripts/compare_sasc_script_update_ctrl_state_machine_trial.sh`
...
- SCRIPT3 `SCRIPT_HandleSerialCtrlCmd` direct alias compare script: `src/decomp/scripts/compare_sasc_script_handle_serial_ctrl_cmd_trial.sh`
- SCRIPT4 `SCRIPT_ResetBannerCharDefaults` SAS/C compare script: `src/decomp/scripts/compare_sasc_script_reset_banner_char_defaults_trial.sh`
- SCRIPT4 `SCRIPT_GetBannerCharOrFallback` SAS/C compare script: `src/decomp/scripts/compare_sasc_script_get_banner_char_or_fallback_trial.sh`
- SCRIPT4 `SCRIPT_DrawInsetTextWithFrame` SAS/C compare script: `src/decomp/scripts/compare_sasc_script_draw_inset_text_with_frame_trial.sh`
- SCRIPT4 `SCRIPT_SetupHighlightEffect` SAS/C compare script: `src/decomp/scripts/compare_sasc_script_setup_highlight_effect_trial.sh`
- SCRIPT `SCRIPT_AllocateBufferArray` SAS/C compare script: `src/decomp/scripts/compare_sasc_script_allocate_buffer_array_trial.sh`
- SCRIPT `SCRIPT_DeallocateBufferArray` SAS/C compare script: `src/decomp/scripts/compare_sasc_script_deallocate_buffer_array_trial.sh`
- SCRIPT `SCRIPT_BuildTokenIndexMap` SAS/C compare script: `src/decomp/scripts/compare_sasc_script_build_token_index_map_trial.sh`
- SCRIPT2 `SCRIPT_ReadHandshakeBit3Flag` SAS/C compare script: `src/decomp/scripts/compare_sasc_script2_read_handshake_bit3_flag_trial.sh`
- SCRIPT2 `SCRIPT_ReadHandshakeBit5Mask` SAS/C compare script: `src/decomp/scripts/compare_sasc_script2_read_handshake_bit5_mask_trial.sh`
- SCRIPT2 `SCRIPT_GetCtrlLineFlag` SAS/C compare script: `src/decomp/scripts/compare_sasc_script2_get_ctrl_line_flag_trial.sh`
- SCRIPT2 `SCRIPT_WriteCtrlShadowToSerdat` SAS/C compare script: `src/decomp/scripts/compare_sasc_script2_write_ctrl_shadow_to_serdat_trial.sh`
- SCRIPT2 `SCRIPT_AssertCtrlLine` SAS/C compare script: `src/decomp/scripts/compare_sasc_script2_assert_ctrl_line_trial.sh`
- SCRIPT2 `SCRIPT_DeassertCtrlLine` SAS/C compare script: `src/decomp/scripts/compare_sasc_script2_deassert_ctrl_line_trial.sh`
- SCRIPT2 `SCRIPT_AssertCtrlLineIfEnabled` SAS/C compare script: `src/decomp/scripts/compare_sasc_script2_assert_ctrl_line_if_enabled_trial.sh`
- SCRIPT2 `SCRIPT_ClearCtrlLineIfEnabled` SAS/C compare script: `src/decomp/scripts/compare_sasc_script2_clear_ctrl_line_if_enabled_trial.sh`
- SCRIPT2 `SCRIPT_AssertCtrlLineNow` SAS/C compare script: `src/decomp/scripts/compare_sasc_script2_assert_ctrl_line_now_trial.sh`
- SCRIPT2 `SCRIPT_DeassertCtrlLineNow` SAS/C compare script: `src/decomp/scripts/compare_sasc_script2_deassert_ctrl_line_now_trial.sh`
- SCRIPT2 `SCRIPT_ReadNextRbfByte` SAS/C compare script: `src/decomp/scripts/compare_sasc_script2_read_next_rbf_byte_trial.sh`
- SCRIPT2 `SCRIPT_ESQ_CaptureCtrlBit4StreamBufferByte` SAS/C compare script: `src/decomp/scripts/compare_sasc_script2_esq_capture_ctrl_bit4_stream_buffer_byte_trial.sh`
- SCRIPT2 `SCRIPT_ESQ_CaptureCtrlBit4StreamBufferByte` direct alias compare script: `src/decomp/scripts/compare_sasc_script_esq_capture_ctrl_bit4_stream_buffer_byte_trial.sh`
- SCRIPT2 `SCRIPT_UpdateSerialShadowFromCtrlByte` SAS/C compare script: `src/decomp/scripts/compare_sasc_script2_update_serial_shadow_from_ctrl_byte_trial.sh`
- SCRIPT2 `SCRIPT_PollHandshakeAndApplyTimeout` SAS/C compare script: `src/decomp/scripts/compare_sasc_script2_poll_handshake_and_apply_timeout_trial.sh`
...
- TEXTDISP `TEXTDISP_HandleScriptCommand` SAS/C compare script: `src/decomp/scripts/compare_sasc_textdisp_handle_script_command_trial.sh`
- TEXTDISP `TEXTDISP_DrawHighlightFrame` SAS/C compare script: `src/decomp/scripts/compare_sasc_textdisp_draw_highlight_frame_trial.sh`
- TEXTDISP `TEXTDISP_FilterAndSelectEntry` SAS/C compare script: `src/decomp/scripts/compare_sasc_textdisp_filter_and_select_entry_trial.sh`
- TEXTDISP `TEXTDISP_BuildEntryDetailLine` SAS/C compare script: `src/decomp/scripts/compare_sasc_textdisp_build_entry_detail_line_trial.sh`
- TEXTDISP `TEXTDISP_BuildNowShowingStatusLine` SAS/C compare script: `src/decomp/scripts/compare_sasc_textdisp_build_now_showing_status_line_trial.sh`
- TEXTDISP `TEXTDISP_BuildEntryPairStatusLine` SAS/C compare script: `src/decomp/scripts/compare_sasc_textdisp_build_entry_pair_status_line_trial.sh`
...
- TEXTDISP `TEXTDISP_SetEntryTextFields` SAS/C compare script: `src/decomp/scripts/compare_sasc_textdisp_set_entry_text_fields_trial.sh`
...
- TEXTDISP `TEXTDISP_ShouldOpenEditorForEntry` SAS/C compare script: `src/decomp/scripts/compare_sasc_textdisp_should_open_editor_for_entry_trial.sh`
...
- Unknown42 `PARALLEL_WriteStringLoop` SAS/C compare script: `src/decomp/scripts/compare_sasc_parallel_write_string_loop_trial.sh` (legacy alias: `compare_sasc_parallel_writestringloop_trial.sh`)
- Unknown42 `PARALLEL_RawDoFmtStackArgs` SAS/C compare script: `src/decomp/scripts/compare_sasc_parallel_raw_dofmt_stack_args_trial.sh` (legacy alias: `compare_sasc_parallel_rawdofmtstackargs_trial.sh`)
- Unknown42 `PARALLEL_RawDoFmtCommon` SAS/C compare script: `src/decomp/scripts/compare_sasc_parallel_raw_dofmt_common_trial.sh` (legacy alias: `compare_sasc_parallel_rawdofmtcommon_trial.sh`)
- Unknown42 `PARALLEL_WriteCharHw` SAS/C compare script: `src/decomp/scripts/compare_sasc_parallel_write_char_hw_trial.sh` (legacy alias: `compare_sasc_parallel_writecharhw_trial.sh`)
- Unknown42 `PARALLEL_RawDoFmt` SAS/C compare script: `src/decomp/scripts/compare_sasc_parallel_raw_dofmt_trial.sh` (legacy alias: `compare_sasc_parallel_rawdofmt_trial.sh`)
- SAS/C trial sources currently used:
- `src/decomp/sas_c/test_memory_pair.c`
- `src/decomp/sas_c/memory_deallocate.c`
...
- `src/decomp/sas_c/parallel_check_ready.c`
- `src/decomp/sas_c/parallel_check_ready_stub.c`
- `src/decomp/sas_c/parallel_wait_ready.c`
...
- `src/decomp/scripts/semantic_filter_sasc_tliba2_find_last_char_in_string.awk`
- `src/decomp/sas_c/tliba2_find_last_char_in_string.c`
- `src/decomp/scripts/compare_sasc_tliba2_resolve_entry_window_with_default_range_trial.sh`
- `src/decomp/scripts/semantic_filter_sasc_tliba2_resolve_entry_window_with_default_range.awk`
- `src/decomp/sas_c/tliba2_resolve_entry_window_with_default_range.c`
...
- `src/decomp/scripts/semantic_filter_sasc_tliba1_parse_style_code_char.awk`
- `src/decomp/sas_c/tliba1_parse_style_code_char.c`
...
- `src/decomp/scripts/semantic_filter_sasc_tliba1_format_clock_format_entry.awk`
- `src/decomp/sas_c/tliba1_format_clock_format_entry.c`
...
- `src/decomp/scripts/semantic_filter_sasc_newgrid_draw_grid_top_bars.awk`
- `src/decomp/sas_c/newgrid_draw_grid_top_bars.c`
- `src/decomp/scripts/compare_sasc_newgrid_clear_highlight_area_trial.sh`
- `src/decomp/scripts/semantic_filter_sasc_newgrid_clear_highlight_area.awk`
- `src/decomp/sas_c/newgrid_clear_highlight_area.c`
- `src/decomp/scripts/compare_sasc_newgrid_fill_grid_rects_trial.sh`
- `src/decomp/scripts/semantic_filter_sasc_newgrid_fill_grid_rects.awk`
- `src/decomp/sas_c/newgrid_fill_grid_rects.c`
- `src/decomp/scripts/compare_sasc_newgrid_draw_grid_frame_trial.sh`
- `src/decomp/scripts/semantic_filter_sasc_newgrid_draw_grid_frame.awk`
- `src/decomp/sas_c/newgrid_draw_grid_frame.c`
...
- `src/decomp/scripts/semantic_filter_sasc_newgrid_compute_day_slot_from_clock_with_offset.awk`
- `src/decomp/sas_c/newgrid_compute_day_slot_from_clock_with_offset.c`
- `src/decomp/scripts/compare_sasc_newgrid_map_selection_to_mode_trial.sh`
- `src/decomp/scripts/semantic_filter_sasc_newgrid_map_selection_to_mode.awk`
- `src/decomp/sas_c/newgrid_map_selection_to_mode.c`
...
- `src/decomp/scripts/semantic_filter_sasc_newgrid_init_grid_resources.awk`
- `src/decomp/sas_c/newgrid_init_grid_resources.c`
- `src/decomp/scripts/compare_sasc_newgrid_shutdown_grid_resources_trial.sh`
- `src/decomp/scripts/semantic_filter_sasc_newgrid_shutdown_grid_resources.awk`
- `src/decomp/sas_c/newgrid_shutdown_grid_resources.c`
- `src/decomp/scripts/compare_sasc_newgrid_select_next_mode_trial.sh`
- `src/decomp/scripts/semantic_filter_sasc_newgrid_select_next_mode.awk`
- `src/decomp/sas_c/newgrid_select_next_mode.c`
- `src/decomp/scripts/compare_sasc_newgrid_draw_wrapped_text_trial.sh`
- `src/decomp/scripts/semantic_filter_sasc_newgrid_draw_wrapped_text.awk`
- `src/decomp/sas_c/newgrid_draw_wrapped_text.c`
- `src/decomp/scripts/compare_sasc_newgrid_draw_clock_format_header_trial.sh`
- `src/decomp/scripts/semantic_filter_sasc_newgrid_draw_clock_format_header.awk`
- `src/decomp/sas_c/newgrid_draw_clock_format_header.c`
