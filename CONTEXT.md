...
- `src/ESQ.asm` is now in-progress: core init, serial CTRL/RBF handling, and jump stubs are being renamed and documented. Continue adding header blocks/aliases after the CTRL buffer routines.
...
2. Keep pushing through `src/ESQ.asm`, adding header blocks/aliases after the CTRL/RBF routines.
3. Propagate new names to all call sites across `src/modules/`, `src/subroutines/`, and `src/data/`.
4. Once the UI highlight path is fully named, revisit `src/data/wdisp.s` to formalize table names and document their relationship to the gcommand routines.
5. Schedule an alias/comment sweep for `src/modules/newgrid.s` and its related tables once the UI naming stabilizes.
6. Re-run `./test-hash.sh` and capture the output hash in commit or PR notes for traceability.
