...
SASC_SRC="flib_append_clock_stamped_log_entry.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/a/r/flib.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="flib_append_clock_stamped_log_entry"
ENTRY="FLIB_AppendClockStampedLogEntry"
...
awk -v e="^${ENTRY}:$" -v e2="$ENTRY_SASC_REGEX" '$0 ~ e || $0 ~ e2 {in_func=1} in_func { if (($0 ~ /^FLIB_[A-Za-z0-9_]+:$/ || $0 ~ /^_?FLIB_[A-Za-z0-9_]+:$/) && $0 !~ e && $0 !~ e2) exit; if ($0 ~ /^XREF / || $0 ~ /^XDEF / || $0 ~ /^ END$/ || $0 ~ /^END$/ || $0 ~ /^__const:$/) exit; print }' \
...
awk -f src/decomp/scripts/semantic_filter_sasc_flib_append_clock_stamped_log_entry.awk \
