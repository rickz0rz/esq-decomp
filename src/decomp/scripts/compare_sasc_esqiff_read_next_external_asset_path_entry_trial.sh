...
SASC_SRC="esqiff_read_next_external_asset_path_entry.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/a/n/esqiff.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="esqiff_read_next_external_asset_path_entry"
ENTRY_ORIG="ESQIFF_ReadNextExternalAssetPathEntry"
ENTRY_SASC_REGEX="^ESQIFF_ReadNextExternalAssetPath[A-Za-z0-9_]*:$"

mkdir -p "$OUT_DIR"

./sc-build-with-dis.sh "$SASC_SRC" >"${OUT_DIR}/sc_build_${BASE}.log" 2>&1

awk -v start="^${ENTRY_ORIG}:$" -v next_label="^ESQIFF_RestoreBasePaletteTriples:$" '$0 ~ start {in_func=1} in_func {if ($0 ~ next_label) exit; print}' "$ORIG_ASM" >"${OUT_DIR}/${BASE}.original.s"
...
awk -f src/decomp/scripts/semantic_filter_sasc_esqiff_read_next_external_asset_path_entry.awk "${OUT_DIR}/${BASE}.original.norm.s" >"${OUT_DIR}/${BASE}.original.semantic.txt"
awk -f src/decomp/scripts/semantic_filter_sasc_esqiff_read_next_external_asset_path_entry.awk "${OUT_DIR}/${BASE}.sasc.norm.s" >"${OUT_DIR}/${BASE}.sasc.semantic.txt"
