...
SASC_SRC="locavail2_display_alert_delay_and_reboot.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/a/z/locavail2.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="locavail2_display_alert_delay_and_reboot"
ENTRY="LOCAVAIL2_DisplayAlertDelayAndReboot"
ENTRY_SASC_REGEX="^LOCAVAIL2_DisplayAlertDelayAndRe[A-Za-z0-9_]*:$"
...
awk -v e="^${ENTRY}:$" -v e2="$ENTRY_SASC_REGEX" '$0 ~ e || $0 ~ e2 {in_func=1} in_func { if (($0 ~ /^LOCAVAIL2_[A-Za-z0-9_]+:$/ || $0 ~ /^_?LOCAVAIL2_[A-Za-z0-9_]+:$/) && $0 !~ e && $0 !~ e2) exit; if ($0 ~ /^XREF / || $0 ~ /^XDEF / || $0 ~ /^ END$/ || $0 ~ /^END$/ || $0 ~ /^__const:$/) exit; print }' \
...
awk -f src/decomp/scripts/semantic_filter_sasc_locavail2_display_alert_delay_and_reboot.awk \
