...
cd "$ROOT_DIR"
SASC_SRC="coi_ensure_anim_object_allocated.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/a/e/coi.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="coi_ensure_anim_object_allocated"
ENTRY="COI_EnsureAnimObjectAllocated"
ENTRY_SASC_REGEX="^COI_EnsureAnimObjectAllocated[A-Za-z0-9_]*:$"
mkdir -p "$OUT_DIR"
...
awk -f src/decomp/scripts/semantic_filter_sasc_coi_ensure_anim_object_allocated.awk "${OUT_DIR}/${BASE}.original.norm.s" >"${OUT_DIR}/${BASE}.original.semantic.txt"
awk -f src/decomp/scripts/semantic_filter_sasc_coi_ensure_anim_object_allocated.awk "${OUT_DIR}/${BASE}.sasc.norm.s" >"${OUT_DIR}/${BASE}.sasc.semantic.txt"
