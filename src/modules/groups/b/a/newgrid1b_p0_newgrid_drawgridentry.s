    XDEF    _NEWGRID_DrawGridEntry


;------------------------------------------------------------------------------
; FUNC: _NEWGRID_DrawGridEntry   (Draw grid entry line)
; ARGS:
;   stack +8: A3 = rastport
...
_NEWGRID_DrawGridEntry:
    ...
    MOVE.W  26(A5),D6
    MOVE.L  28(A5),D5
    LEA     _NEWGRID_GridEntryDelimiterBar,A0
    LEA     -19(A5),A1
    MOVE.B  (A0)+,(A1)+
    MOVE.B  (A0)+,(A1)+
    MOVE.B  (A0)+,(A1)+
...
.check_custom_render:
    ...
    MOVE.L  A0,-(A7)
    MOVE.L  A2,-(A7)
    JSR     _NEWGRID2_JMPTBL_COI_RenderClockFormatEntryVariant(PC)
...
.draw_secondary_line:
    MOVE.L  -8(A5),-(A7)
    MOVE.L  A3,-(A7)
    JSR     _NEWGRID2_JMPTBL_DISPTEXT_LayoutSourceToLines(PC)
