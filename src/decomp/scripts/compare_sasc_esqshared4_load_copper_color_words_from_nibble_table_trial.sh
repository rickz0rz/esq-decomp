...
SASC_SRC="esqshared4_load_copper_color_words_from_nibble_table.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/a/q/esqshared4.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="esqshared4_load_copper_color_words_from_nibble_table"
ENTRY="ESQSHARED4_LoadCopperColorWordsFromNibbleTable"
ENTRY_SASC_REGEX="^ESQSHARED4_LoadCopperColorWords[A-Za-z0-9_]*:$"
...
awk -v e="^${ENTRY}:$" -v e2="$ENTRY_SASC_REGEX" '$0 ~ e || $0 ~ e2 {in_func=1} in_func { if (($0 ~ /^ESQSHARED4_[A-Za-z0-9_]+:$/ || $0 ~ /^_?ESQSHARED4_[A-Za-z0-9_]+:$/) && $0 !~ e && $0 !~ e2) exit; if ($0 ~ /^XREF / || $0 ~ /^XDEF / || $0 ~ /^ END$/ || $0 ~ /^END$/ || $0 ~ /^__const:$/) exit; print }' \
...
awk -f src/decomp/scripts/semantic_filter_sasc_esqshared4_load_copper_color_words_from_nibble_table.awk \
