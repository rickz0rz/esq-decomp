...
BASE="newgrid_should_open_editor_state_flow"
...
awk -v start="^${ENTRY}:$" -v next_label="^;!======$" '
    $0 ~ start { inf=1 }
    inf {
...
awk -f src/decomp/scripts/semantic_filter_sasc_newgrid_should_open_editor_state_flow.awk "${OUT_DIR}/${BASE}.original.norm.s" >"${OUT_DIR}/${BASE}.original.semantic.txt"
awk -f src/decomp/scripts/semantic_filter_sasc_newgrid_should_open_editor_state_flow.awk "${OUT_DIR}/${BASE}.sasc.norm.s" >"${OUT_DIR}/${BASE}.sasc.semantic.txt"
