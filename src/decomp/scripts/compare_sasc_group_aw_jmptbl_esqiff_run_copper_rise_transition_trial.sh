...
BASE="group_aw_jmptbl_esqiff_run_copper_rise_transition"
ENTRY_ORIG="GROUP_AW_JMPTBL_ESQIFF_RunCopperRiseTransition"
ENTRY_SASC="GROUP_AW_JMPTBL_ESQIFF_RunCopper"
TARGET="ESQIFF_RunCopperRiseTransition"

mkdir -p "$OUT_DIR"
./sc-build-with-dis.sh "$SASC_SRC" >"${OUT_DIR}/sc_build_${BASE}.log" 2>&1
awk -v e="^${ENTRY_ORIG}:$" '$0 ~ e {inf=1} inf { if ($0 ~ /^GROUP_AW_JMPTBL_ESQIFF_RunCopperDropTransition:$/) exit; print }' "$ORIG_ASM" >"${OUT_DIR}/${BASE}.original.s"
awk -v e="^${ENTRY_SASC}:$" '$0 ~ e {inf=1} inf { if ($0 ~ /^GROUP_AW_JMPTBL_DISPLIB_DisplayT:$/) exit; print }' "$SASC_DIS" >"${OUT_DIR}/${BASE}.sasc.dis.s"
