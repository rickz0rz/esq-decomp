...
SASC_SRC="esq_poll_ctrl_input.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/a/a/app.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="esq_poll_ctrl_input"
ENTRY="ESQ_PollCtrlInput"
ENTRY_SASC_REGEX="^ESQ_PollCtrlInput[A-Za-z0-9_]*:$"
ENTRY_SEM="ESQ_POLLCTRLINPUT:"
ENTRY_ALT_SEM="ESQ_POLLCTRLINPUT"
...
awk -v ENTRY_PREFIX="$ENTRY_SEM" -v ENTRY_ALT_PREFIX="$ENTRY_ALT_SEM" -f src/decomp/scripts/semantic_filter_sasc_esq_poll_ctrl_input.awk "${OUT_DIR}/${BASE}.original.norm.s" >"${OUT_DIR}/${BASE}.original.semantic.txt"
awk -v ENTRY_PREFIX="$ENTRY_SEM" -v ENTRY_ALT_PREFIX="$ENTRY_ALT_SEM" -f src/decomp/scripts/semantic_filter_sasc_esq_poll_ctrl_input.awk "${OUT_DIR}/${BASE}.sasc.norm.s" >"${OUT_DIR}/${BASE}.sasc.semantic.txt"
