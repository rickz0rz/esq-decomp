...
SASC_SRC="esq_handle_serial_rbf_interrupt.c"
SASC_DIS="src/decomp/sas_c/${SASC_SRC}.dis"
ORIG_ASM="src/modules/groups/a/a/app.s"
OUT_DIR="build/decomp/sasc_trial"
BASE="esq_handle_serial_rbf_interrupt"
ENTRY="ESQ_HandleSerialRbfInterrupt"
ENTRY_SASC_REGEX="^ESQ_HandleSerialRbfInterrupt[A-Za-z0-9_]*:$"
ENTRY_SEM="ESQ_HANDLESERIALRBFINTERRUPT:"
ENTRY_ALT_SEM="ESQ_HANDLESERIALRBFINTERRUPT"
...
awk -v ENTRY_PREFIX="$ENTRY_SEM" -v ENTRY_ALT_PREFIX="$ENTRY_ALT_SEM" -f src/decomp/scripts/semantic_filter_sasc_esq_handle_serial_rbf_interrupt.awk "${OUT_DIR}/${BASE}.original.norm.s" >"${OUT_DIR}/${BASE}.original.semantic.txt"
awk -v ENTRY_PREFIX="$ENTRY_SEM" -v ENTRY_ALT_PREFIX="$ENTRY_ALT_SEM" -f src/decomp/scripts/semantic_filter_sasc_esq_handle_serial_rbf_interrupt.awk "${OUT_DIR}/${BASE}.sasc.norm.s" >"${OUT_DIR}/${BASE}.sasc.semantic.txt"
