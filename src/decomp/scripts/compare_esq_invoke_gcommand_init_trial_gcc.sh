...
SRC_C="src/decomp/c/replacements/esq_invoke_gcommand_init_gcc.c"
ORIG_ASM="src/modules/groups/a/a/app3.s"
OUT_DIR="build/decomp/c_trial_gcc"
GEN_ASM="${OUT_DIR}/esq_invoke_gcommand_init.generated.gcc.s"
ORIG_SLICE="${OUT_DIR}/esq_invoke_gcommand_init.original_slice.s"
GEN_SLICE="${OUT_DIR}/esq_invoke_gcommand_init.generated_slice.s"
ORIG_NORM="${OUT_DIR}/esq_invoke_gcommand_init.original_slice.norm.s"
GEN_NORM="${OUT_DIR}/esq_invoke_gcommand_init.generated_slice.norm.s"
ORIG_SEM="${OUT_DIR}/esq_invoke_gcommand_init.original_slice.semantic.txt"
GEN_SEM="${OUT_DIR}/esq_invoke_gcommand_init.generated_slice.semantic.txt"
...
awk '$0 ~ /^ESQ_InvokeGcommandInit:?$/ {in_func=1} in_func { if ($0 ~ /^;!======/) exit; print }' "$ORIG_ASM" > "$ORIG_SLICE"
awk '$0 ~ /^_?ESQ_InvokeGcommandInit:?$/ {in_func=1} in_func { print }' "$GEN_ASM" > "$GEN_SLICE"
...
awk -f src/decomp/scripts/semantic_filter_esq_invoke_gcommand_init.awk "$ORIG_NORM" > "$ORIG_SEM"
awk -f src/decomp/scripts/semantic_filter_esq_invoke_gcommand_init.awk "$GEN_NORM" > "$GEN_SEM"
