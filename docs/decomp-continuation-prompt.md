...
- Current hybrid-integration note (March 13, 2026, later re-validated): `modules/groups/_main/a/a.s` is now seeded in `src/decomp/replacements.map` as a passthrough hybrid boundary. Treat the primary `_main` startup/shutdown helper cluster (`ESQ_StartupEntry`, `ESQ_ReturnWithStackCode`, `ESQ_ShutdownAndReturn`) as integration-ready at the module boundary, even though the replacement file still includes the canonical asm module verbatim for now.
