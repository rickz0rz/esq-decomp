...
SRC_C="src/decomp/c/replacements/ctasks_start_iff_task_process_gcc.c"
ORIG_ASM="src/modules/groups/a/f/ctasks.s"
OUT_DIR="build/decomp/c_trial_gcc"
GEN_ASM="${OUT_DIR}/ctasks_start_iff_task_process.generated.gcc.s"
ORIG_SLICE="${OUT_DIR}/ctasks_start_iff_task_process.original_slice.s"
GEN_SLICE="${OUT_DIR}/ctasks_start_iff_task_process.generated_slice.s"
ORIG_NORM="${OUT_DIR}/ctasks_start_iff_task_process.original_slice.norm.s"
GEN_NORM="${OUT_DIR}/ctasks_start_iff_task_process.generated_slice.norm.s"
...
awk '
$0 ~ /^CTASKS_StartIffTaskProcess:?$/ {in_func=1}
in_func {
    if ($0 ~ /^;!======/) exit
    print
}
...
awk '
$0 ~ /^_?CTASKS_StartIffTaskProcess:?$/ {in_func=1}
in_func { print }
' "$GEN_ASM" > "$GEN_SLICE"
...
awk -f src/decomp/scripts/semantic_filter_ctasks_start_iff_task_process.awk "$ORIG_NORM" > "$ORIG_SEM"
awk -f src/decomp/scripts/semantic_filter_ctasks_start_iff_task_process.awk "$GEN_NORM" > "$GEN_SEM"
