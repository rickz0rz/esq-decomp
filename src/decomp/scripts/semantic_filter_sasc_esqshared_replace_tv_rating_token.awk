...
}
{
    l=t($0)
    if(l=="")next
    if(l~/^ESQSHARED_REPLACETVRATINGTOKEN:/ || l~/^ESQSHARED_REPLACETVRATINGTOKE[A-Z0-9_]*:/)h_entry=1
    if(l~/^MOVEQ(\.L)? #\$?7,D[0-7]$/ || l~/^CMP\.[WL] D[0-7],D[0-7]$/)h_loop_bound=1
    if(l~/(JSR|BSR).*FINDSUBSTRIN/ || l~/(JSR|BSR).*FINDSUBSTRINGCASE/)h_find=1
    if(l~/ESQPARS2_TVRATINGTOKENGLYPHMA[P]?/)h_glyph=1
    if(l~/GLOBAL_TBL_TV_PROGRAM_RATINGS/)h_table=1
