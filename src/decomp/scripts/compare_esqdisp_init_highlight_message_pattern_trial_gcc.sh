...
SRC_C="src/decomp/c/replacements/esqdisp_init_highlight_message_pattern_gcc.c"
ORIG_ASM="src/modules/groups/a/n/esqdisp.s"
OUT_DIR="build/decomp/c_trial_gcc"
GEN_ASM="${OUT_DIR}/esqdisp_init_highlight_message_pattern.generated.gcc.s"
ORIG_SLICE="${OUT_DIR}/esqdisp_init_highlight_message_pattern.original_slice.s"
GEN_SLICE="${OUT_DIR}/esqdisp_init_highlight_message_pattern.generated_slice.s"
ORIG_NORM="${OUT_DIR}/esqdisp_init_highlight_message_pattern.original_slice.norm.s"
GEN_NORM="${OUT_DIR}/esqdisp_init_highlight_message_pattern.generated_slice.norm.s"
...
awk '$0 ~ /^ESQDISP_InitHighlightMessagePattern:?$/ {in_func=1} in_func { if ($0 ~ /^;!======/) exit; print }' "$ORIG_ASM" > "$ORIG_SLICE"
awk '$0 ~ /^_?ESQDISP_InitHighlightMessagePattern:?$/ {in_func=1} in_func { print }' "$GEN_ASM" > "$GEN_SLICE"
...
awk -f src/decomp/scripts/semantic_filter_esqdisp_init_highlight_message_pattern.awk "$ORIG_NORM" > "$ORIG_SEM"
awk -f src/decomp/scripts/semantic_filter_esqdisp_init_highlight_message_pattern.awk "$GEN_NORM" > "$GEN_SEM"
